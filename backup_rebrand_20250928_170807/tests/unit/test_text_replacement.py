...
class TestTextReplacer:
    ...
    def setup_method(self):
        ...
        Welcome to Suna, the AI assistant by Kortix AI.
        Visit us at https://suna.so/ for more information.
        
        ## Configuration
        KORTIX_ADMIN_API_KEY=secret123
        
        Check out our GitHub: https://github.com/kortix-ai/suna
        Follow us on Twitter: https://x.com/kortixai
        """
        ...
        Check out our GitHub: https://github.com/adentic-ai/suna
    ...
    def test_get_replacement_patterns_for_adentic(self):
        ...
        assert pattern_dict["Kortix"] == "Adentic"
        assert pattern_dict["Suna"] == "Assistant"
        assert pattern_dict["Kortix AI"] == "Adentic AI"
        assert pattern_dict["https://suna.so/"] == "https://adentic.ai/"
        assert pattern_dict["https://x.com/kortixai"] == "https://x.com/adenticai"
        assert pattern_dict["KORTIX_ADMIN_API_KEY"] == "ADENTIC_ADMIN_API_KEY"
    ...
    def test_replace_text_in_string(self):
        ...
        assert "Kortix" not in result
        assert "Suna" not in result
        assert "https://suna.so/" not in result
        assert "KORTIX_ADMIN_API_KEY" not in result
    ...
    def test_replace_text_in_file(self):
        ...
        try:
            ...
            assert "Kortix" not in modified_content
    ...
    def test_replace_text_in_file_with_encoding_issues(self):
        ...
        with tempfile.NamedTemporaryFile(mode='wb', suffix='.py', delete=False) as tmp:
            ...
            tmp.write(b'\xff\xfe invalid utf-8 \x80 Kortix')
            tmp_path = Path(tmp.name)
    ...
    def test_process_directory_tree(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Create test files
            (tmp_path / "app.py").write_text("print('Welcome to Kortix')")
            (tmp_path / "README.md").write_text("# Suna AI Assistant")
            (tmp_path / "config.json").write_text('{"app_name": "Kortix Suna"}')
            
            # Create subdirectory
            subdir = tmp_path / "components"
            subdir.mkdir()
            (subdir / "header.tsx").write_text("const title = 'Kortix AI';")
            
            # Create files that should be skipped
            node_modules = tmp_path / "node_modules"
            node_modules.mkdir()
            (node_modules / "package.json").write_text('{"name": "Kortix"}')
            ...
            assert "Kortix" in (node_modules / "package.json").read_text()
    ...
    def test_preserve_formatting_and_structure(self):
        ...
        {
            "name": "Kortix",
            "description": "Suna AI Assistant by Kortix AI",
            "version": "1.0.0",
            "url": "https://suna.so/"
        }
        """
    ...
    def test_special_replacement_contexts(self):
        ...
        content = """
        // API endpoint
        const API_URL = 'https://api.suna.so/v1/chat';
        ...
        // Documentation link
        [Suna Documentation](https://docs.suna.so)
        """
    ...
    def test_replacement_statistics(self):
        ...
        assert pattern_stats.get("Kortix", 0) > 0
        assert pattern_stats.get("Suna", 0) > 0