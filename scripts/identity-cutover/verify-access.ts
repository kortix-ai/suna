...
function token(userId: string, email: string) {
  ...
  const raw = `${enc({ alg: 'HS256', typ: 'JWT' })}.${enc({ iss: `${process.env.SUPABASE_URL}/auth/v1`, sub: userId, aud: 'authenticated', role: 'authenticated', email, iat: now, exp: now + 300 })}`;
...
for (const row of cases) {
  for (const identity of ['old', 'sso'] as const) {
    const response = await fetch(`https://api.kortix.com/v1/projects/${projectId}/sessions/${row.session}`, {
      headers: { Authorization: `Bearer ${token(row[identity], row.email)}` },
