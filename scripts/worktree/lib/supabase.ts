...
import { type ShResult, portInUse, run, sh } from './exec';
...
export function rewriteConfigToml(toml: string, projectId: string, ports: Ports): string {
  const sectionPort: Record<string, number> = {
    '[api]': ports.sbApi,
    '[db]': ports.sbDb,
    '[db.pooler]': ports.sbPooler,
    '[studio]': ports.sbStudio,
    '[inbucket]': ports.sbInbucket,
    '[analytics]': ports.sbAnalytics,
  };
  ...
  const out = lines.map((line) => {
    ...
    return line
      .replace(/127\.0\.0\.1:54321/g, `127.0.0.1:${ports.sbApi}`)
...
export function renderSupabaseProject(
  name: string,
  worktreePath: string,
  projectId: string,
  ports: Ports,
