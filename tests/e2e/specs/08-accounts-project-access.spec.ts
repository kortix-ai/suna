...
async function createProjectForAccessTest(
  ...
  if (response.status === 409 && body.includes('GitHub App installation required')) {
    const workspaceId = seedSelfHostedProject({ accountId, userId: ownerUserId, name, repoUrl });
    return api<ProjectSummary>(token, 'GET', `/projects/${workspaceId}`);
  }
...
async function openCustomizeSection(
  ...
  await page.goto(`/projects/${workspaceId}/customize/${section}`, { waitUntil: 'domcontentloaded' });
