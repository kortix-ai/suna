...
const password = 'E2eTemplatesUi123!';
...
test.describe('26 — Templates UI', () => {
  ...
  test('renders the catalog from the API and installs through a real session', async ({ page }) => {
    ...
    const email = `e2e-templates-ui-${runId}@example.test`;
    ...
    page.on('request', (request) => {
      if (request.method() === 'POST' && request.url().includes('/templates/install-session')) {
    ...
    try {
      ...
      const catalog = await fetch(`${apiBase}/public/templates`);
      ...
      const listResponse = page.waitForResponse(
        (response) =>
          response.url().includes('/v1/public/templates') &&
          response.request().method() === 'GET' &&
          response.status() === 200,
        { timeout: 60_000 },
      );
      await page.goto(`/projects/${project.id}/customize/templates`, {
        waitUntil: 'domcontentloaded',
      });
      ...
      // callback origin; see TMPL-3 in `tests/src/flows/templates.flow.ts`.
      const installResponse = page.waitForResponse(
        (response) =>
          response.url().includes('/templates/install-session') &&
