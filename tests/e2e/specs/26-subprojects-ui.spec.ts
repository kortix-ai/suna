 ...
 * bytes it can see, and so the subproject's manifest lives beside the assertions
 * that depend on it.
 */
...
test.describe("26 — Subprojects UI", () => {
   ...
   *      the card carries the title of a subproject submitted through the API in
  ...
  test("renders the store from the API and installs through a real session", async ({
    ...
    const email = `e2e-subprojects-ui-${runId}@example.test`;
    ...
    try {
      ...
      repository = await createLocalGitRepository(`subprojects-ui-${runId}`);
      ...
      // Submit a subproject through the real route, with a title unique to this run.
      ...
      const submittedBody = (await submitted.json()) as {
        subproject: { subproject_id: string; title: string; triggers: unknown[] };
      };
      ...
      // An uploaded subproject has no repository. The provenance row must say so
      // rather than linking to a 404 — the mock linked every subproject to GitHub.
      ...
      // leaves the browser, carrying the subproject id the card was built from, and
      ...
      // stack with a reachable callback origin; see the subproject-install assertions
      ...
      // It got PAST authz, the feature gate, and subproject resolution. A 401/403
      // would mean the flag or the capability check is wrong; a 404 would mean
      // the subproject id the card sent does not resolve; a 400 would mean the body
      // is malformed.
