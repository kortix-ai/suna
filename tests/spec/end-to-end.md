...
`SESS-17` Session folders — sidebar organization (`session_folders` + `session_folder_grants` tables; auto-folders for Slack/Email/Scheduled/Webhooks are client-derived from `metadata.source` and have NO server rows or routes). Folder sharing reuses the COMMON team-share model (same `sharing` intent + grants as sessions): `private` | `project` | `restricted` (members/groups). `GET /projects/:id/session-folders` → `read` → 200 array of folders visible to the viewer (their own `private` ones + every `project` one + `restricted` ones they're granted; each carries `visibility` + `sharing`; ordered by `position`, then `created_at`). `POST /projects/:id/session-folders {name, sharing?}` → `read` (any member) → 201 folder (defaults private; `sharing` is the intent shape `{mode:project|private|members}` → invalid mode 400; empty or >120-char name → 400; `position` auto-appends). `PATCH /projects/:id/session-folders/:folderId {name?, sharing?, position?}` → folder **owner or project manager** → 200; a member who can SEE a shared folder but not manage it → 403; someone else's non-visible folder → 404 (hidden, not denied); non-uuid `folderId` → 400; negative/non-integer `position` → 400; invalid `sharing` mode → 400. `DELETE /projects/:id/session-folders/:folderId` → owner or manager → 200 `{ok:true}` — sessions inside are UNFILED (`folder_id` → null via FK SET NULL), never deleted. `PUT /projects/:id/sessions/:sid/folder {folder_id: uuid|null}` → session-visible viewer → 200 updated session (`null` unfiles); target folder must live in the project and be visible to the caller → else 404; body without `folder_id` key or with a non-uuid/non-null value → 400.

`SESS-18` Folder sharing by INHERITANCE — a session filed in a shared folder is visible to that folder's audience in `GET /sessions` (list) and `GET /sessions/:sid` (single read via `loadVisibleSession`), even when the session's own visibility is `private`: a `project` folder → every member; a `restricted` folder → its granted members/groups. Flipping the folder back to `private` restores the session's own visibility rules. Session payloads expose `folder_id` (null = unfiled). `POST /sessions` accepts an optional `folder_id` to file the session at birth — an unknown/foreign/other-member's-private folder id is silently ignored (the create must never fail on a stale folder).
