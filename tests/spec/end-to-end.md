...
`GH-7` `POST /projects/:id/git-token` → mint a fresh managed-GitHub installation token for human principals; **403 for every agent-grant/session/runtime principal**, which must use the Kortix Git proxy; **409 for BYO**; 503 if managed Git is unavailable.
...
3. `POST /projects/:id/goals/:slug/push` on an active goal with `push` → 202 with `status: queued|fired|deduped`, nullable command/session IDs, `evaluation_id`, `evaluation_state`, and `deduped`. Reusing one valid `Idempotency-Key` returns the same evaluation and delivery identity. A paused, achieved, or abandoned goal → 409 `goal_inactive`. An active push-less goal or unavailable generated trigger → 409 `goal_push_unavailable`. Unknown goal → 404.
4. `POST /projects/:id/goals/:slug/observations {evaluation_id,metric,value,source,observed_at?}` → 201 and persists the finite value for one fired evaluation. Omitted `observed_at` uses now. A project session can write only its assigned evaluation; human observations store `session_id:null`. An undeclared metric → 400 `metric_not_declared`; a queued or failed evaluation → 409; an unknown goal → 404.
...
6. `GET /projects/:id/goals/:slug/health` orders only fired evaluations by durable `fired_at`. It reports missing latest values as `unmeasurable`, three equal consecutive finite values as `stalled`, and all other declared metrics as `measuring`. It never changes the Git-authored desired status.
...
2. A second session claiming before expiry → 409 `task_claim_conflict`. It does not replace the live claim. The same task can be adopted after expiry. A session from another project → 400 `session_not_in_project`. An assigned task is claimable only when the referenced `project_sessions` row matches `assignee_agent` or `assignee_user_id`; an unassigned task remains claimable.
3. `POST /projects/:id/tasks/:taskId/done {session_id,evidence:[{ref,summary?}]}` with the session holding a live, unexpired claim and non-empty cited evidence → 200. It sets `status:"done"`, stores `result.evidence`, and releases the claim. Missing or empty evidence → 400. A never-claimed task, an expired unreclaimed claim, or a different owner → 409 `task_transition_conflict` without changing the task.
4. `POST /projects/:id/tasks/:taskId/block {session_id,blocker}` with the session holding a live, unexpired claim and a non-empty blocker → 200. It sets `status:"blocked"`, stores `result.blocker`, and releases the claim. Empty blocker → 400. A never-claimed task, an expired unreclaimed claim, or a different owner → 409 `task_transition_conflict`. A project-session principal must supply its authenticated `session_id`; an unbound human JWT or PAT can coordinate an existing project session.
...
`TASK-3` restart-safe bounded task workers:

1. `POST /projects/:id/tasks/:taskId/worker {session_id,worker_session_id,prompt,contract}` requires the live coordinator claim. It atomically binds the worker and immutable `max_wall_seconds`, `max_tokens`, `max_cost_usd`, and `max_iterations`, then inserts the initial prompt outbox command. The worker must belong to the project and have `metadata.spawned_by_session == session_id`. The 202 response returns `worker.state:"drained"|"queued"`; `queued` remains durable for scheduler retry.
2. `POST /projects/:id/tasks/:taskId/progress {session_id,worker_session_id,settlement_id,ref}` records authenticated semantic progress. The response and a later task read expose `last_progress_at` and `last_progress_ref`. Another worker identity → 409 without changing progress. The same `(task_id,settlement_id)` replays the stored progress snapshot. The ID must equal the server-owned `task.liveness_turn_id`; each accepted outcome advances that ID.
3. `POST /projects/:id/tasks/:taskId/no-progress {session_id,worker_session_id,settlement_id,reason}` measures server-owned usage. The first distinct settlement queues exactly one continuation. Replay of the same `settlement_id` returns its stored result. PostgreSQL rejects progress and no-progress for the same settlement ID, including concurrent calls. A second distinct settlement or exceeded bound atomically blocks and releases the task, then durably queues worker stop and coordinator escalation commands.
4. The LLM gateway atomically admits iterations below the persisted limit. Exhaustion denies provider dispatch and finalizes the task. The leader scheduler enforces wall, token, cost, and iteration bounds without requiring another worker request. Process restarts do not reset any bound or continuation state.
5. A spawned-unbound or bound worker cannot create or claim tasks, bind another worker, fire goal push triggers, or write goal observations. Task and goal reads remain available. Coordinator-only progress, no-progress, and evidence finalization semantics remain unchanged.
6. A task-worker `git-receive-pack` must contain exactly one non-delete command for `refs/heads/<worker_session_id>`. The proxy parses only a 16 KiB command prelude, caps the streamed body at 1 GiB, and rejects malformed, multi-ref, delete, or other-ref input before resolving provider credentials. A concurrent receive-pack loses PostgreSQL admission. Ordinary non-worker Git behavior is unchanged.
7. The proxy aborts a live task-worker receive-pack 5 seconds before the immutable worker deadline. A client abort or transport failure does not settle the durable `live` state. Terminal transition, second no-progress escalation, ledger-failure blocking, and liveness terminalization remain behind the live Git fence. A blocked liveness finalization atomically revokes worker PATs, queues worker stop and coordinator escalation, and denies every later prompt while the task stays nonterminal. The leader repeatedly reconciles the allowed remote ref after the 30-second grace. It changes the matching server-owned request UUID to `settled` only after the requested update completed or a server fence made the abandoned old-OID compare-and-swap impossible. If the requested object is unavailable, that fence deletes an existing session branch or creates an absent session branch at another advertised object. A crashed proxy or provider outage therefore cannot create a deadline-equality terminal race or leave runtime authority active while terminalization waits.
8. Every terminal worker transaction revokes all active `account_tokens` rows with the worker `session_id` before it queues the stop command. The worker therefore receives 401 on subsequent Kortix API and connector calls during the stop delay.
