...
`RTA-4` `GET|HEAD /v1/runtime-assets/entrypoint` → auth → 200 the supervising sandbox entrypoint, `ETag` = `components.entrypoint.sha256`, `Content-Length` = `components.entrypoint.size`. `HEAD` proves the advertised digest without transferring the script. No entrypoint in the image → 404. `ANON` → 401.
...
`SESS-1` `POST /projects/:id/sessions {agent_name?,initial_prompt?,base_ref?,provider?,name?,session_id?,branch_already_created?,runtime_context?}` → `session` (any project member, **M_VIEWER included** — viewer is the base usable role) → 201 status `provisioning` (fire-and-forget sandbox). `runtime_context` is an optional non-secret scalar map (max 64 entries / 16 KiB UTF-8 JSON; lower-case semantic keys only; no credential-like keys); it is stored outside user-editable session metadata and restored into every replacement runtime as the single server-owned `KORTIX_SESSION_CONTEXT` JSON variable. Nested/oversize/reserved/credential-shaped input → 400 before session persistence or provisioning; raw env/MCP fields are unknown and rejected. Create metadata keys `pi_worker_boot`, `sandbox_slug`, `pi_worker_ref`, `pi_worker_sha`, `agent_resources_sha`, and `runtimeArtifact` are server-managed and return 400. An explicit top-level `sandbox_slug: "pi-worker"` returns 400 with code `PI_WORKER_RUNTIME_RESERVED`; only `kortix_version: 3` at the immutable session commit selects Pi. Version 2 selects OpenCode. An optional `runtime` field must match the version; contradictory values return `409 PI_WORKER_RUNTIME_RESOLUTION_FAILED`. Pi selection does not require the `pi_worker` feature flag. A project with a Git/read/parse/invalid-runtime resolution failure at the immutable session Git commit returns `409 PI_WORKER_RUNTIME_RESOLUTION_FAILED` before session persistence; it never falls back to OpenCode. A genuinely absent manifest or v1 manifest preserves the legacy OpenCode runtime. The server persists a valid Pi decision with its immutable Git ref/SHA and the effective Daytona provider. Pi requires the project's effective `llm_gateway` flag and the account's managed-model entitlement because the worker has no native-provider credential path. A Pi selection with the gateway disabled returns `409 PI_WORKER_LLM_GATEWAY_REQUIRED`; an ineligible account returns `403 PI_WORKER_LLM_GATEWAY_ENTITLEMENT_REQUIRED`. Both failures occur before the session row, billing hold, branch, or sandbox exists. Provisioning rechecks both conditions before provider allocation so restart races fail closed. Cold `/start` and replacement `/restart` reuse the stored Pi identity, never rebuild the worker with OpenCode env, and fail closed when that identity is incomplete. MEMBER with no project grant / NONMEMBER → 403 before reserved-field validation. (An invalid `provider` for an allowed caller → 400, proving the role gate passed before provider validation.)
...
Session model selection for `SESS-1` uses the selected source revision for both v2 and v3. An explicit `opencode_model` wins, followed by the saved agent preference, compiled agent model, and project/account/platform defaults. Account entitlement still applies. The response and persisted metadata expose the same `opencode_model` and `opencode_model_source`. Omitting the request model must retain the compiled choice; moving the default branch must not change a session created from an older source revision.

`SESS-10` OpenCode title/tree mirror is server-owned. After a real prompt, the API snapshots the root OpenCode session and its child tree into `metadata.opencode_sessions`. `GET /projects/:id/sessions` and `GET /projects/:id/sessions/:sid` return the same root title and tree. The platform name and runtime title are generated independently; both must become non-placeholder titles. No browser-write sync endpoint exists.
`SESS-12` `POST /projects/:id/sessions/:sid/stop` → `session` (then **owner or project manager** only — `canManageLifecycle`) → **200** status `stopped`, worker and environment paused in place (disk kept, no re-provision — same contract as an idle auto-stop); resumable via `/start`/`SESS-9`. Sandbox not `active` → 409; unsupported provider → 400. If the worker stops but the provider cannot confirm the environment stopped → 502 with `worker_status: stopped` and the observed `environment_status`; the API never reports a complete two-runtime stop as 200 while the environment can still run. For Pi workers, passive proxy retries, background `POST /log` writes, and SSE reconnects cannot wake the provider or change worker/session lifecycle state during or after the stop; explicit `/start` still resumes execution.
`SESS-13` Public shares — `GET/POST /projects/:id/sessions/:sid/public-shares` and `DELETE …/public-shares/:shareId`. LIST and DELETE are gated on `canManageLifecycle` (session owner or project manager); POST is gated on `canManageSharing` (the session owner, or a project manager when the session is machine-owned; see `SESS-14`). POST body `{preview:{port,path?,label?},file?,mode?,expires_at?}` → 201 `{share:{share_id,session_id,project_id,resource_type,label,port,path,file_path,mode,allow_websocket,expires_at,revoked_at,created_at,updated_at,public_token,public_path,proxy_path}}` (`public_token` = `kps_<shareId no dashes>`); a blocked/out-of-range port or an invalid `expires_at` → 400. GET list → `{shares:[…]}`, **not filtered** — a revoked share stays in the list with `revoked_at` set (revoking never deletes the row). DELETE `:shareId` → 200 `{share:{…,revoked_at}}`; **no idempotency guard** — revoking an already-revoked share is 200 again, not 409; unknown `shareId` → 404; non-uuid `sid`/`shareId` → 400. The unauthenticated counterpart is `GET /p/public-share/:token` (§9, mounted public — no auth middleware at all): unknown token → 404 `Share link not found`; **revoked token → 410** `Share link revoked` (checked before the sandbox); a real token whose sandbox has no `external_id` yet → 503 `Sandbox is not ready` (`resolvePublicShare` LEFT JOINs `session_sandboxes` for exactly this case — a freshly-created, not-yet-provisioned session has no sandbox row at all; an INNER JOIN previously misread that as a 404 "not found" on a perfectly valid token); a resolved file share requires a configured isolated preview domain and returns 503 `File sharing requires an isolated preview domain` when that domain is absent; a resolved share includes an origin-based `public_url` whose `public_share` query parameter carries the token; a resolvable `preview` share proxies to `GET {origin}/p/public-share/:token/:port/*` (redirect for the bare `:port` form), a `file` share to `.../file[/*]` (view-only — non-GET/HEAD/OPTIONS → 405).
...
`SESS-16` Anonymous session-share viewing — `GET /public/session-shares/:shareId` and `GET /public/session-shares/:shareId/messages` accept the raw share UUID. Both routes enforce unknown (404), revoked/expired (410), personal connector (403), and malformed ID (400) gates. Metadata returns 200 from the database without requiring compute. Messages prefer the active worker, including for Pi sessions whose environment is separate. If the worker is stopped, absent, or unreachable, the route returns the sanitized database transcript mirror when it matches the pinned conversation. Mirror responses include `source: "mirror"`, `captured_at`, and `complete`. They retain text, tool names/status, file names/MIME types, and a reasoning-omitted flag. They omit tool inputs/outputs, file contents/URLs, and reasoning text. A stopped worker without a mirror returns 503. A live worker without a readable transcript returns 200 with `available: false`. File and preview proxy shares still require their execution environment.
`SESS-21` `GET|PUT /projects/:id/sessions/:sid/scope` and `PUT /projects/:id/sessions/:sid/model` hide an unknown project with 404.

`SESS-28` Model PUT rejects anonymous callers with 401 and nonmembers with 403. Pi stores an authorized model for newly accepted prompts and returns `applies_to: next_prompt`. The agent, source SHA, and other metadata remain unchanged. Invalid or unavailable models return 400 and preserve the saved choice. Model GET returns the selected model and its gateway limits without starting compute. It enforces authentication, project boundaries, session visibility, and Pi runtime identity. A queued OpenCode session retains the existing model-change contract. An older live Pi worker must advertise `next-prompt-v1` before accepting a model change; otherwise PUT returns 409 without changing metadata.
...
`SESS-30` Durable pi transcript log. `POST /projects/:id/sessions/:sid/log` appends one bare mutation and accepts an optional UUID-v4 `Idempotency-Key` during the mixed-version rollout. Replaying identical content under the same key → `204` without a second row. Reusing that key for different content → `409`. `GET /projects/:id/sessions/:sid/log` returns the bare mutations once each in database append order without contacting or starting a runtime.

`SESS-33` Pi history transition fencing through `POST /projects/:id/sessions/:sid/log`. History transitions require a matching UUID-v4 idempotency key and version 1. Stage/restore and new prompt acceptance compare the same history revision under a PostgreSQL session lock. Concurrent operations from one revision return one `204` and one `409`. Rewind rejects unfinished turns. Accepted prompts commit staged history; restore then returns `409`. Identical retries append once. After the first rewind, acceptance without `historyRevision` returns `409`, preventing old workers from bypassing the guard. Malformed transitions return `400`. Reads prove exact append order. The PostgreSQL transcript mirror filters hidden IDs before counting and pagination, returns a known empty transcript when all captured rows are hidden, and restores only staged rows. Accepted replacement prompts keep discarded rows hidden. Operations do not start compute. Prepare/commit/cancel use a stable operation UUID. Prepare blocks admission without changing visible history. Commit moves the native branch and visibility after the environment receipt completes. Cancel keeps history visible and releases the admission fence. Repeated commits append once; wrong operation identities return `409`. File rollback is verified through the worker/environment integration and deployed session route.


The history flow also verifies that a stopped transcript preserves its session-scoped attachment reference. The authenticated attachment API returns the original bytes without changing session status. Mirror capture retains only canonical references belonging to the captured project and session. It strips inline bytes, external URLs, and references to other sessions.

`SESS-29` Immutable session attachments. `PUT /projects/:id/sessions/:sid/attachments/:sha256` stores 1 byte through 8 MiB of raw bytes under their SHA-256, with a parameter-free MIME `Content-Type`; matching retries return `204`. A digest mismatch or malformed digest/type returns `400`; changing the MIME type for existing bytes returns `409`. `GET` returns exact bytes, MIME, ETag, and private immutable cache headers without starting a sandbox. Unknown attachments and sessions return `404`; project nonmembers and sibling-session credentials cannot read or write them. Empty uploads return `400`; uploads above 8 MiB return `413`, including chunked bodies. Deleting the session makes its attachments inaccessible. Pi prompts accept immutable ordinary files as well as native images. Ordinary files work with text-only models and remain downloadable from stopped history. Environment file tools materialize original bytes with the environment credential. Worker/environment tests assert duplicate names, replay, queued-turn isolation, corruption, limits, edits, deletions, and environment replacement. Browser journey 29 downloads exact CSV bytes twice across reload without any sandbox rows.

`SESS-35` Prompt refusal and durable Stop. A disabled optional connector binding permits a prompt and persists its inbox row. Explicitly requiring that same connector returns `409 CONNECTOR_CONNECTION_REQUIRED`. An unavailable required connector returns `409 REQUIRED_CONNECTOR_CONNECTION_UNAVAILABLE` before enqueueing. Stop marks a claimed delivery `waiting/held` immediately; a fresh GET preserves that state. Resume clears both the public hold and delivery-stop marker. The delivery worker checks Stop before each POST and preserves permanent refusal messages without retrying them.
...
session authorization boundary. Pi prompt preflight reads connector requirements from
the session's pinned config commit and agent. A later branch commit changes requirements
for unpinned OpenCode sessions but does not replace the Pi session's requirements.
...
Environment HTTP and terminal WebSocket ingress caches follow the persisted provisioning claim and readiness status. Stop/resume refreshes provider access tokens immediately, including when another API process resumes the environment.

`RUN-1` `POST /p/<sbx>/8000/session` → create OpenCode conversation → returns `{id}`.
`RUN-2` `POST /p/<sbx>/8000/session/<ocId>/prompt_async {parts:[{type:text,text}]}` → **204** (async; agent runs in background). A runtime that advertises `x-kortix-prompt-admission: durable-message-id-v1` owns admission by `messageID`: an exact retry with or without `Idempotency-Key` returns **204**, changed input under that ID returns **409**, and only one user message persists. The proxy preserves that ID. Runtimes without this capability retain the proxy’s **200** duplicate response. An explicit `Idempotency-Key` remains bound to its first message ID. A failed capability read returns **503** with `prompt_admission_unavailable` and `retry:true`; it does not claim delivery. A confirmed legacy runtime retains proxy deduplication. Pi accepts `noReply: true` to append context without model execution: the async route returns **204** after durable admission and the synchronous message route returns **200** with the saved user message. Context enters the next model request once, including after worker replacement. Retrying the same ID preserves the original result; changing `noReply` between context and model execution returns **409**. Completed durable responses include `x-kortix-prompt-message-id` and `x-kortix-prompt-completed: idle|error`; the proxy settles the accepted delivery by exact message identity. A completed retry leaves no active turn in the session snapshot and no Stop control after reload. Pi prompt `tools` controls persist when execution starts, including context-only prompts and worker replacement. A nonempty map replaces previous controls; omitted or empty maps retain them. Disabled tools are absent from the provider registry and cannot execute. Cancelled queued prompts and completed retries do not change the current controls. The user message retains its tool map and the session read exposes active permission rules. A pending Pi question survives abrupt worker replacement with the same request ID, message IDs, and prior tool results. The replacement does not repeat completed actions or the model request that produced the question. Answers commit before HTTP acknowledgment. Stop cancels the restored question. A durable execution-release fence prevents automatic replay after an uncertain subsequent tool or model boundary.
...
`TRG-2` `POST /projects/:id/triggers {name(required),slug?,type,agent?,model?,enabled?,prompt_template,cron?,timezone?,secret_env?}` → `manage` → 201, manifest committed; `name` is required (slug derived from it when omitted); duplicate slug → 409. Select the created trigger by slug and verify its model through a separate GET; seeded triggers may sort before it. `webhook` requires `secret_env` (names a `project_secrets` key, regex `^[A-Z_][A-Z0-9_]*$`). `cron` requires 6-field croner expr + IANA `timezone` (default UTC).
`TRG-3` `PATCH /projects/:id/triggers/:slug` (e.g. `{enabled:false}` or a model-only update) → `manage`. Select the updated trigger by slug, then GET and verify its model and disabled state. Seeded triggers may sort before it.
...
`GH-19` `GET /projects/:id/git/connection` → `read` → `{state,reason?,install_url?}`. State is one of `connected|reconnect_required|unavailable|not_connected`; only an account-fixable reconnect carries `install_url`. `ANON` → 401, non-member → 403/404, unknown project → 404.
...
`GH-18` A fresh YAML v3 project with `pi_worker: false` pushes through Git and downloads its exact compiled Pi artifact → 200, source SHA and SHA-256 match; anonymous → 401; mismatched ref/SHA → 409. `GET /projects/:projectId/sessions/:sessionId/environment/resources` returns only the session agent's environment files, with matching bytes and SHA-256. Changing the default branch does not change an existing session's resources. Anonymous → 401; another account → 403. The resource read does not start compute. Push-time compilation selects Pi only. Shared JSONC commands support comments and trailing commas and appear in the Pi artifact. YAML v2 keeps OpenCode, bundles the same environment declarations, and installs them in its single sandbox before custom code starts. Its compiled checkout accepts the pinned commit as `ref` and preserves that Git HEAD and file content after main moves. Its session stores a server-owned `agent_resources_sha` from the selected source commit; create and PATCH reject caller-supplied pins. The resource route returns only that agent’s pinned files after the default branch moves. A session without a resource pin returns 409; an invalid pin or missing selected artifact entry returns 503. Worker resource declarations remain Pi-only and v2 rejects them.
...
`kind:turn_resume` requires the owning worker credential, a Pi session, and the current owner from its durable admission journal. A project owner JWT receives 403. The request carries `opencode_session_id`, `turn_message_id`, and UUID `turn_owner_id`. A valid interrupted turn creates one active attempt and deadline grant; retries return `already_active`. Terminal or aborted turns cannot resume. A resumed attempt accepts a worker completion only for its `turn_owner_id`.
...
`SBX-4` `POST /sandbox-templates` → 201; bad → 400; the server-owned `default` and `pi-worker` slugs or a duplicate → 409; `PATCH/DELETE/build /:templateId`; unknown → 404. Authorization precedes reserved-slug validation, so a non-member request using `pi-worker` remains 403/404.
...
`SESS-34` creates separate sessions with repository access disabled and enabled.
...
Browser journey 29 opens a stopped session with saved text and an immutable image, but no sandbox row. History remains visible after reload. The displayed image blob matches the original bytes, and no sandbox or environment is created. An empty stopped session retains its restart screen.
