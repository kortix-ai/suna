...
## 12b. Spaces (source of truth = the root manifest's `spaces:` map, one block per space)

A space is a named container inside a project: it groups sessions (one `spaces.<slug>` block of `kortix.yaml` each), owns the triggers that name it, and is an IAM object granted to members/groups exactly like an agent (closed by default — a member with no grant row sees none). The database holds only `project_sessions.space` (the session join) and the grant rows (`resource-grants`, `resource_type: 'space'`); every CRUD write commits `kortix.yaml`. `sessions: private` (default) keeps the ordinary per-session visibility model; `sessions: shared` makes every session in the space readable by everyone granted it, regardless of that session's own visibility (lifecycle rights unchanged). Deleting a space strips `space:` from any trigger naming it and orphans its grant rows (still flagged, never silently reinterpreted); session rows keep their `space` column. The local test profile cannot provision a real session (`503 KORTIX_URL_UNREACHABLE`), so a session-create body carrying `space` is asserted at the space validation/authorization boundary only — never through to a running sandbox (`MKTP-11` precedent).

`SPACE-1` CRUD — `POST /projects/:id/spaces {name(required),slug?,description?,agent?,sessions?}` → `manage` + `project.customize.write` → `201`, slug derived from `name` via `slugify` when omitted, `sessions` defaults `private`; the write commits `kortix.yaml` — the created object's `path` is that manifest and `agents` is `[]` (proven by reading the commit back through `GET /projects/:id/commits`). `GET /projects/:id/spaces` lists the caller's accessible set; `GET /projects/:id/spaces/:slug` reads one. `PATCH` merges partial fields (`description` + `sessions: shared` persist); an empty `PATCH {}` → `200` with no new commit. A body field this version does not have — `instructions`, `context`, both dropped 2026-09-07 — is `400`, never silently swallowed. Duplicate slug → `409 SPACE_SLUG_TAKEN`; unknown `agent` → `400`; invalid slug → `400`; bad `sessions` value → `400`. `DELETE /projects/:id/spaces/:slug` → `200`, then `GET` → `404`.

`SPACE-2` authz — a floor `member` (`team.grantProjectRole(...,'member')`) with no grant: `GET /spaces` → `[]`, `GET /spaces/:slug` → `404` (undeclared and inaccessible are the same answer), `POST /sessions {space}` → `403 space_not_accessible` with `accessible_spaces: []`. A manager grants it via the generic surface — `POST /projects/:id/resource-grants {resource_type:'space',resource_id,principal_type:'member',principal_id}` → `201` — after which the member's list/get both succeed and `POST /sessions {space}` clears the space gate (asserted as "not the `space_not_accessible` 403"; the local profile's next boundary is whatever the ordinary agent-access gate returns, since granting a space does NOT imply the agent grant). `GET /resource-grants` shows `resources.spaces` and the grant row. A member without `project.customize.write` gets `403` on `PATCH`/`DELETE`. `POST /sessions {space:'nope'}` → `400 SPACE_NOT_DECLARED`. `ANON` → `401`.

`SPACE-3` sessions filter/hiding + `sessions: shared` — rows are seeded directly (`project_sessions.space`, the local profile cannot provision a real session) rather than through the API. A space the viewer holds no grant for hides its rows entirely from `GET /sessions?space=<slug>`, even a row the viewer themself created — the IAM gate runs before the ordinary visibility fold. After a grant, the viewer's own row in a `private` space becomes visible; in a `sessions: shared` space, another member's `private`-visibility row becomes visible too, once granted. `?space=<slug>` and `?space=` (rows with none) are both accepted query filters and return `200` arrays.

`SPACE-4` triggers — `POST /projects/:id/triggers {...,space:<slug>}` → `201`, and `GET /triggers` shows it; `space` must name a declared space or `400 SPACE_NOT_DECLARED`. `PATCH {enabled:false}` (an unrelated field) keeps the existing `space` — PATCH merges onto the current spec, so only an explicit `space` key changes it. `PATCH {space:null}` clears it. Deleting the space first strips `space:` from every trigger naming it (a root-manifest commit), then removes the file (a second commit), so the manifest set is valid after each (`GET /triggers` then shows `space: null`).

`SPACE-5` CLI (`apps/cli/src/commands/spaces.ts`) as real processes — `kortix spaces create "<name>" [--description] [--agent] [--sessions] --json` → the created object; `ls --json` lists it; `show <slug> --json` reads it; `update <slug> --sessions shared --json` persists the mode change; `rm <slug> --yes` removes it (exit `0`) and a subsequent `show` exits `1`. `kortix sessions new|ls --space <slug>` and `kortix triggers add|set --space <slug>` carry the same field through to the wire.

`SPACE-6` space-owned agents (spec 2026-09-06) — `spaces.marketing` declares `agents.writer` (and `agent: writer` as its default) and `spaces.sales` references it with `agents.writer: { from: marketing }`; both blocks are committed in ONE `kortix.yaml` straight into the project's local bare repository (no API route writes an `agents:` block), followed by one API write (`POST /spaces {name:'Ops'}`) that invalidates the git mirror. `GET /spaces/:slug` then reports `agents: ['writer']` for marketing and sales and `[]` for ops; `GET /projects/:id/detail` `config.agents` marks `writer` with `space: 'marketing'` and `kortix` with `null`. `POST /sessions {agent_name:'writer'}` at the project level → `400 AGENT_NOT_IN_SPACE` with `usable_agents: ['kortix']`; the same inside `marketing` (owner) or `sales` (reference) passes that gate (the local profile's next boundary is whatever the ordinary create path returns), and inside `ops` → `400 AGENT_NOT_IN_SPACE`. A send into `marketing` with no agent fills in its own default `writer` and passes. `POST /triggers {agent:'writer'}` without a space → `400 AGENT_NOT_IN_SPACE`; with `space:'marketing'` → `201`.

`SPACE-7` moving a session — `PATCH /projects/:id/sessions/:sessionId {space}` files an existing session under a space, moves it between two, and (`{space: null}`) back to the project level; each move is proven by the response body and by `GET /sessions?space=<slug>` / `?space=` afterwards. Rows are seeded directly (`project_sessions.space`, the local profile cannot provision a real session) and each caller moves their OWN row, since the move takes the sharing gate (`can_manage_sharing`) — a `shared` space makes its sessions readable by everyone granted it. An undeclared target → `400 SPACE_NOT_DECLARED`. A member with no grant moving their own session into one → `403 space_not_accessible`; after `POST /resource-grants {resource_type:'space'}` the same move → `200`.
