...
2. `POST /projects/:id/tasks/:taskId/progress {session_id,worker_session_id,ref}` records authenticated semantic progress. The response and a later task read expose `last_progress_at` and `last_progress_ref`. Another worker identity → 409 without changing progress.
3. `POST /projects/:id/tasks/:taskId/no-progress {session_id,worker_session_id,settlement_id,reason}` measures server-owned usage. The first distinct settlement queues exactly one continuation. Replay of the same `settlement_id` returns its stored result. A second distinct settlement or exceeded bound atomically blocks and releases the task, then durably queues worker stop and coordinator escalation commands.
