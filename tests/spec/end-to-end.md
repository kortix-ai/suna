...
`CONN-24` Invalid toolkit slug regression. After declaring a Composio connector with `app:"anthropic"`, `POST /connectors/projects/:id/connectors/:slug/connect` returns `422` with the slug and toolkit reason. The response does not expose a raw Composio 400 or return 500.
