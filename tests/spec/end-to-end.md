...
`CHN-21` `GET /projects/:id/channels/telegram/installation` → `read` → `{botId,botUsername,installedAt}` or null. Telegram is an OPTIONAL channel — nothing else depends on it being connected.
`CHN-22` `POST /projects/:id/channels/telegram/connect {bot_token}` → `manage` + `project.connector.write` → token shape checked LOCALLY (400 before any Telegram call) → `getMe` validate (Telegram rejects → 400; unreachable → 502) → mint webhook secret + server-side `setWebhook` (+ `setMyCommands` best-effort) → encrypted secrets + `chat_installs` (workspaceId = bot id). The token never appears in any response, sandbox env, or CLI.
