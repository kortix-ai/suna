...
`CFG-7` Local target. Repository replacement. A session receives a descriptor, then the project's repository is replaced (a second bare repository, a new `metadata.repository_generation`; the flow writes the columns `persistProjectRepositoryReplacement` writes, because the replacement route needs GitHub). The previous-repository session's descriptor request → 409 `{error:"Session belongs to a previous repository", code:"session_repository_changed"}`, and its archive download → the same 409. Its `GET .../config` → `stale: false`, `latest_etag: null`. A session with the new generation receives a descriptor whose `source_commit` is the new repository's tip, a new `release_id`, and an archive with the new repository's files.
