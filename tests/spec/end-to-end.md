...
`SESS-31` The session list pages on request. With neither `limit` nor `cursor` the list is a bare array, unchanged. `limit` (1–200) returns `{ sessions, next_cursor }` in `updated_at DESC, session_id DESC` order. Walking `next_cursor` returns every listed session exactly once, including two sessions inside one millisecond, and the last page's cursor is null. The `project` scope pages the rows only that scope lists (a soft-deleted session). An out-of-range `limit` or a malformed `cursor` returns 400. A non-member cannot page another project.
