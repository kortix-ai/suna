...
`PROJ-34` `POST /projects/:id/execution-lease {action:acquire|renew|release, session_id, lease_ttl_seconds?}` — the in-sandbox agent reports active OpenCode work; `acquire` returns the provider keep-alive endpoint once, `renew`/`release` each run one conditional update. Runtime-only: one of the handful of paths `middleware/auth.ts` lets a SANDBOX token reach (with clone-credential / turn-stream / turn-question / llm-catalog / boot-timeline), and the handler re-checks it. ANON → 401; an OWNER user session → 403; an account PAT → 403 (`execution lease requires a sandbox token`) — being a project principal buys nothing here. Body is validated before the handler, so an unknown `action` → 400. The lease semantics themselves need a live sandbox token no black-box client can mint; they are owned by the API-latency work (PR #5490) and covered by `apps/api/src/projects/execution-lease*.test.ts`.
`PROJ-18` Project cap by plan: a FREE account may own exactly 3 projects — `POST /projects/provision` for the 4th → 403 `{code:project_limit_reached,limit:3}` (checked before any repo is provisioned); paid/team plans get `MAX_PROJECTS_PER_ACCOUNT`. Requires `managedGit`+`stripe` (billing enforced).
...
## 28. AGI — goals, tasks, observations, requests, liveness (§AGI)

The whole surface is behind ONE experimental key, `agi`, enabled per project via `PATCH /projects/:id/experimental {feature:"agi"}`. The gate runs LAST, after the membership floor and the leaf capability, and answers **404, not 403** — with `agi` off there are no routes, so the surface must not exist (R-44). Spec: `docs/specs/2026-07-26-agi-autonomous-operations.md`.

Storage split: **goals** are authored state in `kortix.yaml` (R-6, applied by `kortix ship`, no HTTP write path — the goal routes here are read-only over the manifest); **tasks, observations and requests** are platform DB state (R-3) with a full HTTP lifecycle. `push` is sugar for exactly one cron trigger in the existing trigger subsystem (R-8), never a second scheduler.

`AGI-1` the `agi` gate — feature OFF: `GET /projects/:id/agi/{goals,tasks,liveness,requests}` → **404 for the project OWNER** (about the feature, never the caller). `PATCH /projects/:id/experimental {feature:"agi",enabled:true}` → 200 → the same four → 200. With the feature ON: NONMEMBER → 403/404, ANON → 401 (authz still precedes the gate). Clearing the override (`enabled:null`) puts the surface back to 404.
`AGI-2` task CRUD — `POST /projects/:id/agi/tasks {title,origin,…}` → 201 `{task,created:true}`; missing `origin` or `title` → 400; `agent` + `assignee_user_id` together → 400 (R-14, one assignee); unknown `parent_id` → 400; the same `origin_fingerprint` twice → 201 then **200 `created:false`** with the SAME task id (R-20 idempotency). `GET …/agi/tasks` → 200; a bad `status`/`priority`/`limit`/`ready`/`cursor` → 400. `GET …/agi/tasks/:taskId` → 200 `{task,children,blockers,missing_blockers}`; unknown id → 404; malformed (non-uuid) id → **404, never a 500** (a non-uuid would otherwise raise 22P02). `PATCH …/agi/tasks/:taskId` → 200; self-blocker and self-parent → 400; unknown task → 404. ANON → 401.
`AGI-3` claim/release — `POST …/agi/tasks/:taskId/claim {session_id,ttl_seconds?,status?}` → 200 `{claimed:true}`; a SECOND session on the same task → **409 `claim_conflict`** carrying the current holder (R-18: the loser picks different work, it never waits); the holder re-claiming renews its own lease → 200. Missing `session_id` → 400; claiming into a terminal status → 400; out-of-range `ttl_seconds` → 400. `POST …/agi/tasks/:taskId/release {session_id}` by a non-holder → **409 `claim_not_held`**; by the holder → 200 `{released:true}`; releasing again → 409. Both on an unknown task → 404.
`AGI-4` goals + observations — `GET …/agi/goals` → 200 `{goals,errors}`; a project with no `goals:` block returns both as empty arrays, and a malformed goal is REPORTED in `errors`, never omitted. An invalid `?status` → 400; each of active/achieved/paused/abandoned → 200. `GET …/agi/goals/:slug`, `POST …/agi/goals/:slug/push`, `POST|GET …/agi/goals/:slug/observations` on a slug the manifest does not declare → **404** (R-12c: a typo'd slug must not write a series nobody reads). Push with a non-string `reason` → 400; observe with a non-numeric `value` → 400 (body validated before the manifest read); series with a bad `limit` → 400. ANON → 401. The pushing-goal happy path needs a manifest commit and is covered at the API layer in `integration-agi-goals-http.test.ts`.
`AGI-5` liveness — `GET …/agi/liveness` → 200 with BOTH halves: `tasks`/`stalled`/`stalled_count` and `goals`/`stalled_goals`/`unmeasurable_goals`/`stalled_total` (R-12e — a stall surface that can only see tasks is the blind spot §4.2 was written about). Bad `limit` → 400. `POST …/agi/liveness/sweep` → 200 `{scanned,stalled,outcomes}`; it is explicit, on-demand and idempotent (R-21/R-32, never a poller) — a second sweep with unchanged evidence reports the same `scanned`. ANON → 401.
`AGI-6` human requests (§4.3, R-12g) — `POST …/agi/tasks/:taskId/requests {kind,need,why?,url?,responder_user_id?}` → 201 `{request,created:true,delivered_via}` with a resolved `responder_user_id`; the SAME ask again → **200 `created:false` and NOT re-delivered** (the fix for an ignored ask is a louder inbox, never a daily repeat). Bad `kind`, empty `need`, or a `responder_user_id` outside the workspace → 400; unknown task → 404. `GET …/agi/requests` → 200; `?responder=me` is resolved server-side; `?undelivered=1` is its own view ("we tried to reach a human and could not"); bad `status`/`responder`/`task`/`undelivered`/`limit` → 400. `POST …/agi/requests/:requestId {status:satisfied|cancelled,note?}` → 200; closing it again → **409 `request_not_pending`**; unknown request → 404; an unknown status → 400.
