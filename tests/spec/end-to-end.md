...
`SESS-20` The session transcript route returns 404 for an unknown session. The turn read reports a fresh session as idle with no `last_ended`, and the turn history read returns an empty list for it. Both reject an unknown session with 404 and an anonymous caller.
