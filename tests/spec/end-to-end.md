...
`IAM-36` `DELETE /accounts/:id/iam/assignments/:assignmentId` removes the canonical row AND the legacy row it mirrors, in one transaction (`project_members` / `project_group_grants` / `iam_policies` / `iam_resource_grants`, and a re-point of `account_members.account_role`). The dual-write triggers run legacy → canonical only, so deleting just the canonical row would let any later UPDATE of the legacy row resurrect the revoked grant from a pre-cutover replica. Where the legacy table holds ONE row per (principal, project) and the canonical store holds two, the revoke re-points the legacy row at the strongest surviving role rather than deleting it. Revoking the account's last `owner` → 409. Revoking a principal's ONLY account-scope role → 409 pointing at `DELETE /accounts/:id/members/:userId`, which is the path that also revokes their tokens and releases the seat; a DEMOTION is a grant of the lower role followed by this revoke. Unknown or malformed id → 404.
