...
A subproject is a named container inside a project: it groups sessions, gives the agent standing instructions and reference files (`kortix.yaml` → `subprojects.<slug>`), owns the triggers that name it, and is an IAM object granted to members/groups exactly like an agent (closed by default — a member with no grant row sees none). The database holds only `project_sessions.subproject` (the session join) and the grant rows (`resource-grants`, `resource_type: 'subproject'`); every CRUD write commits `kortix.yaml`. `sessions: private` (default) keeps the ordinary per-session visibility model; `sessions: shared` makes every session in the subproject readable by everyone granted it, regardless of that session's own visibility (lifecycle rights unchanged). Deleting a subproject strips `subproject:` from any trigger naming it and orphans its grant rows (still flagged, never silently reinterpreted); session rows keep their `subproject` column. The local test profile cannot provision a real session (`503 KORTIX_URL_UNREACHABLE`), so a session-create body carrying `subproject` is asserted at the subproject validation/authorization boundary only — never through to a running sandbox (`MKTP-11` precedent).

`SUBP-1` CRUD — `POST /projects/:id/subprojects {name(required),slug?,description?,instructions?,context?,agent?,sessions?}` → `manage` + `project.customize.write` → `201`, slug derived from `name` via `slugify` when omitted, `sessions` defaults `private`, `context` defaults `[]`; the write commits `kortix-<slug>.yaml` — the created object's `path` is that file and `agents` is `[]` (proven by reading the commit back through `GET /projects/:id/commits`). `GET /projects/:id/subprojects` lists the caller's accessible set; `GET /projects/:id/subprojects/:slug` reads one. `PATCH` merges partial fields (`instructions` + `sessions: shared` persist); an empty `PATCH {}` → `200` with no new commit. Duplicate slug → `409 SUBPROJECT_SLUG_TAKEN`; unknown `agent` → `400`; invalid slug → `400`; bad `sessions` value → `400`. `POST .../:slug/context {path,content}` → `200`, commits the file under `.kortix/subprojects/<slug>/<basename>` and appends it to `context[]`; `DELETE .../:slug/context?path=` drops the entry (never the repo file) and `404`s when the path isn't listed. `DELETE /projects/:id/subprojects/:slug` → `200`, then `GET` → `404`.
...
`SUBP-5` CLI (`apps/cli/src/commands/subprojects.ts`) as real processes — `kortix subprojects create "<name>" [--description] [--agent] [--instructions-file] [--context] [--sessions] --json` → the created object; `ls --json` lists it; `show <slug> --json` reads it; `update <slug> --sessions shared --json` persists the mode change; `rm <slug> --yes` removes it (exit `0`) and a subsequent `show` exits `1`. `kortix sessions new|ls --subproject <slug>` and `kortix triggers add|set --subproject <slug>` carry the same field through to the wire.
