...
`MEM-4` `DELETE /accounts/:id/members/:userId` → `MEMBER_REMOVE`; ADMIN removing an OWNER → 403; removing the **last owner** → 409; also removes every account-scoped assignment and group membership before the `account_memberships` identity row. Re-adding that user does not restore old group access.
