...
Specs in `[[triggers]]`; CRUD commits the manifest; runtime state and account-local session access live in `project_trigger_runtime`. Types: `cron`, `webhook`, and `monitor`.
...
`GW-13` per-project OTLP trace export ("Observability") — `GET /projects/:id/gateway/otel` returns `{enabled,endpoint,has_headers,updated_at,capabilities:{write}}` and NEVER a stored header value; `PUT` saves `{enabled,endpoint,headers?}`, encrypting the headers at rest, and treats an OMITTED `headers` as "leave the stored value untouched"; `DELETE` removes the row. The endpoint is caller-supplied egress, so it is validated against the SSRF guard at write time — private/link-local/cloud-metadata/non-HTTPS targets → 400; `enabled:true` with no endpoint → 400; a header name outside the RFC 7230 token charset or a value containing CR/LF → 400; a rejected write leaves the stored config unchanged. Reading needs `project.gateway.spend.read` (floor `member` holds it, and sees `capabilities.write:false`); writing needs `project.gateway.otel.manage` (manager-only), so a floor member's PUT/DELETE → 403. Project nonmember → 403/404; ANON → 401.
