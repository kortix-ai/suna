...
`CFG-7` Repository replacement. A session receives a descriptor, then the project's repository is replaced by a second project's repository with new config (a new `metadata.repository_generation`; the flow writes the project and git-connection columns `persistProjectRepositoryReplacement` writes, because the replacement route needs a caller-supplied GitHub credential that the runner does not hold; PROJ-36 covers the route). The project's git origin then serves the second repository's tip. The previous-repository session's descriptor request → 409 `{error:"Session belongs to a previous repository", code:"session_repository_changed"}`, and its archive download → the same 409. Its `GET .../config` → `stale: false`, `latest_etag: null`. A session with the new generation receives a descriptor whose `source_commit` is the new repository's tip, a new `release_id`, and an archive with the new repository's files.
