...
Browser journey `27-agent-manual-setup`: **Agents → New → Set up manually** opens a creation form on the Agents page. Empty, invalid, and existing names cannot submit. Cancel resets the form. A failed network write retains the draft for retry. Saving sends the description, instructions, and selected mode through the agent-config API, then opens the new agent's editor. Read-back verifies the configuration and markdown file. Existing agents and the project default remain unchanged. Desktop covers the default Primary mode; mobile covers explicit Subagent mode.
