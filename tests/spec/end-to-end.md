...
`GH-9` `GET /git/:project/info/refs` · `GET …/compiled-checkout` · `GET …/repo-snapshot` · `GET …/repo-snapshot/archive` · `GET …/compiled-runtime` · `GET …/compiled-pi-runtime` · `POST …/git-upload-pack` · `POST …/git-receive-pack` → Git proxy and prepared boot artifacts, git token auth (not JWT); bad/no token → 401/502. `repo-snapshot` returns an object-scoped capability for one EXACT commit: a branch name is rejected (400) because a branch is never immutable identity, and the auth boundary is checked before the feature flag so an anonymous caller never learns whether snapshots are enabled. `repo-snapshot/archive` is the authenticated streaming delivery used where a sandbox cannot reach object storage; it derives the object key from the AUTHORIZED project row, so a caller cannot name another repository's object, and the bucket name never reaches the client.
