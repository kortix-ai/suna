...
Boundary `SBX-1`: sandbox create/start is implicit on session create (`provisionSessionSandbox`); no standalone endpoint exists. The executable contracts are `RUN-1` and `GOLD-1`.
Boundary `SBX-2`: sandbox manual stop is `SESS-12`; destructive teardown is `SESS-7`; restart is `SESS-9`; status read is `SESS-8`. No separate sandbox action exists.
...
Boundary `TRG-6`: the cron scheduler is a global `setInterval` (default 60s), which sweeps at most 200 active projects. Due means `nextCronRun(cron,lastFired,tz) ≤ now`. It marks fired before firing. `KORTIX_TRIGGER_SCHEDULER_ENABLED=false` disables it. This internal scheduler has no HTTP route.
...
Boundary `TRG-8`: `fireGitTrigger` selects the account's first owner. No owner causes a silent failure. It calls `createProjectSession(enforceAccountCap:false, visibility:'project', metadata.trigger_*)`. Trigger sessions are project-visible. Three provisioning sessions or an account tier cap queues the trigger. The executable entrypoint contracts are `TRG-5` and `TRG-7`.
Boundary `TRG-9`: no inbound GitHub event webhook exists. Use a generic `webhook` trigger with `X-Hub-Signature-256`. The executable contract is `TRG-7`.
...
`CHN-6` Slack dispatch — `app_mention`/IM/threaded `message` → existing thread session → deliver to sandbox `/kortix/prompt` (`delivered|transient|stale`); else `createProjectSession` (actor=owner, agent `default`, `visibility:'project'` — channel sessions are team-visible) + record `chat_threads`. A fresh self-host preview has no third-party Slack installation. Its explicit contract is `404 {"error":"Not configured"}` at the project install gate.
