...
`SESS-25` Server-side prompt inbox. `POST /projects/:id/sessions/:sid/prompts` with a client-minted OpenCode wire `message_id` → `202 {prompt_id, state:'queued', message_id, deduped:false}`; the prompt is a DURABLE row in `kortix.session_lifecycle_commands` from that instant, so it survives a closed tab, a second device, and a crash. A `message_id` OpenCode cannot order → `400` (a mis-ordered id reads as already answered and the turn silently never runs). Re-POSTing the SAME `client_message_id` → `200 {deduped:true}` naming the SAME `prompt_id`, enforced by the unique index on `idempotency_key`. The FIFO key is `(lane, clientSentAtMs, wireMessageId, commandId)`, where `lane` is 1 only for an explicit `placement: 'composer'` and 0 otherwise, with `created_at` substituted when an older producer has no client timestamp; listing, admission, batch delivery, strand repair, and terminal promotion use that same tuple. `GET .../prompts` lists what the session still owes the user — `queued` / `waiting` (with `reason`: `turn_active`, `older_prompt_pending`, or `held`) / `delivering` (with `reason: 'forwarded'` once it is at OpenCode but no turn has consumed it yet) / `failed`; a CONSUMED prompt is omitted, because it is in the transcript; the same answer carries `placed`, the pairings `{prompt_id, client_message_id, wire_message_id, message_id, message_ids, placed_at}` of consumed prompts that left the list within the last ten minutes and were delivered under an id other than their `wire_message_id`, so a client that painted a bubble under the wire id retires it even when it never observed the row re-minted (automation rows are never listed there; every id a re-delivered prompt went out under is in `message_ids`, and a client announces each). A steered prompt is placed in SEND order: its delivered id sorts above every open sibling that was sent BEFORE it (a lifted earlier steer included), and below its client id only when every open sibling above was sent AFTER it by both the sender's Enter stamp and the server's `created_at` (a sibling with no inbox row — an automation prompt — keeps the under-placement). A live turn holds Queue List and unplaced prompts, and every Quick Queue prompt that does not steer, until its terminal relay promotes exactly one FIFO head. Worker claims do not change Waiting to Sending. Completion wakes skip burst coalescing; a completion that races admission requeue is rechecked after the write. A confirmed active turn clears the pending UI without waiting for the next inbox poll. The optional `placement` is `transcript` or `composer`; both execute automatically. `transcript` (Quick Queue) sorts ahead of every `composer` row and acts on the active response even while older Queue List rows wait. Its FIFO head STEERS: it is forwarded into the running turn and the model reads it at the next step boundary (a tool call ends a step). A response that is STREAMING — an open text or reasoning part in a step with no pending or running tool — has no step boundary, so a head submitted at or after that turn started ends the response immediately and runs as the next turn (a step with a pending or running tool steers whatever its reasoning state); a head that was already waiting before the turn started never ends it, and a turn with no recorded start is never ended: that prompt waits for the turn to end. Every consecutive pending Quick Queue row of one session that is deliverable now is delivered as ONE GROUP, in the FIFO tuple's order: rows 1..N-1 are posted with `noReply` (OpenCode persists the user message and starts no reply) and row N is posted normally, so N messages produce exactly ONE reply, parented on row N and addressing all N. Each message keeps its own bubble and its own strictly increasing wire id; nothing changes on the client. A group mints as one: if any row of it waited, every row of it is re-minted above the transcript and no row of it is under-placed, so a row lifted out of a live turn can never sort above a later row of its own group. Row N alone carries a hidden `synthetic` instruction naming the group size, which no surface renders. Rows 1..N-1 open no turn and close `delivered` the moment OpenCode accepts them. An unplaced row and a HELD row are never grouped and end the group rather than being skipped; a group is ONE placement, so the first `composer` row ends a Quick Queue group. A group is a CONTIGUOUS run of the session's rows in the FIFO tuple, never a set of rows that happen to be in one claim: a drain that holds any inbox row of a session claims every queued inbox row of that session, whatever its `available_at`, and a row it cannot claim ends the group rather than being skipped over, so no prompt is ever delivered out of send order or left unanswered because its neighbours were grouped around it. If a `noReply` post fails, the group stops there: row N is never posted, and that row and everything after it go back in line. Steering places the whole group into the live turn the same way, and a session that has already been steered holds two recorded turns — a later Quick Queue head steers into the NEWEST one rather than waiting for the response to end. So a burst of Quick Queue prompts loses at most the first successor's partial answer. An unreadable runtime steers, including a phase read that takes longer than 6 s. A prompt that arrives in the gap after a reasoning part ends and before the next part opens steers; if that next part is text, it is read only after the text finishes. Ending a response does not bind what the model writes next: a response stopped before it wrote any answer text leaves no answer in the transcript, so the next turn may answer the cancelled request as well. `composer` (Queue List) waits for the active response to finish naturally, is never forwarded into a running turn, and never ends a response; when the response finishes, every waiting `composer` row is delivered together as ONE grouped turn (rows 1..N-1 `noReply`, row N opens the one reply that addresses all of them, in order). Omitted placement preserves the composer queue. The list returns lossless `full_text` alongside the capped `text` preview. Enter uses **Quick Queue**, a muted pending conversation message; Command+Enter (Ctrl+Enter on Windows/Linux) uses **Queue List**, an editable composer entry. Waiting Quick Queue entries follow delivered turns as tinted bubbles with no status text. A second Enter paints and submits before the first acceptance response returns; a repeated keypress with an empty editor submits nothing. Each entry receives its own response, except Quick Queue entries, which are grouped: one reply answers the whole group, and a steered group is answered by the turn it steered. If the terminal relay is missing, the queue head verifies exact terminal evidence before delivery. Prompt delivery retries run independently of cron leadership and claim only their owning local instance. Shift+Enter adds a newline. Editing removes only the selected entry; resubmission goes to the queue tail. Placement survives reload, retry, and removal/undo. A Queue List row and a waiting Quick Queue bubble both offer **Remove from queue** by one rule: while the inbox lists the prompt as `queued` or `waiting` (any `reason`, `held` included), never while it is `delivering`, unconfirmed, or has a row action in flight; the session's first prompt offers none, and a `failed` Quick Queue bubble keeps Remove on its failure line. The bubble's control sits in its action row (shown on hover, keyboard focus, and always on a touch screen). Either removal shows the "Removed from queue" notice with Undo for 5 s; Undo of a Quick Queue prompt restores it as a Quick Queue bubble under a fresh wire id, at the back of its lane. A restored prompt is a NEW row created at Undo time: unless it was held, it steers into the turn running at that moment, and it counts as submitted at Undo time, so as the head it ends a response that is streaming text even when the removed prompt was waiting before that turn started. While Stop is available the transcript shows exactly one Thinking row: under a prompt the inbox is delivering, otherwise above any queued bubbles; without Stop it shows none. Stop holds that prompt after reload. A delivery failure displays its readable cause and Retry; an unconfirmed Stop displays a notice. An automation's own `continue_session` row (trigger, Slack, approval resume) is never listed, deletable, or retryable here. `POST .../prompts/:promptId/retry` is the ONE primitive behind retry and "send now": it re-queues a `queued`/`failed`/`dead_lettered` row — or a STOP-PAUSED one, which is the row the hold is rendered on — RE-MINTED above the transcript (a stale id reads as already answered), promotes it past the ordering gate, releases the session's hold, and answers the row with `observed_at` stamped after the write. A row that does not exist → `404 {code:'prompt_not_found'}`; a row already on the wire or closed by the drain → `409 {code:'prompt_already_sent'}`. A row that has already been POSTed goes out under a NEW idempotency key, so the proxy's 10-minute delivery claim cannot answer it `duplicate` and swallow it. `POST .../prompts/hold {held:boolean}` is what the Stop button writes — every queued prompt of the session reads `waiting`/`held` until an explicit send or send-now releases it; a non-boolean is `400`. The server applies the same hold on its own before the abort reaches the runtime: for every client-requested abort of the session's root runtime session (web, SDK, mobile, CLI) and in `POST .../stop` before it aborts the box. The hold is ONE statement over queued, claimed and forwarded rows, so a row the drain moves between `running` and `queued` while the hold runs is still held, and a held composer row is never claimed whatever its `available_at`. A prompt the runtime accepted before the Stop but no step read comes back held when a repair (turn-end strand reconcile, reaper) re-queues it while the hold stands; a stopped session is never woken by such a requeue. The hold carries the Stop instant on the client's clock (`stopped_at_ms`); a send whose `client_sent_at_ms` precedes it was typed before the Stop, so when its POST lands after the hold it is created held and releases nothing. `DELETE .../prompts/:promptId` removes a prompt that has not gone out, or a forwarded one no model step has read, and answers `200 {removed}` with the full body and `held` (true for a held or stop-paused row). A prompt the drain has claimed but not yet POSTed is removed outright and never reaches the runtime; the drain's pre-POST commit and the delete are ordered by the row lock, and a row claimed together with a Quick Queue sibling takes the forwarded path instead. A forwarded, unread prompt is deleted from the runtime whole while the loop is idle; while the loop runs, its parts are deleted, and the part-less copy is deleted at the next turn end. No client draws a part-less user message, and a reply parented on one belongs to the turn before it. Every refusal carries a stable `code`: a prompt that does not exist, was already removed, or belongs to another session → `404 prompt_not_found`; one the drain sent or skipped, or a step is answering → `409 prompt_already_sent`; one still being delivered whose runtime cannot be reached → `409 prompt_cancel_unreachable`. A refusal after the cancel re-reads the row, so the loser of two concurrent removes never answers `prompt_cancel_unreachable`. A session the caller cannot see stays a plain `404` with no code. Undo re-POSTs the removed prompt with `restore: true` and its `held` bit: a restore never releases the session hold, and `held: true` re-creates the row held and not due.

`SESS-29` Prompt admission and durable Stop. A prompt is accepted `202` even while the session's stored `required_connectors` column still names an unconnected connector — there is no session-level connector pre-flight; a connector gate applies only at CALL time (`CONN-ACCOUNTS`). A disabled optional connector binding likewise does not hold a prompt. `PUT .../scope` still accepts `require_connectors` on the wire (an old client must not get `400`) and is a no-op: `GET`/`PUT .../scope` both report `required_connectors: null` always. Stop marks a claimed delivery `waiting/held` immediately; a fresh GET preserves that state. The server holds the queue before the abort is forwarded, so a Stop never delivers a queued prompt, whatever order the client's own hold and abort arrive in. Resume clears both the public hold and delivery-stop marker. The delivery worker checks Stop before each POST and preserves permanent refusal messages without retrying them.
