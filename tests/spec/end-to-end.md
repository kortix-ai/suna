...
`PROJ-37` `PUT` and `DELETE /projects/:id/model-defaults` check access in one order: an invisible project → 404, a caller without `project.customize.write` → 403, then a project with the LLM gateway off → 404 `llm_gateway_disabled`. The same caller and project state get the same status from both verbs. A project manager with the gateway on sets the project default (200), reads it back, clears it (200), and reads back `projectDefault: null`.
