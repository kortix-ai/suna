...
## 12b. Subprojects (source of truth = one `kortix-<slug>.yaml` file per subproject, beside the root manifest)

A subproject is a named container inside a project: it groups sessions (one file each, `kortix-<slug>.yaml`), owns the triggers that name it, and is an IAM object granted to members/groups exactly like an agent (closed by default — a member with no grant row sees none). The database holds only `project_sessions.subproject` (the session join) and the grant rows (`resource-grants`, `resource_type: 'subproject'`); every CRUD write commits `kortix.yaml`. `sessions: private` (default) keeps the ordinary per-session visibility model; `sessions: shared` makes every session in the subproject readable by everyone granted it, regardless of that session's own visibility (lifecycle rights unchanged). Deleting a subproject strips `subproject:` from any trigger naming it and orphans its grant rows (still flagged, never silently reinterpreted); session rows keep their `subproject` column. The local test profile cannot provision a real session (`503 KORTIX_URL_UNREACHABLE`), so a session-create body carrying `subproject` is asserted at the subproject validation/authorization boundary only — never through to a running sandbox (`MKTP-11` precedent).

`SUBP-1` CRUD — `POST /projects/:id/subprojects {name(required),slug?,description?,agent?,sessions?}` → `manage` + `project.customize.write` → `201`, slug derived from `name` via `slugify` when omitted, `sessions` defaults `private`; the write commits `kortix-<slug>.yaml` — the created object's `path` is that file and `agents` is `[]` (proven by reading the commit back through `GET /projects/:id/commits`). `GET /projects/:id/subprojects` lists the caller's accessible set; `GET /projects/:id/subprojects/:slug` reads one. `PATCH` merges partial fields (`description` + `sessions: shared` persist); an empty `PATCH {}` → `200` with no new commit. A body field this version does not have — `instructions`, `context`, both dropped 2026-09-07 — is `400`, never silently swallowed. Duplicate slug → `409 SUBPROJECT_SLUG_TAKEN`; unknown `agent` → `400`; invalid slug → `400`; bad `sessions` value → `400`. `DELETE /projects/:id/subprojects/:slug` → `200`, then `GET` → `404`.

`SUBP-2` authz — a floor `member` (`team.grantProjectRole(...,'member')`) with no grant: `GET /subprojects` → `[]`, `GET /subprojects/:slug` → `404` (undeclared and inaccessible are the same answer), `POST /sessions {subproject}` → `403 subproject_not_accessible` with `accessible_subprojects: []`. A manager grants it via the generic surface — `POST /projects/:id/resource-grants {resource_type:'subproject',resource_id,principal_type:'member',principal_id}` → `201` — after which the member's list/get both succeed and `POST /sessions {subproject}` clears the subproject gate (asserted as "not the `subproject_not_accessible` 403"; the local profile's next boundary is whatever the ordinary agent-access gate returns, since granting a subproject does NOT imply the agent grant). `GET /resource-grants` shows `resources.subprojects` and the grant row. A member without `project.customize.write` gets `403` on `PATCH`/`DELETE`. `POST /sessions {subproject:'nope'}` → `400 SUBPROJECT_NOT_DECLARED`. `ANON` → `401`.

`SUBP-3` sessions filter/hiding + `sessions: shared` — rows are seeded directly (`project_sessions.subproject`, the local profile cannot provision a real session) rather than through the API. A subproject the viewer holds no grant for hides its rows entirely from `GET /sessions?subproject=<slug>`, even a row the viewer themself created — the IAM gate runs before the ordinary visibility fold. After a grant, the viewer's own row in a `private` subproject becomes visible; in a `sessions: shared` subproject, another member's `private`-visibility row becomes visible too, once granted. `?subproject=<slug>` and `?subproject=` (rows with none) are both accepted query filters and return `200` arrays.

`SUBP-4` triggers — `POST /projects/:id/triggers {...,subproject:<slug>}` → `201`, and `GET /triggers` shows it; `subproject` must name a declared subproject or `400 SUBPROJECT_NOT_DECLARED`. `PATCH {enabled:false}` (an unrelated field) keeps the existing `subproject` — PATCH merges onto the current spec, so only an explicit `subproject` key changes it. `PATCH {subproject:null}` clears it. Deleting the subproject first strips `subproject:` from every trigger naming it (a root-manifest commit), then removes the file (a second commit), so the manifest set is valid after each (`GET /triggers` then shows `subproject: null`).

`SUBP-5` CLI (`apps/cli/src/commands/subprojects.ts`) as real processes — `kortix subprojects create "<name>" [--description] [--agent] [--sessions] --json` → the created object; `ls --json` lists it; `show <slug> --json` reads it; `update <slug> --sessions shared --json` persists the mode change; `rm <slug> --yes` removes it (exit `0`) and a subsequent `show` exits `1`. `kortix sessions new|ls --subproject <slug>` and `kortix triggers add|set --subproject <slug>` carry the same field through to the wire.

`SUBP-6` subproject-owned agents (spec 2026-09-06) — `kortix-marketing.yaml` declares `agents.writer` (and `agent: writer` as its default) and `kortix-sales.yaml` references it with `agents.writer: { from: marketing }`; both files are committed straight into the project's local bare repository (no API route writes an `agents:` block), followed by one API write (`POST /subprojects {name:'Ops'}`) that invalidates the git mirror. `GET /subprojects/:slug` then reports `agents: ['writer']` for marketing and sales and `[]` for ops; `GET /projects/:id/detail` `config.agents` marks `writer` with `subproject: 'marketing'` and `kortix` with `null`. `POST /sessions {agent_name:'writer'}` at the project level → `400 AGENT_NOT_IN_SUBPROJECT` with `usable_agents: ['kortix']`; the same inside `marketing` (owner) or `sales` (reference) passes that gate (the local profile's next boundary is whatever the ordinary create path returns), and inside `ops` → `400 AGENT_NOT_IN_SUBPROJECT`. A send into `marketing` with no agent fills in its own default `writer` and passes. `POST /triggers {agent:'writer'}` without a subproject → `400 AGENT_NOT_IN_SUBPROJECT`; with `subproject:'marketing'` → `201`.

`SUBP-7` moving a session — `PATCH /projects/:id/sessions/:sessionId {subproject}` files an existing session under a subproject, moves it between two, and (`{subproject: null}`) back to the project level; each move is proven by the response body and by `GET /sessions?subproject=<slug>` / `?subproject=` afterwards. Rows are seeded directly (`project_sessions.subproject`, the local profile cannot provision a real session) and each caller moves their OWN row, since the move takes the sharing gate (`can_manage_sharing`) — a `shared` subproject makes its sessions readable by everyone granted it. An undeclared target → `400 SUBPROJECT_NOT_DECLARED`. A member with no grant moving their own session into one → `403 subproject_not_accessible`; after `POST /resource-grants {resource_type:'subproject'}` the same move → `200`.
