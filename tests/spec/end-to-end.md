...
`SESS-28` `PUT /projects/:id/sessions/:sid/model` rejects anonymous callers with 401 and nonmembers with 403. A Pi session rejects a valid model change with `409 SESSION_MODEL_FIXED_AT_START` before metadata changes or runtime calls; a subsequent session GET preserves all metadata. Pi models remain fixed at session creation. A queued OpenCode session accepts a native model with 200, reports `applied_live: false`, and persists the selected model for its next start.
