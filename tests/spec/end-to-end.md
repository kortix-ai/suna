...
6. A task-worker `git-receive-pack` acquires one PostgreSQL request lease on its bound doing task. A concurrent receive-pack loses admission. Terminal transition, second no-progress escalation, ledger-failure blocking, and liveness sweep cannot commit before the live lease settles or reaches the immutable worker deadline. A server-owned request UUID owns settlement; client request IDs never own the fence.
7. Every terminal worker transaction revokes all active `account_tokens` rows with the worker `session_id` before it queues the stop command. The worker therefore receives 401 on subsequent Kortix API and connector calls during the stop delay. Ordinary non-worker Git behavior is unchanged.
