...
`PROJ-4` `POST /projects/create-repo {name,private?}` (new GitHub repo) → `PROJECT_CREATE` → 201; no account GitHub App install → 409 + `install_url`; a personal GitHub account (`owner_type: User`) → 409 `code:github_personal_account_create_unsupported` before any GitHub call, because GitHub rejects an App installation token on `POST /user/repos`; auto-dedupes name collision.
