...
`PROJ-3` `POST /projects/provision {name,provider?}` (managed) → `PROJECT_CREATE` → 201 `{push_token,repo_id,repo_url}`. `repository_source_project_id` reuses one connected managed repository. The API creates the requested isolated `default_branch` with server-managed credentials. The derived project stores `managed:false`, so only the source project owns upstream deletion. Unconfigured managed backend → 503.
...
`PROJ-31` `PATCH /projects/:id/sandbox-provider {provider}` → 200 discriminated union. An immediate pin returns `{kind:"project",default_sandbox_provider}`. A cross-provider change returns `{kind:"preparation",target_provider,immediate:false}` for polling. Unknown or disabled provider → 400.
`PROJ-33` `GET /projects/:id/sandbox-provider/transition` → 200 public transition state. The response excludes lease tokens, provider credentials, raw provider errors, snapshot names, and external template identifiers.
...
`SESS-12` `POST /projects/:id/sessions/:sid/stop` → `session` (then **owner or project manager** only) → **200** status `stopped`, sandbox paused in place (disk kept, no re-provision — same contract as an idle auto-stop); repeated stop while already `stopped` is idempotent and returns the same **200** state; resumable via `/start`/`SESS-9`. Sandbox in another non-active state → 409; unsupported provider → 400.
