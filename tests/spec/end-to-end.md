...
`CFG-4` The API chooses the config mode from the workspace report in the descriptor request. No report → `mode: "follow-base"` with the archive. Platform output only — `package.json` differing from base by the `@opencode-ai/plugin` pin (text in `package_json`), `bun.lock`, and a file under a managed-overlay skill dir (`skills/kortix-system/`) — → `follow-base`, the same `release_id`, `reason: null`. A file whose blob equals a base-branch revision → `follow-base`. A real agent edit → `mode: "session-files"`, `archive: null`, `files: null`, the base `compiled_governance`, the same `release_id`. An added dependency, a new skill, and a deleted base file each → `session-files`. `committed_scope: "none"` with no changes → `follow-base` and a `reason` naming `committed_scope none`. 64-hex `head` and blob IDs → 200. An unknown `committed_scope` → 400.
