...
`SEC-POOL-6` ChatGPT account needs reconnection → two ChatGPT accounts of a plain project member list with `needs_reauth_at: null`. A real gateway turn from the member's session-bound token on a `codex/*` model, with a stored login the API cannot read, returns `400 provider_reauth_required` naming the member's newest account and marks only that account with an ISO timestamp. The next turn prefers the unmarked older account and names it; a repeated failure keeps the first timestamp. With both accounts selected for the session, the error names both. The owner's list shows both marks. The member deletes both accounts.
