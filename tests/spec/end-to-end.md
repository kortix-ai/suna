...
`GW-5` `GET /projects/:id/llm-catalog/providers` returns the LIVE provider-level catalog (id/name/env/doc/models per provider) from the same 24h-refreshed runtimeModelCatalog `/llm-catalog` and `/model-picker` read — the source the web connect modal now consumes instead of a baked snapshot. Like `/llm-catalog`/`/model-picker`, not gated by any project flag — gateway mode is the only session mode and the retired `llm_gateway` key is rejected by `PATCH /projects/:id/features` with 400 — → 200; project nonmember → 403/404; ANON → 401.
