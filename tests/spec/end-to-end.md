...
`GH-17` `GET /projects/:id/git/connection` → `read` → `{state,reason?,install_url?}`. State is one of `connected|reconnect_required|unavailable|not_connected`; only an account-fixable reconnect carries `install_url`. `ANON` → 401, non-member → 403/404, unknown project → 404.
