...
`MON-1` OWNER enables `monitoring` through `PATCH /projects/:id/features`, creates a session, then `PUT /projects/:id/sessions/:sid/stage {stage:'ready',needs_approval:true,note:'…'}` → `session` (any project member, session must be visible) → **403 `code:'stage_agent_only'`** — a person is not the session's agent. `GET …/sessions/:sid` still reads `stage:null` (the card stays in Backlog). Only the session's own sandbox token (`callerKortixSessionId`) moves a card freely → 200 serialized session with `stage.value`, `stage.needs_approval`, `stage.note`, `stage.updated_at`, `stage.updated_by:'agent'`; a later `{stage:'in_progress'}` replaces the object whole. A person's only allowed writes are the approval decision on a card parked in `ready` with `needs_approval:true`: `{stage:'in_progress'}` (approve) or `{stage:'planning'}` (send back) → 200 with `needs_approval:false` (the approval is consumed) and `updated_by` = the user id. (`daytona`+`funded` — needs a real session; the agent-token path has no black-box flow.)
...
**Question backstop (documented boundary, verified live 2026-09-04).** The sandbox's question relay (`POST /projects/:id/sessions/:sid/question`, sandbox token, `question.asked`) also parks the asking session's card in `ready` + `needs_approval` with the first question's header/text (≤200 chars) as the note — unless the agent already parked it or moved it to `review`/`done`; the web answer (`POST …/question`, human) moves a parked card to `in_progress` stamped with the answerer. Both are no-ops when `monitoring` is off. Pure decision functions: `apps/api/src/projects/lib/session-stage-backstop.ts` (unit-tested).
