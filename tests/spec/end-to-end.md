...
`CFG-8` The `config_releases` feature flag. On by default: the project's serialized `experimental.config_releases` is `true` and the session receives a release. `PATCH /v1/projects/:projectId/features {feature:"config_releases",enabled:false}` → the descriptor route answers `403 {code:"feature_disabled",feature:"config_releases"}` for the sandbox token AND for the owner (`ANON` still → 401), the archive route answers the same `403`, `GET .../config` carries no `release` block and `stale: null`, and no `kortix.config_releases` row is written by a descriptor request or a config read. A base-branch commit while the flag is off changes nothing. `enabled:true` → the descriptor names the NEW base tip as `source_commit` with `mode:"follow-base"`, and its archive downloads. `enabled:null` clears the override and the project returns to the ON default.
