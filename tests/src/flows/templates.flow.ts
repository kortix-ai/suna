/**
 * Templates — the public template catalog (`/v1/public/templates`)
 * and the per-project install (`/v1/projects/:projectId/templates/install-session`).
 *
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step(
      ...
      async () => {
        const r = await ctx.client.as(ctx.P.ANON).get('/v1/public/templates');
    ...
    await ctx.step('the catalog revalidates → 304 on a matching ETag', async () => {
      const r1 = await ctx.client.as(ctx.P.ANON).get('/v1/public/templates');
      r1.status(200);
      const again = await ctx.client.as(ctx.P.ANON).get('/v1/public/templates', {
    ...
    await ctx.step('`q` narrows the catalog to templates that say the word', async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get('/v1/public/templates', { query: { q: first.title.toUpperCase() } });
      r.status(200);
      ...
      const miss = await ctx.client
        .as(ctx.P.ANON)
        .get('/v1/public/templates', { query: { q: 'no-such-template-zzz' } });
      miss.status(200);
    ...
    await ctx.step('one template by slug → 200 with the same card', async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get(`/v1/public/templates/${first.slug}`);
...
// ─── TMPL-2 — the feature gate ───────────────────────────────────────────────
flow(
  'TMPL-2',
  { domain: 'projects', routes: ['POST /v1/projects/:projectId/templates/install-session'] },
  async (ctx) => {
    // No `templates` in metadata: the flag is OFF, and the install must fail
    ...
    await ctx.step('the flag is off → 403 feature_disabled, not a session', async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .post(
          '/v1/projects/:projectId/templates/install-session',
          { slug: 'sre-oncall' },
          { params: { projectId: project.id } },
        );
      r.status(403).body().has('$.code', 'feature_disabled').has('$.feature', 'templates');
    });
...
flow(
  'TMPL-3',
  {
    ...
    routes: [
      ...
      'POST /v1/projects/:projectId/templates/install-session',
    ],
  },
  async (ctx) => {
    const project = await ctx.fixtures.project({
      managedGit: true,
      metadata: { experimental: { templates: true } },
    });
    let slug = '';
