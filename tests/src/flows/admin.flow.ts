...
flow(
  "ADM-1",
  { domain: "admin", routes: ["GET /v1/admin/api/accounts"] },
  async (ctx) => {
    ...
    if (ctx.env.capabilities.admin) {
      await ctx.step("platform admin lists accounts → 200 page", async () => {
        const r = await ctx.client
          ...
          .get("/v1/admin/api/accounts", { query: { limit: "5" } });
        r.status(200);
      });
    }
  },
...
flow(
  "ADM-2",
  { domain: "admin", routes: ["GET /v1/admin/api/accounts/:id/users"] },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/admin/api/accounts/:id/users", { params: { id: NOPE } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      await ctx.step(
        "platform admin reads an account's users → 200",
        async () => {
          const r = await ctx.client
            .withBearer(ctx.env.adminToken!, "ADMIN_TOKEN")
...
flow(
  ...
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/admin/api/accounts/:id/projects", { params: { id: NOPE } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    await ctx.step("canonical Workspace route keeps the same admin gate", async () => {
      const r = await ctx.client
        .as(ctx.P.OWNER)
        .get("/v1/admin/api/accounts/:id/workspaces", { params: { id: NOPE } });
      r.status(403);
    });
    if (ctx.env.capabilities.admin) {
      await ctx.step(
        "platform admin reads an account's workspaces → 200 {workspaces:[]}",
        async () => {
          const r = await ctx.client
            .withBearer(ctx.env.adminToken!, "ADMIN_TOKEN")
            .get("/v1/admin/api/accounts/:id/workspaces", {
      ...
      await ctx.step(
        "platform admin reads an account's projects → 200 {projects:[]}",
        async () => {
          const r = await ctx.client
            .withBearer(ctx.env.adminToken!, "ADMIN_TOKEN")
            .get("/v1/admin/api/accounts/:id/projects", {
...
flow(
  "ADM-3",
  { domain: "admin", routes: ["GET /v1/admin/api/accounts/:id/ledger"] },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/admin/api/accounts/:id/ledger", { params: { id: NOPE } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
...
flow(
  "ADM-4",
  {
    domain: "admin",
    serial: true,
    routes: ["POST /v1/admin/api/accounts/:id/credits"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "admin grants credits → 200 {ok:true, balance}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/credits",
...
flow(
  "ADM-5",
  {
    domain: "admin",
    serial: true,
    routes: ["POST /v1/admin/api/accounts/:id/credits/debit"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "admin debits credits → 200 {ok:true, balance}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/credits/debit",
...
flow(
  "ADM-6",
  { domain: "admin", serial: true, routes: ["PUT /v1/system/maintenance"] },
  async (ctx) => {
    ...
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .put("/v1/system/maintenance", { level: "none" });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      const admin = ctx.client.withBearer(ctx.env.adminToken!, "ADMIN_TOKEN");
      await ctx.step(
        "admin updates maintenance config → 200 (then restores none)",
        async () => {
          const r = await admin.put("/v1/system/maintenance", {
            level: "none",
            title: "",
            message: "",
          });
...
flow(
  "ADM-7",
  { domain: "admin", routes: ["GET /v1/admin/api/provider-analytics"] },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
    ...
    if (ctx.env.capabilities.admin) {
      await ctx.step(
        "platform admin reads provider analytics → 200 aggregate",
        async () => {
          const r = await ctx.client
            .withBearer(ctx.env.adminToken!, "ADMIN_TOKEN")
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step("ANON → 401 (PUT)", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .put("/v1/admin/api/provider-fallback", { enabled: false });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403 (PUT)", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "admin writes back the SAME enabled flag (no-op) → 200",
        async () => {
          const before = await admin.get("/v1/admin/api/provider-fallback");
          before.status(200);
          const enabled = before.json().enabled === true;
          const r = await admin.put("/v1/admin/api/provider-fallback", {
            enabled,
          });
...
flow(
  "ADM-10",
  { domain: "admin", routes: ["GET /v1/admin/api/sandboxes"] },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
...
flow(
  ...
  {
    domain: "admin",
    routes: ["POST /v1/admin/api/sandboxes/:sessionId/migrate"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      const admin = ctx.client.withBearer(ctx.env.adminToken!, "ADMIN_TOKEN");
      await ctx.step(
        "admin: invalid targetProvider → 400 (validated before any lookup)",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/sandboxes/:sessionId/migrate",
      ...
      await ctx.step(
        "admin: unknown session id with a valid provider → 404, no real sandbox ever touched",
        async () => {
...
flow(
  "ADM-12",
  {
    domain: "admin",
    serial: true,
    routes: ["POST /v1/admin/api/accounts/:id/tier"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "admin re-applies the OWNER account's current tier (no-op) → 200",
        async () => {
          ...
          const currentTier: string =
            before.json()?.subscription?.tier_key || "free";
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/tier",
            { tier: currentTier },
            { params: { id: ctx.P.OWNER.accountId! } },
...
flow(
  ...
  async (ctx) => {
    ...
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .post("/v1/admin/api/accounts/:id/trial", grant, {
          params: { id: NOPE },
        });
      r.status(401);
    });
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "admin: tier_key that is not a PAID tier → 400",
        async () => {
          const r = await admin.post(
      ...
      await ctx.step(
        "admin: duration_days beyond the 365-day cap → 400",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/trial",
      ...
      await ctx.step(
        "admin grants a 30-day 'team' trial → 200 {ok:true, trial:{status:'active'}}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/trial",
      ...
      await ctx.step(
        "the accounts-list row reports the active trial",
        async () => {
...
flow(
  ...
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .del("/v1/admin/api/accounts/:id/trial", { params: { id: NOPE } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      const team = await ctx.fixtures.team({
        name: ctx.fixtures.name("adm15-trial"),
      });

      await ctx.step(
        "admin revokes on an account that never had a trial → 400",
        async () => {
          const r = await admin.del("/v1/admin/api/accounts/:id/trial", {
            params: { id: team.id },
          });
          r.status(400);
        },
      ...
      await ctx.step(
        "admin grants a trial to revoke → 200 active",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/trial",
      ...
      await ctx.step(
        "admin revokes it → 200 {ok:true, trial:{status:'revoked'}} (tier/seats kept)",
        async () => {
          const r = await admin.del("/v1/admin/api/accounts/:id/trial", {
            params: { id: team.id },
          });
...
flow(
  ...
  {
    domain: "admin",
    routes: ["POST /v1/admin/api/accounts/:id/managed-models"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      const admin = ctx.client.withBearer(ctx.env.adminToken!, "ADMIN_TOKEN");
      const team = await ctx.fixtures.team({
        name: ctx.fixtures.name("adm16-mm"),
      });
      ...
      await ctx.step(
        "admin grants managed models → 200 {ok:true, override:true}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/managed-models",
            { override: true },
      ...
      await ctx.step(
        "admin forces BYOK-only → 200 {ok:true, override:false}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/managed-models",
            { override: false },
      ...
      await ctx.step(
        "admin clears the override → 200 {ok:true, override:null}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/managed-models",
            { override: null },
...
flow(
  ...
  async (ctx) => {
    ...
    if (ctx.env.capabilities.admin) {
      const admin = ctx.client.withBearer(ctx.env.adminToken!, "ADMIN_TOKEN");
      const team = await ctx.fixtures.team({
        name: ctx.fixtures.name("adm17-demo"),
      });
      ...
      await ctx.step(
        "admin enables the demo → 200 {ok:true, enabled:true}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/enterprise-demo",
            { enabled: true },
      ...
      await ctx.step(
        "the account-scoped IAM read reflects it → 200 true",
        async () => {
          const r = await ctx.client
            .as(ctx.P.OWNER)
            .get("/v1/accounts/:accountId/iam/enterprise-demo", {
      ...
      await ctx.step(
        "admin disables it again → 200 {ok:true, enabled:false}",
        async () => {
          const r = await admin.post(
            "/v1/admin/api/accounts/:id/enterprise-demo",
            { enabled: false },
...
flow(
  ...
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/admin/api/accounts", { query: { limit: "1" } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "a never-granted account carries the entitlement columns at their defaults",
        async () => {
          const r = await admin.get("/v1/admin/api/accounts", {
            query: { search: name, limit: "5" },
          });
...
// ADM-20 — the fleet workspaces list. Asserted against a fresh run-owned workspace
...
flow(
  ...
  {
    domain: "admin",
    routes: ["GET /v1/admin/api/workspaces", "GET /v1/admin/api/projects"],
  },
  async (ctx) => {
    await ctx.step("ANON → 401", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
        .get("/v1/admin/api/workspaces", { query: { limit: "1" } });
      r.status(401);
    });
    await ctx.step("non-admin OWNER → 403", async () => {
      const r = await ctx.client
    ...
    if (ctx.env.capabilities.admin) {
      ...
      const workspace = await ctx.fixtures.project({ name });

      await ctx.step(
        "a never-run workspace carries the account join and zeroed session counts",
        async () => {
          const r = await admin.get("/v1/admin/api/workspaces", {
            query: { search: name, limit: "5" },
          });
          r.status(200)
            .body()
            ...
            .has("$.page", 1)
            .has("$.limit", 5)
            .has("$.workspaces[0].workspaceId", workspace.id)
            .has("$.workspaces[0].name", name)
            .has("$.workspaces[0].status", "active")
            .has("$.workspaces[0].accountId", ctx.P.OWNER.accountId!)
            .has("$.workspaces[0].ownerEmail", ctx.P.OWNER.email!)
            .has("$.workspaces[0].sessionCount", 0)
            .has("$.workspaces[0].activeSessionCount", 0)
            .has("$.workspaces[0].lastSessionAt", null)
            .exists("$.workspaces[0].createdAt");
        },
      );

      await ctx.step(
        "search also matches the owning account's member email",
        async () => {
          const r = await admin.get("/v1/admin/api/workspaces", {
            query: {
              search: ctx.P.OWNER.email!,
              accountId: ctx.P.OWNER.accountId!,
              limit: "100",
            },
          });
          r.status(200).body().exists("$.workspaces[0].workspaceId");
        },
      );

      await ctx.step(
        "every sortBy is accepted, in both directions",
        async () => {
      ...
      await ctx.step(
        "status filter selects and excludes; an unknown value degrades to no filter",
        async () => {
          const active = await admin.get("/v1/admin/api/workspaces", {
            query: { search: name, status: "active", limit: "5" },
          });
          active.status(200).body().has("$.total", 1);

          const archived = await admin.get("/v1/admin/api/workspaces", {
            query: { search: name, status: "archived", limit: "5" },
          });
          archived.status(200).body().has("$.total", 0);

          const bogus = await admin.get("/v1/admin/api/workspaces", {
      ...
      await ctx.step(
        "a non-uuid accountId narrows to an empty page, never to the fleet",
        async () => {
          const r = await admin.get("/v1/admin/api/workspaces", {
            query: { accountId: "not-a-uuid", limit: "5" },
          });
          r.status(200).body().has("$.total", 0).has("$.workspaces", []);
        },
      );
      ...
      await ctx.step(
        "deprecated Project alias preserves the legacy response",
        async () => {
          ...
          r.status(200)
            .body()
            .has("$.projects[0].workspaceId", workspace.id)
...
flow(
  "ADM-19",
  {
    ...
    serial: true,
    global: true,
    routes: ["POST /v1/billing/cron/trial-expiry"],
  },
  async (ctx) => {
    ...
    await ctx.step(
      "wrong bearer → 401 (never the real internal key)",
      async () => {
        const r = await ctx.client
...
flow(
  "ADM-21",
  { domain: "admin", routes: ["GET /v1/admin/analytics/activity"] },
  async (ctx) => {
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "platform admin reads the activity series → 200",
        async () => {
          const r = await admin.get("/v1/admin/analytics/activity", {
            query: { days: "7" },
          });
          r.status(200)
            .body()
          ...
          if (days.length !== 7) {
            throw new Error(
              `days=7 should return 7 dense entries, got ${days.length}`,
            );
          }
          for (const key of [
            "date",
          ...
          if ([...dates].sort().join() !== dates.join()) {
            throw new Error(
              `day entries are not ascending: ${dates.join(",")}`,
            );
          }
        },
      );
      ...
      await ctx.step("days is clamped to [1,90], never rejected", async () => {
        ...
        if (zero !== 1)
          throw new Error(`days=0 should clamp to 1 entry, got ${zero}`);
        const huge = await count("9999");
        if (huge !== 90)
          throw new Error(`days=9999 should clamp to 90 entries, got ${huge}`);
        const junk = await count("abc");
        if (junk !== 30)
          throw new Error(
            `days=abc should fall back to 30 entries, got ${junk}`,
          );
      });
    }
  },
...
flow(
  "ADM-22",
  { domain: "admin", routes: ["GET /v1/admin/analytics/usage"] },
  async (ctx) => {
    await ctx.step("ANON → 401 (inherited gate)", async () => {
      const r = await ctx.client
        .as(ctx.P.ANON)
    ...
    if (ctx.env.capabilities.admin) {
      ...
      await ctx.step(
        "platform admin reads the credit-burn series → 200",
        async () => {
          const r = await admin.get("/v1/admin/analytics/usage", {
            query: { days: "7" },
          });
          r.status(200)
            .body()
