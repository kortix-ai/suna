...
flow(
  ...
  async (ctx) => {
    ...
    for (const explicitKey of [false, true]) {
      const mode = explicitKey ? 'explicit idempotency key' : 'message identity';
      const options = explicitKey ? { headers: { 'Idempotency-Key': crypto.randomUUID() } } : undefined;
      ...
      await ctx.step(`${mode}: POST .../session/<ocId>/prompt_async → 204`, async () => {
        const r = await ctx.client
          .as(ctx.P.OWNER)
          .post(ocPath(sandboxId, `/session/${ocId}/prompt_async`), prompt, options);
        r.status(204);
      ...
      await ctx.step(`${mode}: repeating the messageID preserves the runtime admission contract`, async () => {
      ...
      await ctx.step(
        `${mode}: changed content under the same messageID cannot create a second prompt`,
        async () => {
