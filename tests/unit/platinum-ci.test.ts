...
describe('Platinum CI worker plan', () => {
  test('requires Docker only for lanes that start the local stack', () => {
    expect(sandboxWorkerNeedsDocker([])).toBe(true);
    expect(sandboxWorkerNeedsDocker(['--browser-only'])).toBe(true);
    expect(sandboxWorkerNeedsDocker(['--packages-only'])).toBe(false);
    expect(sandboxWorkerNeedsDocker(['--sdk-only'])).toBe(false);
  });
