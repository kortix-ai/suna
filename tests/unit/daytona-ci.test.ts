...
describe("Daytona CI worker plan", () => {
  test("accepts bounded provider identifiers and rejects report injection", () => {
    expect(providerMetadataIdentifier("sandbox:abc-123", "sandbox ID")).toBe(
      "sandbox:abc-123",
    );
    expect(() =>
      providerMetadataIdentifier("sandbox\nforged=1", "sandbox ID"),
    ).toThrow("invalid sandbox ID");
    expect(() =>
      providerMetadataIdentifier("x".repeat(129), "sandbox ID"),
    ).toThrow("invalid sandbox ID");
  });

  test("uses one content-addressed warm snapshot for one lockfile", () => {
    expect(DAYTONA_CI_SNAPSHOT_VERSION).toBe("v8");
    expect(daytonaSnapshotName(lockHash)).toBe(
      "kortix-ci-daytona-v8-bbbbbbbbbbbbbbbb",
    );
    expect(daytonaBaseSnapshotName(lockHash)).toBe(
      "kortix-ci-daytona-v3-bbbbbbbbbbbbbbbb-base",
    );
  });
  ...
  test("starts and removes the local stack before capturing the warm snapshot", () => {
    const script = buildDaytonaWarmScript();
    expect(script).toContain("pnpm exec supabase start --ignore-health-check");
    expect(script).toContain("pnpm exec supabase stop --no-backup");
  ...
  test("uses the unchanged root test command inside Daytona", () => {
    ...
    expect(script).toContain("start_daytona_docker warm-overlay");
    expect(script).toContain(
      "--data-root=/workspace/daytona-vfs --storage-driver=vfs",
    );
    expect(script).toContain("start_daytona_docker vfs-fallback");
    expect(script).not.toContain("tests/test-results/platinum");
  });

  test("does not start Docker for the package-only worker", () => {
    const script = buildWorkerScript({
      repository: "kortix-ai/suna",
      ref: sha,
      sha,
    ...
    expect(script).toContain("[daytona-ci] docker_skipped=1");
    expect(script).not.toContain("nohup dockerd");
    expect(script).not.toContain("docker network inspect bridge");
  });
  ...
  test("validates provider input before making requests", () => {
    ...
    expect(() => validateDaytonaCiInput({ ...valid, apiKey: "" })).toThrow(
      "DAYTONA_API_KEY",
    );
    expect(() =>
      validateDaytonaCiInput({ ...valid, target: "us west" }),
    ).toThrow("invalid Daytona target");
  });
  ...
  test("retries rate limits and provider outages with bounded backoff", async () => {
    ...
    expect(
      isRetryableDaytonaError(new DaytonaHttpError("rate limit", 429)),
    ).toBe(true);
    expect(
      isRetryableDaytonaError(new DaytonaHttpError("bad request", 400)),
    ).toBe(false);
  });
