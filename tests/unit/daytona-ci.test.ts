...
describe('Daytona CI worker plan', () => {
  ...
  test('uses one content-addressed warm snapshot for one lockfile', () => {
    ...
    expect(daytonaSnapshotName(lockHash)).toBe('kortix-ci-daytona-v8-bbbbbbbbbbbbbbbb');
  ...
  test('uses the unchanged root test command inside Daytona', () => {
    ...
    expect(script).toContain('--data-root=/workspace/daytona-vfs --storage-driver=vfs');
