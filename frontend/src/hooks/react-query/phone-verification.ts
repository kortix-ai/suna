...
export const useListFactors = () => {
  ...
  return useQuery({
    ...
    enabled: false, // Disabled for Cognito auth - phone verification is Supabase-specific
...
export const useGetAAL = () => {
  ...
  return useQuery({
    ...
    enabled: false, // Disabled for Cognito auth - MFA is handled by Cognito
    staleTime: Infinity, // 1 minute
    retry: 0, // Don't retry - this feature not used with Cognito
  });
};
