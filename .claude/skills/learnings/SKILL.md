...
`x-forwarded-host` against `origin` and aborts the action when they differ:
`Invalid Server Actions request`, HTTP 500, surfaced in the browser only as
**minified React error #441**. It kills EVERY Server Action, not the one you
are looking at — on the preview stack that meant nobody could sign in, because
the whole auth flow is Server Actions.
...
server's** stdout, so it is only reachable from the container
...
(2) A minified React error with no stack is a SERVER error: fetch the server's
own log before reading any client code. (3) `digest` is a hash, not a message —
grep the container for it.

*Incident:* every `preview`-labelled PR was impossible to sign into. Fixed by
pinning the host in `buildPreviewCaddyfile`.
