...
### File hooks share SDK cache keys; history replay ends with current status (2026-09-14)
...
### Pinned commits must work through every checkout transport (2026-09-15)
...
**Enforcement.** `SESS-35` exercises refusal, Stop, fresh GET, and Resume through
...
requirements. `SESS-35` stores a disabled bound connector, admits a prompt through
...
### Preserve pinned agent configuration in prompt preflight

**Near-miss (2026-09-16, pi-worker integration):** merging the connector admission
gate made every prompt read the current project branch. Pi sessions execute an
artifact pinned to an older commit. Six regression cases failed before the fix.

**Rule:** read Pi connector requirements from the persisted runtime SHA and agent.
Never fall back to the current branch when Pi identity is incomplete. Continue
to check current connection availability, explicit session requirements, and IAM.
OpenCode sessions retain their existing current-branch behavior.

**Enforcement:** `prompt-connector-preflight-pi.test.ts` checks pinned source, agent,
missing identity, branch removal, requirement union, and unavailable source.
`SESS-31` changes agent requirements through HTTP, checks the unpinned refusal,
and verifies that the pinned Pi session still persists one attachment prompt.

### Reconfigure readiness without inheriting an idle timer

**Near-miss (2026-09-16, pi-worker integration):** the full daemon suite caught
an OpenCode reconfigure remaining `starting` for five seconds. Its live process
already answered, but the previous idle probe interval remained scheduled.

**Rule:** replace the idle timer with a startup probe when configuration changes.
An in-flight probe from the previous configuration cannot mark the new one ready
or schedule another probe loop. Reconfigure before start must not start probing.
