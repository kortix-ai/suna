...
**Second follow-up (dev, 2026-09-25):** a visibility change is a scope change too. `PUT /sessions/:id/sharing` answered 200 for a private session whose model ran on a key granted only to its owner; the shared session then answered `400 INVALID_SESSION_MODEL` to its own model. A share now resolves the scope against the pending visibility (`resolveSessionPersonalOwner({ visibility })`): a model that loses its keys switches to every key shared with the whole project, or the share is refused with `409 SHARED_SESSION_NEEDS_PROJECT_KEY`. Still open: the first prompt from another human (an admin continuing a private session under session oversight) clears `on_behalf_of` for good and narrows the scope the same way, with no check (`projects/lib/on-behalf-of.ts`).

**Enforcement:** `unit-channel-model-access.test.ts` (a follow-up in a shared session is checked with `personalUserId: null`), `unit-channel-vision-model.test.ts` (probe inputs and cache key carry the scope), `default-model.test.ts` (a shared session's default is checked without personal keys), `unit-session-model-keys.test.ts` (model change and share), `provider-secret-pools.test.ts` (a shared session refuses a member-granted key), flow `SEC-POOL-2` (`403 SHARED_SESSION_PERSONAL_KEY`, `400 INVALID_SESSION_MODEL`, `409 SHARED_SESSION_NEEDS_PROJECT_KEY`, the switch to the project's key).
