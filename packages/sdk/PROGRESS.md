...
| B14 `defaultControls` restore-or-cut — did the composer need a footer restored? | Jay   | **Applied the brief's own pre-authorized fallback (WS5-P0-c): "cut", not "restore".** No architecture work was needed either way — grep-verified `modelDefaultControls` had zero live call sites in the composer before this task (see B14's row and the session log). If this reasoning is wrong (e.g. another surface _was_ relying on it reaching the composer), flag here for a real restore pass; nothing was silently dropped — the flag-ON and flag-OFF composer both render zero default-controls footer today. |
...
### 2026-07-20 — session `acp-harness-runtime-v2`: D2 reload-drop fix (Task W5) — CLAIM

Claiming Task W5 / Backlog **B11**: persisted synthetic outputs `show` dropped on
reload of an error-terminated ACP run. Root cause reproduced from the real codex
stream (`8023ee8f…`) and minimized: `reduceEnvelope`'s `session/load`
...
`name` _argument_ like `project_create({ name, description })` is not mistaken
...
suffix rule drops any _unrecognized_ `*_update` kind before the `raw` fallback.
...
### 2026-07-23 — session `pr-4510-sdk-readiness` (completion)

Added deadline-based `ensureReady()` polling and progress callbacks. Concurrent
callers share a start driver only when the session, deadline, and poll interval
match. Different readiness policies keep independent deadlines. Progress
callbacks cannot cancel readiness when a host callback throws or rejects. The
session types now expose the server-derived `origin` and backend-only
`origin_ref` fields.

**TDD evidence:** the different-deadline test first failed because one shared
driver applied the first caller's policy to both callers. The final focused
client suite reported **65 pass / 0 fail** with 239 assertions.

**Final SDK gates:** `pnpm typecheck` exited 0. The full SDK suite reported
**1348 pass / 0 fail** across 98 files with 5622 assertions.
`pnpm smoke:install` built, packed, installed, imported, and constructed
`@kortix/sdk` successfully.

**Shippable to production: YES** for the SDK readiness and KaaB type slice.
PR-wide CI and human review remain outside this SDK verdict.

---

### 2026-07-23 — session `pr-4510-sdk-readiness-audit` (claim)

Reopened the readiness slice after final review found three defects. Different
caller policies can duplicate one session's `/start` request. A stalled request
can exceed `deadlineMs`. Non-finite options can create an unbounded poll loop.
This correction will keep one start driver per session, enforce caller-specific
deadlines, reject non-finite options, and finish with all SDK gates.

**Status:** IN PROGRESS.

---

### 2026-07-23 — session `pr-4510-sdk-readiness-audit` (completion)

Readiness now uses one driver per `(projectId, sessionId)`. Each caller keeps
its own deadline, poll interval, and progress callback. A late caller receives
the current progress value and can wake a slower poll delay. The last caller
aborts a stalled request. `stop()`, `restart()`, and `delete()` cancel the
driver and reject stale ready responses before they update the runtime
registry. The HTTP client composes caller abort signals with its timeout
signal. Non-finite readiness options fail before any request.

**TDD evidence:** the focused RED run reported three failures. The client
ignored caller abort signals. A late caller waited 200ms behind the first
caller's poll interval. `stop()` allowed a stale start response to complete.
The focused GREEN run reported **86 pass / 0 fail** with 292 assertions.

**Final SDK gates:** `pnpm typecheck` exited 0. The full SDK suite reported
**1354 pass / 0 fail** across 98 files with 5638 assertions.
`pnpm smoke:install` built, packed, installed, imported, and constructed
`@kortix/sdk` successfully.
...
### 2026-07-23 — session `pr-4510-sdk-readiness-portability-audit` (main merge addendum)

Merged `origin/main` at `643756043` after GitHub reported the PR as
conflicting. The only textual conflict was this append-only log. The resolution
preserves both the PR readiness entries and main's GitHub-selector entries.

**Post-merge SDK gates:** `pnpm typecheck` exited 0. The full SDK suite reported
**1360 pass / 0 fail** across 98 files with 5657 assertions. `pnpm
smoke:install` built, packed, installed, imported, and constructed
`@kortix/sdk` successfully. The local landing visual suite reported **2 pass /
0 fail**.
