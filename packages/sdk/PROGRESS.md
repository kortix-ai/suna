...
- `pnpm --filter @kortix/sdk test`: `2230 pass`, `0 fail`, `10063 expect()` calls across `183` files.
...
Discovered, not fixed (see _Discovered this session_): `apps/mobile` and
...
directive; an _unused_ `@ts-expect-error` is itself a typecheck error, so the
...
its shape changed (interface → type alias) but the exported _name_ didn't,
...
_Assistant Turn UX_); no Linear identifiers appear in code, branches, or commits.
...
Read plainly: _"if this page holds any real user message at all, every optimistic
user message is a duplicate."_ In an ongoing conversation that is always true —
...
- `pnpm --filter @kortix/sdk test`: `1396 pass`, `0 fail`, `5985 expect() calls`
  across `117` files.
- `pnpm --filter @kortix/sdk run smoke:install`: exit `0` — _"OK: @kortix/sdk
  imports and constructs from a packed tarball"_.
...
Bug work, not part of the **Now** chain. Linear JAY-286 (_Assistant Turn UX_).
...
Left on the ticket rather than folded in — see _Found work mid-task_.
...
Linear JAY-290 (_Assistant Turn UX_), plus a **gap in JAY-285 found while doing
...
Linear JAY-287 (_Assistant Turn UX_). Fourth entry in the send-reliability
...
- `session.prompt` (v2) → `/api/session/{id}/prompt` — _"Durably admit one
  session input and schedule agent-loop execution"_ — with
...
read: _"with exactly ONE optimistic message in flight there is nothing to be
ambiguous about, so retire it"_ — regardless of `dispatched`.
