...
| B43 | **Preserve and classify `AGENT_SWITCH_REQUIRES_NEW_SESSION` prompt failures.** The OpenCode response carries `error`, `code`, `expected_agent`, and `requested_agent`, but `promptOpenCodeMessage` prefers a missing `message` field and emits the generic `Failed to send message`. Hosts cannot render the required new-session recovery. | Production session `c887b2f0-712d-4cb2-afce-51c68204aec9` returned `409` with `expected_agent=default` and `requested_agent=kortix`; `/ecs/kortix-prod` logged nine `AgentSecretGrantMismatchError` refusals. `src/react/use-opencode-sessions/messages.ts:324` discards the response `error` field. | **IN PROGRESS 2026-07-31** — session `agent-switch-conflict`; RED SDK tests, typed classification, web recovery action, local HTTP/browser proof, PR, merge, Deploy Dev, and deployed proof required. |
