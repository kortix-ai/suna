...
| B8  | **Retire the experimental project-app deployment SDK surface with the Freestyle-backed platform capability.** This is intentionally subtractive because the user explicitly requested complete removal of the underlying capability. | `src/core/rest/projects-client/apps.ts` and `src/core/client/kortix.ts` expose the project-app CRUD/deploy surface; public-surface snapshots lock the current names. | **IN PROGRESS 2026-07-13** — session `remove-freestyle`; RED-first removal contract and full SDK gates required |
