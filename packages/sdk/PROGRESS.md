...
| B13 | **Keep an initialized session's SSE and message recovery active when a boot-only repository gate later reports `runtimeReady: false`.** | Live session `8383ad27-bf58-4989-a865-4b13fdef3596` returns `opencode: "ok"`, `runtimeReady: false`, and a working `HTTP 200` `/global/event`, while `useRuntimeReconnect`, `useOpenCodeEventStream`, and `useSessionSync` convert that boot-readiness value into a transport shutdown. | **IN PROGRESS 2026-07-23** — session `session-stream-recovery`; RED-first SDK and web regression coverage required |
