...
| B14 `defaultControls` restore-or-cut — did the composer need a footer restored? | Jay | **Applied the brief's own pre-authorized fallback (WS5-P0-c): "cut", not "restore".** No architecture work was needed either way — grep-verified `modelDefaultControls` had zero live call sites in the composer before this task (see B14's row and the session log). If this reasoning is wrong (e.g. another surface *was* relying on it reaching the composer), flag here for a real restore pass; nothing was silently dropped — the flag-ON and flag-OFF composer both render zero default-controls footer today. |
