...
| B7  | **Hidden `auto` assumes every platform default belongs to the `kortix` provider.** Normalize provider-qualified platform defaults such as `codex/gpt-5.6` into the correct `ModelKey`. | `src/react/use-opencode-local.ts:197` hardcodes `{ providerID: 'kortix', modelID: AUTO_DEFAULT_MODEL_ID }`. | **IN PROGRESS** — session `gateway-fallbacks`, 2026-07-12 |
