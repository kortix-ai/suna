...
| `kortix.accounts` | list · get · create · members · invites · `tokens.{list,create,revoke}` (account-scoped CLI PATs, `kortix_pat_…`) · `audit.{log,export,webhooks.*}` (filterable project/session reconstruction log) · `branding.{get,uploadAsset,removeAsset,reset}` (Enterprise organization branding: logo / icon / favicon, light + dark) (+ more: `updateName`, `leave`, `invite`, `removeMember`, `updateMemberRole`) |
