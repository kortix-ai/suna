...
export const useSyncStore = create<SyncState>()((set, get) => ({
	...
	buildSessionMessages: (sessionID, msgs, parts) => {
		...
		const rebuilt: MessageWithParts[] = [];
		for (const info of msgs) {
			const messageParts = parts[info.id];
			partRefs.push(messageParts);
			rebuilt.push({ info, parts: messageParts ?? [] });
		}
		// Hand back the PREVIOUS wrapper object for every message whose `info`
		// and `parts` are reference-unchanged.
		//
		// The cache above is all-or-nothing: one streamed part invalidates it and
		// every message gets a brand new `{info, parts}` wrapper. Downstream that
		// looks like "every message changed", so `groupMessagesIntoTurns` cannot
		// reuse a single turn object, and the transcript's per-turn memoization
		// degrades to recomputing every turn on every token — O(all messages) per
		// token on a long thread.
