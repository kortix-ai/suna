import type { WorkingProjection } from '../../session/working';
...
export interface BusyRowInput<T extends TurnLike, P extends BusyRowPrompt> {
  ...
  /** The runtime is parked on a question or a permission prompt: no row draws
   *  anywhere, because the session is waiting on the user, not working. */
  awaitingUser?: boolean;
}
...
export function resolveBusyRow<T extends TurnLike, P extends BusyRowPrompt>(
  ...
  // A busy session always draws one row. The boot stand-in draws its own. A
  // session parked on a question is the one case where zero rows is right, so
  // it is excluded here rather than catching the row the working turn declined.
