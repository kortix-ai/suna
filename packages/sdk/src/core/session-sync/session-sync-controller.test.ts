...
describe('SessionSyncController', () => {
  ...
  test('a tail read that never settles is abandoned after readTimeoutMs and later reconciles read again', async () => {
    ...
    try {
      ...
      expect(calls).toBe(1);
      // While the read is inside its budget, a turn-end reconcile does not
      // issue a second read: it is owed ONE follow-up read after this one.
      const owed = controller.reconcile('turn-end');
      expect(calls).toBe(1);
      ...
      // The released slot issues the owed read, and it lands.
      await owed;
      expect(calls).toBe(2);
      expect(hydrated).toEqual([['message-2']]);

      // A later reconcile is not held by the abandoned read either.
      await controller.reconcile('turn-end');
      expect(calls).toBe(3);
      expect(hydrated).toEqual([['message-2'], ['message-2']]);
