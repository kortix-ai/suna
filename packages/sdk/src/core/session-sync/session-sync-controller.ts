...
export class SessionSyncController {
  ...
  private async loadPage(
    ...
    try {
      const page = await this.boundedRead((signal) =>
  ...
  /**
   * Run one read under its own signal: aborted when the controller is
   * destroyed, and when `readTimeoutMs` elapses. The timeout also rejects the
   * returned promise directly, so a loader that ignores its signal still
   * releases the single-flight slot.
   */
  private boundedRead<T>(read: (signal: AbortSignal) => Promise<T>): Promise<T> {
