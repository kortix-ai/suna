...
test('catalog uses the workspace-scoped route and the shared token seam', async () => {
  const result = await getConnectorCatalog('workspace/one');

  expect(result).toEqual(catalog.connectors);
  expect(calls[0]?.url).toBe(
    'http://test.local/v1/connectors/workspaces/workspace%2Fone/catalog',
  );
...
test('the platform fetch seam supports compatibility clients without replacing global fetch', async () => {
  ...
  expect(await getConnectorCatalog('workspace-one')).toEqual([]);
...
test('tools flatten the catalog into stable connector.action identifiers', async () => {
  const tools = await listConnectorTools('workspace-one');
...
test('search matches non-contiguous query tokens and applies the limit', async () => {
  expect((await searchConnectorTools('workspace-one', 'channel history')).map((tool) => tool.tool))
    .toEqual(['slack.get_history']);
  expect(await searchConnectorTools('workspace-one', '', { limit: 1 })).toHaveLength(1);
});

test('describe resolves one tool and returns null for an unknown tool', async () => {
  expect(await describeConnectorTool('workspace-one', 'slack.send_message')).toMatchObject({
    connector: 'slack',
    action: 'send_message',
  });
  expect(await describeConnectorTool('workspace-one', 'slack.missing')).toBeNull();
});
...
test('call accepts one tool identifier and sends the canonical gateway payload', async () => {
  ...
  expect(calls[0]?.url).toBe(
    'http://test.local/v1/connectors/workspaces/workspace-one/call',
  );
...
test('call rejects an invalid tool identifier before making a request', async () => {
  await expect(callConnector('workspace-one', 'send_message')).rejects.toThrow(
...
test('call preserves an asynchronous approval handoff', async () => {
  ...
  expect(
    await callConnector('workspace-one', 'gmail.send_email', {
...
test('attachment upload sends raw bytes through the shared token seam', async () => {
  ...
  const result = await uploadConnectorAttachment('workspace/one', bytes, {
    filename: 'chart.png',
  ...
  expect(calls[0]?.url).toBe(
    'http://test.local/v1/connectors/workspaces/workspace%2Fone/attachments',
  );
  expect(calls[0]?.method).toBe('POST');
  expect(calls[0]?.headers.get('authorization')).toBe('Bearer agent-token');
...
test('gateway failures use the SDK ApiError with the server reason', async () => {
  ...
  try {
    await callConnector('workspace-one', 'slack.send_message');
