...
export interface ListAccountAuditOptions {
  ...
  until?: string;
  /** Case-insensitive action, resource, workspace, session, or correlation search. */
  q?: string;
...
/** Canonical workspace-scoped audit timeline. The API binds workspace scope server-side. */
export async function listWorkspaceAudit(workspaceId: string, options?: ListAccountAuditOptions) {
  const search = new URLSearchParams();
  if (options?.action) search.set('action', options.action);
  if (options?.actor) search.set('actor', options.actor);
  if (options?.actorType) search.set('actor_type', options.actorType);
  ...
  const qs = search.toString();
  return unwrap(
    await backendApi.get<AuditEventList>(`/workspaces/${workspaceId}/audit${qs ? `?${qs}` : ''}`),
  );
}
