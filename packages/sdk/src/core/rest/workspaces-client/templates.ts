...
/** Public — template detail + a requirement preview (built against an empty workspace). */
export async function getTemplate(id: string): Promise<TemplateDetail> {
  return unwrap(await backendApi.get<TemplateDetail>(`/templates/${encodeURIComponent(id)}`));
}

/** Apply a template into a workspace: renders inputs, merges the manifest, commits. */
export async function installTemplate(
  id: string,
  body: { workspace_id: string; inputs: Record<string, string> },
