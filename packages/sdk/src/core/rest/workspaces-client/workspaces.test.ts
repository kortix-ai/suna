...
test('ExperimentalFeatureKey includes the workspace-scoped Apps gate', () => {
...
test('GitHub workspace creation accepts a marketplace workspace template', () => {
  const createInput: CreateWorkspaceRepoInput = {
    account_id: 'acc-1',
    name: 'support-agent',
    source_item_id: 'kortix-workspaces:support-agent-kit',
  };

  expect(createInput.source_item_id).toBe('kortix-workspaces:support-agent-kit');
});

test('CreateWorkspaceRepoInput accepts an optional icon', () => {
  const createInput: CreateWorkspaceRepoInput = {
    account_id: 'acc-1',
    name: 'support-agent',
    icon: '🚀',
...
test('CreateWorkspaceRepoInput accepts an optional icon_glyph', () => {
  const createInput: CreateWorkspaceRepoInput = {
    account_id: 'acc-1',
    name: 'support-agent',
...
test('createWorkspaceRepo sends icon_glyph on the wire, same as provision and link-repository', async () => {
  // The type-level test above only proves `CreateWorkspaceRepoInput` accepts the
  // field on a locally-built object — it stays green even if `createWorkspaceRepo`
  // silently dropped `icon_glyph` before POSTing. This asserts what actually
  // reaches `fetch`, matching the wire assertion `provisionWorkspace` gets
  ...
  globalThis.fetch = mock(async (_input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ workspace_id: 'proj-1' }), {
      status: 200,
    ...
    async (_input: RequestInfo | URL, init?: RequestInit): Promise<Response> =>
      await new Promise<Response>((resolve, reject) => {
        const timer = setTimeout(
          () =>
            resolve(
              new Response(JSON.stringify({ workspace_id: 'proj-too-late' }), {
                status: 200,
                headers: { 'content-type': 'application/json' },
              }),
            ),
...
// getWorkspaceDetail goes through backendApi/unwrap — the same parsing path
// createWorkspace, getWorkspace, and updateWorkspace use to return a KortixWorkspace.
// provisionWorkspaceWithToken (covered above) bypasses backendApi entirely with
// its own explicit-token fetch, so it does not exercise this path.
test('a workspace response carries the icon through the backendApi/unwrap parsing path', async () => {
  configureKortix({
    backendUrl: 'http://backend.test/v1',
    getToken: async () => 'tok',
  });
  nextResponse = () =>
    new Response(
      JSON.stringify({
        workspace: { workspace_id: 'proj-1', name: 'Iconic', icon: '🚀' },
        config: {
  ...
  expect(detail.workspace.icon).toBe('🚀');
});

test('provisionWorkspace sends the icon in the request body', async () => {
  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
  ...
  await provisionWorkspace({ account_id: 'acc-1', name: 'Iconic', icon: '🚀' });

  expect(sentBody).toMatchObject({ icon: '🚀' });
});

test('a workspace response carries the icon through to KortixWorkspace', async () => {
  nextResponse = () =>
    new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'Iconic', icon: '🚀' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });

  const result = await provisionWorkspaceWithToken(opts, { ...input, icon: '🚀' });

  expect(result.ok).toBe(true);
  expect(result.ok && result.workspace.icon).toBe('🚀');
});

// ── B44: `icon` on the updateWorkspace body ────────────────────────────────────
//
// `PATCH /v1/workspaces/:workspaceId` reads THREE states off `icon`, and only the
// request body can tell them apart (apps/api/src/workspaces/routes/r5.ts):
//
...
// handed, not the object handed to `updateWorkspace`.

/** Runs `updateWorkspace` against a mocked fetch and returns what it sent. */
async function captureUpdate(input: Partial<WorkspaceInput>) {
  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
  ...
  globalThis.fetch = mock(async (target: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'Iconic', icon: null }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  }) as unknown as typeof fetch;

  await updateWorkspace('proj-1', input);
...
test('updateWorkspace PATCHes the workspace route', async () => {
  const sent = await captureUpdate({ name: 'Iconic' });

  expect(sent.method).toBe('PATCH');
  expect(sent.url).toBe('http://backend.test/v1/workspaces/proj-1');
});

test('updateWorkspace puts an explicit null icon on the wire, not an absent key', async () => {
...
test('updateWorkspace sends a chosen emoji verbatim', async () => {
...
test('updateWorkspace can send a name and an icon in one body', async () => {
 ...
 * direction fails if `KortixWorkspace['icon']` is ever narrowed to `string`.
 *
 * A clear is only useful if the caller can SEE that it happened — the workspace
 * card re-renders its lettered fallback from exactly this field. Found while
 * mutation-testing B44: narrowing `KortixWorkspace.icon` to `string` left every
 * runtime assertion here green, because `expect(x).toBeNull()` accepts any type.
 */
const workspaceIconAcceptsNull: KortixWorkspace['icon'] = null;

test('a workspace response with a null icon reaches the caller as null', async () => {
  expect(workspaceIconAcceptsNull).toBeNull();
  ...
  globalThis.fetch = mock(
    async () =>
      new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'Iconic', icon: null }), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      }),
  ) as unknown as typeof fetch;

  const workspace = await updateWorkspace('proj-1', { icon: null });

  expect(workspace.icon).toBeNull();
});

// ── B45: `icon_glyph` on the workspace types ───────────────────────────────────
//
...
// updateWorkspace body — omit leaves the stored glyph alone, `null` removes it,
// an object replaces it (and clears `icon` server-side; not this package's
// concern to prove, only to type correctly).

test('icon_glyph round-trips on a workspace read', async () => {
  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
  nextResponse = () =>
    new Response(
      JSON.stringify({
  ...
  const workspace = await getWorkspace('proj-1');

  expect(workspace.icon_glyph).toEqual({ name: 'Rocket', color: 'blue' });
});
...
test('idempotency_key is sent on provision', async () => {
  // POST /workspaces/provision mints a brand-new managed repo per call. The key
  // is how a caller makes a retry (second tab, reload, lost response) return
  // the workspace the first call already created instead of a duplicate — so it
  // has to survive `provisionWorkspace`'s body construction, not just typecheck.
  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
...
test('ProvisionWorkspaceInput accepts an optional idempotency_key', () => {
  const withKey: ProvisionWorkspaceInput = {
    account_id: 'acc-1',
    name: 'My First Workspace',
    idempotency_key: 'onboarding-8f1c0f6e-1a2b-4c3d-9e8f-000000000001',
  };
  const without: ProvisionWorkspaceInput = { name: 'My First Workspace' };
...
test('updateWorkspace distinguishes omitted icon_glyph from null', () => {
  // Same three-way contract `icon` already documents: omit to leave alone,
  // null to remove, an object to replace. `updateWorkspace`'s real body type is
  // `Partial<WorkspaceInput>` (see its signature below) — `WorkspaceInput` itself
  // requires `repo_url`, so that's what a bare `{}` would fail against.
  const omitted: Partial<WorkspaceInput> = {};
  const cleared: Partial<WorkspaceInput> = { icon_glyph: null };
  expect('icon_glyph' in omitted).toBe(false);
  expect(cleared.icon_glyph).toBeNull();
});

test('updateWorkspace puts an explicit null icon_glyph on the wire, not an absent key', async () => {
...
test('updateWorkspace sends a chosen glyph verbatim', async () => {
 ...
 * `workspaceIconAcceptsNull` above, found necessary during B44's mutation pass:
 * narrowing `KortixWorkspace.icon` to `string` left every runtime assertion
 * green because `expect(x).toBeNull()` accepts any type. Assigning INTO the
 * member is what pins it.
 */
const workspaceIconGlyphAcceptsNull: KortixWorkspace['icon_glyph'] = null;

test('a workspace response with a null icon_glyph reaches the caller as null', async () => {
  expect(workspaceIconGlyphAcceptsNull).toBeNull();

  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
  globalThis.fetch = mock(
    async () =>
      new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'Glyphic', icon_glyph: null }), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      }),
  ) as unknown as typeof fetch;

  const workspace = await updateWorkspace('proj-1', { icon_glyph: null });

  expect(workspace.icon_glyph).toBeNull();
});

// ── B-default-branch: `default_branch` on `ProvisionWorkspaceInput` ───────────
//
// Carried Minor: apps/web sends `default_branch` on provision and the server
// (apps/api/src/workspaces/routes/r1.ts:546) reads it, but the SDK's
// `ProvisionWorkspaceInput` never declared it — forcing a double-cast at the
// web call site. Additive only: a new optional field, no existing member
// touched.

test('ProvisionWorkspaceInput accepts an optional default_branch', () => {
  const withBranch: ProvisionWorkspaceInput = {
    name: 'My First Workspace',
...
test('provisionWorkspace sends default_branch on the wire', async () => {
  configureKortix({ backendUrl: 'http://backend.test/v1', getToken: async () => 'tok' });
  ...
  await provisionWorkspace({ account_id: 'acc-1', name: 'x', default_branch: 'develop' });
...
// ── provisionWorkspaceStream ───────────────────────────────────────────────────
//
// POST /workspaces/provision-stream reports the same create as provisionWorkspace,
// but as a series of data-only SSE frames (`data: {"type":…}\n\n`, no `event:`
// line — see apps/api/src/workspaces/routes/r1.ts). Frame parsing is line-by-line
...
describe('provisionWorkspaceStream', () => {
  test('is exported', () => {
    expect(typeof provisionWorkspaceStream).toBe('function');
  });

  test('reports each phase in order and resolves with the workspace', async () => {
    const seen: string[] = [];
    const body = [
      ...
      'data: {"type":"done","workspace":{"workspace_id":"p1","name":"suna-web"}}\n\n',
    ].join('');

    const workspace = await provisionWorkspaceStream(
      { name: 'suna-web' },
      (event) => {
    ...
    expect(workspace.workspace_id).toBe('p1');
  });
  ...
  test('rejects with the server error when the stream ends in an error event', async () => {
    ...
    await expect(
      provisionWorkspaceStream({ name: 'x' }, () => {}, { fetch: stubStreamingFetch([body]) }),
    ).rejects.toThrow('Owner or admin role required');
  });

  test('rejects when the stream closes with no terminal event', async () => {
    await expect(
      provisionWorkspaceStream({ name: 'x' }, () => {}, { fetch: stubStreamingFetch([]) }),
    ).rejects.toThrow();
  });
  ...
  test('reassembles a frame split mid-JSON across two chunks', async () => {
    const frame = 'data: {"type":"done","workspace":{"workspace_id":"p1","name":"suna-web"}}\n\n';
    const splitPoint = frame.indexOf('"workspace_id"') + 5; // land inside the JSON body
    ...
    expect(workspace.workspace_id).toBe('p1');
    expect(workspace.name).toBe('suna-web');
  });
  ...
  test('reassembles a frame split mid multi-byte UTF-8 character across two chunks', async () => {
    ...
    const name = 'suna-🚀';
    const frame = `data: ${JSON.stringify({ type: 'done', workspace: { workspace_id: 'p1', name } })}\n\n`;
    ...
    const workspace = await provisionWorkspaceStream(
      ...
      },
      {
        fetch: stubStreamingFetch([
          combinedChunk,
          'data: {"type":"done","workspace":{"workspace_id":"p1"}}\n\n',
        ]),
      },
    );
  ...
  test('wraps a JSON parse failure with context, and still cancels the reader', async () => {
    ...
    await expect(
      provisionWorkspaceStream({ name: 'x' }, () => {}, { fetch: stub }),
    ).rejects.toThrow('provisionWorkspaceStream: received an unparseable SSE frame');

    expect(cancelled).toBe(true);
  });

  // Guards the pre-stream denial path documented in
  // apps/api/src/workspaces/routes/r1.ts: an unauthorized caller gets a plain
  ...
  // ── Final-review FIX 1 ───────────────────────────────────────────────────
  //
  // Every failure `provisionWorkspaceStream` threw used to be a bare `new
  // Error(message)` — no `.status`, no `.code`, even though the server sends
  // both (`apps/api/src/workspaces/routes/r1.ts`'s error frame, and the
  ...
  test('FIX 1: an in-band error frame propagates status and code onto the thrown error', async () => {
    ...
    let caught: unknown;
    try {
      await provisionWorkspaceStream({ name: 'x' }, () => {}, { fetch: stubStreamingFetch([body]) });
    } catch (err) {
      caught = err;
    }
  ...
  test('FIX 1: a pre-stream denial (non-2xx, no stream ever opened) propagates status and code the same way', async () => {
    ...
    let caught: unknown;
    try {
      await provisionWorkspaceStream({ name: 'x' }, () => {}, { fetch: stub });
    } catch (err) {
      caught = err;
    }
    expect(caught).toBeInstanceOf(ApiError);
  ...
  test('a runtime switch over ProvisionStreamEvent.type reaches all three wire shapes', () => {
    const describeEvent = (event: ProvisionStreamEvent): string => {
      switch (event.type) {
        ...
        case 'done':
          return `done:${event.workspace.workspace_id}`;
        case 'error':
    ...
    expect(describeEvent({ type: 'done', workspace: { workspace_id: 'p1' } as KortixWorkspace })).toBe(
      'done:p1',
    );
...
async function captureFeatureCall(
  ...
  globalThis.fetch = mock(async (target: RequestInfo | URL, init?: RequestInit) => {
    request = { url: String(target), method: init?.method, body: String(init?.body ?? '') };
    return new Response(JSON.stringify({ workspace_id: 'proj-1', name: 'Flagged' }), {
      status: 200,
      headers: { 'content-type': 'application/json' },
    });
  }) as unknown as typeof fetch;
...
test('updateFeatureFlag PATCHes the canonical /features route', async () => {
  ...
  expect(sent.method).toBe('PATCH');
  expect(sent.url).toBe('http://backend.test/v1/workspaces/proj-1/features');
...
test('updateExperimentalFeature keeps its legacy /experimental wire path', async () => {
  ...
  expect(sent.url).toBe('http://backend.test/v1/workspaces/proj-1/experimental');
