// Review Center — per-workspace human-in-the-loop inbox (change requests, tool
...
export async function listReviewItems(
  ...
  return unwrap(
    await backendApi.get<{ review_items: ApiReviewItem[] }>(
      `/workspaces/${workspaceId}/review/items${qs ? `?${qs}` : ''}`,
...
export async function getReviewItem(workspaceId: string, reviewItemId: string) {
  return unwrap(
    await backendApi.get<{ review_item: ApiReviewItem }>(
      `/workspaces/${workspaceId}/review/items/${reviewItemId}`,
    ),
  );
}
...
export async function submitReviewItem(
  ...
  return unwrap(await backendApi.post<ApiReviewItem>(`/workspaces/${workspaceId}/review/items`, input));
}
...
export async function actReviewItem(
  ...
  return unwrap(
    await backendApi.post<ApiReviewItem>(
      `/workspaces/${workspaceId}/review/items/${reviewItemId}/act`,
      input,
    ),
  );
}
...
export async function bulkActReviewItems(
  ...
  return unwrap(
    await backendApi.post<{ updated: number; review_items: ApiReviewItem[] }>(
      `/workspaces/${workspaceId}/review/bulk`,
      input,
    ),
  );
}
