...
test('listSessionCosts serializes account, workspace, limit, and zero offset', async () => {
  nextResponse = page;
  ...
  expect(last()).toEqual({
    url: 'http://test.local/usage/session-costs?account_id=account-1&workspace_id=workspace-1&limit=25&offset=0',
    method: 'GET',
  });
...
test('getSessionCostRecord encodes the session id and serializes scope filters', async () => {
  ...
  expect(last()).toEqual({
    url: 'http://test.local/usage/session-costs/session%2Fwith%20space?account_id=account-1&workspace_id=workspace-1',
    method: 'GET',
  });
...
const workspaceCostPage = {
  ...
  offset: 0,
  next_offset: null,
} satisfies WorkspaceCostPage;

test('listCostByWorkspace targets the rollup route with account, window, sort and paging', async () => {
  nextResponse = workspaceCostPage;

  const result = await listCostByWorkspace({
    accountId: 'acct-1',
    from: '2026-07-01T00:00:00.000Z',
    to: '2026-08-01T00:00:00.000Z',
    sort: 'total_desc',
  ...
  expect(last()).toEqual({
    url:
      'http://test.local/usage/cost-by-workspace?account_id=acct-1' +
      '&from=2026-07-01T00%3A00%3A00.000Z&to=2026-08-01T00%3A00%3A00.000Z' +
      '&sort=total_desc&limit=10&offset=20',
    method: 'GET',
  });
  expect(result).toEqual(workspaceCostPage);
});

test('listCostByWorkspace omits the query string when no options are supplied', async () => {
  nextResponse = workspaceCostPage;

  await listCostByWorkspace();

  expect(last().url).toBe('http://test.local/usage/cost-by-workspace');
});

test('listCostByWorkspace accepts the workspace-only name_asc sort', async () => {
  nextResponse = workspaceCostPage;

  await listCostByWorkspace({ sort: 'name_asc' });

  expect(last().url).toBe('http://test.local/usage/cost-by-workspace?sort=name_asc');
});
...
test('getCostSummary forwards account, workspace, session scope and window', async () => {
  ...
  expect(last()).toEqual({
    url:
      'http://test.local/usage/cost-summary?account_id=acct-1&workspace_id=proj-1' +
...
test('costExportUrl builds the workspaces CSV export URL with format=csv', () => {
  const url = costExportUrl('workspaces', {
    accountId: 'acct-1',
    from: '2026-07-01T00:00:00.000Z',
    to: '2026-08-01T00:00:00.000Z',
    sort: 'total_desc',
...
test('costExportUrl builds the sessions CSV export URL with workspace and owner scope', () => {
  ...
  expect(url).toBe(
    'http://test.local/usage/session-costs?account_id=acct-1&workspace_id=proj-1' +
...
test('costExportUrl emits only format=csv when no options are supplied', () => {
  ...
  expect(costExportUrl('workspaces')).toBe('http://test.local/usage/cost-by-workspace?format=csv');
});

// `workspace_id`/`owner_id` have no meaning on `/cost-by-workspace` (it has no
// per-session filter), and `name_asc` has no meaning on `/session-costs` (a
// session has no workspace name to sort on, mirroring SESSION_COST_SORTS on the
...
test('costExportUrl and fetchCostExportCsv reject the wrong kind\'s fields at compile time', () => {
  // @ts-expect-error workspace_id has no meaning on the /cost-by-workspace route
  costExportUrl('workspaces', { workspaceId: 'proj-1' });
  // @ts-expect-error owner_id has no meaning on the /cost-by-workspace route
  costExportUrl('workspaces', { ownerId: 'user-9' });
  // @ts-expect-error name_asc is valid only for the workspace rollup, not sessions
  costExportUrl('sessions', { sort: 'name_asc' });
  // @ts-expect-error workspace_id has no meaning on the /cost-by-workspace route
  void fetchCostExportCsv('workspaces', { workspaceId: 'proj-1' });
  // @ts-expect-error name_asc is valid only for the workspace rollup, not sessions
...
// fetchWorkspaceArchive in ./files.ts does, per the architecture rule that
...
test('fetchCostExportCsv targets the workspaces rollup route for kind "workspaces"', async () => {
  let capturedUrl = '';
  globalThis.fetch = mock(async (url: unknown) => {
    capturedUrl = String(url);
    return new Response('workspace_id,total_cost\n', { status: 200 });
  }) as unknown as typeof fetch;

  await fetchCostExportCsv('workspaces', { accountId: 'acct-1' });

  expect(capturedUrl).toBe('http://test.local/usage/cost-by-workspace?account_id=acct-1&format=csv');
});

test('fetchCostExportCsv returns rowCap null when the response carries no row-cap header', async () => {
  globalThis.fetch = mock(
    async () => new Response('workspace_id,total_cost\n', { status: 200 }),
  ) as unknown as typeof fetch;

  const result = await fetchCostExportCsv('workspaces');
...
test('fetchCostExportCsv throws with the response body on a non-OK response', async () => {
  ...
  await expect(fetchCostExportCsv('workspaces')).rejects.toThrow('Forbidden');
});
