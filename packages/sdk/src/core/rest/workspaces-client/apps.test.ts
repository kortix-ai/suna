...
test('Apps publishes default-private access modes and grant inputs', () => {
  const modes: AppAccessMode[] = ['private', 'workspace', 'restricted', 'public', 'password'];
...
test('Apps CRUD uses the workspace-scoped API contract', async () => {
  const app = {
    app_id: 'app-1',
  ...
  expect(await listApps('workspace-1')).toEqual([app]);
  expect(last()).toMatchObject({ method: 'GET', url: 'http://backend.test/v1/workspaces/workspace-1/apps' });

  await createApp('workspace-1', { slug: 'demo', name: 'Demo' });
  expect(last()).toMatchObject({
    method: 'POST',
    url: 'http://backend.test/v1/workspaces/workspace-1/apps',
    body: { slug: 'demo', name: 'Demo' },
  });

  await getApp('workspace-1', 'app-1');
  expect(last().url).toBe('http://backend.test/v1/workspaces/workspace-1/apps/app-1');

  await updateApp('workspace-1', 'app-1', { name: 'Renamed' });
  expect(last()).toMatchObject({ method: 'PATCH', body: { name: 'Renamed' } });

  await deleteApp('workspace-1', 'app-1');
  expect(last().method).toBe('DELETE');
});

test('App access reads, updates, and creates a browser exchange URL through workspace-scoped REST routes', async () => {
  const policy = {
  ...
  expect(await getAppAccess('workspace-1', 'app-1')).toEqual(policy);
  expect(last()).toMatchObject({
    method: 'GET',
    url: 'http://backend.test/v1/workspaces/workspace-1/apps/app-1/access',
  });

  expect(await updateAppAccess('workspace-1', 'app-1', {
    mode: 'restricted',
  ...
  expect(await createAppAccessSession('workspace-1', 'app-1')).toEqual(session);
  expect(last()).toMatchObject({
    method: 'POST',
    url: 'http://backend.test/v1/workspaces/workspace-1/apps/app-1/access-session',
    body: {},
  });
});
...
test('artifact registration, finalization, and deployment preserve the wire spec', async () => {
  ...
  await registerAppArtifact('workspace-1', { kind: 'oci_image', image: 'ghcr.io/kortix/demo:1' });
  expect(last().body).toEqual({ kind: 'oci_image', image: 'ghcr.io/kortix/demo:1' });

  await finalizeAppArtifact('workspace-1', 'artifact-1', {
    sha256: 'a'.repeat(64),
    size_bytes: 42,
  });
  expect(last()).toMatchObject({
    method: 'POST',
    url: 'http://backend.test/v1/workspaces/workspace-1/apps/artifacts/artifact-1/finalize',
  });

  await createAppDeployment('workspace-1', 'app-1', {
...
test('archive upload sends immutable bytes to the signed URL and finalizes their SHA-256', async () => {
  ...
  await uploadAppArtifactArchive('workspace-1', bytes, {
...
test('deployment inspection, logs, lifecycle, and rollback use bound identifiers', async () => {
  ...
  await listAppDeployments('workspace-1', 'app-1');
  await getAppDeployment('workspace-1', 'app-1', 'deployment-1');
  await getAppDeploymentLogs('workspace-1', 'app-1', 'deployment-1', { after: 3, limit: 50 });
  expect(last().url).toBe(
    'http://backend.test/v1/workspaces/workspace-1/apps/app-1/deployments/deployment-1/logs?after=3&limit=50',
  );
  await startApp('workspace-1', 'app-1');
  await stopApp('workspace-1', 'app-1');
  await rollbackApp('workspace-1', 'app-1', 'deployment-1');
