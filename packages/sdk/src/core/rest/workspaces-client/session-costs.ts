...
import type { WorkspaceSessionStatus } from './sessions';
 ...
 * `apps/api/src/router/routes/usage.ts`). A session page has no workspace name
 * to sort on, so `name_asc` is deliberately excluded here.
 */
export type SessionCostSort = 'total_desc' | 'total_asc' | 'recent';

/**
 * The four sorts `GET /usage/cost-by-workspace` accepts (`WORKSPACE_COST_SORTS`
 * in the same route file) — every `SessionCostSort` plus `name_asc`, since a
 * workspace rollup row has a name to sort on.
 */
export type WorkspaceCostSort = SessionCostSort | 'name_asc';
...
// ── Workspace rollup — GET /usage/cost-by-workspace ────────────────────────────
// Mirrors `WorkspaceCostRow` / `WorkspaceCostPage` in
...
// attribute to any workspace is folded into the account-wide totals returned
...
export interface ListCostByWorkspaceOptions extends CostWindowOptions {
  accountId?: string;
...
export async function listCostByWorkspace(
  options: ListCostByWorkspaceOptions = {},
  ...
  if (options.limit != null) query.set('limit', String(options.limit));
  if (options.offset != null) query.set('offset', String(options.offset));
  return unwrap(await backendApi.get<WorkspaceCostPage>(`/usage/cost-by-workspace${suffix(query)}`));
}
...
// ── CSV export ────────────────────────────────────────────────────────────
// `GET /usage/cost-by-workspace` and `GET /usage/session-costs` (format=csv)
...
// token, fetch, return a Blob — the same pattern `fetchWorkspaceArchive` in
// `./files.ts` already uses for the workspace-archive download (it calls
...
/** Options accepted by `costExportUrl('workspaces', …)` / `fetchCostExportCsv('workspaces', …)`.
 *  No `workspaceId`/`ownerId` — `/cost-by-workspace` has no such query param. */
export interface WorkspaceCostExportOptions extends CostWindowOptions {
  accountId?: string;
  sort?: WorkspaceCostSort;
}

/** Options accepted by `costExportUrl('sessions', …)` / `fetchCostExportCsv('sessions', …)`.
 *  `sort` is `SessionCostSort`, not `WorkspaceCostSort` — `name_asc` is only
 *  valid on the workspace rollup (SESSION_COST_SORTS excludes it on the API,
...
export type CostExportOptions = WorkspaceCostExportOptions | SessionCostExportOptions;
...
export function costExportUrl(kind: 'workspaces', options?: WorkspaceCostExportOptions): string;
...
export function costExportUrl(
  ...
  options: WorkspaceCostExportOptions | SessionCostExportOptions = {},
): string {
  const query = new URLSearchParams();
  if (options.accountId) query.set('account_id', options.accountId);
  if (kind === 'sessions') {
    const sessionOptions = options as SessionCostExportOptions;
    if (sessionOptions.workspaceId) query.set('workspace_id', sessionOptions.workspaceId);
    if (sessionOptions.ownerId) query.set('owner_id', sessionOptions.ownerId);
  }
  appendWindow(query, options);
  if (options.sort) query.set('sort', options.sort);
  query.set('format', 'csv');
  const path = kind === 'workspaces' ? '/usage/cost-by-workspace' : '/usage/session-costs';
 ...
 * mirrors `fetchWorkspaceArchive` in `./files.ts`: resolves the current token
...
export function fetchCostExportCsv(
  kind: 'workspaces',
  options?: WorkspaceCostExportOptions,
...
export async function fetchCostExportCsv(
  ...
  // non-narrowed 'workspaces' | 'sessions' variable directly). This also
  ...
  const url =
    kind === 'workspaces'
      ? costExportUrl('workspaces', options as WorkspaceCostExportOptions)
