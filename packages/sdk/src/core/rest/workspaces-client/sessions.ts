 ...
 * metadata-only create. Callers that rotate workspace secrets or connector
...
/** @deprecated Use `WorkspaceOpenCodeSession`. */
export type WorkspaceRuntimeSession = WorkspaceOpenCodeSession;

/** @param options.scope - `workspace` asks for the manager-only full inventory. */
 ...
 *
 * Await any workspace-secret or connector-credential mutations that the runtime
 * must observe before calling this function. `startWorkspaceSession` does not
...
export async function ensureWarmWorkspaceSession(workspaceId: string) {
  const result = unwrap(
    await backendApi.post<WarmWorkspaceSessionResult>(`/workspaces/${workspaceId}/sessions/warm`, {}),
  );
...
export async function claimWarmWorkspaceSession(
  ...
  const session = unwrap(
    await backendApi.post<WorkspaceSession>(`/workspaces/${workspaceId}/sessions/warm/claim`, input, {
      showErrors: false,
    }),
  );
 ...
 * Visible to anyone who can see the session (its launcher + workspace managers). */
export async function getSessionAudit(
  ...
  return unwrap(
    await backendApi.get<SessionAudit>(
      `/workspaces/${workspaceId}/sessions/${sessionId}/audit${qs ? `?${qs}` : ''}`,
      {
        showErrors: options?.showErrors,
      },
    ),
 ...
 * `reloadWorkspaceSessionConfig` is how you fix it.
 */
export interface SessionConfigState {
   ...
   * unreachable, or the workspace has no compiled config at all (a v1
   * `kortix.toml` workspace). Branch on `=== true` and `=== false`; `!stale`
...
/**
 * Not cheap — the server drops the workspace's git-mirror TTL, recompiles the
 ...
 */
export async function getWorkspaceSessionConfigState(
  workspaceId: string,
  sessionId: string,
): Promise<SessionConfigState> {
  return unwrap(
    await backendApi.get<SessionConfigState>(
      `/workspaces/${workspaceId}/sessions/${encodeURIComponent(sessionId)}/config`,
      { showErrors: false },
    ),
  );
}
 ...
 */
export async function reloadWorkspaceSessionConfig(
  workspaceId: string,
  sessionId: string,
  ...
  return unwrap(
    await backendApi.post<SessionReloadResult>(
      `/workspaces/${workspaceId}/sessions/${encodeURIComponent(sessionId)}/reload`,
      input,
      { showErrors: false },
    ),
  );
...
export interface SessionScopeInput {
   ...
   * (fall back to the agent's own grant); `[]` means "no workspace secrets at
...
export async function getWorkspaceSessionScope(
  workspaceId: string,
  sessionId: string,
): Promise<SessionScope> {
  return unwrap(
    await backendApi.get<SessionScope>(
      `/workspaces/${workspaceId}/sessions/${encodeURIComponent(sessionId)}/scope`,
    ),
  );
}
 ...
 */
export async function setWorkspaceSessionScope(
  workspaceId: string,
  sessionId: string,
