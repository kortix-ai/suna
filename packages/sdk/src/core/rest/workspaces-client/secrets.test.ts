...
test('upsertWorkspaceSecret sends an explicit server consumer without a plaintext transition', async () => {
  nextResponse = { status: 200, body: { name: 'OPENAI_API_KEY' } };
  await upsertWorkspaceSecret('P1', {
    name: 'OPENAI_API_KEY',
    value: 'key',
    strategy: 'broker',
...
test('setWorkspaceSecretStrategy PUTs the strategy to the encoded identifier route', async () => {
  nextResponse = {
    status: 200,
    body: {
      identifier: 'API/key',
  ...
  const result = await setWorkspaceSecretStrategy('P1', 'API/key', 'denied');

  expect(last().url).toContain('/workspaces/P1/secrets/API%2Fkey/strategy');
  expect(last().method).toBe('PUT');
  expect(last().body).toEqual({ strategy: 'denied' });
  expect(result.strategy).toBe('denied');
});

test('setWorkspaceSecretStrategy sends broker policy options', async () => {
  nextResponse = { status: 200, body: { identifier: 'API_KEY', strategy: 'broker' } };

  await setWorkspaceSecretStrategy('P1', 'API_KEY', 'broker', {
...
test('brokerWorkspaceSecretRequest POSTs a policy-bound HTTPS request', async () => {
  nextResponse = {
    status: 200,
    body: { status: 201, headers: { 'content-type': 'application/json' }, body_base64: 'e30=' },
  };

  const result = await brokerWorkspaceSecretRequest('P1', 'primary/key', {
  ...
  expect(last()).toMatchObject({
    method: 'POST',
    url: expect.stringContaining('/workspaces/P1/secrets/primary%2Fkey/broker'),
    body: {
      url: 'https://api.example.com/v1/messages',
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
test('deleteWorkspaceProviderOAuth DELETEs the encoded provider', async () => {
  nextResponse = { status: 200, body: { ok: true } };
  await deleteWorkspaceProviderOAuth('P1', 'openai/codex');
  expect(last().url).toContain('/workspaces/P1/oauth/openai%2Fcodex');
  expect(last().method).toBe('DELETE');
});
