// Public sandbox share links — `/v1/p/share`. Sandbox-scoped, NOT workspace-
// scoped: the route takes a `sandbox_id` (the sandbox's external id), never a
// Kortix workspace id, and proxies through to the sandbox daemon's own
// `/kortix/share` endpoints (see apps/api/src/sandbox-proxy/routes/share.ts).
// Lives in `workspaces-client` for barrel/discoverability reasons even though
// its URLs sit outside `/workspaces`.
