...
// workspace-scoped install surface in `./marketplace.ts` (`/workspaces/:id/marketplace/*`,
// `/workspaces/:id/registry/*`). Read-only catalog routes are public; the
...
export async function createMarketplaceInstallSession(
  ...
  return unwrap(
    await backendApi.post<{ session_id: string }>(
      `/workspaces/${encodeURIComponent(workspaceId)}/marketplace/install-session`,
      { id: itemId },
    ),
