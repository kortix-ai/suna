...
test('createMarketplaceInstallSession starts a typed workspace install session', async () => {
  nextResponse = { status: 200, body: { session_id: 'session-1' } };
  const result = await createMarketplaceInstallSession('workspace-1', 'kortix:researcher');
  expect(last().url).toContain('/workspaces/workspace-1/marketplace/install-session');
  expect(last().method).toBe('POST');
