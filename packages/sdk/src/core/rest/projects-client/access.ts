...
export type ResourceGrantType = 'agent' | 'skill' | 'secret' | 'space';
...
export interface ProjectResourceGrantsResponse {
  resources: {
    ...
    /** The declared spaces, keyed by slug — the second closed object type
     *  (a member with zero grant rows sees zero spaces). Optional, so an
