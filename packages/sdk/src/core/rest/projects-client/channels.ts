...
export async function getWhatsAppInstallation(
  ...
  const res = await backendApi.get<WhatsAppInstallation | null>(
    `/projects/${encodeURIComponent(projectId)}/channels/whatsapp/installation`,
    { showErrors: false },
  );
  if (!res.success) return null;
  return res.data ?? null;
...
/** Step 1: which numbers can this gateway URL + API key reach? */
export async function listWhatsAppConnections(
  projectId: string,
  input: { gateway_url: string; api_key: string },
): Promise<{ gatewayUrl: string; connections: WhatsAppConnection[] }> {
  return unwrap(
    await backendApi.post<{ gatewayUrl: string; connections: WhatsAppConnection[] }>(
      `/projects/${encodeURIComponent(projectId)}/channels/whatsapp/connections`,
      input,
      { showErrors: false },
    ),
    'Could not reach that WhatsApp gateway',
  );
}

/** Step 2: bind one connection and register the webhook on the gateway. */
export async function connectWhatsApp(
  projectId: string,
  input: { gateway_url: string; api_key: string; account_id: string },
): Promise<WhatsAppInstallation> {
  return unwrap(
    await backendApi.post<WhatsAppInstallation>(
      `/projects/${encodeURIComponent(projectId)}/channels/whatsapp/connect`,
      input,
      { showErrors: false },
    ),
...
export async function disconnectWhatsApp(projectId: string): Promise<void> {
  unwrap(
