// Project apps/deployments — the `/v1/projects/:id/apps/*` family. Mirrors
// the response shapes from suna/apps/api/src/projects/index.ts. The platform
// serializes apps in snake_case so we keep that on the wire and translate at
// the component boundary when needed.
...
export async function deployProjectApp(
  ...
  return unwrap(
    await backendApi.post<DeployProjectAppResponse>(`${base(projectId)}/${slug}/deploy`, {}),
  );
}
...
export async function stopProjectApp(
  ...
  return unwrap(
    await backendApi.post<StopProjectAppResponse>(`${base(projectId)}/${slug}/stop`, {}),
  );
}
