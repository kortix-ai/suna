...
const { runtimeKeys } = await import('../use-runtime-sessions');
const { fileListKeys, gitStatusKeys, fileContentKeys } = await import('../file-keys');
const { ptyKeys } = await import('../use-runtime-pty');

// ============================================================================
// Test harness — a fresh QueryClient + real sync store + spy callbacks for
// every test, mirroring exactly what `use-runtime-events/index.ts` wires up
...
describe('session lifecycle cache mutations', () => {
  test('session.created inserts into an existing session list, newest first', () => {
    ...
    queryClient.setQueryData(runtimeKeys.sessions(), [session('ses_old', { time: { created: 1, updated: 1 } })]);
    ...
    expect(queryClient.getQueryData(runtimeKeys.session('ses_new'))).toMatchObject({ id: 'ses_new' });
  });
  ...
  test('session.created on an empty/uninitialized list still seeds it', () => {
    ...
    expect(queryClient.getQueryData<Session[]>(runtimeKeys.sessions())).toEqual([session('ses_new')]);
  });
  ...
  test('session.updated patches the individual session cache and re-sorts the list', () => {
    ...
    queryClient.setQueryData(runtimeKeys.session('ses_a'), session('ses_a', { title: 'Old title' }));
    ...
    expect(queryClient.getQueryData<Session>(runtimeKeys.session('ses_a'))?.title).toBe('New title');
    expect(queryClient.getQueryData<Session[]>(runtimeKeys.sessions())?.[0].title).toBe('New title');
  });
  ...
  test('session.deleted removes the session from the list and clears its query cache', () => {
    ...
    queryClient.setQueryData(runtimeKeys.session('ses_a'), session('ses_a'));
    ...
    expect(queryClient.getQueryData<Session[]>(runtimeKeys.sessions())?.map((s) => s.id)).toEqual(['ses_b']);
    expect(queryClient.getQueryData(runtimeKeys.session('ses_a'))).toBeUndefined();
  });
});
...
describe('session.error', () => {
  test('patches .error onto the last assistant message in the messages cache', () => {
    const { handleEvent, queryClient } = buildHandler();
    const key = runtimeKeys.messages('ses_1');
