...
import { ptyKeys } from '../use-runtime-pty';
import { type MessageWithParts, runtimeKeys, type Session } from '../use-runtime-sessions';
...
export function createEventHandler(deps: {
  ...
  applySyncEvent: (event: RuntimeSdkEvent) => void;
  ...
  function getSessionTitle(sessionID: string): string | undefined {
    ...
    const session = queryClient.getQueryData<Session>(runtimeKeys.session(sessionID));
  ...
  function handleEvent(event: RuntimeEvent) {
    ...
    // This matches Runtime's architecture where the SolidJS store is
    ...
    applySyncEvent(event as RuntimeSdkEvent);
    ...
    switch (event.type) {
      ...
      case 'session.created': {
        ...
        if (info) {
          ...
          queryClient.setQueryData(runtimeKeys.session(info.id), info);
      ...
      case 'session.updated': {
        const info = readSessionInfo(event);
        if (info) {
          // Runtime auto-titles after the first message via session.updated.
          ...
          const prevTitle =
            ...
            queryClient.getQueryData<Session>(runtimeKeys.session(info.id))?.title ??
            null;
      ...
      case 'session.deleted': {
        ...
        if (info) {
          ...
          queryClient.removeQueries({ queryKey: runtimeKeys.session(info.id) });
          queryClient.removeQueries({ queryKey: runtimeKeys.messages(info.id) });
          ...
          client.session
            ...
            .then((res) => {
              if (res.data) {
                ...
                queryClient.setQueryData(runtimeKeys.session(sessionID), session);
      ...
      case 'session.error': {
        ...
        if (props.sessionID && props.error) {
          ...
          const key = runtimeKeys.messages(sessionID);
      ...
      case 'project.updated': {
        ...
        // but Runtime can emit bursts while tools are running. Coalesce
