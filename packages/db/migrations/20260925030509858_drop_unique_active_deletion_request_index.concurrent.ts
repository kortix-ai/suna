// Migration: drop_unique_active_deletion_request_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
// mixed-version-safe: no code reads this index or relies on its 23505. Every writer checks
// for a pending row first, and the previous migration already enforces one pending row.
export const up = (pgm) => {
  pgm.noTransaction();
  ...
  // Retired: uniq_account_deletion_requests_pending (previous migration) is the
  // one-pending-request guard now. This index is keyed on flags the application
  // never writes, so it refused every re-request after a cancel. Prod never had
  // it (faked baseline); IF EXISTS makes it a no-op there.
  pgm.sql(`drop index concurrently if exists kortix.unique_active_deletion_request`);
};
