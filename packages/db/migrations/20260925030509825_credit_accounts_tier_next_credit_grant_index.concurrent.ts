// Migration: credit_accounts_tier_next_credit_grant_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // The free-tier rotation sweep (`getFreeAccountsDueForRotation`) reads
  // `tier = 'free' AND (next_credit_grant IS NULL OR next_credit_grant <= now())`.
  // No index served it: prod ran it 3,868 times at 915 ms mean as a parallel
  // seq scan over ~234k rows to return ~96 due rows (pg_stat_statements,
  // read-only, 2026-09-25). With (tier, next_credit_grant) the planner answers
  // the OR with a BitmapOr of two probes. A plain composite, not a partial
  // index on `tier = 'free'`, so a generic plan with `tier = $1` can use it too.
  // The columns change only on a grant, so balance updates stay HOT.
  pgm.sql(`
    create index concurrently if not exists idx_credit_accounts_tier_next_credit_grant
      on kortix.credit_accounts (tier, next_credit_grant)
  `);
};
