// Migration: account_memberships_pkey_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Prod's account_memberships has no primary key (faked baseline; the table
  // was account_members before the RBAC cutover renamed it). Every other
  // database has account_members_pkey PRIMARY KEY (user_id, account_id).
  //
  // Step 1 of 2: build the key's unique index without blocking writes. The
  // next migration (..._prod_missing_constraints_not_valid.sql) attaches it
  // with ADD CONSTRAINT ... PRIMARY KEY USING INDEX, which only needs a brief
  // lock because the index already exists. Elsewhere account_members_pkey is
  // already the primary key's index, so IF NOT EXISTS makes this a no-op.
  //
  // Prod held 0 duplicate (user_id, account_id) pairs and 0 NULLs in either
  // column on 2026-09-25, and idx_account_members_user_account already
  // enforces the same uniqueness there, so the build cannot fail on data.
  pgm.sql(`
    create unique index concurrently if not exists account_members_pkey
      on kortix.account_memberships (user_id, account_id)
  `);
};
