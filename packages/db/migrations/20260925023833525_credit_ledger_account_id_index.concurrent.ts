// Migration: credit_ledger_account_id_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Every account-scoped ledger read filters on account_id and orders or
  // ranges on created_at: the usage breakdown (4,804 calls, 2.0 s mean on
  // prod), the transactions page (225 calls, 1.96 s mean), the per-type sums
  // (6,019 calls, 64 ms mean) and the row count (761 calls, 342 ms mean).
  //
  // The baseline built this index. Prod's baseline was faked, so prod never
  // had it: every one of those reads is a parallel seq scan of 2.7M rows
  // (EXPLAIN on prod, 2026-09-25). Dev, staging and fresh databases have it,
  // so IF NOT EXISTS makes this a no-op there. Same name and definition as
  // 20260621094136410_baseline.sql.
  pgm.sql(`
    create index concurrently if not exists idx_credit_ledger_account_id
      on kortix.credit_ledger (account_id, created_at desc)
  `);
};
