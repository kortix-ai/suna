// Migration: audit_events_session_sequence_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Serves the per-session audit read behind
  // GET /v1/projects/:projectId/sessions/:sessionId/audit:
  //   where session_id = $1
  //   order by session_sequence asc, event_id asc
  //   limit $2
  //
  // The route filters on session_id ALONE by design: rows written before
  // account resolution (auth.login.success) or from project-neutral endpoints
  // belong to the session's integrity chain, so an account predicate would cut
  // the middle out of a persisted chain. Every other index on this table leads
  // with account_id/actor_user_id/resource — none can serve a bare session_id
  // predicate — so this query seq-scanned the whole ledger (21M+ rows in
  // 2026-07, larger now) and was cancelled by the request-path 25 s
  // statement_timeout (57014), surfacing as a 503 + a Sentry report per call
  // (prod, 2026-09-25; Better Stack pattern 97669531, KRTX-211).
  //
  // Column order matters: the equality predicate leads, then the ordering
  // tuple in the SAME direction the query asks for, so Postgres walks the
  // index and stops at LIMIT instead of scanning and sorting. event_id is a
  // random uuid, so (session_id, session_sequence, event_id) is unique — the
  // route's keyset cursor compares exactly this tuple.
  pgm.sql(`
    create index concurrently if not exists idx_audit_events_session_sequence
      on kortix.audit_events (session_id, session_sequence, event_id)
  `);
};
