// Migration: subprojects_upload_identity_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Gives an UPLOADED subproject an identity, so re-uploading a fixed archive
  // REPLACES it instead of adding a duplicate row. `idx_subprojects_repo_ref` cannot
  // do this: an upload has NULL repo columns, and a btree unique treats every
  // NULL as distinct, so every upload would be a new subproject forever.
  //
  // Partial on `source_kind = 'upload'` so a github subproject is unaffected — two
  // github subprojects from different repos may legitimately share a slug, and only
  // (repo_owner, repo_name, ref) decides their identity.
  pgm.sql(`
    create unique index concurrently if not exists idx_subprojects_upload_identity
      on kortix.subprojects (account_id, slug)
