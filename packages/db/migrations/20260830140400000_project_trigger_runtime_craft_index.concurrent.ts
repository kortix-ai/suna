// Migration: project_trigger_runtime_craft_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Serves the craft-runs query: given (project_id, craft_slug), find the
  // trigger slugs this craft owns, then join project_trigger_executions on
  // (project_id, slug). Partial on `craft_slug IS NOT NULL` because only a
  // craft-installed trigger carries one — a small fraction of the table, and
  // nothing at all for a project that has installed no craft.
  pgm.sql(`
    create index concurrently if not exists idx_project_trigger_runtime_craft
      on kortix.project_trigger_runtime (project_id, craft_slug)
      where craft_slug is not null
  `);
};
