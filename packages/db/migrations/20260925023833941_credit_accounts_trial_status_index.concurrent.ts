// Migration: credit_accounts_trial_status_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // The billing cron's trial sweep (`sweepExpiredTrials`) and trial re-grant
  // (`regrantActiveTrials`) filter `trial_status = 'active'`. On prod 4 of
  // 234k rows have a status other than 'none', so this partial index answers
  // both from a few index entries. Without it each run is a parallel seq scan
  // of credit_accounts (prod: 4,102 calls at 885 ms mean and 3,866 calls at
  // 112 ms mean).
  //
  // Built by the baseline everywhere except prod (faked baseline). Same name
  // and definition as 20260621094136410_baseline.sql; a no-op elsewhere.
  pgm.sql(`
    create index concurrently if not exists idx_credit_accounts_trial_status
      on kortix.credit_accounts (trial_status)
      where ((trial_status)::text <> 'none'::text)
  `);
};
