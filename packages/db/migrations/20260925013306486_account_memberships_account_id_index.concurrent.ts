// Migration: account_memberships_account_id_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // Account-scoped membership reads (member lists, seat counts, IAM cache
  // invalidation, SCIM) filter `account_memberships` by `account_id` alone.
  // The primary key leads with `user_id`, so without this index each of them
  // scans every membership of every account.
  //
  // `kortix.ts` has declared this index since before the RBAC cutover, and
  // prod has it (built outside the migration history, 2026-09-25 read-only
  // check). No migration ever built it, so dev, staging, self-host and every
  // fresh database lack it. Same name, IF NOT EXISTS: a no-op on prod.
  pgm.sql(`
    create index concurrently if not exists idx_account_members_account_id
      on kortix.account_memberships (account_id)
  `);
};
