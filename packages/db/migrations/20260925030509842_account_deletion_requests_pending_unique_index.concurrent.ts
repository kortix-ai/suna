// Migration: account_deletion_requests_pending_unique_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // At most one pending deletion request per account. Replaces
  // unique_active_deletion_request (dropped by the next migration), which was
  // keyed on is_cancelled / is_deleted: the application writes `status` and
  // never sets those flags, so on dev and staging a request after a cancel
  // failed with 23505, and prod (faked baseline) never had the index at all.
  //
  // A duplicate pending row would fail this build and leave an INVALID index
  // (see MIGRATIONS.md "When it has already failed"). Checked read-only
  // 2026-09-25: 0 accounts with more than one pending row on prod, staging and
  // dev.
  pgm.sql(`
    create unique index concurrently if not exists uniq_account_deletion_requests_pending
      on kortix.account_deletion_requests (account_id)
