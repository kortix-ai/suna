// Migration: drop_account_members_user_id_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
// mixed-version-safe: non-unique index; its only readers are user_id lookups that the
// unique (user_id, account_id) indexes on the same table already serve.
export const up = (pgm) => {
  pgm.noTransaction();
  ...
  // Prod-only index: no migration ever built it, and no other database has
  // it. Its lookups (`user_id = $1`) are served by the leading column of
  // idx_account_members_user_account and of account_members_pkey
  // (user_id, account_id), which is how every other environment runs them.
  // Dropping it removes one index write per membership change on prod and
  // removes the drift. IF EXISTS makes it a no-op elsewhere.
  pgm.sql(`drop index concurrently if exists kortix.idx_account_members_user_id`);
};
