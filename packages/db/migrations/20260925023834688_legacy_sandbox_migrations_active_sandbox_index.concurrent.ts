// Migration: legacy_sandbox_migrations_active_sandbox_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // kortix.ts declares this unique index: at most one live migration per
  // sandbox. Prod never had it (faked baseline), so nothing enforces that
  // rule there. Prod held 0 duplicate live rows on 2026-09-25 (195 rows in
  // the table), so the build succeeds. Same name and definition as
  // 20260621094136410_baseline.sql; a no-op elsewhere.
  pgm.sql(`
    create unique index concurrently if not exists idx_legacy_sandbox_migrations_active_sandbox
      on kortix.legacy_sandbox_migrations (sandbox_id)
      where ((status)::text = any ((array['planned'::character varying, 'running'::character varying, 'applied'::character varying, 'verified'::character varying, 'completed'::character varying])::text[]))
  `);
};
