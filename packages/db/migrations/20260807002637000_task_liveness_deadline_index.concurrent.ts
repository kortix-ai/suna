// Migration: task_liveness_deadline_index (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  pgm.sql(`
    create index concurrently if not exists idx_project_tasks_liveness_deadline
      on kortix.project_tasks (status, liveness_deadline_at)
