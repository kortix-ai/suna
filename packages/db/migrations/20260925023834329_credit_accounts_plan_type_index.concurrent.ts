// Migration: credit_accounts_plan_type_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // The yearly-renewal cron (`getYearlyAccountsDueForRefill`) filters
  // `plan_type = 'yearly'`: 150 of 234k rows on prod. Without this index the
  // query is a parallel seq scan (prod: 3,871 calls at 929 ms mean).
  // idx_credit_accounts_yearly_renewal cannot serve it: its predicate requires
  // `next_credit_grant IS NOT NULL`, and the query also accepts NULL.
  //
  ...
  pgm.sql(`
    create index concurrently if not exists idx_credit_accounts_plan_type
      on kortix.credit_accounts (plan_type)
  `);
};
