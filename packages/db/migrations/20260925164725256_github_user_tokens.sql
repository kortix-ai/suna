...
-- already built by 20260925164209388_github_installations_one_per_owner_index
-- CONCURRENTLY (the table is live; a plain CREATE INDEX would block its
-- writers). Removed on purpose -- the snapshot now carries it, so it will not
-- be proposed again.

-- Both indexes above are on a table created in this same migration, so they
-- take no lock anyone can see and do not need the CONCURRENTLY escape hatch.