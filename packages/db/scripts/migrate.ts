...
function resolveUrl(argv: string[]): string {
  ...
  if (target) {
    const key = target.toUpperCase() === 'LOCAL' ? 'DATABASE_URL' : `${target.toUpperCase()}_DB_URL`;
    const v = readEnvKey(DOTENV, key);
    if (!v) { console.error(`--target=${target}: ${key} not set in apps/api/.env`); process.exit(1); }
    return v;
  }
...
const fmtUrl = (u: string) => { try { const x = new URL(u); if (x.password) x.password = '***'; return x.toString(); } catch { return u.replace(/:[^:@/]+@/, ':***@'); } };
...
async function main() {
  ...
  switch (cmd) {
    case 'up':
      ...
      return;
    case 'fake':
      await runner({ ...base, direction: 'up', count: Infinity, fake: true });
      return;
    case 'down':
      await runner({ ...base, direction: 'down', count: countArg ? parseInt(countArg, 10) : 1 });
      return;
    case 'status': {
      const pending = await runner({ ...base, direction: 'up', count: Infinity, dryRun: true });
      if (pending.length === 0) console.log('Up to date — no pending migrations.');
      else { console.log(`${pending.length} pending migration(s):`); for (const m of pending) console.log(`  pending  ${m.name}`); }
