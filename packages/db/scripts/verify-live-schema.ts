 ...
 *
 * Both databases are read through catalog.ts: one catalog query and one
 * ledger query each, on a read-only session. The script writes nothing.
 *
 * Exit 0 = nothing missing (waivers aside).  Exit 1 = drift.  Exit 2 = usage/connection error.
 */
import { type Catalog, type CatalogConstraint, connectReadOnly, readCatalog, readLedger } from './catalog';
...
export function diffStructure(
  ...
  const canonicalTables = tablesOf(canonical);
  const liveTables = tablesOf(live);
  const shared = (table: string) => canonicalTables.has(table) && liveTables.has(table);

  const indexKey = (definition: string) => definitionKey(normalizeIndexDef(definition));
  const liveIndexDefs = new Set([...live.indexes.values()].map((i) => indexKey(i.definition)));
  const canonicalIndexDefs = new Set([...canonical.indexes.values()].map((i) => indexKey(i.definition)));
  for (const [name, index] of canonical.indexes) {
    if (!shared(index.table) || liveIndexDefs.has(indexKey(index.definition))) continue;
    if (name in waivers.indexes) drift.waived.push(`index ${name}: ${waivers.indexes[name]}`);
    else drift.missingIndexes.push(`${name}: ${normalizeIndexDef(index.definition)}`);
  }
  for (const [name, index] of live.indexes) {
    if (!index.valid) drift.invalidIndexes.push(`${name} on ${index.table}`);
    if (shared(index.table) && !canonicalIndexDefs.has(indexKey(index.definition))) {
      drift.extraIndexes.push(`${name}: ${normalizeIndexDef(index.definition)}`);
    }
  }

  const constraintKey = (c: CatalogConstraint) =>
    `${c.table}|${c.type}|${definitionKey(normalizeConstraintDef(c.definition))}`;
  ...
  for (const [name, c] of canonical.constraints) {
    ...
    if (!match) {
      ...
      else drift.missingConstraints.push(`${name} on ${c.table}: ${normalizeConstraintDef(c.definition)}`);
    } else if (c.validated && !match.validated) {
      drift.unvalidatedConstraints.push(`${name} on ${c.table}: ${normalizeConstraintDef(c.definition)}`);
    }
  }
  for (const [name, c] of live.constraints) {
    if (shared(c.table) && !canonicalConstraintKeys.has(constraintKey(c))) {
      drift.extraConstraints.push(`${name} on ${c.table}: ${normalizeConstraintDef(c.definition)}`);
    }
  }
...
/** Catalog and ledger of one database, read through catalog.ts's read-only session. */
async function readDatabase(databaseUrl: string): Promise<{ catalog: Catalog; ledger: string[] }> {
  const client = await connectReadOnly(databaseUrl);
  try {
    return { catalog: await readCatalog(client, SCHEMA), ledger: await readLedger(client) };
  } finally {
    await client.end();
  }
}
...
async function main() {
  ...
  const [canon, target] = await Promise.all([readDatabase(canonical), readDatabase(live)]);
  ...
  const counts = ({ catalog }: { catalog: Catalog }) =>
    `${tablesOf(catalog).size} tables, ${catalog.columns.size} columns, ${catalog.enumValues.size} enum values, ` +
    `${catalog.indexes.size} indexes, ${catalog.constraints.size} constraints.`;
  console.log(`Canonical: ${counts(canon)}\nLive:      ${counts(target)}`);
  section(
