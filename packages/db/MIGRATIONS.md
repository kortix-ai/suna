...
- If main overtakes an unmerged migration already applied in an isolated preview, a filename change requires an explicit checksum-guarded entry in `scripts/migration-ledger-repair.ts`. Keep every SQL byte unchanged. The attachment repair maps `20260908152048390_prompt_attachments` to `20260912000000000_prompt_attachments`. It executes the two exact Sept9/Sept10 index-removal prerequisites if missing, then moves only the known attachment ledger row after their actual timestamps. The normal runner retains strict ordering. Never fake applied rows or rename an arbitrary ledger entry. `scripts/attachment-migration-rename.integration.test.ts` proves fresh, legacy, repaired, duplicate, and checksum-drift paths against disposable databases, including retained attachment/reference rows and unchanged unrelated ledger rows.
