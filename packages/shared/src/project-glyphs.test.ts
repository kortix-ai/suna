...
import {
  WORKSPACE_GLYPH_COLORS,
  WORKSPACE_GLYPH_GROUPS,
  WORKSPACE_GLYPH_NAMES,
  isProjectGlyphColor,
  isProjectGlyphName,
} from './project-glyphs';
...
describe('the glyph catalogue', () => {
  test('is 202 names in 17 groups: 16 of 12 and one of 10', () => {
    expect(WORKSPACE_GLYPH_NAMES).toHaveLength(202);
    expect(WORKSPACE_GLYPH_GROUPS).toHaveLength(17);
    ...
    const sizes = WORKSPACE_GLYPH_GROUPS.map((group) => group.names.length);
    const numbersIndex = WORKSPACE_GLYPH_GROUPS.findIndex((group) => group.label === 'Numbers');
  ...
  test('no name is duplicated', () => {
    expect(new Set(WORKSPACE_GLYPH_NAMES).size).toBe(WORKSPACE_GLYPH_NAMES.length);
  });
  ...
  test('names are PascalCase Phosphor identifiers', () => {
    ...
    for (const name of WORKSPACE_GLYPH_NAMES) {
