...
OpenAI and Anthropic models are not in the managed lineup. `packages/llm-catalog/src/managed.test.ts` fails if a managed entry routes to either vendor. Members reach them through their own account:

- ChatGPT plan: `codex/<id>`, for example `codex/gpt-6-sol` and `codex/gpt-6-luna` (`apps/api/src/llm-gateway/models/codex-models.ts`).
- BYOK key: `openai/<id>` or `anthropic/<id>`, for example `anthropic/claude-opus-5-5`.

`CATALOG` carries the models.dev records for `openai/gpt-6-sol`, `openai/gpt-6-luna`, and `anthropic/claude-opus-5-5`. The served catalog takes reasoning effort, modalities, and `temperature` for those routes from them.

`claude-opus-5.5`, `gpt-6-sol`, and `gpt-6-luna` are retired managed IDs (`RETIRED_MANAGED_MODEL_IDS` in `apps/api/src/llm-gateway/models/managed-models.ts`). A saved default that names one falls back to a served managed model.
