...
// OpenAI and Anthropic models are reached through a member's ChatGPT plan
// (`codex/<id>`) or a BYOK key (`anthropic/<id>`, `openai/<id>`). Kortix
// credits never pay for them.
describe('OpenAI and Anthropic models stay out of the managed lineup', () => {
  test('no managed model routes to an OpenAI or Anthropic model', () => {
    for (const model of MANAGED_MODELS) {
      expect(model.upstreamModelId, model.id).not.toMatch(/^(openai|anthropic)\//);
      expect(model.pricingRef, model.id).not.toMatch(/^(openrouter\/)?(openai|anthropic)\//);
    }
  });

  // The served catalog takes temperature and reasoning_options from these
  // records for the ChatGPT and BYOK routes.
  test.each([
  ...
  test('the ChatGPT GPT-6 records reject a client temperature', () => {
