...
public struct KortixClient: Sendable {
    ...
    public func getAccount(_ accountID: String) async throws -> Account {
    ...
    private func get<Value: Decodable>(path: String, query: [URLQueryItem] = []) async throws -> Value {
        ...
        do { return try decoder.decode(Value.self, from: response.data) }
        catch { throw KortixSDKError.decoding(String(describing: error)) }
    }
