...
- **The project owns the install.** Installing copies files into the project
  repo: skills under `skills/<name>/`, agents under `agents/`, memory items
  under `memory/`. This holds in every project, including those created
  before 2026-09 that keep other files under `.kortix/`.
...
4. Verify installed state:
   ...
   ```
5. In the next session, the skill is available from `skills/`.
