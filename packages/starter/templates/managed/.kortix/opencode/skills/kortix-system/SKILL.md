...
skills arrive with a template (`<templates>` below) or are authored in the
...
<templates>
A **template** is a public GitHub repository whose `kortix.yaml` declares
agents, skills, connectors and triggers — an example project setup. The catalog
is curated and static; browse it before authoring a capability from scratch.

```sh
kortix templates list [--query <text>] --json
kortix templates show <slug> --json
kortix templates install <slug> --project <project-id>
```

`install` starts an agent session that reads both manifests, merges the
template into the project root (never as a subproject unless the user asks),
and opens ONE change request the user reviews. Nothing records what is
installed: that change request is the record, and reverting it is the
uninstall. The web equivalent is Customize → Templates and the public
`/templates` catalog. The project must have the `templates` feature flag on.
</templates>
...
check the template catalog (`<templates>` above) — a skill that already
