...
| Command | Effect |
| --- | --- |
| `kortix sessions ls [--space <slug>]` | All sessions on the project. `--space <slug>` narrows to one space; `--space ""` narrows to sessions with none. `--json` for machine-readable output. |
...
| `kortix sessions new [--prompt "<text>"] [--space <slug>] [--wait] [--json]` | Start a new session. `--space <slug>` groups it under a declared space (default: inherits `$KORTIX_SPACE` when running inside one — pass `--space ""` to opt OUT). `--wait` blocks until it's running; `--json` prints the session object so you can capture `session_id` to orchestrate. `--with-file <local path>` (repeatable) uploads each file to `/workspace/incoming/<name>` **before** the prompt is delivered, and appends a manifest of the paths to the prompt. |
...
| `kortix triggers add <slug> --type cron\|webhook\|monitor --prompt "<text>" [--space <slug>] [--apply] […]` | Append a `[[triggers]]` block. `--apply` commits to the cloud project now instead of writing locally. |
| `kortix triggers set <slug> [--space <slug>] […]` | Change a LIVE trigger. Only the flags you pass are written; `--space ""` clears it. Always applies now. |
...
`--space <slug>` (on `add`/`set`) scopes the fired session to a declared
space — must name a key in `spaces:`; see **Spaces** below.

### Spaces

A space groups sessions under a named effort, with its own standing
instructions, reference files, default agent, and scheduled work. The
manifest (`kortix.yaml` `spaces.<slug>`) is the source of truth — every
write below commits to it. Real `kortix spaces --help` output:

```
Usage: kortix spaces <subcommand> [options]
...
Subcommands:
  ...
  context add <slug> <file> [--as name]
                                  Upload a local file as space context.
...
`kortix spaces create "<name>" --json` prints the created object
(capture `.slug`). `grant`/`revoke` are sugar over the generic
`POST|DELETE /projects/:id/resource-grants {resource_type:'space',...}`
