---
name: talent-pool-rendering
description: "Turn a list of scored candidates into a shareable Talent Pool leaderboard — a ranked, tiered page with a per-candidate signal breakdown, methodology, and workflow sections. Use to visualize or deploy a candidate ranking ('build a leaderboard from these scores', 'make a shareable talent pool page', 'deploy the candidate dashboard'). Read-only by default; add a backend when reviewers need to add candidates that persist."
---

# Talent Pool Rendering

> **Marketplace skill — requires a Kortix ATS connector (e.g. an applicant-tracking system). Install when configured.** Strictly, this skill renders a page from scores you already have, so it can run with no connector. It's grouped with the recruitment connector skills because its input usually comes from `ats-ranker` / `rtm-ranker` / `automated-sourcing`, which do use an ATS. Connect one in-chat via the `kortix-system` credentials reference when you want the upstream scoring too.

Take a set of **already-scored candidates** and render a **Talent Pool**: a ranked leaderboard bucketed into tiers, where each candidate opens a breakdown of the signals behind their score, with methodology and workflow context below the grid. This skill is the *presentation layer* for the recruitment rankers — it does no scoring of its own.

## Two modes

```
   scored candidates (from ats-ranker / rtm-ranker / a manual list)
                              │
                              ▼
...
This is exactly the shape `ats-ranker` and `rtm-ranker` already produce (rank + score + per-field signals + rationale). Pass theirs straight in.
...
1. Get the scored bundle (from `ats-ranker` / `rtm-ranker`, or assemble one from a manual list).
...
This skill **renders and deploys** a ranking. It does not:

- score or re-rank candidates — that's `ats-ranker` / `rtm-ranker`;
- collect Yes/Maybe/No review decisions — that's `automated-sourcing`'s review surface (this skill's only write path is *add a candidate*);
