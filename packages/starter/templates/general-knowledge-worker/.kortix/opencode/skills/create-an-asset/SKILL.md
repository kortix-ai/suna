...
> **Visual design for web-based formats:** When building interactive landing pages or any HTML/web-based sales asset, do NOT follow the visual design guidance (Phase 4) in this skill. Instead, read and follow `skills/website-building/SKILL.md` for all design tokens, typography, color systems, layout, and CSS. This skill's Phase 4 applies only to non-web outputs (decks, one-pagers). The sales content structure, research, and methodology in this skill still applies to all formats.
...
**Design foundation:** Follow the design-foundations principle of **1 accent + neutrals** (see `skills/design-foundations/SKILL.md`). The prospect's brand primary becomes the accent; everything else should be neutral. If no brand colors are available, fall back to the Kortix neutral palette defaults: black/white or off-white neutrals plus one accent. Reserve color for emphasis — the viewer's eye goes where color is.
...
| Energy | `#16a34a` (green) |
| Default | Kortix teal `#22808D` (see `skills/design-foundations/SKILL.md`) |
