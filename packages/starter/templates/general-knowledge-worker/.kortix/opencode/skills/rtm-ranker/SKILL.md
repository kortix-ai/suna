---
name: rtm-ranker
description: "Rank sourcing candidates by how ready they are to move jobs, so you outreach the most reachable people first. Use when you have (or can source) a list of profiles for a role and want them ordered by likelihood to respond and switch employers — 'who on this list is most likely to move', 'prioritize these profiles for outreach', 'who's overdue for a change'. Scores readiness-to-move from fetched profiles, not CV-vs-JD fit. Not an interview shortlist and not a quality ranking."
---

# RTM Ranker — Ready to Move
...
This score says **nothing** about whether someone is good, hireable, or would pass an interview. A mediocre candidate can be wide open to a move; a star can be perfectly happy and locked in. If you want CV-vs-JD interview fit, that's `ats-ranker`. If you want to *find* people, that's `people-search`. This skill only answers: *of the people we already have, who's most reachable right now?*
...
If the user wants a shareable view, build a **read-only leaderboard** with the `talent-pool-rendering` skill (feed it the readiness scores + rubric breakdowns). No outreach, no profile writes from this skill — drafting messages is `draft-outreach`'s job.
