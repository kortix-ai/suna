...
This skill owns a **standing, repeatable sourcing pipeline** for a role — not a single search. Each run sources fresh candidates, removes anyone already in your ATS or already reviewed, ranks the rest, refreshes a review surface a recruiter actually clicks through, and folds their decisions back in so the next run is sharper. It's the recurring machine; the one-shot search underneath it is `people-search`.

## When to load this — and when not to

Load it to **run, schedule, debug, rebuild, or modify the recurring workflow**. Triggers: "do the next sourcing run," "set up weekly sourcing for the Staff SRE role," "the review app isn't saving decisions," "regenerate the candidate review page."

Do **not** load it for a one-off lookup ("find me 5 senior PMs in Berlin") — that's `people-search`. Don't load it to write a Slack/email announcement *about* the workflow, to summarize recruiter feedback, or to discuss recruiting strategy. The request has to be about *operating the pipeline itself*.
...
```
     ...
     └──────────────────────────────────────────────┘
        ...
        dedupe against ATS pipeline + past reviews + hired patterns
                              │
                              ▼
        rank by readiness (rtm-ranker) or fit signals
                              │
                              ▼
...
This skill is the **orchestrator**. It composes other skills rather than re-implementing them:
...
- **`rtm-ranker`** (or `ats-ranker` if you're scoring against a JD) does the ranking. Don't hand-roll a scorer.
- **`talent-pool-rendering`** / **`webapp`** build the review surface. Don't hand-author one-off HTML.
...
Rank the new candidates so the recruiter reviews the best first. For outreach prioritization use `rtm-ranker` (readiness to move); if you're screening against a written JD, use `ats-ranker`. Either way the ranking is a transparent rubric, not a black box — carry each candidate's score and reason.
...
- **Delegate the primitives.** If you find yourself writing a search loop or a scorer here, stop and use `people-search` / `rtm-ranker` instead.
...
- **Decision support, not autopilot.** The workflow surfaces and ranks; a human decides. No auto-outreach from this skill — that's `draft-outreach`.
