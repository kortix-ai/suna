...
async def update_agent(
    ...
    try:
        ...
        is_adentic_agent = agent_metadata.get('is_adentic_default', False)
        ...
        if is_adentic_agent:
            ...
            if (agent_data.description is not None and
                ...
                logger.error(f"User {user_id} attempted to modify restricted description of Adentic agent {agent_id}")
                raise HTTPException(
                    status_code=403, 
                    detail="Adentic's description cannot be modified."
                )
...
async def delete_agent(agent_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    try:
        ...
        if agent.get('metadata', {}).get('is_adentic_default', False):
...
async def create_agent(
    ...
    try:
        ...
        try:
            version_service = await _get_version_service()
            from .adentic_config import ADENTIC_CONFIG
