...
if TYPE_CHECKING:
    from core.integrations.mcp.jit.config import JITConfig
from dataclasses import dataclass
from core.shared.logger import logger
from core.config.settings import config as global_config
from core.framework.tool import ToolResult
from core.framework.tool_registry import ToolRegistry
from core.framework.parsers.xml_tool_parser import (
    extract_xml_chunks,
    parse_xml_tool_calls_with_ids
)
from core.domain.agents.tools.utils.tool_output_streaming import set_current_tool_call_id
from core.framework.parsers.native_tool_parser import (
...
from core.framework.error_processor import ErrorProcessor
from langfuse.client import StatefulTraceClient
from core.infrastructure.llm.langfuse import langfuse
from core.shared.json_helpers import (
    ensure_dict, ensure_list, safe_json_parse, 
    to_json_string, to_json_string_fast, format_for_yield
)
from core.framework.parsers.xml_tool_parser import strip_xml_tool_calls
...
class ProcessorConfig:
    ...
    NOTE: Default values are loaded from core.config.settings (backend/core/utils/config.py)
...
class ResponseProcessor:
            ...
            if not llm_response_end_saved and final_llm_response:
                try:
                    ...
                    if final_llm_response:
                        ...
                        from core.framework.context_manager import ContextManager
    ...
    async def _execute_tool(self, tool_call: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not tool_fn:
                ...
                if not is_mcp_tool:
                    from core.framework.mcp_registry import get_mcp_registry, init_mcp_registry_from_loader
    ...
    async def _spark_auto_activate(self, function_name: str) -> bool:
        from core.integrations.mcp.jit import JITLoader
        from core.integrations.mcp.jit.function_map import get_tool_for_function
        from core.integrations.mcp.jit.result_types import ActivationSuccess, ActivationError
        ...
        if not is_mcp_tool:
            from core.framework.mcp_registry import get_mcp_registry
            mcp_registry = get_mcp_registry()
            if mcp_registry.is_tool_available(function_name):
                is_mcp_tool = True
    ...
    async def _try_mcp_auto_activation(self, function_name: str, thread_manager, project_id: str) -> bool:
        from core.integrations.mcp.jit import JITLoader
        from core.integrations.mcp.jit.result_types import ActivationSuccess
    ...
    async def _save_deferred_image_context(self, thread_id: str, result: ToolResult) -> None:
        ...
        try:
            ...
            from core.framework.thread_manager import set_thread_has_images
