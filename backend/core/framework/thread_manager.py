...
from core.infrastructure.llm.client import make_llm_api_call, LLMError
from core.framework.prompt_caching import apply_anthropic_caching_strategy, validate_cache_blocks
from core.framework.tool import Tool
from core.framework.tool_registry import ToolRegistry
from core.framework.context_manager import ContextManager
from core.framework.response_processor import ResponseProcessor, ProcessorConfig
...
class ThreadManager:
    ...
    async def add_message(
        ...
        try:
            ...
            if saved_message and 'message_id' in saved_message:
                ...
                if is_llm_message:
                    try:
                        from core.infrastructure.cache.runtime_cache import invalidate_message_history_cache
    ...
    async def get_llm_messages(self, thread_id: str, lightweight: bool = False) -> List[Dict[str, Any]]:
        ...
        if not lightweight:
            from core.infrastructure.cache.runtime_cache import get_cached_message_history
        ...
        try:
            ...
            if not lightweight:
                from core.infrastructure.cache.runtime_cache import set_cached_message_history
