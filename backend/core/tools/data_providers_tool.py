...
class DataProvidersTool(Tool):
    ...
    @openapi_schema({
        "type": "function",
        "function": {
            "name": "load_data_provider_instructions",
            "description": "REQUIRED FIRST STEP BEFORE USING THE DATA PROVIDERS TOOL: Load detailed data provider usage workflow and requirements. You MUST call this before using the data providers tool to understand the data provider usage workflow, best practices, and limitations.",
            "parameters": {
                "type": "object",
                "properties": {},
                "required": []
    ...
    async def load_data_provider_instructions(self) -> ToolResult:
        """Load detailed data provider usage workflow and requirements"""
        try:
            return self.success_response({
                "message": "Data provider usage workflow and requirements loaded successfully",
                "instructions": """
                    ### DATA PROVIDERS
            ...
            return self.fail_response(f"Failed to load data provider usage workflow and requirements: {str(e)}")
