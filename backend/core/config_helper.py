...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    is_adentic_default = metadata.get('is_adentic_default', False)
    
    # Debug logging
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_adentic_default={is_adentic_default}")
    ...
    if is_adentic_default:
        return _extract_adentic_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_adentic_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def build_unified_config(
    ...
    adentic_metadata: Optional[Dict[str, Any]] = None,
    ...
    if adentic_metadata:
        config['adentic_metadata'] = adentic_metadata
