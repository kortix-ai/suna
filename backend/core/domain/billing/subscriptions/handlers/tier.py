...
class TierHandler:
    ...
    async def get_user_subscription_tier(account_id: str, skip_cache: bool = False) -> Dict:
        ...
        if not skip_cache:
            try:
                from core.infrastructure.cache.runtime_cache import get_cached_tier_info
        ...
        try:
            from core.infrastructure.cache.runtime_cache import set_cached_tier_info
    ...
    async def get_allowed_models_for_user(user_id: str, client=None) -> List[str]:
        try:
            ...
            from core.domain.billing.shared.config import is_model_allowed
