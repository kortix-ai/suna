...
from core.middleware.utils import verify_and_get_user_id_from_jwt, verify_and_authorize_thread_access, require_thread_access, AuthorizedThreadAccess, get_optional_user_id
from core.shared.logger import logger
from core.integrations.sandbox.sandbox import create_sandbox, delete_sandbox
from core.config.settings import config, EnvMode

from core.shared.api_models.threads import CreateThreadResponse, MessageCreateRequest
...
async def get_user_threads(
    ...
    from core.domain.threads.repo import list_user_threads as repo_list_threads
...
async def get_project(
    ...
    from core.domain.threads import repo as threads_repo
...
async def create_thread_in_project(
    ...
    from core.domain.threads.repo import get_project_access, create_thread as repo_create_thread
    ...
    try:
        ...
        if config.ENV_MODE != EnvMode.LOCAL:
            from core.domain.billing.limits_checker import check_thread_limit
        ...
        from core.shared.logger import structlog
        ...
        try:
            from core.infrastructure.cache.runtime_cache import increment_thread_count_cache
...
async def delete_project(
    ...
    try:
        ...
        from core.domain.resources import ResourceService
        ...
        try:
            from core.infrastructure.cache.runtime_cache import invalidate_thread_count_cache, invalidate_project_cache
...
async def get_thread(
    ...
    from core.domain.threads import repo as threads_repo
    from core.middleware.utils import get_optional_user_id
        ...
        if thread.get('project_id'):
            ...
            if sandbox_info and sandbox_info.get('id'):
                ...
                async def start_sandbox_background():
                    try:
                        from core.integrations.sandbox.sandbox import get_or_start_sandbox
...
async def create_thread(
    ...
    try:
        if config.ENV_MODE != EnvMode.LOCAL:
            from core.domain.billing.limits_checker import check_thread_limit, check_project_count_limit
        ...
        try:
            from core.domain.resources import ResourceService, ResourceType, ResourceStatus
            resource_service = ResourceService(client)
        ...
        try:
            from core.infrastructure.cache.runtime_cache import set_cached_project_metadata
            sandbox_cache_data = {
                'id': sandbox_id,
                'pass': sandbox_pass,
                'vnc_preview': vnc_url,
...
async def get_thread_messages(
    ...
    await verify_and_authorize_thread_access(client, thread_id, user_id)
    try:
        from core.domain.threads.message_migration import migrate_thread_messages, needs_migration
...
async def add_message_to_thread(
    ...
    if thread_account_id != user_id:
        client = await db.client
        from core.middleware.utils import verify_and_authorize_thread_access
        await verify_and_authorize_thread_access(client, thread_id, user_id)
    
    try:
        thread_name = await threads_repo.get_thread_name(thread_id)
        if thread_name in ('New Chat', None):
            from core.domain.threads.thread_name_generator import generate_and_update_thread_name
...
async def delete_thread(
    ...
    from core.domain.threads.repo import (
    ...
    try:
        ...
        try:
            from core.infrastructure.cache.runtime_cache import invalidate_thread_count_cache
        ...
        if project_id:
            ...
            if remaining_thread_count == 0:
                ...
                try:
                    from core.infrastructure.cache.runtime_cache import invalidate_project_cache
                    await invalidate_project_cache(project_id)
                except Exception:
                    pass
            else:
