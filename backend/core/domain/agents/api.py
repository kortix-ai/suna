...
from core.middleware.utils import (
...
from core.domain.billing.credits.integration import billing_integration
from core.config.settings import config, EnvMode
from core.infrastructure.cache import redis
from core.config import model_manager
from core.shared.api_models.threads import UnifiedAgentStartResponse
from core.infrastructure.database.supabase import DBConnection

# Import from new modules
from core.domain.agents.runner import execute_agent_run
from core.domain.files import (
...
async def _get_agent_run_with_access_check(agent_run_id: str, user_id: str):
    ...
    from core.domain.agents import repo as agents_repo
...
async def _check_billing_and_limits(
    ...
    from core.domain.billing.limits_checker import (
...
async def _create_agent_run_record(
    ...
    from core.domain.agents import repo as agents_repo
    from core.shared.instance import get_instance_id
    ...
    # Invalidate caches
    try:
        from core.infrastructure.cache.runtime_cache import invalidate_running_runs_cache
...
async def start_agent_run(
    ...
    from core.domain.agents.config import load_agent_config
    from core.domain.threads import repo as threads_repo
    from core.domain.threads.project_helpers import generate_and_update_project_name
...
async def stop_agent(agent_run_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    """Stop an agent run."""
    from core.domain.agents.run_management import stop_agent_run_with_helpers as stop_agent_run
...
async def get_active_agent_runs(user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    from core.domain.agents.repo import get_active_agent_runs as repo_get_active_runs
...
async def get_agent_runs(thread_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    from core.domain.agents.repo import get_thread_agent_runs as repo_get_thread_runs
...
async def get_agent_run(agent_run_id: str, user_id: str = Depends(verify_and_get_user_id_from_jwt)):
    ...
    from core.domain.agents.repo import get_agent_run_by_id as repo_get_run
