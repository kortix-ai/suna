...
from core.framework.tool import SchemaType
from core.domain.agents.prompts.agent_builder_prompt import get_agent_builder_prompt
from core.domain.agents.prompts.prompt import get_system_prompt
from core.domain.agents.tools.guide_registry import get_minimal_tool_index, get_tool_guide
...
class PromptManager:
    ...
    async def build_system_prompt(model_name: str, agent_config: Optional[dict], 
        ...
        if agent_config and agent_config.get('system_prompt'):
            ...
            from core.domain.agents.prompts.core_prompt import get_core_system_prompt
    ...
    async def _fetch_mcp_config(agent_id: str, user_id: str) -> Optional[dict]:
        try:
            from core.domain.agents.versioning.version_service import get_version_service
    ...
    def _get_preloaded_tool_guides() -> str:
        from core.integrations.mcp.jit.loader import JITLoader
    ...
    async def _fetch_knowledge_base(agent_config: Optional[dict], client) -> Optional[str]:
        ...
        try:
            # Check cache first
            from core.infrastructure.cache.runtime_cache import get_cached_kb_context, set_cached_kb_context
...
"""
    ...
    async def _fetch_user_context_data(user_id: Optional[str], client) -> Optional[str]:
        ...
        # Check cache first
        from core.infrastructure.cache.runtime_cache import get_cached_user_context, set_cached_user_context
        ...
        async def fetch_locale():
            try:
                from core.shared.user_locale import get_user_locale
        ...
        if locale:
            from core.shared.user_locale import get_locale_context_prompt
    ...
    async def _fetch_user_memories(user_id: Optional[str], thread_id: str, client) -> Optional[str]:
        ...
        try:
            from core.domain.memory.retrieval_service import memory_retrieval_service
            from core.domain.billing import subscription_service
    ...
    async def _fetch_file_context(thread_id: Optional[str]) -> Optional[str]:
        ...
        try:
            from core.domain.files import get_cached_file_context, format_file_context_for_agent
