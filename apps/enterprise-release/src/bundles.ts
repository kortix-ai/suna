...
function appInstallScript(): string {
  return [
    ...
    '    --supabase-kong-origin) supabase_kong_origin="${2:-}"; shift 2 ;;',
    ...
    '  esac',
    'done',
    'for value in "$release" "$runtime_env" "$api_image" "$gateway_image" "$frontend_image" "$caddy_image" "$api_domain" "$frontend_domain" "$supabase_kong_origin"; do',
