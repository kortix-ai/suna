...
Minimal Electron wrapper for the SprintLab web application with deep linking support.

## Features

- Native macOS/Windows/Linux desktop app
- Deep linking for magic link authentication (`sprintlab://` protocol)
...
By default, it loads `https://sprintlab.id/`.
...
The app registers the `sprintlab://` protocol for magic link authentication:

1. User enters email in desktop app
2. Magic link email contains `sprintlab://auth/callback?code=xxx`
3. User clicks link in email
4. Operating system opens SprintLab Desktop app
