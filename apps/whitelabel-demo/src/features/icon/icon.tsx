import { cn } from '@/lib/utils';
import { CgClose } from 'react-icons/cg';

/**
 * Slim icon set used by the design-system primitives.
 *
 * The full Kortix `Icon` registry carries dozens of brand glyphs; the
 * white-label starter only needs the handful the shared UI primitives
 * reference, plus a neutral brand mark surfaces can reuse.
 */
export const Icon = {
  Close: ({ className, ...props }: React.ComponentProps<typeof CgClose>) => {
    return <CgClose className={cn('size-4 stroke-1', className)} {...props} />;
  },

  /** Neutral, swappable brand mark (used by the app shell / auth). */
  Brand: ({ className }: { className?: string }) => {
    return (
      <svg
        width="30"
        height="25"
        viewBox="0 0 30 25"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className={cn('size-4', className)}
      >
        <path
          d="M25.5614 24.916H29.8268C29.8268 19.6306 26.9378 15.0039 22.6171 12.4587C26.9377 9.91355 29.8267 5.28685 29.8267 0.00146484H25.5613C25.5613 5.00287 21.8906 9.18692 17.0654 10.1679V0.00146484H12.8005V10.1679C7.9526 9.20401 4.3046 5.0186 4.3046 0.00146484H0.0391572C0.0391572 5.28685 2.92822 9.91355 7.24884 12.4587C2.92818 15.0039 0.0390625 19.6306 0.0390625 24.916H4.30451C4.30451 19.8989 7.95259 15.7135 12.8005 14.7496V24.9206H17.0654V14.7496C21.9133 15.7134 25.5614 19.8989 25.5614 24.916Z"
          fill="currentColor"
        />
      </svg>
    );
  },
};
