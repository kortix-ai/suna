...
import { 
  ...
  useSprintLabComputerPendingToolNavIndex,
  useSprintLabComputerClearPendingToolNav,
} from '@/stores/sprintlab-computer-store';
...
interface SprintLabComputerProps {
  isOpen: boolean;
  onClose: () => void;
...
const FLOATING_LAYOUT_ID = 'sprintlab-computer-float';

export const SprintLabComputer = memo(function SprintLabComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useSprintLabComputerPendingToolNavIndex();
  const clearPendingToolNav = useSprintLabComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[SprintLabComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the SprintLab Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the SprintLab Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="sprintlab-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
