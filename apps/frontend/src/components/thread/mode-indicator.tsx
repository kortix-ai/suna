...
const ModeLogo = memo(function ModeLogo({ 
  ...
  return (
    <span className="flex-shrink-0 relative" style={{ height: `${height}px`, width: 'auto' }}>
      ...
      <img
        src={lightSrc}
        alt={mode === 'advanced' ? 'SprintLab Advanced' : 'SprintLab Basic'}
...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'kortix/basic' || m.label === 'SprintLab Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'kortix/power' || m.label === 'SprintLab Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'kortix/basic' && m.id !== 'kortix/power' && 
             m.label !== 'SprintLab Basic' && m.label !== 'SprintLab Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access SprintLab Advanced mode',
        });
      }
    }
