...
import { SprintLabComputer, ToolCallInput } from '@/components/thread/sprintlab-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // SprintLab Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useSprintLabComputerStore();
  ...
  // Handle file click - now opens in SprintLab Computer instead of modal
  ...
  const selectedFilePath = useSprintLabComputerStore((state) => state.selectedFilePath);
