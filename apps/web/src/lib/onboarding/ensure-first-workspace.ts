import { type KortixWorkspace, listWorkspacesForAccount, provisionWorkspace } from '@kortix/sdk';

import { isValidWorkspaceId } from '@/lib/onboarding/landing-destination';
...
/** Name + template for every auto-provisioned first workspace. */
export const FIRST_WORKSPACE_NAME = 'My First Workspace';
export const FIRST_WORKSPACE_TEMPLATE = 'general-knowledge-worker';

/**
 * Set when the user archives their LAST workspace. Auto-provisioning is otherwise
 * unconditional, and without this flag deleting your only workspace would
 ...
 */
const SUPPRESS_AUTO_WORKSPACE_KEY = 'kortix:suppress-auto-workspace';
...
export function suppressAutoWorkspaceAfterDelete(): void {
  safeSessionStorage()?.setItem(SUPPRESS_AUTO_WORKSPACE_KEY, '1');
}

export function isAutoWorkspaceSuppressed(): boolean {
  return safeSessionStorage()?.getItem(SUPPRESS_AUTO_WORKSPACE_KEY) === '1';
}

export function clearAutoWorkspaceSuppression(): void {
  safeSessionStorage()?.removeItem(SUPPRESS_AUTO_WORKSPACE_KEY);
}
 ...
 * timeout (`provisionWorkspace`). At 6x that ceiling the bound can never expire
...
/**
 * Stable `idempotency_key` for one account's first-workspace auto-provision
 * attempt.
 *
 * WHY LOCALSTORAGE, NOT A REF OR SESSIONSTORAGE. `autoCreateAttempted`
 * (workspaces/page.tsx) and `resolving` (start/page.tsx) are per-mount refs —
 ...
 * opened in a new tab, attempts the same logical create. `/workspaces` and
 * `/workspaces/start` are two independent doors that can both run this exact
 ...
 * server is intentionally NOT scoped to active workspaces — an archived row
 ...
 * workspace and a fresh auto-create fires) would replay it and get back the
 * archived workspace instead of creating a new one.
 *
 * TWO BOUNDS, because one is not enough. `clearProvisionAttemptKey` drops the
 * key the moment `ensureFirstWorkspace` resolves — but that function is only
 * reached from `/workspaces/start`, and from `/workspaces` when the account has
 * zero workspaces. So a provision that COMMITS server-side while every client
 * attempt errors (a 120s SDK timeout, then the retry budget exhausted, then
 * the "We could not open your workspace" screen) leaves the key behind with no
 * caller left to clear it: the user clicks "View all workspaces", the workspace is
 * now there, and nothing calls `ensureFirstWorkspace` again. That stale key is
 * exactly what would later replay onto the ARCHIVED workspace. The second bound
...
export function getOrCreateProvisionAttemptKey(accountId: string, now = Date.now()): string {
  ...
  if (store) {
    try {
      ...
      const minted = `onboarding-first-workspace:${randomAttemptToken()}`;
  ...
  return `onboarding-first-workspace:${randomAttemptToken()}`;
}

/**
 * Drop the persisted attempt key for this account. Called the moment
 * `ensureFirstWorkspace` resolves to a workspace — by definition the attempt this
...
/**
 * Whether this navigation may CREATE a workspace, as opposed to only opening one
 * that already exists.
 *
 ...
 * Both auto-provisioning entry points — the landing door and the empty workspaces
 * list — gate creation on this. Opening an EXISTING workspace stays allowed from
 * anywhere.
 *
 ...
 *    workspaces list instead of their workspace.
 ...
 * for an account with ZERO workspaces, and the free tier caps such an account at
 * one workspace anyway — so the worst case is the user getting the workspace that
 * sign-up would have created for them regardless.
 */
export function navigationMayCreateWorkspace(): boolean {
...
/**
 * True for the `409` `POST /workspaces/provision` returns when another call
 * carrying the SAME `idempotency_key` is mid-provision — see
 * `apps/api/src/workspaces/lib/provision-idempotency.ts`'s `in_flight` case.
...
/**
 * Pick which existing workspace to open: the one the browser last had open, else
 * the first. `preferredWorkspaceId` is untrusted because it comes from a cookie, so it
...
): KortixWorkspace | null {
  if (workspaces.length === 0) return null;
  if (isValidWorkspaceId(preferredWorkspaceId)) {
...
/**
 * The two network calls `ensureFirstWorkspace` needs, injectable so tests can
 ...
 * this parameter.
 */
export type EnsureFirstWorkspaceClient = {
  listWorkspacesForAccount: (accountId?: string) => Promise<KortixWorkspace[]>;
  provisionWorkspace: typeof provisionWorkspace;
};

/**
 * Return the workspace this account should open, creating one when the account
 * has none.
 *
 * This deliberately provisions. The previous behaviour returned null for an
 * empty account and made the UI open the create-workspace modal, on the reasoning
 ...
 * committed safe (the list finds the workspace). What it cannot cover is a
 ...
 * dedupe in `provision-idempotency.ts` collapses them to one workspace.
 */
export async function ensureFirstWorkspace(
  accountId: string,
  opts: { preferredWorkspaceId?: string | null; allowCreate?: boolean } = {},
  ...
  client: EnsureFirstWorkspaceClient = { listWorkspacesForAccount, provisionWorkspace },
): Promise<KortixWorkspace | null> {
  const existing = await client.listWorkspacesForAccount(accountId);
  const picked = pickLandingWorkspace(existing, opts.preferredWorkspaceId);
  if (picked) {
    ...
    // (e.g. after the user deletes their only workspace) never replays a stale
  ...
  try {
    const created = await client.provisionWorkspace({
      account_id: accountId,
      name: FIRST_WORKSPACE_NAME,
      seed_starter: true,
      starter_template: FIRST_WORKSPACE_TEMPLATE,
    ...
    // a workspace, go look before giving up":
    //  - `isWorkspaceLimitError` — losing a create race against another tab.
    ...
    // this catch.
    if (isWorkspaceLimitError(err) || isProvisionInFlightError(err)) {
      const retry = await client.listWorkspacesForAccount(accountId);
      const retryPicked = pickLandingWorkspace(retry, opts.preferredWorkspaceId);
      if (retryPicked) {
        clearProvisionAttemptKey(accountId);
        return retryPicked;
      }
