...
describe('menu registry feature-flag gating', () => {
  ...
  test('Review Center and Voice are reachable from the palette, behind their flags', () => {
    // Ported at the settings-panel merge: `main` authored these two entries
    // against `/customize/<section>`, the overlay this branch deleted. The
    // gating half is `main`'s unchanged; only the destination vocabulary is
    // this branch's, and the "must resolve to a real tab" check now runs
    // against `parseSettingsTab` — the live parser, not a source scan.
    ...
    expect(review?.href).toBe('/projects/{projectId}/settings/review');
    expect(voice?.requiresFlag).toBe('voice');
    expect(voice?.href).toBe('/projects/{projectId}/settings/voice');
    // Both must resolve to a real settings tab, or the palette opens the
    // overlay on nothing.
