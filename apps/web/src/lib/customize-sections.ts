/**
 * FROZEN LEGACY VOCABULARY — do not "helpfully" alias this to the new module.
 *
 * This file is the pre-merge Customize-overlay id space, kept byte-for-byte
 * so every current importer keeps compiling and behaving exactly as before
 * while the settings-panel merge is in progress. It is deliberately NOT a
 * re-export of `@/features/workspace/settings/settings-tabs` — that module's
 * `SettingsTab` union does not contain `git`, `commands`, `settings`,
 * `llm-management`, or `upgrade`, so aliasing the two here would silently
 * break every caller below (that was tried and reverted — see the settings
 * panel plan, Task 3 fix round 1). The two vocabularies coexist on purpose
 * until the task that rewires the panel deletes this file and its callers
 * move over to `settings-tabs.ts`, the new source of truth.
 *
