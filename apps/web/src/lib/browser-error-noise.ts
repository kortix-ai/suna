...
// `app:///_next/static/chunks/…` (see `isBrowserBundleSource`) — and a loaded
// script asset carries its own `.js` url as its filename, not the document url.
// Distinct from the bare `app:///` (empty path) WebView instrumentation source
// in `IOS_WEBVIEW_INSTRUMENTED_FRAME_SOURCE`.
 ...
 * INLINE into the document (the Electron desktop bootstrap in
 * `apps/web/src/lib/desktop.ts`, the analytics-init snippets in the locale
 * layout) shares this source, so an overflow in one of those snippets would be
 * dropped. Those are small, non-recursive bootstraps, and the already-shipped
...
export function shouldIgnoreBrowserRuntimeNoise(input: {
  ...
  // iOS-WebView in-document inline-script stack-overflow noise — the sibling
  // shape where `window.onerror` DID keep a frame, but it is the WebView's
  // in-page document source (`app:///<route>`) rather than a `_next` bundle
  // chunk. Browser-injected inline scripts only; a first-party `apps/web/src/…`
  // or `_next` chunk frame keeps reporting. See
  // `isIosWebViewInjectedStackOverflowNoise`.
  if (isIosWebViewInjectedStackOverflowNoise({ message, filename: input.filename })) {
    return true;
  }
...
export function shouldIgnoreSentryBrowserNoise(event: {
  ...
  // iOS-WebView in-document inline-script stack-overflow noise — the sibling
  // shape with frames that are ALL the WebView's in-page document source
  // (`app:///<route>`) rather than a `_next` bundle chunk. Browser-injected
  // inline scripts only; a first-party `apps/web/src/…` or `_next` chunk frame
  // keeps reporting. See `isIosWebViewInjectedStackOverflowNoise`. NOT in
  // `ignoreErrors` (no frame context there).
  if (isIosWebViewInjectedStackOverflowNoise({ message, frames, requestUrl })) {
    return true;
  }
