...
function Input({
  ...
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'border-input bg-background file:text-foreground placeholder:text-muted-foreground/70 flex h-10 w-full min-w-0 rounded-md border px-3 py-1 text-sm shadow-xs transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50',
        ...
        variant === 'secondary' &&
          'bg-input text-secondary-foreground border-none shadow-xs',
        size === 'xs' && 'h-8 text-xs',
        size === 'sm' && 'h-9 text-sm',
