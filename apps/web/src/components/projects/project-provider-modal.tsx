    ...
    <div className="border-border/50 bg-muted/20 rounded-2xl border p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-muted-foreground mt-0.5 text-xs leading-5">
            Use OpenCode headless auth, then save the resulting login as an encrypted project secret
            for future sessions.
          </p>
        </div>
      </div>
      {(authInstructions || authUrl) && (
        <div className="border-border/50 bg-background/70 mt-3 rounded-2xl border p-3">
          <div className="text-foreground text-xs font-medium">Complete authorization</div>
          {authUrl && (
            <Button
              type="button"
              size="sm"
