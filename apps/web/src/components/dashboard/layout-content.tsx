...
import { isTabRecentlyClosed, useTabStore, type TabType } from '@/stores/tab-store';
...
const TabPane = memo(function TabPane({
  ...
  return (
    <div className={cn('absolute inset-0 flex flex-col', className, !isActive && 'hidden')}>
      {children}
    </div>
  );
});
...
const PAGE_PANE_TYPES: ReadonlySet<TabType> = new Set(['settings', 'page', 'project', 'dashboard']);
...
function SessionTabsContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={cn('bg-background relative flex min-h-0 flex-1 flex-col overflow-hidden')}>
      {mounted.map(({ id, type }) => (
        <TabPane
          key={id}
          id={id}
      ...
      <div className={cn('flex min-h-0 flex-1 flex-col overflow-y-auto', hasActiveTab && 'hidden')}>
        {children}
      </div>
    </div>
  );
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button variant="outline" size="sm" className="absolute top-3 right-3 z-20">
          {tHardcodedUi.raw('componentsDashboardLayoutContent.line421JsxTextSkipOnboarding')}
          <ChevronRight className="h-3.5 w-3.5" />
        </Button>
      </AlertDialogTrigger>
      <AlertDialogContent className="max-w-sm rounded-2xl">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-foreground/90 text-base font-medium">
            {tHardcodedUi.raw('componentsDashboardLayoutContent.line427JsxTextSkipOnboarding')}
          </AlertDialogTitle>
          <AlertDialogDescription className="text-muted-foreground/60 text-sm leading-relaxed">
            {tHardcodedUi.raw(
              'componentsDashboardLayoutContent.line429JsxTextYouCanSetUpYourProfileAnytimeYour',
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter className="gap-2 sm:gap-2">
          ...
          <AlertDialogAction
            onClick={onConfirm}
            className="bg-foreground text-background hover:bg-foreground/90 rounded-full text-sm"
          >
            {tHardcodedUi.raw('componentsDashboardLayoutContent.line438JsxTextSkipOnboarding')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export default function DashboardLayoutContent({
  ...
  const [onboardingChecked, setOnboardingChecked] = useState(() => {
    ...
    try {
      return (
        localStorage.getItem(`kortix-onboarding-complete:${routeInstanceId || 'default'}`) ===
        'true'
      );
    } catch {
      return false;
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        const res = await f(`${instanceUrl}/env/ONBOARDING_COMPLETE`, undefined, {
        ...
        if (!res.ok) {
          ...
          console.warn(
            `[onboarding] /env/ONBOARDING_COMPLETE returned ${res.status} — deferring, not entering onboarding`,
          );
          return;
        }
        ...
        if (data?.ONBOARDING_COMPLETE === 'true') {
          ...
          try {
            localStorage.removeItem(`kortix-onboarding-complete:${routeInstanceId || 'default'}`);
          } catch {
            /* non-fatal */
          }
        ...
        console.warn(
          '[onboarding] Failed to check ONBOARDING_COMPLETE — deferring, not entering onboarding',
        );
      }
    };
    check();
    ...
    (async () => {
      try {
        ...
        if (needsCmd && !obCmdFired.current) {
          ...
          void getClient()
            .session.command({ sessionID: sid, command: 'onboarding', arguments: '' })
            .catch(() => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        persistEnv('ONBOARDING_COMMAND_FIRED', 'true');
        void c.session
          .command({ sessionID: ob.sessionId!, command: 'onboarding', arguments: '' })
          .catch(() => {
            obCmdFired.current = false;
          });
      } catch {}
  ...
  useEffect(() => {
    ...
    const unsubscribe = useServerStore.subscribe((s) => {
      if (cancelled) return;
      const match = s.servers.find((server) => server.instanceId === routeInstanceId);
      if (!match) return;
  ...
  useEffect(() => {
    ...
    const connectionSettled = connectionStatus !== 'connecting' || hasNoSandbox;
  ...
  const maintenanceBlock =
    isMaintenanceActive && !systemStatusLoading && !isCheckingAdminRole && !isAdmin;
        ...
        <NovuInboxProvider>
          ...
          >
            ...
            <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
              ...
              {!hideChrome && disableTabSelector && (
                <div className="bg-sidebar h-3 flex-shrink-0" />
              )}
                  ...
                  <div className="bg-background absolute inset-0 z-10 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-muted-foreground text-xs">
                        {tHardcodedUi.raw(
                          'componentsDashboardLayoutContent.line1114JsxTextSettingUpYourWorkspace',
                        )}
                      </p>
                    </div>
                  </div>
            ...
            {maintenanceConfig &&
              maintenanceConfig.level !== 'none' &&
              maintenanceConfig.level !== 'blocking' && (
                <Suspense fallback={null}>
