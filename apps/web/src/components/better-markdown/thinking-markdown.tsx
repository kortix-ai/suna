...
const components: Components = {
  ...
  p: ({ children }) => (
    <p className="text-primary/80 text-[15px] leading-relaxed font-medium">{children}</p>
  ),
  span: ({ children }) => (
    <span className="text-primary/80 text-[15px] leading-relaxed font-medium">{children}</span>
  ),
  ul: ({ children }) => (
  ...
  table: ({ children }) => (
    <div className="border-border mt-6 w-full overflow-hidden overflow-y-auto rounded-xl border">
...
export function ThinkingMarkdown({ children }: ThinkingMarkdownProps) {
  return (
    <div
      className={cn(
