// Generated from tests/spec/routes.generated.json. Do not edit directly.
// Run: node apps/web/scripts/generate-audit-http-routes.mjs

const AUDIT_HTTP_ROUTE_KEYS = [
  "GET|health",
  "GET|health|live",
  "GET|health|ready",
  "POST|internal|gateway|authenticate",
  "POST|internal|gateway|authorize",
  "POST|internal|gateway|billing",
  "POST|internal|gateway|budget-check",
  "POST|internal|gateway|models",
  "POST|internal|gateway|resolve-route",
  "POST|internal|gateway|resolve-upstream",
  "POST|internal|gateway|trace",
  "POST|internal|gateway|usage",
  "GET|metrics",
  "GET|scim|v2|accounts|:accountId|Groups",
  "POST|scim|v2|accounts|:accountId|Groups",
  "DELETE|scim|v2|accounts|:accountId|Groups|:groupId",
  "GET|scim|v2|accounts|:accountId|Groups|:groupId",
  "PATCH|scim|v2|accounts|:accountId|Groups|:groupId",
  "PUT|scim|v2|accounts|:accountId|Groups|:groupId",
  "GET|scim|v2|accounts|:accountId|ResourceTypes",
  "GET|scim|v2|accounts|:accountId|ResourceTypes|:id",
  "GET|scim|v2|accounts|:accountId|Schemas",
  "GET|scim|v2|accounts|:accountId|Schemas|:id",
  "GET|scim|v2|accounts|:accountId|ServiceProviderConfig",
  "GET|scim|v2|accounts|:accountId|Users",
  "POST|scim|v2|accounts|:accountId|Users",
  "DELETE|scim|v2|accounts|:accountId|Users|:userId",
  "GET|scim|v2|accounts|:accountId|Users|:userId",
  "PATCH|scim|v2|accounts|:accountId|Users|:userId",
  "PUT|scim|v2|accounts|:accountId|Users|:userId",
  "POST|v1|access|check-email",
  "POST|v1|access|request-access",
  "GET|v1|access|signup-status",
  "GET|v1|account-invites|:inviteId",
  "POST|v1|account-invites|:inviteId|accept",
  "POST|v1|account-invites|:inviteId|decline",
  "POST|v1|account|cancel-deletion",
  "DELETE|v1|account|delete-immediately",
  "GET|v1|account|deletion-status",
  "POST|v1|account|request-deletion",
  "GET|v1|accounts",
  "POST|v1|accounts",
  "GET|v1|accounts|:accountId",
  "PATCH|v1|accounts|:accountId",
  "GET|v1|accounts|:accountId|audit",
  "GET|v1|accounts|:accountId|audit|export",
  "POST|v1|accounts|:accountId|audit|reconcile",
  "GET|v1|accounts|:accountId|audit|webhooks",
  "POST|v1|accounts|:accountId|audit|webhooks",
  "DELETE|v1|accounts|:accountId|audit|webhooks|:webhookId",
  "PATCH|v1|accounts|:accountId|audit|webhooks|:webhookId",
  "GET|v1|accounts|:accountId|audit|webhooks|:webhookId|deliveries",
  "POST|v1|accounts|:accountId|audit|webhooks|:webhookId|deliveries|:deliveryId|replay",
  "GET|v1|accounts|:accountId|iam|actions",
  "GET|v1|accounts|:accountId|iam|agent-identities",
  "GET|v1|accounts|:accountId|iam|enterprise-demo",
  "PUT|v1|accounts|:accountId|iam|enterprise-demo",
  "GET|v1|accounts|:accountId|iam|groups",
  "POST|v1|accounts|:accountId|iam|groups",
  "DELETE|v1|accounts|:accountId|iam|groups|:groupId",
  "GET|v1|accounts|:accountId|iam|groups|:groupId",
  "PATCH|v1|accounts|:accountId|iam|groups|:groupId",
  "GET|v1|accounts|:accountId|iam|groups|:groupId|members",
  "POST|v1|accounts|:accountId|iam|groups|:groupId|members",
  "DELETE|v1|accounts|:accountId|iam|groups|:groupId|members|:userId",
  "GET|v1|accounts|:accountId|iam|groups|:groupId|project-grants",
  "GET|v1|accounts|:accountId|iam|members|:userId|effective",
  "POST|v1|accounts|:accountId|iam|members|:userId|effective:batch",
  "GET|v1|accounts|:accountId|iam|members|:userId|groups",
  "GET|v1|accounts|:accountId|iam|members|:userId|project-access",
  "PATCH|v1|accounts|:accountId|iam|members|:userId|super-admin",
  "GET|v1|accounts|:accountId|iam|mfa-required",
  "PATCH|v1|accounts|:accountId|iam|mfa-required",
  "GET|v1|accounts|:accountId|iam|mfa-required|preview",
  "GET|v1|accounts|:accountId|iam|pat-policy",
  "PATCH|v1|accounts|:accountId|iam|pat-policy",
  "GET|v1|accounts|:accountId|iam|policies",
  "POST|v1|accounts|:accountId|iam|policies",
  "POST|v1|accounts|:accountId|iam|policies:bulk-delete",
  "POST|v1|accounts|:accountId|iam|policies:bulk-import",
  "DELETE|v1|accounts|:accountId|iam|policies|:policyId",
  "PATCH|v1|accounts|:accountId|iam|policies|:policyId",
  "GET|v1|accounts|:accountId|iam|roles",
  "POST|v1|accounts|:accountId|iam|roles",
  "DELETE|v1|accounts|:accountId|iam|roles|:roleId",
  "PATCH|v1|accounts|:accountId|iam|roles|:roleId",
  "GET|v1|accounts|:accountId|iam|roles|:roleId|permissions",
  "PUT|v1|accounts|:accountId|iam|roles|:roleId|permissions",
  "GET|v1|accounts|:accountId|iam|roles|:roleId|usage",
  "GET|v1|accounts|:accountId|iam|scim|tokens",
  "POST|v1|accounts|:accountId|iam|scim|tokens",
  "DELETE|v1|accounts|:accountId|iam|scim|tokens|:tokenId",
  "GET|v1|accounts|:accountId|iam|service-accounts",
  "POST|v1|accounts|:accountId|iam|service-accounts",
  "DELETE|v1|accounts|:accountId|iam|service-accounts|:saId",
  "POST|v1|accounts|:accountId|iam|service-accounts|:saId|disable",
  "GET|v1|accounts|:accountId|iam|session-policy",
  "PATCH|v1|accounts|:accountId|iam|session-policy",
  "GET|v1|accounts|:accountId|iam|sessions",
  "POST|v1|accounts|:accountId|iam|sessions|:sessionId|revoke",
  "GET|v1|accounts|:accountId|iam|sso|mappings",
  "POST|v1|accounts|:accountId|iam|sso|mappings",
  "DELETE|v1|accounts|:accountId|iam|sso|mappings|:mappingId",
  "DELETE|v1|accounts|:accountId|iam|sso|provider",
  "GET|v1|accounts|:accountId|iam|sso|provider",
  "PUT|v1|accounts|:accountId|iam|sso|provider",
  "POST|v1|accounts|:accountId|iam|sso|provider|from-metadata",
  "GET|v1|accounts|:accountId|invites",
  "DELETE|v1|accounts|:accountId|invites|:inviteId",
  "POST|v1|accounts|:accountId|invites|:inviteId|resend",
  "POST|v1|accounts|:accountId|leave",
  "GET|v1|accounts|:accountId|members",
  "POST|v1|accounts|:accountId|members",
  "DELETE|v1|accounts|:accountId|members|:userId",
  "PATCH|v1|accounts|:accountId|members|:userId",
  "GET|v1|accounts|me",
  "GET|v1|accounts|tokens",
  "POST|v1|accounts|tokens",
  "DELETE|v1|accounts|tokens|:tokenId",
  "GET|v1|admin|analytics|activity",
  "GET|v1|admin|analytics|usage",
  "GET|v1|admin|api|accounts",
  "POST|v1|admin|api|accounts|:id|credits",
  "POST|v1|admin|api|accounts|:id|credits|debit",
  "POST|v1|admin|api|accounts|:id|enterprise-demo",
  "POST|v1|admin|api|accounts|:id|enterprise-entitlement",
  "GET|v1|admin|api|accounts|:id|ledger",
  "POST|v1|admin|api|accounts|:id|managed-models",
  "GET|v1|admin|api|accounts|:id|projects",
  "POST|v1|admin|api|accounts|:id|session-limit",
  "POST|v1|admin|api|accounts|:id|tier",
  "DELETE|v1|admin|api|accounts|:id|trial",
  "POST|v1|admin|api|accounts|:id|trial",
  "GET|v1|admin|api|accounts|:id|users",
  "GET|v1|admin|api|projects",
  "GET|v1|admin|api|provider-analytics",
  "GET|v1|admin|api|provider-distribution",
  "PUT|v1|admin|api|provider-distribution",
  "GET|v1|admin|api|provider-fallback",
  "PUT|v1|admin|api|provider-fallback",
  "GET|v1|admin|api|sandboxes",
  "POST|v1|admin|api|sandboxes|:sessionId|migrate",
  "GET|v1|approval-links|:token",
  "POST|v1|auth|logout",
  "GET|v1|billing|account-state",
  "GET|v1|billing|account-state|minimal",
  "POST|v1|billing|account|cancel-deletion",
  "DELETE|v1|billing|account|delete-immediately",
  "GET|v1|billing|account|deletion-status",
  "POST|v1|billing|account|request-deletion",
  "POST|v1|billing|auto-topup|configure",
  "GET|v1|billing|auto-topup|settings",
  "GET|v1|billing|auto-topup|setup-status",
  "POST|v1|billing|cancel-scheduled-change",
  "POST|v1|billing|cancel-subscription",
  "GET|v1|billing|checkout-session|:sessionId",
  "POST|v1|billing|claim-per-seat",
  "POST|v1|billing|confirm-checkout-session",
  "POST|v1|billing|confirm-inline-checkout",
  "POST|v1|billing|create-checkout-session",
  "POST|v1|billing|create-inline-checkout",
  "POST|v1|billing|create-per-seat-checkout",
  "POST|v1|billing|create-portal-session",
  "GET|v1|billing|credit-breakdown",
  "GET|v1|billing|credit-usage",
  "POST|v1|billing|cron|free-tier-rotation",
  "POST|v1|billing|cron|trial-expiry",
  "POST|v1|billing|cron|yearly-rotation",
  "POST|v1|billing|deduct",
  "POST|v1|billing|deduct-usage",
  "GET|v1|billing|proration-preview",
  "POST|v1|billing|purchase-credits",
  "POST|v1|billing|reactivate-subscription",
  "POST|v1|billing|schedule-downgrade",
  "POST|v1|billing|sync-seat-quantity",
  "POST|v1|billing|sync-subscription",
  "GET|v1|billing|tier-configurations",
  "GET|v1|billing|transactions",
  "GET|v1|billing|transactions|summary",
  "GET|v1|billing|usage-history",
  "POST|v1|billing|webhook|revenuecat",
  "POST|v1|billing|webhook|stripe",
  "POST|v1|billing|webhooks|revenuecat",
  "POST|v1|billing|webhooks|stripe",
  "POST|v1|channels|slack|identity|bind",
  "GET|v1|channels|slack|identity|login|:token",
  "POST|v1|channels|teams|identity|bind",
  "GET|v1|channels|teams|identity|login|:token",
  "POST|v1|connectors|attachments",
  "POST|v1|connectors|call",
  "GET|v1|connectors|catalog",
  "GET|v1|connectors|connect-status",
  "GET|v1|connectors|connectors",
  "GET|v1|connectors|oauth2|callback",
  "POST|v1|connectors|projects|:projectId|attachments",
  "POST|v1|connectors|projects|:projectId|call",
  "GET|v1|connectors|projects|:projectId|catalog",
  "GET|v1|connectors|projects|:projectId|connectors",
  "POST|v1|connectors|projects|:projectId|connectors",
  "DELETE|v1|connectors|projects|:projectId|connectors|:slug",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|authorization-strategy",
  "GET|v1|connectors|projects|:projectId|connectors|:slug|config",
  "POST|v1|connectors|projects|:projectId|connectors|:slug|connect",
  "POST|v1|connectors|projects|:projectId|connectors|:slug|connect|finalize",
  "DELETE|v1|connectors|projects|:projectId|connectors|:slug|credential",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|credential",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|credential-mode",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|name",
  "GET|v1|connectors|projects|:projectId|connectors|:slug|policies",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|policies",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|secret-binding",
  "PUT|v1|connectors|projects|:projectId|connectors|:slug|sensitive",
  "POST|v1|connectors|projects|:projectId|connectors|auth-discovery",
  "POST|v1|connectors|projects|:projectId|connectors|sync",
  "GET|v1|connectors|projects|:projectId|discover|connectors",
  "GET|v1|connectors|projects|:projectId|discover|connectors|detail",
  "GET|v1|connectors|projects|:projectId|pipedream|apps",
  "GET|v1|connectors|projects|:projectId|policies",
  "PUT|v1|connectors|projects|:projectId|policies",
  "POST|v1|connectors|webhook|pipedream",
  "GET|v1|docs",
  "GET|v1|generation",
  "POST|v1|git|:project|git-receive-pack",
  "POST|v1|git|:project|git-upload-pack",
  "GET|v1|git|:project|info|refs",
  "GET|v1|health",
  "GET|v1|health|live",
  "GET|v1|health|ready",
  "POST|v1|llm|chat|completions",
  "GET|v1|llm|health",
  "POST|v1|llm|messages",
  "GET|v1|llm|models",
  "POST|v1|llm|v1|chat|completions",
  "POST|v1|llm|v1|messages",
  "GET|v1|llm|v1|models",
  "GET|v1|marketplace|items",
  "GET|v1|marketplace|items|:id",
  "GET|v1|marketplace|items|:id|file",
  "GET|v1|marketplace|marketplaces",
  "GET|v1|marketplace|marketplaces|featured",
  "GET|v1|marketplace|sources",
  "POST|v1|marketplace|sources",
  "DELETE|v1|marketplace|sources|:id",
  "GET|v1|oauth|authorize",
  "POST|v1|oauth|authorize|consent",
  "GET|v1|oauth|authorize|consent|:requestId",
  "POST|v1|oauth|token",
  "GET|v1|oauth|userinfo",
  "GET|v1|openapi.json",
  "GET|v1|ops|overview",
  "OPTIONS|v1|p|auth",
  "POST|v1|p|auth",
  "GET|v1|p|public-share|:token",
  "GET|v1|p|share",
  "POST|v1|p|share",
  "DELETE|v1|p|share|:token",
  "GET|v1|platform",
  "POST|v1|platform|boot-timeline",
  "DELETE|v1|platform|github-app",
  "POST|v1|platform|github-app|app",
  "GET|v1|platform|github-app|install-callback",
  "GET|v1|platform|github-app|manifest-callback",
  "POST|v1|platform|github-app|manifest-start",
  "POST|v1|platform|github-app|pat",
  "GET|v1|platform|github-app|status",
  "GET|v1|platform|sandbox|version",
  "GET|v1|platform|sandbox|version|all",
  "GET|v1|platform|sandbox|version|changelog",
  "GET|v1|platform|sandbox|version|latest",
  "POST|v1|prewarm",
  "GET|v1|projects",
  "POST|v1|projects",
  "DELETE|v1|projects|:projectId",
  "GET|v1|projects|:projectId",
  "PATCH|v1|projects|:projectId",
  "GET|v1|projects|:projectId|access",
  "GET|v1|projects|:projectId|access-requests",
  "POST|v1|projects|:projectId|access-requests",
  "POST|v1|projects|:projectId|access-requests|:requestId|approve",
  "POST|v1|projects|:projectId|access-requests|:requestId|reject",
  "DELETE|v1|projects|:projectId|access|:userId",
  "PUT|v1|projects|:projectId|access|:userId",
  "POST|v1|projects|:projectId|access|invite",
  "GET|v1|projects|:projectId|access|pending-invites",
  "DELETE|v1|projects|:projectId|access|pending-invites|:inviteId",
  "POST|v1|projects|:projectId|access|pending-invites|:inviteId|resend",
  "GET|v1|projects|:projectId|agents|:agentName|config",
  "PUT|v1|projects|:projectId|agents|:agentName|config",
  "PUT|v1|projects|:projectId|agents|:agentName|scope",
  "GET|v1|projects|:projectId|approvals",
  "POST|v1|projects|:projectId|approvals|:executionId",
  "GET|v1|projects|:projectId|approvals|needs-input",
  "GET|v1|projects|:projectId|apps",
  "POST|v1|projects|:projectId|apps",
  "DELETE|v1|projects|:projectId|apps|:appId",
  "GET|v1|projects|:projectId|apps|:appId",
  "PATCH|v1|projects|:projectId|apps|:appId",
  "GET|v1|projects|:projectId|apps|:appId|access",
  "PATCH|v1|projects|:projectId|apps|:appId|access",
  "POST|v1|projects|:projectId|apps|:appId|access-session",
  "GET|v1|projects|:projectId|apps|:appId|deployments",
  "POST|v1|projects|:projectId|apps|:appId|deployments",
  "GET|v1|projects|:projectId|apps|:appId|deployments|:deploymentId",
  "GET|v1|projects|:projectId|apps|:appId|deployments|:deploymentId|logs",
  "POST|v1|projects|:projectId|apps|:appId|rollback",
  "POST|v1|projects|:projectId|apps|:appId|start",
  "POST|v1|projects|:projectId|apps|:appId|stop",
  "POST|v1|projects|:projectId|apps|artifacts",
  "POST|v1|projects|:projectId|apps|artifacts|:artifactId|finalize",
  "GET|v1|projects|:projectId|audit",
  "GET|v1|projects|:projectId|branches",
  "GET|v1|projects|:projectId|change-requests",
  "POST|v1|projects|:projectId|change-requests",
  "GET|v1|projects|:projectId|change-requests|:crId",
  "PATCH|v1|projects|:projectId|change-requests|:crId",
  "POST|v1|projects|:projectId|change-requests|:crId|close",
  "GET|v1|projects|:projectId|change-requests|:crId|diff",
  "POST|v1|projects|:projectId|change-requests|:crId|merge",
  "GET|v1|projects|:projectId|change-requests|:crId|merge-preview",
  "POST|v1|projects|:projectId|change-requests|:crId|reopen",
  "POST|v1|projects|:projectId|change-requests|:crId|request-changes",
  "GET|v1|projects|:projectId|channels|bindings",
  "PATCH|v1|projects|:projectId|channels|bindings|:bindingId",
  "POST|v1|projects|:projectId|channels|email|connect",
  "DELETE|v1|projects|:projectId|channels|email|installation",
  "GET|v1|projects|:projectId|channels|email|installation",
  "PATCH|v1|projects|:projectId|channels|email|installation",
  "GET|v1|projects|:projectId|channels|email|mode",
  "PUT|v1|projects|:projectId|channels|meet|name",
  "POST|v1|projects|:projectId|channels|slack|bind-thread",
  "POST|v1|projects|:projectId|channels|slack|connect",
  "GET|v1|projects|:projectId|channels|slack|file",
  "POST|v1|projects|:projectId|channels|slack|file|upload",
  "DELETE|v1|projects|:projectId|channels|slack|installation",
  "GET|v1|projects|:projectId|channels|slack|installation",
  "GET|v1|projects|:projectId|channels|slack|mode",
  "POST|v1|projects|:projectId|channels|teams|connect",
  "GET|v1|projects|:projectId|channels|teams|file",
  "POST|v1|projects|:projectId|channels|teams|file|upload",
  "DELETE|v1|projects|:projectId|channels|teams|installation",
  "GET|v1|projects|:projectId|channels|teams|installation",
  "GET|v1|projects|:projectId|channels|teams|manifest",
  "GET|v1|projects|:projectId|channels|teams|mode",
  "GET|v1|projects|:projectId|cli-token",
  "POST|v1|projects|:projectId|cli-token",
  "DELETE|v1|projects|:projectId|cli-token|:tokenId",
  "GET|v1|projects|:projectId|commits",
  "GET|v1|projects|:projectId|commits|:sha",
  "GET|v1|projects|:projectId|commits|:sha|diff",
  "POST|v1|projects|:projectId|connect-requests",
  "GET|v1|projects|:projectId|connections",
  "POST|v1|projects|:projectId|connections",
  "PUT|v1|projects|:projectId|connections|:connectionId|activate",
  "POST|v1|projects|:projectId|connections|:connectionId|connect",
  "POST|v1|projects|:projectId|connections|:connectionId|connect|finalize",
  "PUT|v1|projects|:projectId|connections|:connectionId|credential",
  "PUT|v1|projects|:projectId|connections|:connectionId|default",
  "GET|v1|projects|:projectId|connections|:connectionId|oauth2|application",
  "PUT|v1|projects|:projectId|connections|:connectionId|oauth2|application",
  "POST|v1|projects|:projectId|connections|:connectionId|oauth2|authorize",
  "POST|v1|projects|:projectId|connections|:connectionId|oauth2|device",
  "POST|v1|projects|:projectId|connections|:connectionId|oauth2|device|:sessionId",
  "POST|v1|projects|:projectId|connections|:connectionId|oauth2|discover",
  "GET|v1|projects|:projectId|connections|:connectionId|oauth2|status",
  "PUT|v1|projects|:projectId|connections|:connectionId|revoke",
  "GET|v1|projects|:projectId|connections|all",
  "POST|v1|projects|:projectId|connections|me",
  "POST|v1|projects|:projectId|connectors|:slug|oauth2|connection",
  "PUT|v1|projects|:projectId|default-agent",
  "GET|v1|projects|:projectId|detail",
  "PATCH|v1|projects|:projectId|experimental",
  "PATCH|v1|projects|:projectId|features",
  "GET|v1|projects|:projectId|files",
  "GET|v1|projects|:projectId|files|archive",
  "GET|v1|projects|:projectId|files|content",
  "GET|v1|projects|:projectId|files|history",
  "GET|v1|projects|:projectId|files|search",
  "GET|v1|projects|:projectId|gateway|breakdown",
  "GET|v1|projects|:projectId|gateway|budgets",
  "PUT|v1|projects|:projectId|gateway|budgets",
  "DELETE|v1|projects|:projectId|gateway|budgets|:budgetId",
  "GET|v1|projects|:projectId|gateway|errors",
  "GET|v1|projects|:projectId|gateway|keys",
  "POST|v1|projects|:projectId|gateway|keys",
  "DELETE|v1|projects|:projectId|gateway|keys|:keyId",
  "GET|v1|projects|:projectId|gateway|logs",
  "GET|v1|projects|:projectId|gateway|logs|:logId",
  "GET|v1|projects|:projectId|gateway|overview",
  "POST|v1|projects|:projectId|gateway|playground",
  "POST|v1|projects|:projectId|gateway|providers|:providerId|verify",
  "DELETE|v1|projects|:projectId|gateway|routing-policy",
  "GET|v1|projects|:projectId|gateway|routing-policy",
  "PUT|v1|projects|:projectId|gateway|routing-policy",
  "POST|v1|projects|:projectId|gateway|routing-policy|preview",
  "GET|v1|projects|:projectId|gateway|series",
  "GET|v1|projects|:projectId|gateway|sessions",
  "PUT|v1|projects|:projectId|git-credential",
  "POST|v1|projects|:projectId|git-token",
  "GET|v1|projects|:projectId|git|clone-credential",
  "POST|v1|projects|:projectId|git|collaborators",
  "GET|v1|projects|:projectId|group-grants",
  "POST|v1|projects|:projectId|group-grants",
  "DELETE|v1|projects|:projectId|group-grants|:groupId",
  "PATCH|v1|projects|:projectId|group-grants|:groupId",
  "GET|v1|projects|:projectId|llm-catalog",
  "GET|v1|projects|:projectId|llm-catalog|providers",
  "POST|v1|projects|:projectId|manifest|validate",
  "POST|v1|projects|:projectId|marketplace|install-session",
  "DELETE|v1|projects|:projectId|model-defaults",
  "GET|v1|projects|:projectId|model-defaults",
  "PUT|v1|projects|:projectId|model-defaults",
  "PUT|v1|projects|:projectId|model-enablement",
  "GET|v1|projects|:projectId|model-picker",
  "GET|v1|projects|:projectId|oauth",
  "DELETE|v1|projects|:projectId|oauth|:provider",
  "POST|v1|projects|:projectId|oauth|:provider|poll",
  "POST|v1|projects|:projectId|oauth|:provider|start",
  "PATCH|v1|projects|:projectId|onboarding",
  "GET|v1|projects|:projectId|resource-grants",
  "POST|v1|projects|:projectId|resource-grants",
  "DELETE|v1|projects|:projectId|resource-grants|:grantId",
  "POST|v1|projects|:projectId|review|bulk",
  "GET|v1|projects|:projectId|review|items",
  "POST|v1|projects|:projectId|review|items",
  "GET|v1|projects|:projectId|review|items|:reviewItemId",
  "POST|v1|projects|:projectId|review|items|:reviewItemId|act",
  "GET|v1|projects|:projectId|sandbox-health",
  "PATCH|v1|projects|:projectId|sandbox-provider",
  "GET|v1|projects|:projectId|sandbox-provider|transition",
  "GET|v1|projects|:projectId|sandbox-templates",
  "POST|v1|projects|:projectId|sandbox-templates",
  "DELETE|v1|projects|:projectId|sandbox-templates|:templateId",
  "PATCH|v1|projects|:projectId|sandbox-templates|:templateId",
  "POST|v1|projects|:projectId|sandbox-templates|:templateId|build",
  "GET|v1|projects|:projectId|sandboxes",
  "POST|v1|projects|:projectId|secret-requests",
  "GET|v1|projects|:projectId|secrets",
  "POST|v1|projects|:projectId|secrets",
  "POST|v1|projects|:projectId|secrets|:identifier|broker",
  "PUT|v1|projects|:projectId|secrets|:identifier|strategy",
  "DELETE|v1|projects|:projectId|secrets|:name",
  "DELETE|v1|projects|:projectId|secrets|:name|personal",
  "PUT|v1|projects|:projectId|secrets|:name|personal",
  "POST|v1|projects|:projectId|secrets|sync",
  "GET|v1|projects|:projectId|sessions",
  "POST|v1|projects|:projectId|sessions",
  "DELETE|v1|projects|:projectId|sessions|:sessionId",
  "GET|v1|projects|:projectId|sessions|:sessionId",
  "PATCH|v1|projects|:projectId|sessions|:sessionId",
  "GET|v1|projects|:projectId|sessions|:sessionId|audit",
  "POST|v1|projects|:projectId|sessions|:sessionId|audit|events",
  "POST|v1|projects|:projectId|sessions|:sessionId|commit-push",
  "GET|v1|projects|:projectId|sessions|:sessionId|config",
  "POST|v1|projects|:projectId|sessions|:sessionId|mcp|voice",
  "PUT|v1|projects|:projectId|sessions|:sessionId|model",
  "GET|v1|projects|:projectId|sessions|:sessionId|previews",
  "GET|v1|projects|:projectId|sessions|:sessionId|public-shares",
  "POST|v1|projects|:projectId|sessions|:sessionId|public-shares",
  "DELETE|v1|projects|:projectId|sessions|:sessionId|public-shares|:shareId",
  "GET|v1|projects|:projectId|sessions|:sessionId|question",
  "POST|v1|projects|:projectId|sessions|:sessionId|question",
  "POST|v1|projects|:projectId|sessions|:sessionId|reload",
  "POST|v1|projects|:projectId|sessions|:sessionId|restart",
  "GET|v1|projects|:projectId|sessions|:sessionId|scope",
  "PUT|v1|projects|:projectId|sessions|:sessionId|scope",
  "PUT|v1|projects|:projectId|sessions|:sessionId|sharing",
  "POST|v1|projects|:projectId|sessions|:sessionId|start",
  "POST|v1|projects|:projectId|sessions|:sessionId|stop",
  "GET|v1|projects|:projectId|sessions|:sessionId|transcript",
  "GET|v1|projects|:projectId|sessions|:sessionId|voice-transcript",
  "POST|v1|projects|:projectId|sessions|warm",
  "POST|v1|projects|:projectId|sessions|warm|claim",
  "GET|v1|projects|:projectId|snapshots",
  "POST|v1|projects|:projectId|snapshots|fix-with-agent",
  "POST|v1|projects|:projectId|snapshots|rebuild",
  "GET|v1|projects|:projectId|triggers",
  "POST|v1|projects|:projectId|triggers",
  "DELETE|v1|projects|:projectId|triggers|:slug",
  "PATCH|v1|projects|:projectId|triggers|:slug",
  "POST|v1|projects|:projectId|triggers|:slug|fire",
  "PATCH|v1|projects|:projectId|triggers|activation",
  "POST|v1|projects|:projectId|turn-question",
  "POST|v1|projects|:projectId|turn-stream",
  "GET|v1|projects|:projectId|version-diff",
  "POST|v1|projects|create-repo",
  "DELETE|v1|projects|github|installation",
  "GET|v1|projects|github|installation",
  "POST|v1|projects|github|installation",
  "GET|v1|projects|github|installations",
  "DELETE|v1|projects|github|installations|:installationId",
  "POST|v1|projects|github|installations|link",
  "POST|v1|projects|github|installations|linkable",
  "GET|v1|projects|github|repositories",
  "GET|v1|projects|github|repository-branches",
  "POST|v1|projects|link-repository",
  "GET|v1|projects|managed-git|status",
  "POST|v1|projects|provision",
  "GET|v1|projects|suna-migration|eligibility",
  "POST|v1|projects|suna-migration|start",
  "GET|v1|projects|suna-migration|status",
  "GET|v1|public|session-shares|:shareId",
  "GET|v1|public|session-shares|:shareId|messages",
  "GET|v1|public|voice-join|:token",
  "GET|v1|public|voice-join|:token|transcript",
  "POST|v1|router|chat|completions",
  "GET|v1|router|health",
  "POST|v1|router|image-search",
  "GET|v1|router|models",
  "GET|v1|router|models|:model",
  "POST|v1|router|web-search",
  "GET|v1|setup-links|connectors|:token",
  "POST|v1|setup-links|connectors|:token|start",
  "GET|v1|setup-links|secret|:token",
  "POST|v1|setup-links|secret|:token",
  "GET|v1|skills",
  "GET|v1|skills|:name",
  "GET|v1|skills|:name|file",
  "POST|v1|system|demo-request",
  "GET|v1|system|maintenance",
  "PUT|v1|system|maintenance",
  "GET|v1|system|status",
  "GET|v1|tunnel|audit|:tunnelId",
  "GET|v1|tunnel|connections",
  "POST|v1|tunnel|connections",
  "DELETE|v1|tunnel|connections|:tunnelId",
  "GET|v1|tunnel|connections|:tunnelId",
  "PATCH|v1|tunnel|connections|:tunnelId",
  "POST|v1|tunnel|connections|:tunnelId|rotate-token",
  "POST|v1|tunnel|device-auth",
  "POST|v1|tunnel|device-auth|:code|approve",
  "POST|v1|tunnel|device-auth|:code|deny",
  "GET|v1|tunnel|device-auth|:code|info",
  "GET|v1|tunnel|device-auth|:code|status",
  "GET|v1|tunnel|permission-requests",
  "POST|v1|tunnel|permission-requests|:requestId|approve",
  "POST|v1|tunnel|permission-requests|:requestId|deny",
  "GET|v1|tunnel|permission-requests|stream",
  "GET|v1|tunnel|permissions|:tunnelId",
  "POST|v1|tunnel|permissions|:tunnelId",
  "DELETE|v1|tunnel|permissions|:tunnelId|:permissionId",
  "POST|v1|tunnel|rpc|:tunnelId",
  "GET|v1|usage",
  "GET|v1|usage|cost-by-project",
  "GET|v1|usage|cost-summary",
  "GET|v1|usage|session-costs",
  "GET|v1|usage|session-costs|:sessionId",
  "GET|v1|user-roles",
  "POST|v1|webhooks|email|agentmail",
  "POST|v1|webhooks|projects|:projectId|:slug",
  "POST|v1|webhooks|sandbox|daytona",
  "POST|v1|webhooks|sandbox|platinum",
  "POST|v1|webhooks|slack",
  "POST|v1|webhooks|slack|:projectId",
  "POST|v1|webhooks|slack|:projectId|commands",
  "POST|v1|webhooks|slack|:projectId|interactivity",
  "GET|v1|webhooks|slack|:projectId|manifest",
  "POST|v1|webhooks|slack|commands",
  "POST|v1|webhooks|slack|interactivity",
  "GET|v1|webhooks|slack|oauth|callback",
  "POST|v1|webhooks|teams|:projectId|messages",
  "POST|v1|webhooks|teams|messages",
  "GET|v1|webhooks|teams|oauth|callback",
  "POST|v1|webhooks|telegram|:projectId",
  "GET|v1|workspaces",
  "POST|v1|workspaces",
  "DELETE|v1|workspaces|:projectId",
  "GET|v1|workspaces|:projectId",
  "PATCH|v1|workspaces|:projectId",
  "GET|v1|workspaces|:projectId|access",
  "GET|v1|workspaces|:projectId|access-requests",
  "POST|v1|workspaces|:projectId|access-requests",
  "POST|v1|workspaces|:projectId|access-requests|:requestId|approve",
  "POST|v1|workspaces|:projectId|access-requests|:requestId|reject",
  "DELETE|v1|workspaces|:projectId|access|:userId",
  "PUT|v1|workspaces|:projectId|access|:userId",
  "POST|v1|workspaces|:projectId|access|invite",
  "GET|v1|workspaces|:projectId|access|pending-invites",
  "DELETE|v1|workspaces|:projectId|access|pending-invites|:inviteId",
  "POST|v1|workspaces|:projectId|access|pending-invites|:inviteId|resend",
  "GET|v1|workspaces|:projectId|agents|:agentName|config",
  "PUT|v1|workspaces|:projectId|agents|:agentName|config",
  "PUT|v1|workspaces|:projectId|agents|:agentName|scope",
  "GET|v1|workspaces|:projectId|approvals",
  "POST|v1|workspaces|:projectId|approvals|:executionId",
  "GET|v1|workspaces|:projectId|approvals|needs-input",
  "GET|v1|workspaces|:projectId|apps",
  "POST|v1|workspaces|:projectId|apps",
  "DELETE|v1|workspaces|:projectId|apps|:appId",
  "GET|v1|workspaces|:projectId|apps|:appId",
  "PATCH|v1|workspaces|:projectId|apps|:appId",
  "GET|v1|workspaces|:projectId|apps|:appId|access",
  "PATCH|v1|workspaces|:projectId|apps|:appId|access",
  "POST|v1|workspaces|:projectId|apps|:appId|access-session",
  "GET|v1|workspaces|:projectId|apps|:appId|deployments",
  "POST|v1|workspaces|:projectId|apps|:appId|deployments",
  "GET|v1|workspaces|:projectId|apps|:appId|deployments|:deploymentId",
  "GET|v1|workspaces|:projectId|apps|:appId|deployments|:deploymentId|logs",
  "POST|v1|workspaces|:projectId|apps|:appId|rollback",
  "POST|v1|workspaces|:projectId|apps|:appId|start",
  "POST|v1|workspaces|:projectId|apps|:appId|stop",
  "POST|v1|workspaces|:projectId|apps|artifacts",
  "POST|v1|workspaces|:projectId|apps|artifacts|:artifactId|finalize",
  "GET|v1|workspaces|:projectId|audit",
  "GET|v1|workspaces|:projectId|branches",
  "GET|v1|workspaces|:projectId|change-requests",
  "POST|v1|workspaces|:projectId|change-requests",
  "GET|v1|workspaces|:projectId|change-requests|:crId",
  "PATCH|v1|workspaces|:projectId|change-requests|:crId",
  "POST|v1|workspaces|:projectId|change-requests|:crId|close",
  "GET|v1|workspaces|:projectId|change-requests|:crId|diff",
  "POST|v1|workspaces|:projectId|change-requests|:crId|merge",
  "GET|v1|workspaces|:projectId|change-requests|:crId|merge-preview",
  "POST|v1|workspaces|:projectId|change-requests|:crId|reopen",
  "POST|v1|workspaces|:projectId|change-requests|:crId|request-changes",
  "GET|v1|workspaces|:projectId|channels|bindings",
  "PATCH|v1|workspaces|:projectId|channels|bindings|:bindingId",
  "POST|v1|workspaces|:projectId|channels|email|connect",
  "DELETE|v1|workspaces|:projectId|channels|email|installation",
  "GET|v1|workspaces|:projectId|channels|email|installation",
  "PATCH|v1|workspaces|:projectId|channels|email|installation",
  "GET|v1|workspaces|:projectId|channels|email|mode",
  "PUT|v1|workspaces|:projectId|channels|meet|name",
  "POST|v1|workspaces|:projectId|channels|slack|bind-thread",
  "POST|v1|workspaces|:projectId|channels|slack|connect",
  "GET|v1|workspaces|:projectId|channels|slack|file",
  "POST|v1|workspaces|:projectId|channels|slack|file|upload",
  "DELETE|v1|workspaces|:projectId|channels|slack|installation",
  "GET|v1|workspaces|:projectId|channels|slack|installation",
  "GET|v1|workspaces|:projectId|channels|slack|mode",
  "POST|v1|workspaces|:projectId|channels|teams|connect",
  "GET|v1|workspaces|:projectId|channels|teams|file",
  "POST|v1|workspaces|:projectId|channels|teams|file|upload",
  "DELETE|v1|workspaces|:projectId|channels|teams|installation",
  "GET|v1|workspaces|:projectId|channels|teams|installation",
  "GET|v1|workspaces|:projectId|channels|teams|manifest",
  "GET|v1|workspaces|:projectId|channels|teams|mode",
  "GET|v1|workspaces|:projectId|cli-token",
  "POST|v1|workspaces|:projectId|cli-token",
  "DELETE|v1|workspaces|:projectId|cli-token|:tokenId",
  "GET|v1|workspaces|:projectId|commits",
  "GET|v1|workspaces|:projectId|commits|:sha",
  "GET|v1|workspaces|:projectId|commits|:sha|diff",
  "POST|v1|workspaces|:projectId|connect-requests",
  "GET|v1|workspaces|:projectId|connections",
  "POST|v1|workspaces|:projectId|connections",
  "PUT|v1|workspaces|:projectId|connections|:connectionId|activate",
  "POST|v1|workspaces|:projectId|connections|:connectionId|connect",
  "POST|v1|workspaces|:projectId|connections|:connectionId|connect|finalize",
  "PUT|v1|workspaces|:projectId|connections|:connectionId|credential",
  "PUT|v1|workspaces|:projectId|connections|:connectionId|default",
  "GET|v1|workspaces|:projectId|connections|:connectionId|oauth2|application",
  "PUT|v1|workspaces|:projectId|connections|:connectionId|oauth2|application",
  "POST|v1|workspaces|:projectId|connections|:connectionId|oauth2|authorize",
  "POST|v1|workspaces|:projectId|connections|:connectionId|oauth2|device",
  "POST|v1|workspaces|:projectId|connections|:connectionId|oauth2|device|:sessionId",
  "POST|v1|workspaces|:projectId|connections|:connectionId|oauth2|discover",
  "GET|v1|workspaces|:projectId|connections|:connectionId|oauth2|status",
  "PUT|v1|workspaces|:projectId|connections|:connectionId|revoke",
  "GET|v1|workspaces|:projectId|connections|all",
  "POST|v1|workspaces|:projectId|connections|me",
  "POST|v1|workspaces|:projectId|connectors|:slug|oauth2|connection",
  "PUT|v1|workspaces|:projectId|default-agent",
  "GET|v1|workspaces|:projectId|detail",
  "PATCH|v1|workspaces|:projectId|experimental",
  "PATCH|v1|workspaces|:projectId|features",
  "GET|v1|workspaces|:projectId|files",
  "GET|v1|workspaces|:projectId|files|archive",
  "GET|v1|workspaces|:projectId|files|content",
  "GET|v1|workspaces|:projectId|files|history",
  "GET|v1|workspaces|:projectId|files|search",
  "GET|v1|workspaces|:projectId|gateway|breakdown",
  "GET|v1|workspaces|:projectId|gateway|budgets",
  "PUT|v1|workspaces|:projectId|gateway|budgets",
  "DELETE|v1|workspaces|:projectId|gateway|budgets|:budgetId",
  "GET|v1|workspaces|:projectId|gateway|errors",
  "GET|v1|workspaces|:projectId|gateway|keys",
  "POST|v1|workspaces|:projectId|gateway|keys",
  "DELETE|v1|workspaces|:projectId|gateway|keys|:keyId",
  "GET|v1|workspaces|:projectId|gateway|logs",
  "GET|v1|workspaces|:projectId|gateway|logs|:logId",
  "GET|v1|workspaces|:projectId|gateway|overview",
  "POST|v1|workspaces|:projectId|gateway|playground",
  "POST|v1|workspaces|:projectId|gateway|providers|:providerId|verify",
  "DELETE|v1|workspaces|:projectId|gateway|routing-policy",
  "GET|v1|workspaces|:projectId|gateway|routing-policy",
  "PUT|v1|workspaces|:projectId|gateway|routing-policy",
  "POST|v1|workspaces|:projectId|gateway|routing-policy|preview",
  "GET|v1|workspaces|:projectId|gateway|series",
  "GET|v1|workspaces|:projectId|gateway|sessions",
  "PUT|v1|workspaces|:projectId|git-credential",
  "POST|v1|workspaces|:projectId|git-token",
  "GET|v1|workspaces|:projectId|git|clone-credential",
  "POST|v1|workspaces|:projectId|git|collaborators",
  "GET|v1|workspaces|:projectId|group-grants",
  "POST|v1|workspaces|:projectId|group-grants",
  "DELETE|v1|workspaces|:projectId|group-grants|:groupId",
  "PATCH|v1|workspaces|:projectId|group-grants|:groupId",
  "GET|v1|workspaces|:projectId|llm-catalog",
  "GET|v1|workspaces|:projectId|llm-catalog|providers",
  "POST|v1|workspaces|:projectId|manifest|validate",
  "POST|v1|workspaces|:projectId|marketplace|install-session",
  "DELETE|v1|workspaces|:projectId|model-defaults",
  "GET|v1|workspaces|:projectId|model-defaults",
  "PUT|v1|workspaces|:projectId|model-defaults",
  "PUT|v1|workspaces|:projectId|model-enablement",
  "GET|v1|workspaces|:projectId|model-picker",
  "GET|v1|workspaces|:projectId|oauth",
  "DELETE|v1|workspaces|:projectId|oauth|:provider",
  "POST|v1|workspaces|:projectId|oauth|:provider|poll",
  "POST|v1|workspaces|:projectId|oauth|:provider|start",
  "PATCH|v1|workspaces|:projectId|onboarding",
  "GET|v1|workspaces|:projectId|resource-grants",
  "POST|v1|workspaces|:projectId|resource-grants",
  "DELETE|v1|workspaces|:projectId|resource-grants|:grantId",
  "POST|v1|workspaces|:projectId|review|bulk",
  "GET|v1|workspaces|:projectId|review|items",
  "POST|v1|workspaces|:projectId|review|items",
  "GET|v1|workspaces|:projectId|review|items|:reviewItemId",
  "POST|v1|workspaces|:projectId|review|items|:reviewItemId|act",
  "GET|v1|workspaces|:projectId|sandbox-health",
  "PATCH|v1|workspaces|:projectId|sandbox-provider",
  "GET|v1|workspaces|:projectId|sandbox-provider|transition",
  "GET|v1|workspaces|:projectId|sandbox-templates",
  "POST|v1|workspaces|:projectId|sandbox-templates",
  "DELETE|v1|workspaces|:projectId|sandbox-templates|:templateId",
  "PATCH|v1|workspaces|:projectId|sandbox-templates|:templateId",
  "POST|v1|workspaces|:projectId|sandbox-templates|:templateId|build",
  "GET|v1|workspaces|:projectId|sandboxes",
  "POST|v1|workspaces|:projectId|secret-requests",
  "GET|v1|workspaces|:projectId|secrets",
  "POST|v1|workspaces|:projectId|secrets",
  "POST|v1|workspaces|:projectId|secrets|:identifier|broker",
  "PUT|v1|workspaces|:projectId|secrets|:identifier|strategy",
  "DELETE|v1|workspaces|:projectId|secrets|:name",
  "DELETE|v1|workspaces|:projectId|secrets|:name|personal",
  "PUT|v1|workspaces|:projectId|secrets|:name|personal",
  "POST|v1|workspaces|:projectId|secrets|sync",
  "GET|v1|workspaces|:projectId|sessions",
  "POST|v1|workspaces|:projectId|sessions",
  "DELETE|v1|workspaces|:projectId|sessions|:sessionId",
  "GET|v1|workspaces|:projectId|sessions|:sessionId",
  "PATCH|v1|workspaces|:projectId|sessions|:sessionId",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|audit",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|audit|events",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|commit-push",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|config",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|mcp|voice",
  "PUT|v1|workspaces|:projectId|sessions|:sessionId|model",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|previews",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|public-shares",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|public-shares",
  "DELETE|v1|workspaces|:projectId|sessions|:sessionId|public-shares|:shareId",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|question",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|question",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|reload",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|restart",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|scope",
  "PUT|v1|workspaces|:projectId|sessions|:sessionId|scope",
  "PUT|v1|workspaces|:projectId|sessions|:sessionId|sharing",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|start",
  "POST|v1|workspaces|:projectId|sessions|:sessionId|stop",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|transcript",
  "GET|v1|workspaces|:projectId|sessions|:sessionId|voice-transcript",
  "POST|v1|workspaces|:projectId|sessions|warm",
  "POST|v1|workspaces|:projectId|sessions|warm|claim",
  "GET|v1|workspaces|:projectId|snapshots",
  "POST|v1|workspaces|:projectId|snapshots|fix-with-agent",
  "POST|v1|workspaces|:projectId|snapshots|rebuild",
  "GET|v1|workspaces|:projectId|triggers",
  "POST|v1|workspaces|:projectId|triggers",
  "DELETE|v1|workspaces|:projectId|triggers|:slug",
  "PATCH|v1|workspaces|:projectId|triggers|:slug",
  "POST|v1|workspaces|:projectId|triggers|:slug|fire",
  "PATCH|v1|workspaces|:projectId|triggers|activation",
  "POST|v1|workspaces|:projectId|turn-question",
  "POST|v1|workspaces|:projectId|turn-stream",
  "GET|v1|workspaces|:projectId|version-diff",
  "POST|v1|workspaces|create-repo",
  "DELETE|v1|workspaces|github|installation",
  "GET|v1|workspaces|github|installation",
  "POST|v1|workspaces|github|installation",
  "GET|v1|workspaces|github|installations",
  "DELETE|v1|workspaces|github|installations|:installationId",
  "POST|v1|workspaces|github|installations|link",
  "POST|v1|workspaces|github|installations|linkable",
  "GET|v1|workspaces|github|repositories",
  "GET|v1|workspaces|github|repository-branches",
  "POST|v1|workspaces|link-repository",
  "GET|v1|workspaces|managed-git|status",
  "POST|v1|workspaces|provision",
] as const;

export const AUDIT_HTTP_ROUTES = AUDIT_HTTP_ROUTE_KEYS.map((key) => {
  const [method, ...segments] = key.split("|");
  return `${method} /${segments.join("/")}`;
});
