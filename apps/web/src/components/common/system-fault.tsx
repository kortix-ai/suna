...
/**
 * The full-screen "System fault" view: brandmark, the error name + message
 * with a collapsible stack, and a diagnostics block (url, digest, Sentry event,
 * env, viewport, language, online, time, user-agent). Self-contained — it
 * centers itself on a `.fault-root` wrapper and ships its own `<style>`, so it
 * renders identically whether mounted as the top-level `global-error` boundary
 * (wrapped in `<html><body>`) or dropped into a normal preview page.
 *
 * Pass `report={false}` to skip Sentry capture / console noise (e.g. previews).
 */
