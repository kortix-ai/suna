...
describe('sessionDisplayStatus', () => {
  const cases: Array<[WorkspaceSessionStatus, SessionDisplayStatus]> = [
    ['queued', 'starting'],
    ['branching', 'starting'],
  ...
  test('an unknown lifecycle value degrades instead of throwing', () => {
    // WorkspaceSessionStatus is a published SDK union: an API that grows an
    ...
    const session = makeSession({ status: 'hibernating' as WorkspaceSessionStatus });
