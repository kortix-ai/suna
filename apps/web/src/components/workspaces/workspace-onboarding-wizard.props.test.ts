...
/**
 * `WorkspaceOnboardingWizard` cannot be rendered here: `apps/web`'s `bun test`
 ...
 */
const source = readFileSync(join(import.meta.dir, 'workspace-onboarding-wizard.tsx'), 'utf8');
const code = source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/(^|[^:])\/\/.*$/gm, '$1');

const shellSource = readFileSync(
  join(import.meta.dir, '../../features/workspace/workspace-layout/workspace-shell.tsx'),
  'utf8',
);

describe('WorkspaceOnboardingWizard: completion wiring', () => {
  test('the scan found the component', () => {
    // Guard the guard: an empty string passes every `.not.toContain` below.
    expect(code.length).toBeGreaterThan(0);
    expect(code).toContain('export function WorkspaceOnboardingWizard');
  });
...
describe('WorkspaceOnboardingWizard: the skip control is opt-in', () => {
  ...
  /**
   * THE no-regression assertion. The workspace shell must keep passing neither
   ...
   * a prop here, the workspace-page behaviour changed without anyone deciding
   * that it should.
   */
  test('workspace-shell renders the wizard with workspaceId ONLY', () => {
    expect(shellSource.length).toBeGreaterThan(0);
    expect(shellSource).toContain('<WorkspaceOnboardingWizard workspaceId={workspaceId} />');
 ...
 *
 * It used to leave the workspace unstamped, on the theory that the workspace
 ...
 * `qk.workspace.detail(id)` entry the shell reads, so on arrival at
 ...
 */
describe('WorkspaceOnboardingWizard: skipping stamps, exactly like finishing', () => {
