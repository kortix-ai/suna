...
/**
 * Canonical, framework-free helpers for reading a workspace session the way the
...
export function isMetaCoordinatorSession(session: WorkspaceSession): boolean {
...
export function spawnedBySessionId(session: WorkspaceSession): string | null {
 ...
 *
 * `WorkspaceSessionStatus` is a seven-value SANDBOX lifecycle. Users get five
 ...
 */
export function isLegacyMigratedSession(session: WorkspaceSession): boolean {
  const meta = (session.metadata ?? {}) as Record<string, unknown>;
 ...
 *
 * The `default` is load-bearing, not defensive noise. `WorkspaceSessionStatus`
