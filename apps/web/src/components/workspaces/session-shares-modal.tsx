 ...
 */
function shareUrl(share: WorkspaceSessionPublicShare): string | null {
...
function ShareRow({
  ...
  share: WorkspaceSessionPublicShare;
  onRevoke: () => void;
  isRevoking: boolean;
}) {
...
export function SessionSharesModal({
  ...
  const { shares, liveShares, isLoading, isError } = useSessionPublicShares(workspaceId, sessionId);
  const { revoke, revokingId } = useRevokePublicShare(workspaceId, sessionId);
  const [pendingRevoke, setPendingRevoke] = useState<WorkspaceSessionPublicShare | null>(null);
        ...
        <ModalContent className="lg:max-w-lg">
          ...
          <ModalBody className="max-h-[60vh] space-y-6 overflow-y-auto">
            {listState === 'loading' ? (
              ...
              // The list 403s for a workspace member who neither created the
              // session nor has manage rights (`canManageSharing` = owner OR
              // workspace manager). Falling through to the empty state would tell
