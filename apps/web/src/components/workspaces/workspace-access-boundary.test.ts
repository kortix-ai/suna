...
} from './workspace-access-boundary';

const componentSource = readFileSync(
  fileURLToPath(new URL('./workspace-access-boundary.tsx', import.meta.url)),
  'utf8',
);
...
function copyBlock(locale: string): Record<string, string> {
  ...
  return JSON.parse(readFileSync(path, 'utf8')).hardcodedUi?.workspaceAccessBoundary ?? {};
}
...
describe('gateStateForError', () => {
  ...
  test('404 reports the workspace as gone', () => {
...
describe('gateStateForRequestResult', () => {
  ...
  // switches over RequestWorkspaceAccessResult['status'] with no default, so a
  ...
  test('returns null for already_has_access so the caller re-fetches the workspace', () => {
...
describe('resolveGateState', () => {
  ...
  test('a deleted project outranks a request that is still waiting', () => {
    // The opposite failure: without this, deleting the workspace mid-wait leaves
...
describe('polling lifecycle', () => {
  test('the poll stops as soon as the workspace becomes readable', () => {
    // This boundary wraps the workspace shell for the whole session, so a poll
    // that only checks `waiting` keeps calling getWorkspace every 15s while the
...
describe('copy wiring', () => {
  ...
  test('every locale carries every key this component renders', () => {
    ...
    // literal `t.raw('workspaceAccessBoundary.x')` is scraped by regex, and the
    // template-literal call sites are covered by gateCopyKeys.
    const used = new Set<string>();
    for (const match of componentSource.matchAll(/workspaceAccessBoundary\.(\w+)/g)) {
      used.add(match[1]);
    }
    for (const state of ALL_STATES) {
      const keys = gateCopyKeys(state);
...
describe('copy quality', () => {
  ...
  test('no internal navigation paths or product jargon reach the user', () => {
    // "Customize → Members" shipped in the previous copy. It does not help
    // someone who cannot open the workspace.
    const banned = [
      /customize/i,
      /→/,
      /viewer/i,
