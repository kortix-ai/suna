...
import { WorkspaceIconField, type WorkspaceIconValue } from './workspace-icon-field';

const read = (relative: string) =>
  readFileSync(fileURLToPath(new URL(relative, import.meta.url)), 'utf8');

const source = read('./workspace-icon-field.tsx');
...
const render = (
  props: Partial<Parameters<typeof WorkspaceIconField>[0]> & { value: WorkspaceIconValue },
) => renderToStaticMarkup(<WorkspaceIconField onChange={noop} onGlyphChange={noop} {...props} />);
 ...
 * `(value: WorkspaceIconValue) => void`: one wide callback would overload a
...
const signaturePin: (props: PinnedProps) => unknown = WorkspaceIconField;
...
const pickerEnd = popoverBody.indexOf('/>', popoverBody.indexOf('<WorkspaceIconPicker')) + 2;
/** The `<WorkspaceIconPicker … />` element alone — both `onEmojiSelect` and
 *  `onGlyphSelect`, and the `defaultTab`/`defaultColor` wiring. */
const pickerElement = popoverBody.slice(popoverBody.indexOf('<WorkspaceIconPicker'), pickerEnd);
...
describe('WorkspaceIconField trigger', () => {
  ...
  test('an unrenderable glyph name falls back to the neutral glyph too', () => {
    ...
    // stale cached data — a workspace row whose glyph catalogue shrank — must
  ...
  test('is type="button", declared here and not only inherited', () => {
    ...
    // type submits it, so opening the picker would create the workspace.
    //
  ...
  test('the value prop is the union, and the setters cannot produce it', () => {
    ...
    expect(signaturePin).toBe(WorkspaceIconField);
 ...
 */
describe('WorkspaceIconField popover geometry', () => {
...
describe('WorkspaceIconField conventions', () => {
 ...
 */
describe('WorkspaceIconField trigger tint — emoji', () => {
  const classesOf = (value: WorkspaceIconValue) =>
 ...
 * glyph itself.
 */
describe('WorkspaceIconField trigger tint — glyph', () => {
  const classesOf = (value: WorkspaceIconValue) =>
    (render({ value }).match(/class="([^"]*)"/)?.[1] ?? '').split(/\s+/);
 ...
 */
describe('WorkspaceIconField remove control', () => {
  test('the popover slices were all found', () => {
    ...
    expect(popoverBody).toContain('<WorkspaceIconPicker');
  ...
  test('it is an X, not a trash can', () => {
    ...
    // removing a PERSISTED row (gateway-routing fallbacks, the workspace card's
