...
import type { WorkspaceInput } from '@kortix/sdk';

import type { WorkspaceIconValue } from './workspace-icon-field';
 ...
 * harder once a workspace could hold a GLYPH as well as an emoji. `PATCH
 * /v1/projects/:workspaceId` reads THREE states off EACH of `icon` and
 ...
 * icon_glyph" as two independent fields: the draft (`WorkspaceEditDraft.icon`)
 * is the SAME union `WorkspaceIconField` itself uses — `{ emoji } | { glyph } |
 ...
 */
export interface WorkspaceEditSubject {
  /** The workspace's stored name. */
  name?: string;
  /** The workspace's stored emoji, or null/undefined when it has none. */
  icon?: string | null;
  /** The workspace's stored glyph, or null/undefined when it has none. At most
...
export function buildWorkspaceEditPatch(
  ...
  // (null) compare equal — otherwise opening the modal on a workspace with no
 ...
 */
export function summarizeWorkspaceEdit(patch: Partial<WorkspaceInput>, savedName: string): string {
