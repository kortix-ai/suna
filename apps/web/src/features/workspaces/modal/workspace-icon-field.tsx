 ...
 * `WorkspaceCard` (workspace Settings, edit — `settings-view.tsx`) holds
 * exactly this type as their icon state and pass it straight through.
 */
export type WorkspaceIconValue = { emoji: string } | { glyph: GlyphSelection } | null;
 ...
 * in the registry (stale cached data: a workspace's `icon_glyph.name` outlived
...
export function WorkspaceIconField({
   ...
   * the switcher and the workspace cards draw.
   *
   ...
   * `(value: WorkspaceIconValue) => void`, because a single wide callback would
  ...
  return (
    ...
    <Popover open={open && !disabled} onOpenChange={setOpen} modal>
      ...
      {/*
        ...
        `GlyphPicker`, rendered on the Icon tab inside `WorkspaceIconPicker`,
      ...
      */}
      ...
      >
        <WorkspaceIconPicker
          ...
          // `undefined` here is what lets WorkspaceIconPicker's own default
