...
import { buildWorkspaceEditPatch, summarizeWorkspaceEdit } from './workspace-edit-patch';

/** A workspace as the modal receives it: named, with an emoji already saved. */
const ICONED = { name: 'Atlas', icon: '🚀' };
/** A workspace that has never had an icon of either kind. */
const PLAIN = { name: 'Atlas', icon: null };
/** A workspace with a glyph already saved (never an emoji at the same time —
...
function readyPatch(result: ReturnType<typeof buildWorkspaceEditPatch>) {
...
describe('nothing to save', () => {
  test('an untouched draft is unchanged', () => {
    expect(buildWorkspaceEditPatch(ICONED, { name: 'Atlas', icon: { emoji: '🚀' } })).toEqual({
      status: 'unchanged',
    });
  });

  test('an untouched draft on a workspace with NO icon is unchanged', () => {
    ...
    expect(buildWorkspaceEditPatch(PLAIN, { name: 'Atlas', icon: null })).toEqual({
      status: 'unchanged',
    });
  });

  test('an undefined stored icon also compares equal to a null draft', () => {
    // `KortixWorkspace.icon` is optional, so a response that simply omits the
    // member arrives as undefined, not null.
    expect(buildWorkspaceEditPatch({ name: 'Atlas' }, { name: 'Atlas', icon: null })).toEqual({
      status: 'unchanged',
    });
  });
  ...
  test('an untouched GLYPH draft is unchanged too', () => {
    expect(
      buildWorkspaceEditPatch(GLYPHED, {
        name: 'Atlas',
  ...
  test('a stored name with whitespace still compares equal', () => {
    expect(
      buildWorkspaceEditPatch({ name: ' Atlas ', icon: null }, { name: 'Atlas', icon: null }),
    ).toEqual({
      status: 'unchanged',
    });
  });
...
describe('the name is required', () => {
  test('an emptied name is not savable', () => {
    expect(buildWorkspaceEditPatch(ICONED, { name: '', icon: { emoji: '🚀' } })).toEqual({
      status: 'empty-name',
    });
  });
...
describe('renaming only', () => {
  test('the patch carries the new name and NO icon key at all', () => {
    ...
    const patch = readyPatch(
      buildWorkspaceEditPatch(ICONED, { name: 'Atlas 2', icon: { emoji: '🚀' } }),
    );
  ...
  test('the name is trimmed before it is sent', () => {
    expect(
      readyPatch(buildWorkspaceEditPatch(ICONED, { name: '  Atlas 2  ', icon: { emoji: '🚀' } })),
    ).toEqual({ name: 'Atlas 2' });
  });

  test('renaming a workspace with no icon still sends no icon key', () => {
    const patch = readyPatch(buildWorkspaceEditPatch(PLAIN, { name: 'Atlas 2', icon: null }));
...
describe('changing the icon only', () => {
  test('a different emoji is savable on its own, with no name key', () => {
    ...
    const patch = readyPatch(
      buildWorkspaceEditPatch(ICONED, { name: 'Atlas', icon: { emoji: '🎯' } }),
    );
  ...
  test('adding a first emoji to a workspace that had none', () => {
    expect(
      readyPatch(buildWorkspaceEditPatch(PLAIN, { name: 'Atlas', icon: { emoji: '🎯' } })),
    ).toEqual({ icon: '🎯' });
  });
  ...
  test('adding a first glyph to a workspace that had none', () => {
    expect(
      readyPatch(
        buildWorkspaceEditPatch(PLAIN, {
          name: 'Atlas',
          icon: { glyph: { name: 'Rocket', color: 'blue' } },
        }),
      ),
...
describe('switching between an emoji and a glyph', () => {
  test('switching an emoji project to a glyph sends icon_glyph, and NEVER icon: null', () => {
    ...
    // produce.
    const patch = readyPatch(
      buildWorkspaceEditPatch(ICONED, {
        name: 'Atlas',
        icon: { glyph: { name: 'Rocket', color: 'blue' } },
      }),
    );
  ...
  test('switching a glyph project to an emoji sends icon, and NEVER icon_glyph: null', () => {
    ...
    const patch = readyPatch(
      buildWorkspaceEditPatch(GLYPHED, { name: 'Atlas', icon: { emoji: '🚀' } }),
    );
...
describe('removing the icon', () => {
  test('the patch carries an explicit null, and the key is present', () => {
    const patch = readyPatch(buildWorkspaceEditPatch(ICONED, { name: 'Atlas', icon: null }));
  ...
  test('a rename and a removal travel in one patch', () => {
    const patch = readyPatch(buildWorkspaceEditPatch(ICONED, { name: 'Atlas 2', icon: null }));
  ...
  test('clearing a GLYPH project sends icon_glyph: null, and NEVER icon: null', () => {
    ...
    // `icon` for this workspace, so it must not appear in the patch at all —
    // only the key that actually held a value gets the explicit clear.
    const patch = readyPatch(buildWorkspaceEditPatch(GLYPHED, { name: 'Atlas', icon: null }));
...
describe('summarizeWorkspaceEdit', () => {
  test('a rename names the SERVER-returned name, not the draft', () => {
    ...
    expect(summarizeWorkspaceEdit({ name: 'typed' }, 'Stored Name')).toBe('Renamed to "Stored Name"');
  });

  test('a rename is the headline when the icon moved too', () => {
    expect(summarizeWorkspaceEdit({ name: 'Atlas 2', icon: '🎯' }, 'Atlas 2')).toBe(
      'Renamed to "Atlas 2"',
    );
  });

  test('an icon-only change says so, and does not claim a rename', () => {
    const message = summarizeWorkspaceEdit({ icon: '🎯' }, 'Atlas');

    expect(message).toBe('Workspace icon updated');
    expect(message).not.toContain('Renamed');
  });

  test('a glyph-only change says so too', () => {
    const message = summarizeWorkspaceEdit(
  ...
  test('a removal is reported as a removal, not as an update', () => {
    ...
    expect(summarizeWorkspaceEdit({ icon: null }, 'Atlas')).toBe('Workspace icon removed');
  });

  test('a glyph removal is reported as a removal too', () => {
    expect(summarizeWorkspaceEdit({ icon_glyph: null }, 'Atlas')).toBe('Workspace icon removed');
  });
  ...
  test('an absent icon key is not treated as a removal', () => {
    ...
    expect(summarizeWorkspaceEdit({ name: 'Atlas 2' }, 'Atlas 2')).not.toContain('removed');
  });

  test('an empty patch still says something', () => {
    expect(summarizeWorkspaceEdit({}, 'Atlas')).toBe('Workspace updated');
  });
});
