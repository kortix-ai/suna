...
const code = readFileSync(fileURLToPath(new URL('./workspace-card.tsx', import.meta.url)), 'utf8')
  .replace(/\/\*[\s\S]*?\*\//g, '')
  .replace(/(^|[^:])\/\/.*$/gm, '$1');

const BASE: KortixWorkspace = {
  workspace_id: 'p1',
  account_id: 'a1',
  name: 'Turtle Shop',
...
const render = (workspace: Partial<KortixWorkspace>) =>
 ...
 * anything: the card also renders the workspace NAME, so a whole-markup
...
describe('WorkspaceCard — the workspace’s own icon', () => {
  test('a workspace that set an icon shows that icon on its card', () => {
    // The payoff of the whole feature: the emoji picked in the create modal has
    // to survive the round trip and land on the card in the /workspaces grid.
    expect(tileTextOf(render({ icon: '🐢' }))).toBe('🐢');
  });

  test('the card shows the workspace’s OWN icon, not a fixed one', () => {
    expect(tileTextOf(render({ icon: '🍕' }))).toBe('🍕');
    expect(tileTextOf(render({ icon: '🚀' }))).toBe('🚀');
  });

  test('a workspace with no icon still shows its initial, unchanged', () => {
    // `null` is what the API returns for every workspace created before the
  ...
  test('a response with no icon field at all still shows the initial', () => {
    // `icon` is optional on KortixWorkspace, so a stale/cached payload omits it.
  ...
  test('two workspaces that picked different emoji do not share a tile', () => {
...
describe('WorkspaceCard — the workspace’s own glyph', () => {
  test('a workspace that set a glyph shows that glyph on its card', () => {
  ...
  test('a workspace with no glyph and no icon still shows its initial', () => {
  ...
  test('a response with no glyph field at all still shows the initial', () => {
    // `icon_glyph` is optional on KortixWorkspace, so a stale/cached payload
  ...
  test('the glyph is passed to EntityAvatar, which is what actually enforces precedence', () => {
    ...
    expect(code).toContain('glyph={workspace.icon_glyph}');
    expect(code).toContain('emoji={workspace.icon}');
  });

  test('a glyph workspace does not also render the emoji tint', () => {
