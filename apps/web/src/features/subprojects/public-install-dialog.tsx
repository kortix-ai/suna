...
import type { Subproject } from './subprojects-catalog';
 ...
 * one, or many projects), unlike `SubprojectInstallModal` which is always
 ...
 * (`createSubprojectInstallSession`) the in-project modal makes, so the two
 * surfaces can never install a subproject two different ways.
 */
export function PublicSubprojectInstallDialog({
  subproject,
  open,
  onOpenChange,
}: {
  ...
  useEffect(() => {
    ...
    setError(null);
    setOpening(false);
    setNewProjectName(subproject.title);
  }, [open, subproject.title]);
    ...
    try {
      ...
      if (target === NEW_PROJECT) {
        ...
        const project = await provisionProject({
          ...
          name: newProjectName.trim() || subproject.title,
          starter_template: 'general-knowledge-worker',
        });
        queryClient.invalidateQueries({ queryKey: qk.projects.scope() });
        projectId = project.project_id;
      }
      const { session_id } = await createSubprojectInstallSession(
        projectId,
        ...
        <ModalBody className="space-y-3">
          ...
          {projectsQuery.isPending ? (
            ...
            <div className="border-border/60 bg-muted/30 rounded-md border px-4 py-4">
              <p className="text-foreground text-sm font-medium">Couldn&apos;t load your projects</p>
              <p className="text-muted-foreground mt-0.5 text-xs">
                {(projectsQuery.error as Error)?.message || 'The request failed.'}
              </p>
              <Button size="sm" variant="outline" className="mt-3" onClick={() => projectsQuery.refetch()}>
                Try again
              </Button>
            </div>
          ) : (
            <>
              ...
              {target === NEW_PROJECT && (
                <div className="space-y-1.5">
                  <Label className="text-sm" htmlFor="subproject-new-project-name">
                    Project name
                  </Label>
                  <Input
                    id="subproject-new-project-name"
