...
import { PublicSubprojectInstallDialog } from './public-install-dialog';
import type { Subproject } from './subprojects-catalog';

/**
 * The action button on `/marketplace/<slug>` — split out from
 * `PublicSubprojectDetail` for exactly one reason: it is the only piece that
 ...
 * thing allowed to opt out of that, and `PublicSubprojectDetail` wraps it in
 ...
 *  - signed IN: `Install`, opening `PublicSubprojectInstallDialog` — the same
 *    `createSubprojectInstallSession` call the in-project install modal
 *    makes, after a project-picker step that modal never needs (it already
 *    knows `projectId`).
 */
export function SubprojectInstallCta({ subproject }: { subproject: Subproject }) {
