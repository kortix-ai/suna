...
import { qk, useSubprojects, useProjectSubprojects } from '@kortix/sdk/react';
...
/**
 * The project-scoped subprojects store (`/projects/[id]/subprojects`).
 *
...
export function SubprojectsStore({ projectId }: { projectId: string }) {
  ...
  const counts = useMemo(() => {
    const installed = subprojects.filter((subproject) => installedSlugs.has(subproject.slug)).length;
  ...
  return (
    <div data-subprojects-store className="mx-auto w-full max-w-6xl space-y-8 px-4 pt-8 pb-16 sm:px-6">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-foreground text-2xl font-semibold tracking-tight">Subprojects</h1>
          <p className="text-muted-foreground text-sm text-balance">
            Open-source subprojects you can install into this project — pick one, install it, review what
            it delivers.
          </p>
        </div>
