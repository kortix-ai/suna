/**
 * Display names and marks for the third-party apps a subproject plugs into.
 *
 * This is a PRESENTATION table, not data: the subproject's requirement list comes
 * from its `kortix.yaml` through `Subproject.connectors`, and this module only
 * answers "what is this app called, and what is its logo". An app with no row
 * here still renders — {@link connectorFor} falls back to initials — so a subproject
 ...
 * two are deliberately not merged: a subproject's requirements are global, a
 ...
 * crashing the modal. Called with whatever the subproject's manifest declared, which
