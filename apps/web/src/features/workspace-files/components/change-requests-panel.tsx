...
import type { WorkspaceChangeRequest, ChangeRequestStatus } from '../api/change-requests';
...
function tsFromCr(cr: WorkspaceChangeRequest): number {
...
function crTimeLabel(cr: WorkspaceChangeRequest): string {
  if (cr.status === 'merged' && cr.merged_at) {
...
function groupByDate(crs: WorkspaceChangeRequest[]) {
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const yesterday = new Date(today.getTime() - 86_400_000);
  const thisWeekStart = new Date(today.getTime() - today.getDay() * 86_400_000);

  const groups = new Map<string, WorkspaceChangeRequest[]>();
  for (const cr of crs) {
