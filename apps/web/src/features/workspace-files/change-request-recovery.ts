...
export function buildChangeRequestRecoveryPrompt(
  ...
  if (blocker.kind === 'manifest_invalid') {
    return [
      `Change request #${target.number} cannot merge because its workspace manifest fails validation.`,
      '',
      ...
      '2. Locate the workspace manifest and run the canonical manifest validation.',
      '3. Fix every validation error.',
      '4. Run the manifest validation and the relevant workspace checks again.',
  ...
  return [
    ...
    '6. Run the relevant workspace checks.',
