...
// ── Pure function: buildWorkspaceTableRows ────────────────────────────────────
...
describe('buildWorkspaceTableRows', () => {
  test('appends unassigned spend as a row so the footer reconciles', () => {
    ...
    expect(rows[1]).toMatchObject({ workspace_id: null, workspace_name: 'Unassigned', total_cost: 668.35 });
  });
  ...
  // `workspaces`, so subtracting it from the account total yields "everything
  ...
  // it changed from `offset > 0` to `workspaces.length !== total`, the outcome
  // here did not (1 workspace rendered, total 40).
    ...
    const rows = buildWorkspaceTableRows(
      {
        workspaces: [
          {
            workspace_id: 'p26',
            workspace_name: 'Page 2 workspace',
            session_count: 1,
            llm_cost: 1,
            compute_cost: 1,
            total_cost: 2,
  ...
  // subtraction ran against 25 of 40 workspaces and rendered the other 15
  // workspaces' spend ($0.34425 on the seed account) under the "Unassigned"
  // label.
  //
  ...
  // over `twoWorkspacePage`) is exactly `workspaces.length < total` at
  ...
  test('omits the unassigned row on a first page that is shorter than the whole result', () => {
    ...
    expect(rows).toHaveLength(1);
    expect(rows.some((row) => row.workspace_id === null)).toBe(false);
  });

  // The positive half of the same guard: a single page that holds every
  // workspace in the result is the one shape where the subtraction is
  // meaningful, and there the row must still appear with the exact figure.
  // `next_offset: null` and `workspaces.length === total` together are what
  // "this page is the whole result" looks like on the wire.
  test('appends the unassigned row when one page holds every workspace in the result', () => {
    const rows = buildWorkspaceTableRows(
      {
        workspaces: [
          {
    ...
    expect(rows[2]).toMatchObject({ workspace_id: null, workspace_name: 'Unassigned', total_cost: 1.5 });
  });
...
describe('nextWorkspaceSort', () => {
  ...
  test('Total toggles between the two directions the route accepts', () => {
    expect(nextWorkspaceSort('total_desc', 'total')).toBe('total_asc');
    expect(nextWorkspaceSort('total_asc', 'total')).toBe('total_desc');
  });

  test('a first click on Total lands on descending, from any other sort', () => {
    expect(nextWorkspaceSort('name_asc', 'total')).toBe('total_desc');
    expect(nextWorkspaceSort('recent', 'total')).toBe('total_desc');
  });

  // Project does NOT toggle. `GET /usage/cost-by-workspace` accepts `name_asc`
  ...
  test('Project is one-way — a repeat click never asks for a name_desc the API has no token for', () => {
    expect(nextWorkspaceSort('total_desc', 'name')).toBe('name_asc');
    expect(nextWorkspaceSort('name_asc', 'name')).toBe('name_asc');
  });
  ...
  test('every reachable result is a sort the route accepts', () => {
    const accepted: WorkspaceCostSort[] = ['total_desc', 'total_asc', 'recent', 'name_asc'];
    for (const active of accepted) {
      for (const column of ['name', 'total'] as const) {
        expect(accepted).toContain(nextWorkspaceSort(active, column));
      }
    }
  });
});

describe('workspaceSortDirection', () => {
  test('reports the active column and nothing else', () => {
    expect(workspaceSortDirection('total_desc', 'total')).toBe('descending');
    expect(workspaceSortDirection('total_desc', 'name')).toBeUndefined();

    expect(workspaceSortDirection('total_asc', 'total')).toBe('ascending');
    expect(workspaceSortDirection('total_asc', 'name')).toBeUndefined();

    expect(workspaceSortDirection('name_asc', 'name')).toBe('ascending');
    expect(workspaceSortDirection('name_asc', 'total')).toBeUndefined();
  });
  ...
  test('claims nothing under a sort no header offers', () => {
    expect(workspaceSortDirection('recent', 'total')).toBeUndefined();
    expect(workspaceSortDirection('recent', 'name')).toBeUndefined();
  });
  ...
  test('never returns the string "none"', () => {
    ...
    const sorts: WorkspaceCostSort[] = ['total_desc', 'total_asc', 'recent', 'name_asc'];
    for (const sort of sorts) {
      for (const column of columns) {
        expect(workspaceSortDirection(sort, column)).not.toBe('none');
      }
    }
  });
});
...
// different under a different order. Re-sorting on page 2 of 40 workspaces
...
describe('applyWorkspaceSort', () => {
  ...
  test('resets to page 1 on the Project column too, not just Total', () => {
    expect(applyWorkspaceSort({ sort: 'total_desc', offset: 25 }, 'name')).toEqual({
      sort: 'name_asc',
      offset: 0,
    });
  });
  ...
  test('resets the page even when the sort itself does not change', () => {
    expect(applyWorkspaceSort({ sort: 'name_asc', offset: 75 }, 'name')).toEqual({
      sort: 'name_asc',
      offset: 0,
    });
  });

  test('carries the sort `nextWorkspaceSort` chose, rather than deciding again', () => {
    const sorts: WorkspaceCostSort[] = ['total_desc', 'total_asc', 'recent', 'name_asc'];
    for (const sort of sorts) {
      for (const column of ['name', 'total'] as const) {
        expect(applyWorkspaceSort({ sort, offset: 25 }, column).sort).toBe(
          nextWorkspaceSort(sort, column),
        );
      }
    }
  });
...
describe('buildWorkspacesLevelExportFilters', () => {
  test('sends whichever sort is active, not a fixed default', () => {
    expect(buildWorkspacesLevelExportFilters('name_asc')).toEqual({ sort: 'name_asc' });
    expect(buildWorkspacesLevelExportFilters('total_asc')).toEqual({ sort: 'total_asc' });
    expect(buildWorkspacesLevelExportFilters('total_desc')).toEqual({ sort: 'total_desc' });
  });
  ...
  test('carries no page — an export is the whole filtered query', () => {
    const filters = buildWorkspacesLevelExportFilters('total_desc');
    expect(filters).not.toHaveProperty('offset');
    expect(filters).not.toHaveProperty('limit');
  });
});

// ── Pure function: isWorkspaceRowClickable ────────────────────────────────────

describe('isWorkspaceRowClickable', () => {
  test('a row with a real workspace_id is clickable', () => {
    ...
    expect(isWorkspaceRowClickable(row)).toBe(true);
  });

  test('the unassigned row (workspace_id: null) is not clickable', () => {
    const row: WorkspaceTableRow = {
      workspace_id: null,
      workspace_name: 'Unassigned',
      session_count: 0,
    ...
    expect(isWorkspaceRowClickable(row)).toBe(false);
  });
});
...
// ── Component: WorkspacesLevelContent ─────────────────────────────────────────
...
// `WorkspacesLevel` actually mounts; a second one in this component would be
...
function baseContentProps(overrides: Partial<Parameters<typeof WorkspacesLevelContent>[0]> = {}) {
  return {
    range: baseRange,
    onRangeChange: noop,
    onResetRange: noop,
...
function renderContent(overrides: Partial<Parameters<typeof WorkspacesLevelContent>[0]> = {}): string {
  return renderToStaticMarkup(
    <TooltipProvider>
      <WorkspacesLevelContent {...baseContentProps(overrides)} />
    </TooltipProvider>,
  );
}
...
const twoWorkspacePage: WorkspaceCostPage = {
  workspaces: [
    {
      workspace_id: 'p1',
      workspace_name: 'Alpha',
...
describe('WorkspacesLevelContent', () => {
  test('offers the CSV export beside the range picker', () => {
    const html = renderContent({ page: twoWorkspacePage });
  ...
  test('the export button is handed the active sort, not a fixed default', () => {
    for (const sort of ['total_desc', 'total_asc', 'name_asc', 'recent'] as const) {
      const tree = WorkspacesLevelContent(baseContentProps({ page: twoWorkspacePage, sort }) as never);
      ...
      expect(exports[0]!.props.kind).toBe('workspaces');
  ...
  test('keeps the CSV export available while the table is still loading', () => {
    ...
    const html = renderContent({ isWorkspacesLoading: true });
    expect(html).toContain('Export CSV');
  });

  test('shows a loading skeleton, never a spinner icon, while workspaces have not loaded', () => {
    const html = renderContent({ isWorkspacesLoading: true });
    expect(html).toContain('aria-label="Loading workspaces"');
    expect(html).not.toContain('animate-spin');
    expect(html).not.toContain('No spend');
  });

  test('renders the workspace error inline without losing the shell (range picker still present)', () => {
    const html = renderContent({ workspacesError: new Error('workspaces-error-marker') });
    expect(html).toContain('workspaces-error-marker');
  ...
  // `QueryObserver`): `isWorkspacesLoading` is React Query's `isPending &&
  ...
  // offline. There `page` is undefined and `workspacesError` is null, and
  ...
  test('renders the loading state, not a "no spend" claim, when no page has been read and nothing is in flight', () => {
    const html = renderContent({ page: undefined, isWorkspacesLoading: false, workspacesError: null });
    expect(html).toContain('aria-label="Loading workspaces"');
  ...
  test('a failed refetch over an already-read empty page shows the error, not a "no spend" claim', () => {
    const html = renderContent({
      page: { workspaces: [], total: 0, limit: 25, offset: 0, next_offset: null },
      summary: summaryWithTotal(0),
      workspacesError: new Error('workspaces-error-marker'),
    });
    expect(html).toContain('workspaces-error-marker');
    expect(html).not.toContain('No spend recorded yet');
    expect(html).not.toContain('No spend in this range');
  ...
  // orderings `GET /usage/cost-by-workspace` accepts. The rest must stay plain:
  ...
  test('aria-sort marks only the active column, descending by default', () => {
    const html = renderContent({ page: twoWorkspacePage, sort: 'total_desc' });
    const header = html.match(/<thead[\s\S]*?<\/thead>/)![0];
  ...
  test('aria-sort flips to ascending on the Total column', () => {
    const header = renderContent({ page: twoWorkspacePage, sort: 'total_asc' }).match(
      /<thead[\s\S]*?<\/thead>/,
    )![0];
    expect((header.match(/aria-sort=/g) ?? []).length).toBe(1);
  ...
  test('aria-sort moves to the Project column under name_asc', () => {
    const header = renderContent({ page: twoWorkspacePage, sort: 'name_asc' }).match(
      /<thead[\s\S]*?<\/thead>/,
    )![0];
    const workspaceCell = (header.match(/<th[\s\S]*?<\/th>/g) ?? [])[0]!;
    expect(workspaceCell).toContain('aria-sort="ascending"');
    expect((header.match(/aria-sort=/g) ?? []).length).toBe(1);
  });
  ...
  test('no column claims aria-sort under a sort no header offers', () => {
    const header = renderContent({ page: twoWorkspacePage, sort: 'recent' }).match(
      /<thead[\s\S]*?<\/thead>/,
    )![0];
  ...
  test('the Total header keeps the numeric right alignment', () => {
    const header = renderContent({ page: twoWorkspacePage }).match(/<thead[\s\S]*?<\/thead>/)![0];
  ...
  test('clicking a sortable header reports which column was clicked', () => {
    ...
    const tree = WorkspacesLevelContent(
      baseContentProps({ page: twoWorkspacePage, onSort: (column) => clicked.push(column) }) as never,
    );

    const headers = collectElementsByType(tree, CostSortHeader);
    expect(headers.map((header) => header.props.label)).toEqual(['Workspace', 'Total']);
  ...
  test('renders the column headers in order: Workspace, Sessions, LLM, Compute, Total', () => {
    const html = renderContent({ page: twoWorkspacePage });
    ...
    expect(sessionsIndex).toBeGreaterThan(workspaceIndex);
  ...
  test('renders workspace rows in the order the page provides, with unassigned always last', () => {
    const html = renderContent({ page: twoWorkspacePage, summary: summaryWithTotal(20) });
  ...
  test('a real workspace row is clickable — cursor-pointer and hover:bg-accent on its <tr>', () => {
    const html = renderContent({ page: twoWorkspacePage });
  ...
  // bypassing isWorkspaceRowClickable or by a copy/paste of the real-row
  ...
  test('the footer reconciles on all four numeric columns, including unassigned', () => {
    ...
    // row-HIDDEN state. See `buildWorkspaceTableRows`' consequence 2.
  });
  ...
  // On a page after the first, `buildWorkspaceTableRows` never appends an
  ...
  test('a first page shorter than the whole result renders no Unassigned row, and the footer sums only that page', () => {
    const html = renderContent({
      page: { ...twoWorkspacePage, total: 40, offset: 0, next_offset: 25 },
  ...
  // Defect 2, workspaces level. The footer sums one page; the Total tile above
  ...
  // Defect 3. Workspace names are user-supplied and unbounded; at 1440px a
  ...
  test('a long workspace name is truncated in its own cell rather than widening the column', () => {
    const longName = 'A'.repeat(69);
    const html = renderContent({
      page: {
        ...twoWorkspacePage,
        workspaces: [{ ...twoWorkspacePage.workspaces[0]!, workspace_name: longName }],
        total: 1,
      },
    });
  ...
  test('pagination caption and Previous/Next disabled state reflect the page', () => {
    const html = renderContent({ page: { ...twoWorkspacePage, total: 30, offset: 0, next_offset: 25 } });
    expect(html).toContain('Showing 1-2 of 30 workspaces');
  ...
  test('Previous is enabled and Next is disabled on the last page', () => {
    const html = renderContent({ page: { ...twoWorkspacePage, total: 27, offset: 25, next_offset: null } });
    expect(html).toContain('Showing 26-27 of 27 workspaces');
    const previousMatch = html.match(/<button[^>]*>Previous<\/button>/);
    const nextMatch = html.match(/<button[^>]*>Next<\/button>/);
