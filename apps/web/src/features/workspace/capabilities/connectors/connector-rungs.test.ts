...
describe('the canonical rung order', () => {
  test('CONNECTOR_RUNG_ORDER is exactly overview, accounts, permissions, settings', () => {
    expect(CONNECTOR_RUNG_ORDER.length).toBe(4);
    expect(CONNECTOR_RUNG_ORDER[0]).toBe('overview');
    expect(CONNECTOR_RUNG_ORDER[1]).toBe('accounts');
    expect(CONNECTOR_RUNG_ORDER[2]).toBe('permissions');
    expect(CONNECTOR_RUNG_ORDER[3]).toBe('settings');
  });

  test('a connector that yields all four rungs emits them in that literal sequence', () => {
    const rungs = visibleRungs(conn({ provider: 'pipedream' }), { canWrite: true });
    expect(rungs.length).toBe(4);
    expect(rungs[0]).toBe('overview');
    expect(rungs[1]).toBe('accounts');
    expect(rungs[2]).toBe('permissions');
    expect(rungs[3]).toBe('settings');
  });
});
