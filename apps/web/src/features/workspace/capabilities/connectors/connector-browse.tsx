...
function IntegrationIcon({ icon }: { icon: string | null }) {
  ...
  return (
    <span className="bg-card relative flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-sm outline outline-black/10 dark:outline-white/10">
      <Image
        src={icon}
        alt=""
...
function CatalogEntryCard({
  ...
  return (
    <CatalogCard
      ...
      className={reveal === null ? undefined : 'kx-fade-up'}
      style={
        reveal === null
          ? undefined
          : // The same 40ms step and 200ms ceiling `.kx-stagger` uses in
            // globals.css. `.kx-stagger > *` itself is unusable here: it would
            // animate the first `CATEGORY_ROW_CAP` cards too, and with ~15
            // sections on screen that is ~90 elements animating on first paint.
            { animationDelay: `${Math.min(reveal, 5) * 40}ms` }
      }
    />
  );
}
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {items.length > CATEGORY_ROW_CAP ? (
          <Button
            ...
            aria-label={expanded ? `Show fewer ${label} connectors` : `View all ${label} connectors`}
...
export function ConnectorBrowse({
  ...
  return (
    <div className="space-y-6">
      ...
      {activeCategory === ALL_CATEGORIES && state.source === 'discover' && entries.length < total ? (
