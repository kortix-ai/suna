    ...
    <div className="space-y-4">
      ...
      {connector.actions.length > SEARCH_THRESHOLD ? (
        <InputGroupSearch>
          ...
          <InputGroupSearchInput
            placeholder={`Search ${connector.actions.length} tools`}
            value={query}
            ...
            <ul className="space-y-2">
              {group.actions.map((action) => {
                ...
                return (
                  <li
                    key={action.path}
                    className="bg-popover flex flex-wrap items-center gap-x-3 gap-y-2 rounded-md border px-4 py-2"
                  >
                    <div className="min-w-0 flex-1 basis-40">
                      ...
                      {row.description ? (
                        // No `block` alongside `line-clamp-2`: Tailwind emits
                        // `.block` last and it would kill the clamp.
