...
export function EntityDetailModal({
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent
        className="space-y-0 lg:max-w-7xl"
    ...
    <>
      ...
      <ModalBody className="max-h-[70vh] overflow-hidden p-0">
        <div className="flex min-h-0 flex-col overflow-y-auto lg:h-[70vh] lg:flex-row lg:overflow-hidden">
          {/* Left — "Source" label, source path, edit action, then the file
              tree. Always present; the tree portion only renders once there
              is more than one file to pick from. No separate "About"
              description here: the header already carries that exact sentence
              in the same viewport (`ModalDescription` above), so repeating it
              in a 256px rail would be pure duplication — for a single-file
              entity (most commands) the whole rail would otherwise restate
              the header and nothing else. The source path is the one thing
              the header doesn't already say.
          ...
          <div className="border-border/60 flex shrink-0 flex-col border-b p-3.5 lg:h-full lg:w-64 lg:overflow-y-auto lg:border-r lg:border-b-0">
