...
describe('the shell zoom does not drag the band off the OS controls', () => {
  ...
  test('the in-band control is sized in window px like its offset', () => {
    ...
    const shell = readFileSync(join(import.meta.dir, 'workspace-shell.tsx'), 'utf8');
...
/**
 * One panel, one opener. The workspace sidebar is `collapsible="offcanvas"`, so
 * five views grew their own "bring it back" control. On the desktop shell
 * `WorkspaceShellLayout` draws the canonical one in the band; the view-level
...
describe('page-level sidebar openers all use the shared gate', () => {
  const views = {
    'workspace-home.tsx': join(import.meta.dir, 'workspace-home.tsx'),
    'workspace-sessions-view.tsx': join(
      repoRoot,
      'apps/web/src/features/workspace/workspace-sessions/workspace-sessions-view.tsx',
    ),
