...
import { useDesktopShell } from '@/features/workspace/workspace-layout/sidebar-opener';
...
} from '@/lib/onboarding/last-workspace-cookie';
...
export function WorkspaceShell({ workspaceId, initialSidebarOpen, children }: WorkspaceShellProps) {
  ...
  const { data: workspaceDetail, error: workspaceDetailError } = useQuery({
  ...
  // Remember the workspace so the next `/` hit and the next sign-in land straight
  // back here instead of going through the id-free landing door. Gated on a
  // successful detail fetch: recording a workspace the user cannot actually read
  // would send them into a 404 bounce loop on their next visit.
  useEffect(() => {
    if (!workspaceDetail) return;
    writeLastWorkspaceId(user?.id, workspaceId);
  }, [workspaceDetail, workspaceId, user?.id]);

  // Self-heal a stale remembered project. The cookie outlives the workspace it
  ...
  useEffect(() => {
    if (!workspaceDetailError) return;
    const status = (workspaceDetailError as { status?: number }).status;
    if (status !== 403 && status !== 404) return;
    if (readLastWorkspaceId(user?.id) !== workspaceId) return;
    clearLastWorkspaceId();
    router.replace(WORKSPACE_LANDING_PATH);
  }, [workspaceDetailError, workspaceId, router, user?.id]);
  ...
  useEffect(() => {
    ...
    const redirect = legacyCustomizeRedirect(workspaceId, searchParams.get('customize'));
    if (redirect) {
  ...
  // New-session navigation opens the workspace composer. The first send creates
  // the durable session.
  ...
  return (
    ...
    // until project-detail lands we don't know this workspace's account, and a
    ...
    <BillingAccountProvider
      accountId={workspaceDetail?.workspace?.account_id ?? null}
      resolved={!!workspaceDetail}
    >
      <AppProviders
        showSidebar
      ...
      >
        <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          <WorkspaceShellLayout>{children}</WorkspaceShellLayout>
        </div>
...
const WorkspaceShellLayout = ({ children }: { children: React.ReactNode }) => {
