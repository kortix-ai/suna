...
import { contract, qk, useWorkspaceName, type Command } from '@kortix/sdk/react';
...
export function WorkspaceHome({
  ...
  const sandboxesQuery = useQuery({
    queryKey: qk.workspace.sandboxes(workspaceId),
...
function MetaRuntimeIndicator() {
  return (
    <Hint label="Meta uses a fixed minimal sandbox. It starts specialized sessions for workspace work.">
  ...
  return (
    <div className="relative z-10 flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex shrink-0 justify-center px-4 pb-6">
        <WorkspaceHomeSections workspaceId={workspaceId} />
      </div>
    </div>
  );
}
...
function WorkspaceHomeSections({ workspaceId }: { workspaceId: string }) {
  const openCustomize = useCustomizeStore((s) => s.openCustomize);
            ...
            <Button
              ...
              onClick={() =>
                isCapabilityTabKey(section)
                  ? router.push(capabilityTabHref(workspaceId, section))
