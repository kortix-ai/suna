...
describe('/new hosts the onboarding wizard', () => {
  ...
  test('it reads the workspace id from the onboarding param', () => {
  ...
  test('the wizard is mounted only when the param is present', () => {
    expect(code).toContain('onboardingWorkspaceId && (');
    expect(code).toContain('<WorkspaceOnboardingWizard');
    expect(code).toContain('workspaceId={onboardingWorkspaceId}');
  });
  ...
  test('it leaves for the workspace with replace, never push', () => {
    ...
    expect(code).toContain('<WorkspaceOnboardingWizard');
    const wizardBlock = code.slice(code.indexOf('<WorkspaceOnboardingWizard'));
  ...
  test('every trip into the workspace percent-encodes the id, matching onboardingPath', () => {
    expect(code).not.toContain('`/workspaces/${onboardingWorkspaceId}`');
    ...
    expect(
      code.match(/\/workspaces\/\$\{encodeURIComponent\(onboardingWorkspaceId\)\}/g)?.length,
    ).toBe(2);
  });

  /**
   * `index`, `domain` and the survey `answers` are plain `useState` inside the
   * wizard, none keyed on the workspace. Without a key, a change of
   * `onboardingWorkspaceId` on a mounted instance would PATCH workspace A's
   * answers onto workspace B.
   */
  test('the wizard is keyed on the workspace so state never crosses workspaces', () => {
    expect(code).toContain('key={onboardingWorkspaceId}');
  });
});
 ...
 * `getWorkspaceDetail` was still in flight. The Name input carries `autoFocus`
...
describe('/new: the onboarding param owns the page', () => {
  ...
  test('the form is the OTHER branch of that swap, so the two can never both mount', () => {
    ...
    // autoFocus> and all — while `getWorkspaceDetail` is still in flight.
   ...
   * link now, which is why it takes the workspace id at all.
   */
  test('the handoff is handed the workspace id, so it can offer a way into the workspace', () => {
    expect(code).toContain('workspaceId={onboardingWorkspaceId}');
  });
   ...
   * other host) is mounted by `workspace-shell.tsx` and the share page, never by
