 ...
 * IS already valid v2 Runtime behavior — a stock Runtime agent file, no
...
- **You do NOT need to read each agent's \`.md\` frontmatter to migrate it.** v1's frontmatter (mode/model/temperature/permission/prompt/…) is ALREADY valid v2 Runtime behavior — it stays exactly where it is, unchanged. This migration is governance-only.
...
- If an agent has no \`.md\` today (a bare \`[[agents]]\` entry with no matching Runtime agent file), still declare it in \`agents:\` with its governance grants carried over — don't drop it. It will simply have no behavior until someone adds \`.kortix/opencode/agents/<name>.md\`.
