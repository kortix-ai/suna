 ...
 *
 * Headerless on purpose: the screen shell (`ProjectSectionPage`, driven by
 * `settings-tab-meta`) owns the `<h1>` and the one line under it, so this file
 * renders only the body. It used to wrap itself in a `CustomizeSectionWrapper`
 * titled "Settings", which sat directly under the page's own "Settings" title.
 *
 * Repository configuration moved to the Repository tab
 * (`features/workspace/settings/repository-config-card.tsx`) — it duplicated
 * the repository metadata the Git surface already showed. Everything else,
 * including the danger zone at the bottom, is unchanged.
 */
    ...
    <div className="space-y-8">
      ...
      {project && (
        <>
          ...
          {/* Moves to the Automations screen in a later change; it stays here so
              the kill-switch never stops being reachable in the meantime. */}
          {canManage && (
          ...
          {/* Destructive actions stay last, after everything routine. */}
          {canManage && <DangerZone onArchive={() => setArchiveOpen(true)} />}
        </>
      )}
...
/** The bottom of General: irreversible project-level actions, kept apart. */
function DangerZone({ onArchive }: { onArchive: () => void }) {
  const tHardcodedUi = useTranslations('hardcodedUi');

  return (
    <section className="space-y-4">
      <Label>
        {tHardcodedUi.raw('appProjectsIdCustomizeSettingsPage.line110JsxAttrTitleDangerZone')}
      </Label>
      <div className="bg-popover rounded-md border px-4 py-3">
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            <p className="text-foreground text-sm font-medium">
              {tHardcodedUi.raw('appProjectsIdCustomizeSettingsPage.line116JsxTextArchiveProject')}
            </p>
          ...
          <Button variant="destructive" className="shrink-0" size="sm" onClick={onArchive}>
