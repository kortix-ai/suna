...
import { removeGroupMember } from '@/lib/iam-client';
 ...
 * Invite"), not persistent state. The legacy store used to clear it as a
 * side effect of the NEXT `openCustomize` call for ANY section — so a stale
 * 'invite' request only ever cleared itself once something else happened to
 * touch the store again. `navigate()` (the shared nav's replacement for
 ...
 * (`setSection`, untouched) → clicks "Manage providers"
 * (`navigate('llm-providers')`, which — correctly — does not touch
 * `membersTab`) → clicks back to Members. `SectionContent` unmounts and
 * remounts `MembersView` on that round trip, which seeds its tab from
 * `membersTab` at mount — a stale `'invite'` would replay instead of
 * landing on People. See `members-view.test.ts` for that exact sequence
...
export function MembersView({ projectId }: { projectId: string }) {
  ...
  const { membersTab: requestedTabRaw, activeTab, navigate } = useSettingsNav();
  const requestedTab = (requestedTabRaw ?? 'people') as 'people' | 'invite';
  const [tab, setTab] = useState<'people' | 'invite'>(() => requestedTab);

  // `membersTab` is a one-shot deep-link INTENT ("land on Invite"), not
  // persistent state — see `consumeMembersTabIntent`'s doc comment below.
  // Consume it exactly once per 'invite' request: follow it locally, then
  // clear it on the shared nav so a later, unrelated visit to this view
  // never replays a stale request. Only fires for an actual 'invite' ask —
  // a 'people' value means "nothing pending," so it's never itself a reason
  // to touch local `tab` (that would fight a user who's already looking at
  // Invite from an earlier request this same effect just cleared).
  useEffect(() => {
    const consumed = consumeMembersTabIntent({ membersTab: requestedTabRaw, activeTab, navigate });
    if (consumed) setTab(consumed);
  }, [requestedTabRaw, activeTab, navigate]);
  ...
  // moved (cut, not copied) to `tabs/members-tab.tsx` — see that file's
  // header comment. Their gate conditions and props are preserved there
  // EXACTLY as they were mounted here (verified against this diff, not
  // re-derived): `ProjectGroupGrantsCard` on `project?.account_id` alone;
  // `ResourceAccessCard`/`ProjectRoleAssignmentsCard` additionally on
  // `canManage` (manager-only DATA — the grants list and the account
  // policies route both deny non-managers).
  const peopleContent = (
