...
/**
 * The two project-configuration entries in the sidebar. They are NOT the same
 * surface, and the split is deliberate:
 *
 *  - ProjectCustomizeNavItem — top of the panel, under New session. Opens the
 *    Customize overlay, which floats over the current page. Ungated.
 *  - ProjectSettingsNavItem — bottom of the footer group, on the line the old
 *    Customize row held. Navigates to the capability pages
 *    (Connectors / Skills / Commands) and carries the Mod+, keycap.
 */
 ...
 * project page is active so you never lose your place (see customize-store).
 * That rules out a `<Link prefetch>` — there is no href to prefetch — and it is
 * why active state reads the store's `open` flag rather than the pathname.
 *
 * Ungated on purpose. The Settings row gates on the three capability leaves
