 ...
 * Templates / Secrets / Settings). Gated, because those pages can 403.
 * Templates is additionally flag-gated and appears only when a project turns
 * `templates` on — see `CapabilityTab.flag`.
 *
...
export const TAB_PREFERENCE: readonly { key: CapabilityTab['key']; action: string }[] = [
  ...
  // Templates reuses `project.read`, the SAME leaf Models above it carries,
  // because that is what the API gates the catalog read on
  // (`PROJECT_ACTIONS.PROJECT_WRITE` in
  // `apps/api/src/projects/routes/templates.ts`). Sharing it has a second
  ...
  // Templates for a caller whose `templates` flag is off — a row to a page
  ...
  // flag.
  { key: 'templates', action: PROJECT_ACTIONS.PROJECT_READ },
