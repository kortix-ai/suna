 ...
 * The Crafts row.
 *
 * Crafts had NO entry point at all before this: the store was reachable only
 ...
 * Crafts and Apps are the same KIND of thing — a project surface you configure
 ...
 * grid is neither.
 */
export function ProjectCraftsNavItem() {
  const pathname = usePathname();
  const params = useParams<{ id: string }>();
  const projectId = params?.id;
  const isMobile = useIsMobile();
  const { setOpenMobile } = useSidebar();
  const craftsGate = useFeatureFlag(projectId, 'crafts');
  ...
  /* Fail-closed like every other flagged surface: the row exists once the
     project turns the `crafts` flag on in Settings → Feature flags. Loading
     counts as disabled. */
  if (!craftsGate.enabled) return null;
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        asChild
        /* `/crafts` covers the store AND `/crafts/runs`, so the row stays active
           across the whole surface rather than going dark on the report page. */
        isActive={pathname?.startsWith(`/projects/${projectId}/crafts`) === true}
        tooltip="Crafts"
      ...
      >
        {/* Hover-gated prefetch, same reason as Apps: prefetching on mount cost
            ...
            visit. */}
        <HoverPrefetchLink href={`/projects/${projectId}/crafts`} prefetch onClick={handleClick}>
