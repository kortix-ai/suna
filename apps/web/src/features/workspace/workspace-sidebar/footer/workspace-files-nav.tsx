 ...
 * something to cache, because the workspace layout awaits cookies() and the
 ...
 * layout, so four rows bought nothing over one: WorkspaceSettingsNavItem
...
export function WorkspaceFilesNavItem() {
  ...
  const canReadFiles = useWorkspaceCan(workspaceId, WORKSPACE_ACTIONS.WORKSPACE_FILE_READ);
  ...
  // No workspace id means no valid href. The old onClick already no-op'd in this
  ...
  return (
    <SidebarMenuItem>
      ...
      >
        <Link href={`/workspaces/${workspaceId}/files`} prefetch onClick={handleClick}>
          <FolderOpen />
          Files
        </Link>
      </SidebarMenuButton>
