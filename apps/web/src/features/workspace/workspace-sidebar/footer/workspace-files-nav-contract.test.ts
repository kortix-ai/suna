...
const SOURCE = readFileSync(resolve(import.meta.dir, 'workspace-files-nav.tsx'), 'utf8');

/** The WorkspaceFilesNavItem function body, isolated from its neighbours. */
function filesNavItemSource(): string {
  const start = SOURCE.indexOf('export function WorkspaceFilesNavItem');
  expect(start).toBeGreaterThan(-1);
  const after = SOURCE.indexOf('export function', start + 1);
  return after === -1 ? SOURCE.slice(start) : SOURCE.slice(start, after);
}

describe('workspace Files sidebar entry', () => {
  test('navigates with a prefetching Link, not router.push', () => {
    ...
    expect(navItem).toMatch(/href=\{`\/workspaces\/\$\{workspaceId\}\/files`\}/);
  ...
  test('still hides itself for callers without project.file.read', () => {
    ...
    expect(navItem).toContain('WORKSPACE_FILE_READ');
  ...
  test('does not retain the router.push helper the rail used', () => {
    ...
    // (WorkspaceFilesRailItem) is dead code, so keeping it around would just
    // leave a trap for the next editor.
    expect(SOURCE).not.toContain('useFilesActivate');
    expect(SOURCE).not.toContain('WorkspaceFilesRailItem');
  });
});
