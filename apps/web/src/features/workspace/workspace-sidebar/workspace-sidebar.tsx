...
import { WorkspaceAppsNavItem } from '@/features/workspace/workspace-sidebar/footer/workspace-apps-nav';
...
import { WorkspaceFilesNavItem } from '@/features/workspace/workspace-sidebar/footer/workspace-files-nav';
...
} from '@/features/workspace/workspace-sidebar/workspace-settings-nav';
...
export function WorkspaceSidebar({ workspaceId }: { workspaceId: string }) {
  ...
  // Open the workspace composer without creating a durable session.
      ...
      <SidebarContent className="relative min-h-0 flex-1 [scrollbar-width:'none'] overflow-hidden [-ms-overflow-style:'none'] [&::-webkit-scrollbar]:hidden">
        <div className="flex h-full min-h-0 flex-col space-y-4">
          ...
          {/* Sessions are always expanded — no collapse toggle. The `Sessions`
              header (label + ⋯ filter menu) now lives inside WorkspaceSessionList
          ...
          <SidebarGroup className="min-h-0 flex-1 flex-col py-0" ref={sessionsGroupRef}>
            <WorkspaceSessionList workspaceId={workspaceId} />
          </SidebarGroup>
