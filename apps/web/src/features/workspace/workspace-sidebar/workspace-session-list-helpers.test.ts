...
describe('workspaceSessionsRefetchInterval', () => {
  ...
  test('provisioning polls fast, and outranks the open-session interval', () => {
    ...
    expect(workspaceSessionsRefetchInterval({ sessions, hasOpenSession: false })).toBe(5_000);
    expect(workspaceSessionsRefetchInterval({ sessions, hasOpenSession: true })).toBe(5_000);
  });

  test('an open session polls slowly so its row can change section as it is used', () => {
    expect(workspaceSessionsRefetchInterval({ sessions: settled, hasOpenSession: true })).toBe(
      60_000,
    );
  });

  test('a workspace page with no session open does not poll', () => {
    expect(workspaceSessionsRefetchInterval({ sessions: settled, hasOpenSession: false })).toBe(
      false,
    );
    expect(workspaceSessionsRefetchInterval({ sessions: undefined, hasOpenSession: false })).toBe(
      false,
    );
  });
});
