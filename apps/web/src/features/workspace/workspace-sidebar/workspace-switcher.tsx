...
/**
 * The workspace sidebar's one control: which workspace you are in, and everything
...
import { WorkspaceMenuSection } from '@/features/workspace/workspace-sidebar/workspace-menu-section';
...
export function WorkspaceSwitcher({ workspaceId }: { workspaceId: string }) {
  ...
  // `qk.workspace.summary(id)` is the canonical `getWorkspace` slot — the same entry
  // the rest of the workspace shell reads, so naming the workspace here costs no
  // extra request. NOT the workspaces LIST: that waits on `accounts` first, and
          ...
          <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
            ...
            >
              ...
              {workspace?.account_id ? (
                <DropdownMenuItem
                  onSelect={() =>
                    deferAfterClose(() => router.push(`/accounts/${workspace.account_id}`))
                  }
                  size="sm"
                >
                  <CogOne />
