 ...
 *
 *  - WorkspaceCustomizeNavItem — top of the panel, under New session. Navigates
 *    to the capability pages (Connectors / Agents / Skills). Gated, because
 *    those pages can 403.
 *  - WorkspaceSettingsNavItem — bottom of the footer group, on the line the old
...
const TAB_PREFERENCE: readonly { key: CapabilityTab['key']; action: string }[] = [
  { key: 'connectors', action: WORKSPACE_ACTIONS.WORKSPACE_CONNECTOR_READ },
  { key: 'agent', action: WORKSPACE_ACTIONS.WORKSPACE_AGENT_READ },
  { key: 'skills', action: WORKSPACE_ACTIONS.WORKSPACE_SKILL_READ },
];

/**
 * First tab the caller may open, or null when every one of them is an explicit
 * deny. Optimistic while a probe loads — same rule as WorkspaceFilesNavItem: the
 ...
 */
function useCapabilityTab(workspaceId: string | undefined): CapabilityTab['key'] | null {
  const canConnectors = useWorkspaceCan(workspaceId, TAB_PREFERENCE[0].action);
  const canAgents = useWorkspaceCan(workspaceId, TAB_PREFERENCE[1].action);
  const canSkills = useWorkspaceCan(workspaceId, TAB_PREFERENCE[2].action);
 ...
 * navigates to the workspace's capability pages (Connectors / Agents / Skills),
 ...
 * WorkspaceFilesNavItem: the button form cannot be prefetched, so every click
...
export function WorkspaceCustomizeNavItem() {
  ...
  const isMobile = useIsMobile();
  const { setOpenMobile } = useSidebar();
  const tab = useCapabilityTab(workspaceId);
  ...
  if (!tab) return null;
  // No workspace id means no valid href, so there is nothing to render.
  ...
  return (
    <SidebarMenuItem>
      ...
      >
        <Link href={capabilityTabHref(workspaceId, tab)} prefetch onClick={handleClick}>
 ...
 * workspace page is active so you keep your place (see customize-store). That
