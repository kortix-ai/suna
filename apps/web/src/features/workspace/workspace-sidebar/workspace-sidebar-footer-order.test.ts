 ...
 */
const source = readFileSync(join(import.meta.dir, 'workspace-sidebar.tsx'), 'utf8');
...
describe('workspace sidebar footer ordering', () => {
  test('late-arriving billing items render above the permanent nav', () => {
    expect(orderOf('SidebarUpgradeButton')).toBeLessThan(orderOf('WorkspaceFilesNavItem'));
    expect(orderOf('SidebarBalanceWarning')).toBeLessThan(orderOf('WorkspaceFilesNavItem'));
  });
  ...
  test('the permanent nav keeps its own order', () => {
    ...
    expect(orderOf('WorkspaceFilesNavItem')).toBeLessThan(orderOf('WorkspaceSettingsNavItem'));
    expect(orderOf('WorkspaceSettingsNavItem')).toBeLessThan(
      orderOf('WorkspaceChatGptConnectNavItem'),
    );
  });
  ...
  test('Settings stays in the footer group; only Customize sits up top', () => {
    ...
    expect(orderOf('WorkspaceCustomizeNavItem')).toBeLessThan(orderOf('WorkspaceSessionList'));
    expect(orderOf('WorkspaceSessionList')).toBeLessThan(orderOf('WorkspaceSettingsNavItem'));
    // The three capability rows are gone for good — they are tabs of the one
    // page both remaining entries link to. `WorkspaceCustomizeNavItem` is NOT in
    // this list: the name was reused for the top row.
    expect(source).not.toContain('<WorkspaceConnectorsNavItem');
    expect(source).not.toContain('<WorkspaceSkillsNavItem');
    expect(source).not.toContain('<WorkspaceCommandsNavItem');
  });
});
