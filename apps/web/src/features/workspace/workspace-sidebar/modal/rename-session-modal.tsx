...
export function RenameSessionModal({
  ...
  // instead, so the 'workspace'-scoped inventory page (never painted
  // optimistically) still catches up via a real refetch.
  const sessionsQueryKey = qk.workspace.sessions(workspaceId);
  ...
  const renameMutation = useMutation({
    ...
    onSuccess: (updated, name) => {
      ...
      queryClient.setQueryData<WorkspaceSession[]>(sessionsQueryKey, (sessions) =>
