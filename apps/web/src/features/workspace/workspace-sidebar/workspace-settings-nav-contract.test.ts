...
const SOURCE = readFileSync(resolve(import.meta.dir, 'workspace-settings-nav.tsx'), 'utf8');
 ...
 */
describe('workspace Customize sidebar entry (the routed one)', () => {
  test('navigates with a prefetching Link, not router.push', () => {
    ...
    const navItem = fnSource('WorkspaceCustomizeNavItem');

    expect(navItem).toContain('<Link');
    expect(navItem).toMatch(/prefetch(\s|>|$)/);
    expect(navItem).not.toContain('prefetch={false}');
    expect(navItem).toContain('capabilityTabHref(workspaceId, tab)');
  ...
  test('reads all three capability leaves, not one', () => {
    ...
    expect(SOURCE).toContain('WORKSPACE_AGENT_READ');
    expect(SOURCE).toContain('WORKSPACE_CONNECTOR_READ');
    expect(SOURCE).toContain('WORKSPACE_SKILL_READ');
    expect(fnSource('WorkspaceCustomizeNavItem')).toContain('useCapabilityTab(workspaceId)');
  });

  test('probes every tab in TAB_PREFERENCE', () => {
    // useWorkspaceCan is a hook, so it cannot be called from a loop that
    ...
    const probeCount = (hook.match(/useWorkspaceCan\(/g) ?? []).length;
  ...
  test('stays visible while a probe is loading', () => {
    // Optimistic until an explicit deny — same rule as WorkspaceFilesNavItem.
    expect(SOURCE).toContain('p.allowed || p.isLoading');
  });

  test('closes the mobile drawer on navigate', () => {
    expect(fnSource('WorkspaceCustomizeNavItem')).toContain('setOpenMobile(false)');
  });
  ...
  test('does not open the overlay', () => {
    ...
    expect(fnSource('WorkspaceCustomizeNavItem')).not.toContain('openCustomize');
  });
  ...
  test('carries no keycap', () => {
    ...
    expect(fnSource('WorkspaceCustomizeNavItem')).not.toContain('<Kbd>');
  });
});

describe('workspace Settings sidebar entry (the overlay one)', () => {
  test('opens the Customize overlay, it does not navigate', () => {
    ...
    const navItem = fnSource('WorkspaceSettingsNavItem');
  ...
  test('is ungated and takes its active state from the overlay flag', () => {
    ...
    expect(navItem).not.toContain('useWorkspaceCan');
  ...
  test('closes the mobile drawer on open', () => {
    expect(fnSource('WorkspaceSettingsNavItem')).toContain('setOpenMobile(false)');
  });
