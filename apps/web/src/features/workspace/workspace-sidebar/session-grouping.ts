...
import { sessionDisplayStatus, sessionSource } from '@/components/workspaces/session-label';

import { getSessionDisplayTitle, sessionLastActivityAt } from './workspace-session-list-helpers';
...
function statusBucketFor(session: WorkspaceSession, reviewCount: number): SessionSectionId {
...
): (a: WorkspaceSession, b: WorkspaceSession) => number {
  if (order === 'created') {
...
export function groupSessions(
  ...
  const buckets = new Map<string, WorkspaceSession[]>(declared.map((section) => [section.id, []]));
