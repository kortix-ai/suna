...
describe('workspace sidebar header', () => {
  test('the workspace switcher leads the row', () => {
    expect(header).toContain('<WorkspaceSwitcher workspaceId={workspaceId} />');
  });
  ...
  // the workspace name and search that looked clickable and was not.
