    ...
    <DropdownMenuContent align={align} side="right" className="w-56 p-1">
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent className="w-40 p-1">
          <DropdownMenuRadioGroup
            value={groupMode}
            onValueChange={(value) => setGroupMode(workspaceId, value as SessionGroupMode)}
          >
      ...
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent className="w-40 p-1">
          <DropdownMenuRadioGroup
            value={orderMode}
            onValueChange={(value) => setOrderMode(workspaceId, value as SessionOrderMode)}
          >
          ...
          <DropdownMenuSubContent className="w-48 p-1">
            {showOptions.map((option) => (
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={() => toggleSectionHidden(workspaceId, option.id)}
                onSelect={(event) => event.preventDefault()}
              >
                {option.label}
              </DropdownMenuCheckboxItem>
              ...
              <DropdownMenuSubContent className="w-48 p-1">
                {statusOptions.map((option) => (
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => toggleStatusFilter(workspaceId, option.value)}
                    onSelect={(event) => event.preventDefault()}
                  >
                ...
                {sourceOptions.map((option) => {
                  ...
                  return (
                    <DropdownMenuCheckboxItem
                      ...
                      onCheckedChange={() => toggleSourceFilter(workspaceId, option.value)}
                      onSelect={(event) => event.preventDefault()}
                    >
