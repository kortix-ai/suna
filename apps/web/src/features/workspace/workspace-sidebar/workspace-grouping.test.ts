...
const workspace = (workspaceId: string, accountId: string, name: string, lastOpenedAt: string | null) =>
  ({
...
describe('groupWorkspacesByAccount', () => {
  ...
  test('sorts workspaces inside a group by last_opened_at, most recent first', () => {
    ...
    expect(groups[0].workspaces.map((w) => w.workspace_id)).toEqual(['new', 'old', 'never']);
  });
