...
describe('prefersPreviewLink', () => {
  test('uses a link-only preview for document urls', () => {
    expect(prefersPreviewLink('https://api.dosco.live/v1/p/sandbox/3210/deck.pdf')).toBe(true);
    expect(prefersPreviewLink('https://api.dosco.live/v1/p/sandbox/3210/deck.pptx?download=1')).toBe(true);
  ...
  test('keeps regular web previews in iframes', () => {
    expect(prefersPreviewLink('https://api.dosco.live/v1/p/sandbox/3000/')).toBe(false);
    expect(prefersPreviewLink('https://api.dosco.live/v1/p/sandbox/3000/index.html')).toBe(false);
