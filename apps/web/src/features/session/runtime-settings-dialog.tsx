...
import { modelKeyToWire, setGlobalDefaultModel } from '@/hooks/runtime/use-model-store';
import { clearModelDefault, setModelDefault } from '@kortix/sdk/projects-client';
import { useParams } from 'next/navigation';
import type { Config } from '@/hooks/runtime/use-runtime-config';
import { useRuntimeConfig, useUpdateRuntimeConfig } from '@/hooks/runtime/use-runtime-config';
import type { McpStatus } from '@/hooks/runtime/use-runtime-mcp';
import {
  useAddMcpServer,
  useConnectMcpServer,
  ...
  useMcpAuthStart,
  useRuntimeMcpStatus,
} from '@/hooks/runtime/use-runtime-mcp';
import { useRuntimeProviders, useRuntimeToolIds } from '@/hooks/runtime/use-runtime-sessions';
...
export type RuntimeSettingsTab = 'general' | 'providers' | 'permissions' | 'mcp';

interface RuntimeSettingsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialTab?: RuntimeSettingsTab;
}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
          {tHardcodedUi.raw(
            'componentsSessionRuntimeSettingsDialog.line152JsxTextCustomInstructions',
          )}
        </label>
        <p className="text-muted-foreground/60 text-xs">
          {tHardcodedUi.raw(
            'componentsSessionRuntimeSettingsDialog.line155JsxTextAdditionalInstructionFilePathsOnePerLineE',
          )}
        </p>
        <Textarea
          ...
          placeholder={tHardcodedUi.raw(
            'componentsSessionRuntimeSettingsDialog.line165JsxAttrPlaceholderDocsRulesMd10Cursorrules10AgentsMd',
          )}
          rows={4}
      ...
      <div className="space-y-2">
        <label className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
          {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line174JsxTextDefaultModel')}
        </label>
        <div className="flex items-center gap-2">
          ...
          {selectedModel && (
            ...
            >
              {tHardcodedUi.raw(
                'componentsSessionRuntimeSettingsDialog.line190JsxTextResetToAuto',
              )}
            </Button>
          )}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground/60 mt-0.5 text-xs">
              {tHardcodedUi.raw(
                'componentsSessionRuntimeSettingsDialog.line207JsxTextCreateAGitSnapshotAtEachAgenticStep',
              )}
            </p>
          </div>
    ...
    <div className="space-y-3 overflow-y-auto">
      <div className="flex items-center justify-between">
        <span className="text-muted-foreground/40 text-xs font-medium tracking-wider uppercase">
          {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line242JsxTextConnected')}
...
function PermissionsSection({
  ...
  const tHardcodedUi = useTranslations('hardcodedUi');
  const { data: toolIds } = useRuntimeToolIds();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
          {tHardcodedUi.raw(
            'componentsSessionRuntimeSettingsDialog.line333JsxTextGlobalPermissionMode',
          )}
        </label>
        <p className="text-muted-foreground/60 text-xs">
          {tHardcodedUi.raw(
            'componentsSessionRuntimeSettingsDialog.line336JsxTextSetABlanketPermissionLevelOrConfigurePer',
          )}
        </p>
      ...
      {!isGlobalMode && (
        <div className="space-y-2">
          <label className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line375JsxTextPerToolPermissions',
            )}
          </label>
      ...
      {builtinToolIds.length > 0 && (
        <div className="space-y-2">
          <label className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line411JsxTextToolOverrides',
            )}
          </label>
          <p className="text-muted-foreground/60 text-xs">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line414JsxTextEnableOrDisableIndividualTools',
            )}
          </p>
  ...
  if (s === 'needs_auth' || s === 'needs_client_registration') {
    return (
      <Badge size="sm" variant="warning">
        ...
        {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line467JsxTextNeedsAuth')}
      </Badge>
    );
  }
  // failed
...
function McpServersSection() {
  const tHardcodedUi = useTranslations('hardcodedUi');
  const { data: mcpStatus, isLoading } = useRuntimeMcpStatus();
  const { data: toolIds } = useRuntimeToolIds();
      ...
      <div className="space-y-4 overflow-y-auto pr-1">
        ...
        {authStartMutation.isPending && !authUrl && (
          <div className="flex items-center gap-3 py-4">
            ...
            <span className="text-muted-foreground text-sm">
              {tHardcodedUi.raw(
                'componentsSessionRuntimeSettingsDialog.line660JsxTextStartingAuthorization',
              )}
            </span>
          </div>
        )}
        ...
        {authUrl && (
          <form onSubmit={handleAuthCallback} className="space-y-4">
            <p className="text-muted-foreground text-sm">
              {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line667JsxTextVisitThe')}{' '}
              <a
                href={authUrl}
                target="_blank"
              ...
              >
                {tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line669JsxTextAuthorizationPage',
                )}
              </a>{' '}
              {tHardcodedUi.raw(
                'componentsSessionRuntimeSettingsDialog.line671JsxTextAndAfterItRedirectsToLocalhostPasteThe',
              )}
            </p>
            <div>
              <label className="text-muted-foreground mb-1 block text-xs">
                {tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line674JsxTextLocalhostRedirectUrl',
                )}
              </label>
              <Input
                type="text"
                placeholder={tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line676JsxAttrPlaceholderPasteHttpLocalhostCallback',
                )}
                value={authCode}
        ...
        {authError && !authUrl && (
          <div className="space-y-3">
            ...
            >
              {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line715JsxTextTryAgain')}
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 overflow-y-auto pr-1">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">
            {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line736JsxTextAddMcpServer')}
          </h3>
        </div>
        ...
        <form onSubmit={handleAddServer} className="space-y-4">
          ...
          <div>
            <label className="text-muted-foreground mb-1 block text-xs">
              {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line742JsxTextServerName')}
            </label>
            <Input
              type="text"
          ...
          <div className="space-y-2">
            <label className="text-muted-foreground mb-1 block text-xs">
              {tHardcodedUi.raw(
                'componentsSessionRuntimeSettingsDialog.line753JsxTextTransportType',
              )}
            </label>
            <div className="flex gap-1.5">
              <Button
              ...
              >
                {tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line762JsxTextStdioCommand',
                )}
              </Button>
              <Button
                type="button"
              ...
              >
                {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line771JsxTextHttpUrl')}
              </Button>
            </div>
          </div>
          ...
          {addForm.transportType === 'stdio' ? (
            <div>
              ...
              <Input
                type="text"
                placeholder={tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line781JsxAttrPlaceholderNpxYModelcontextprotocolServerGithub',
                )}
              ...
              <p className="text-muted-foreground/60 mt-1 text-xs">
                {tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line786JsxTextFullCommandWithArgumentsSpaceSeparated',
                )}
              </p>
            </div>
          ) : (
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-muted-foreground text-xs">
                {tHardcodedUi.raw(
                  'componentsSessionRuntimeSettingsDialog.line803JsxTextEnvironmentVariables',
                )}
              </label>
    ...
    <div className="space-y-4 overflow-y-auto pr-1">
      <div className="flex items-center justify-between">
        <span className="text-muted-foreground text-xs font-medium tracking-wider uppercase">
          {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line874JsxTextServers')}
          {servers.length})
        </span>
        <Button
          variant="outline"
        ...
        >
          <Plus className="h-3 w-3" />
          {tHardcodedUi.raw('componentsSessionRuntimeSettingsDialog.line883JsxTextAddServer')}
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line1007JsxTextNoMcpServersConfigured',
            )}
          </p>
          <p className="text-muted-foreground/60 mt-1 text-xs">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line1009JsxTextAddAnMcpServerToExtendAvailableTools',
            )}
          </p>
        </div>
      )}
...
export function RuntimeSettingsDialog({
  ...
  const updateMutation = useUpdateRuntimeConfig();
  const params = useParams();
  const settingsProjectId = typeof params?.id === 'string' ? params.id : null;
  const [draft, setDraft] = useState<Record<string, unknown>>({});
  const [activeTab, setActiveTab] = useState<RuntimeSettingsTab>(initialTab);
  ...
  const handleSave = useCallback(() => {
    ...
    updateMutation.mutate(draft as Partial<Config>, {
      onSuccess: () => {
        ...
        // chain in use-runtime-local.ts picks it up immediately.
        if ('model' in draft) {
  ...
  const handleTabChange = useCallback((value: string) => {
    setActiveTab(value as RuntimeSettingsTab);
  }, []);
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        ...
        aria-describedby="runtime-settings-desc"
      >
        <DialogHeader className="flex-shrink-0 px-6 pt-5 pb-0">
          ...
          <DialogDescription id="runtime-settings-desc" className="sr-only">
            {tHardcodedUi.raw(
              'componentsSessionRuntimeSettingsDialog.line1097JsxTextConfigureOpencodeSettingsIncludingGeneralOptionsProvidersAnd',
            )}
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex-shrink-0 px-6 pt-3">
              <TabsList className="w-full">
                ...
                <TabsTrigger value="mcp" className="flex-1 gap-1.5">
                  ...
                  {tHardcodedUi.raw(
                    'componentsSessionRuntimeSettingsDialog.line1124JsxTextMcpServers',
                  )}
                </TabsTrigger>
              </TabsList>
            </div>
            ...
            {hasDraftChanges && (
              <div className="border-border/40 bg-background flex flex-shrink-0 items-center justify-end gap-2 border-t px-6 py-3">
                ...
                >
                  ...
                  {tHardcodedUi.raw(
                    'componentsSessionRuntimeSettingsDialog.line1178JsxTextSaveChanges',
                  )}
                </Button>
              </div>
            )}
