    ...
    <div className="bg-background flex h-full min-h-0 flex-col" data-testid="acp-session-chat">
      ...
      <div className="w-full">
        ...
        <ComposerChatInput
          ...
          inputSlot={
            // The permission prompt now lives inline in the transcript (see the
            // `AcpSessionPermissionPrompt` mount above), so the composer footer
            // only carries the active question form. Same rule as the permission
            // card above: an answer to a pending question travels the same dead
            // channel, so a terminal error retires the prompt rather than leaving
            // an un-submittable form pinned over the composer.
