...
export function FileThumbnail({ filePath, fileName, className }: FileThumbnailProps) {
  ...
  return (
    <div className={cn('flex items-center justify-center relative overflow-hidden bg-muted/10', className)}>
      ...
      {thumbnailType === 'fallback' && getFileIcon(fileName, { className: 'h-10 w-10 text-muted-foreground/60', variant: 'monochrome' })}

      {/* File type badge (shown for non-image types) */}
      {ext && thumbnailType !== 'image' && (
        <span className="absolute bottom-2 right-2 text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-wider bg-background/80 backdrop-blur-sm border border-border/40 px-1.5 py-0.5 rounded-md z-10">
          {ext}
        </span>
      )}
    </div>
