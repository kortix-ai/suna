...
export function FileContentRenderer({
  ...
  // Download handler — project-files API requires (projectId, ref, path).
  const handleDownload = useCallback(async () => {
    if (!fileName || !projectCtx) return;
    try {
      await downloadFile(projectCtx.projectId, projectCtx.ref, filePath, fileName);
    } catch {
      toast.error(`Failed to download ${fileName}`);
    }
  }, [filePath, fileName, projectCtx]);
