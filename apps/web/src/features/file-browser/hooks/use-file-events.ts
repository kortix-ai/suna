 ...
 * old harness event endpoint for file change events. That duplicate connection
 * caused `ERR_INCOMPLETE_CHUNKED_ENCODING` errors and interfered with the
 * canonical ACP session stream.
 *
...
export function useFileEventInvalidation() {
  // No-op — file events are handled by the ACP session stream.
}
