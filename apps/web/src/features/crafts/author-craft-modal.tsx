...
/** Three concrete examples, because "describe a craft" is not a prompt anyone
...
/**
 * "Grow your crafts" — describe a craft and have one built.
 *
 ...
 * to delegate.
 *
 * Submitting starts a real session and navigates to it. It never claims a craft
 * was created — nothing exists when the call resolves except the session.
 */
export function AuthorCraftModal({
  projectId,
  open,
  onOpenChange,
}: {
  ...
  const { author } = useProjectCrafts(projectId);
  ...
  return (
    ...
    >
      <ModalContent className="lg:max-w-lg" aria-label="Grow a craft">
        <ModalHeader>
          <ModalTitle>Grow a craft</ModalTitle>
          <ModalDescription>
            Say what the job is. Kortix creates the repository, writes the agent and its triggers,
            validates it, and publishes it to your crafts.
          </ModalDescription>
        </ModalHeader>
        <form onSubmit={submit}>
          <ModalBody className="space-y-4">
            <Field>
              <FieldLabel htmlFor="craft-description">What should it do?</FieldLabel>
              <Textarea
