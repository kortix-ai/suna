...
/**
 * A craft's tile, DERIVED from its slug rather than stored.
 *
 * The mock carried `icon` / `color` / `bgColor` on every craft. Two reasons
 * they are gone:
 *
 *  1. The API cannot supply them. A craft is a GitHub repo or an uploaded zip;
 *     nothing in a `kortix.yaml` names a Phosphor component, and inventing a
 *     field for one would make every craft author pick a React import.
 ...
 *     icons here, and only here, is what lets `crafts-catalog.ts` be read from
 *     anywhere.
 *
 * Deterministic by slug, so one craft looks the same on the store card, the
 ...
 * chart.
 */
const VISUALS: readonly CraftVisual[] = [
...
/**
 * Slugs whose meaning is obvious enough to earn a specific icon. A craft named
...
/** The tile for one craft. Stable for a given slug, forever. */
export function craftVisual(slug: string): CraftVisual {
  ...
  // crafts that both match `/report/` still read as different cards.
