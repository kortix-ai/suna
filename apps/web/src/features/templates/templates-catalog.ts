...
/**
 * The template view model — a thin adapter over the SDK's `Template`.
 *
 * It holds no data: the store reads `useTemplateCatalog()`, and everything
...
/** `owner/repo`. */
export const templateRepoSlug = (template: Template): string => template.repo;

/** Link-out target for the provenance row. */
export const templateRepoUrl = (template: Template): string =>
 ...
 */
export function templateConnectorRows(template: Template): TemplateConnectorRow[] {
  return (template.connectors ?? []).map((connector: TemplateConnector) => {
