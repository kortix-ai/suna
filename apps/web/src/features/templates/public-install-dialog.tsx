...
/**
 * Install-from-the-public-templates dialog — `/templates/<slug>` for a
 * SIGNED-IN visitor.
 *
 ...
 * mounted inside one project's `/templates` tab and already knows
 ...
 * (`createTemplateInstallSession`) the in-project modal makes, so the two
...
export function PublicTemplateInstallDialog({
  ...
  async function install() {
    ...
    try {
      ...
      const { session_id } = await createTemplateInstallSession(projectId, template.slug);
