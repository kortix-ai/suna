...
import { renameOnError, renameOnMutate, renameOnSettled } from './workspace-rename-cache';
...
describe('renameOnMutate', () => {
  test('writes the optimistic name and returns a restorable snapshot', () => {
    const qc = client();
    qc.setQueryData(qk.workspaces.list('acct_1'), [{ workspace_id: ID, name: 'Old' }]);
    qc.setQueryData(qk.workspace.detail(ID), { workspace: { workspace_id: ID, name: 'Old' } });

    const context = renameOnMutate(qc, ID, 'New');

    expect(context).toBeDefined();
    expect(qc.getQueryData(qk.workspaces.list('acct_1'))).toEqual([
      { workspace_id: ID, name: 'New' },
    ]);
    expect(qc.getQueryData(qk.workspace.detail(ID))).toEqual({
      workspace: { workspace_id: ID, name: 'New' },
    });
  });
  ...
  test('is a no-op and returns undefined for an icon-only edit (no name in the patch)', () => {
    ...
    expect(qc.getQueryData(qk.workspace.detail(ID))).toEqual({
      workspace: { workspace_id: ID, name: 'Old' },
    });
  });

  test('is a no-op and returns undefined without a workspaceId', () => {
    const qc = client();
...
describe('renameOnError — the Critical rollback path', () => {
  ...
  test('restores the exact prior name everywhere it was cached', () => {
    const qc = client();
    qc.setQueryData(qk.workspaces.list('acct_1'), [
      { workspace_id: ID, name: 'Original' },
      { workspace_id: 'other', name: 'Untouched' },
    ]);
    qc.setQueryData(qk.workspace.detail(ID), { workspace: { workspace_id: ID, name: 'Original' } });
    ...
    expect(qc.getQueryData(qk.workspace.detail(ID))).toEqual({
      workspace: { workspace_id: ID, name: 'Original' },
    });
  });
...
describe('renameOnSettled', () => {
  test('invalidates every account-scoped list and the detail entry', async () => {
    const qc = client();
    qc.setQueryData(qk.workspaces.list('acct_1'), [{ workspace_id: ID, name: 'New' }]);
    qc.setQueryData(qk.workspace.detail(ID), { workspace: { workspace_id: ID, name: 'New' } });

    await renameOnSettled(qc, ID);

    expect(qc.getQueryState(qk.workspaces.list('acct_1'))?.isInvalidated).toBe(true);
    expect(qc.getQueryState(qk.workspace.detail(ID))?.isInvalidated).toBe(true);
  });

  test('is a no-op without a workspaceId', () => {
    const qc = client();
...
// cache entries must not move when this workspace renames. Moved here from
...
describe('blast radius', () => {
  test('leaves an unrelated project untouched', async () => {
    ...
    const OTHER = 'proj_2';
    qc.setQueryData(qk.workspaces.list('acct_1'), [
      { workspace_id: ID, name: 'Old' },
      { workspace_id: OTHER, name: 'Keep' },
    ]);
    qc.setQueryData(qk.workspace.detail(OTHER), { workspace: { workspace_id: OTHER, name: 'Keep' } });

    renameOnMutate(qc, ID, 'New');
    await renameOnSettled(qc, ID);

    const list = qc.getQueryData(qk.workspaces.list('acct_1')) as Array<{
      workspace_id: string;
      name: string;
    }>;
    expect(list.find((p) => p.workspace_id === OTHER)?.name).toBe('Keep');
    expect(qc.getQueryState(qk.workspace.detail(OTHER))?.isInvalidated).toBe(false);
  });
});
...
describe('the full success and failure round trips', () => {
  ...
  test('failure: rollback then settle leaves the ORIGINAL name, invalidated', async () => {
    const qc = client();
    qc.setQueryData(qk.workspaces.list('acct_1'), [{ workspace_id: ID, name: 'Original' }]);
    qc.setQueryData(qk.workspace.detail(ID), { workspace: { workspace_id: ID, name: 'Original' } });
