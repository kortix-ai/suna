...
export function resolveCreateFailure(
  ...
  // It means the workspace has no such connector at all, so the gate would ask
  // someone to connect an account to something that does not exist; the toast is
  // the honest outcome until a workspace owner adds it.
...
function isConnectorGateConnection(value: unknown): value is ConnectorGateConnection {
  ...
  return (
    ...
    value.name.length > 0 &&
    (value.authorization_strategy === 'workspace' || value.authorization_strategy === 'user')
  );
}
