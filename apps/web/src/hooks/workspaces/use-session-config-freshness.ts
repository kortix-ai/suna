...
export function sessionConfigKey(workspaceId?: string, sessionId?: string) {
  return ['session-config', workspaceId ?? '', sessionId ?? ''] as const;
}
...
export function reloadNotAppliedCopy(reason?: string): string {
  switch (reason) {
    ...
    case 'no compiled agent config':
      return 'This workspace has no compiled agent config to load.';
...
export function useSessionConfigFreshness(workspaceId?: string, sessionId?: string) {
  const query = useQuery({
    queryKey: sessionConfigKey(workspaceId, sessionId),
    queryFn: () => getWorkspaceSessionConfigState(workspaceId as string, sessionId as string),
    enabled: !!workspaceId && !!sessionId,
    // NOT polled, on purpose. Each call drops the workspace's git-mirror TTL,
...
export function useReloadSessionConfig(workspaceId: string, sessionId: string) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result: SessionReloadResult) => {
      ...
      setBusyReason(null);
      queryClient.invalidateQueries({ queryKey: sessionConfigKey(workspaceId, sessionId) });
      if (!result.applied) {
      ...
      queryClient.invalidateQueries({
        queryKey: qk.workspace.sessionSandbox(workspaceId, sessionId),
      });
