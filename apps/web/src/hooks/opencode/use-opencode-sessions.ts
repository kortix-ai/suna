...
export function clearProjectProviderCache(projectId: string): void {
  providersCache.remove(`proj:${projectId}`);
}
...
export function useOpenCodeProviders() {
  ...
  // the persisted placeholder is scoped per project — not a global scope.
  const cacheScope = projectId ? `proj:${projectId}` : CACHE_SCOPE_GLOBAL;
  return useQuery<ProviderListResponse>({
    queryKey: projectId ? ['project-providers', projectId] : opencodeKeys.providers(),
    queryFn: async () => {
      // opencode-native is the only LLM path now: /provider/list returns the
      // always-on managed `kortix` provider PLUS every BYOK provider whose key
      // has been injected (P2). No gateway/native fork, no `/llm-catalog` hop.
      const client = getClient();
      const result = await client.provider.list();
      let providers = normalizeProviderList(unwrap(result));
      if (projectId) {
        // Merge project-secret-backed providers into `connected` so a freshly
        // connected BYOK provider lights up the instant its secret lands —
        // before opencode finishes reloading and reports it natively (P3). The
        // managed `kortix` provider is intentionally NOT stripped here; the
        // picker reconciles it down to the managed catalog (+ ChatGPT).
        ...
        providers = mergeProjectSecretConnectedProviders(providers, secretNames, LLM_PROVIDERS);
      }
      ...
      // AND persisted to the localStorage cache below — poisoning the first
      // frame of every future session too. That is the "model picker never
      // shows up" bug. Treat a model-less response as a transient boot state:
      // throw so React Query retries it (with backoff), never cache or persist.
