...
import { isLegacyMigratedSession, sessionDisplayLabel } from '@/components/workspaces/session-label';
...
import { useRestartWorkspaceSession } from '@/hooks/workspaces/use-restart-workspace-session';
import {
  billingDialogArgs,
  billingGateCopy,
 ...
 * so clicking a session with hours of history painted the empty workspace-home
 ...
 */
function WorkspaceSessionView({ workspaceId, sessionId }: { workspaceId: string; sessionId: string }) {
  ...
  const currentWorkspaceSession = workspaceSessions?.find((item) => item.session_id === sessionId);
  const pendingPrompt = pendingSessionPromptFromMetadata(currentWorkspaceSession?.metadata);
  ...
  const restart = useRestartWorkspaceSession(workspaceId, sessionId);
  ...
  }, [billingBlocked, billingState, accountState, openUpgradeDialog, workspaceAccountId]);
  ...
  const inner = (() => {
    ...
    if (dormantWithoutRuntime) {
      ...
      if (currentWorkspaceSession && isLegacyMigratedSession(currentWorkspaceSession)) {
        return (
          <InlineSessionError
            title="Legacy session"
            message="This conversation was imported from Suna. Restore the session to load its chat history — its files are already in the workspace under legacy/."
            detail={restart.errorMessage ?? undefined}
            action={
  ...
  return (
    <>
      ...
      <SessionDeleteModal
        ...
        sessionId={sessionId}
        sessionLabel={
          currentWorkspaceSession ? sessionDisplayLabel(currentWorkspaceSession) : 'Failed session'
        }
        open={deleteOpen}
...
function ActiveSessionChat({
  ...
  // producer (workspace-home composer, `useConfigureThread`, the instant shell)
  ...
  useEffect(() => {
    ...
    const sb = queryClient.getQueryData<{ metadata?: Record<string, unknown> }>(
      qk.workspace.sessionSandbox(workspaceId, sessionId),
    );
