...
/**
 * Shared shell for /workspaces/[id]/{agent,connectors,skills}. The `(capabilities)`
 ...
 * `min-h-svh`, and `SidebarInset` plus the three workspace shells are all
 ...
 * child of `WorkspaceShellLayout`, whose own ancestors add no vertical offset —
...
export default function CapabilitiesLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-svh flex-col overflow-hidden">
      <CapabilityTabs workspaceId={workspaceId} />
      {children}
    </div>
  );
}
