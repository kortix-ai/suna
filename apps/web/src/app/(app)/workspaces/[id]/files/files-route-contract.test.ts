...
const SKELETON = resolve(
  WEB_ROOT,
  'src/features/workspace/workspace-layout/workspace-files-skeleton.tsx',
);

/** Modules too heavy to sit in the loading boundary's payload. */
const HEAVY = ['@/features/workspace-files', '@/features/file-viewer'];
 ...
 * `not.toContain('@/features/workspace-files')` cannot tell an import from a
...
describe('files route loading boundary', () => {
  test('the route has a loading.tsx navigation boundary', () => {
    // Without it the click freezes the previous page, AND — because the workspace
  ...
  test('the loading boundary imports no heavy module', () => {
    ...
    expect(source).toContain('WorkspaceFilesSkeleton');
  });
  ...
  test('the heavy-import detector actually detects', () => {
    ...
    expect(heavyImports("import { X } from '@/features/workspace-files';")).toEqual([
    ...
    expect(heavyImports("import '@/features/workspace-files/some-styles.css';")).toEqual([
      '@/features/workspace-files/some-styles.css',
    ]);
    expect(heavyImports('// mentions @/features/workspace-files in prose only')).toEqual([]);
  });
});
