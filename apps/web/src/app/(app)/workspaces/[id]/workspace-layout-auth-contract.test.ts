...
const LAYOUT = resolve(WEB_ROOT, 'src/app/(app)/workspaces/[id]/layout.tsx');
const MIDDLEWARE = resolve(WEB_ROOT, 'src/middleware.ts');

/**
 * The workspace layout deliberately does NOT verify the session: middleware
 ...
 * either fails this suite loudly instead of silently rendering the workspace
...
describe('workspace layout auth contract', () => {
  ...
  test('the workspace layout does not create a Supabase server client', () => {
