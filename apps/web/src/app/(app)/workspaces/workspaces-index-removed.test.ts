...
describe('/projects index', () => {
  test('is a redirect, not a list', () => {
    ...
    expect(page).not.toContain('WorkspaceCard');
    expect(page).not.toContain('WorkspacesViewMode');
    expect(page).not.toContain('archiveWorkspace');
  });

  test('redirects to the landing door, which resolves a real workspace', () => {
    expect(page).toContain('WORKSPACE_LANDING_PATH');
  });
