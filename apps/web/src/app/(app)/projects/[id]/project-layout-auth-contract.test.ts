...
const PROXY = resolve(WEB_ROOT, 'src/proxy.ts');

/**
 * The project layout deliberately does NOT verify the session: the proxy
 * (Next 16: previously named `middleware.ts`) already did, and doing it
 * again cost a second GoTrue round-trip on every project switch and hard
 * load.
 *
 * That is only safe while the proxy default-denies `/projects`. The proxy
 * has TWO route lists that skip its auth gate — `PUBLIC_ROUTES` and the
 * adjacent `STATIC_PUBLIC_ROUTES` — so both are pinned here. Adding
 * `/projects` to either fails this suite loudly instead of silently
 * rendering the project shell to a signed-out visitor.
 */
describe('project layout auth contract', () => {
  test('proxy does not treat /projects as a public route', () => {
  ...
  test('proxy does not treat /projects as a static public route', () => {
    const source = readFileSync(PROXY, 'utf8');
    const staticPublicRoutesStart = source.indexOf('const STATIC_PUBLIC_ROUTES');
    ...
    // list sitting directly below PUBLIC_ROUTES (proxy.ts ~102-105):
  ...
  test('proxy still redirects unauthenticated non-public traffic to /auth', () => {
    const source = readFileSync(PROXY, 'utf8');
