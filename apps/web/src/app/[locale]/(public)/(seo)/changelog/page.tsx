...
async function getReleases(): Promise<GitHubRelease[]> {
  ...
  // On Vercel the page is ISR (static per locale, regenerated hourly). A failed
  // GitHub read during a background regeneration throws, so Next keeps serving
  // the last good page instead of caching the empty state for an hour. The
  // build (no page to fall back to) and per-request rendering (standalone
  // builds) render the "couldn't load" state instead.
  const keepLastGoodPage =
    runtimeConfigIsBakedAtBuild() && process.env.NEXT_PHASE !== 'phase-production-build';
  ...
  try {
    ...
    if (!res.ok) throw new Error(`GitHub releases returned HTTP ${res.status}`);
    data = ((await res.json()) as GitHubRelease[]) ?? [];
  } catch (error) {
    if (keepLastGoodPage) throw error;
    return [];
  }
  return data
