...
export default function TeamsLoginPage() {
  ...
  useEffect(() => {
    if (isLoading) return;
    if (!user) {
      const target = `/teams/login/${token}`;
    ...
    return (
      <Centered>
        <ResultCard
          ok={false}
          title="Missing link"
          message="This page is opened from a Kortix message in Teams. Start the login from Teams to get a fresh link."
        />
      </Centered>
    );
  }
  ...
  async function connect() {
    ...
    setError(null);
    try {
      const res = await teamsIdentityApi.bind(token);
    ...
    return (
      <Centered>
        <ResultCard
          ...
          message={
            !hasAccess
              ? `Your Kortix account is connected${workspaceName ? ` in ${workspaceName}` : ''}. Head back to Teams and request project access to continue.`
              : resumed
                ? `Your Kortix account is connected${workspaceName ? ` in ${workspaceName}` : ''}. Kortix is picking up your Teams message now.`
                : `Your Kortix account is connected${workspaceName ? ` in ${workspaceName}` : ''}. Head back to Teams and mention Kortix with a task.`
          }
        />
      </Centered>
    );
    ...
    <Centered>
      <div className="bg-background rounded-md border shadow-sm">
        <div className="p-7">
          <div className="mb-6 flex items-center gap-3">
            <div className="bg-muted/40 grid size-11 place-items-center rounded-sm border">
              <Icon.MicrosoftTeams className="size-5" />
            </div>
            <div>
              <div className="text-base font-semibold">Connect Teams to Kortix</div>
              <div className="text-muted-foreground text-xs">kortix.com · Microsoft Teams</div>
            </div>
          </div>

          <p className="text-muted-foreground text-sm">
            Connect or create a Kortix account so the Teams bot can run as <strong className="text-foreground">you</strong> —
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            ...
            <Button onClick={connect} disabled={busy} size="lg">
              {busy ? (
                ...
                <>
                  <Icon.MicrosoftTeams className="size-4" /> Connect account
                </>
              )}
            </Button>
          </div>
        ...
        <div className="bg-muted/30 text-muted-foreground border-t px-7 py-3 text-xs">
          You can disconnect anytime with the <strong className="text-foreground">logout</strong> command in Teams.
        </div>
      </div>
    </Centered>
  );
