    ...
    <div className="grid grid-cols-1 gap-3 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="border-border bg-card overflow-hidden rounded-lg border">
        ...
        <div className="overflow-x-auto p-2 text-sm">
          <CodeBlockCode code={KORTIX_TOML} language="toml" />
        </div>
      </div>
    </div>
  );
      ...
      <section className="mx-auto flex max-w-6xl flex-col gap-10 px-6 py-16 sm:gap-12 sm:py-24 lg:px-0">
        <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <Reveal delay={0.1} className="lg:order-1">
            <div className="border-border bg-card space-y-2.5 rounded-lg border p-5">
              <div className="border-border bg-background flex items-center gap-2.5 rounded-md border p-3">
                <Boxes className="text-muted-foreground/70 size-4 shrink-0" />
                <span className="text-foreground text-sm font-medium">sandbox</span>
                <span className="border-border bg-muted/40 text-muted-foreground ml-auto flex items-center gap-1.5 rounded px-2 py-1 font-mono text-[11px]">
                  <KeyRound className="size-3" /> KORTIX_TOKEN
                </span>
              </div>
              <div className="text-muted-foreground flex items-center justify-center gap-2 text-[11px]">
                <ArrowRight className="size-3.5 rotate-90" /> only a scoped token leaves the box
              </div>
              <div className="border-border bg-background flex items-center gap-2.5 rounded-md border p-3">
                <Server className="text-foreground size-4 shrink-0" />
                <span className="text-foreground text-sm font-medium">Kortix Executor</span>
                <span className="text-muted-foreground ml-auto text-xs">connect proxy · gateway</span>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  >
                    <img src={fav(d)} alt={n} width={16} height={16} loading="lazy" className="size-4" />
                    <span className="text-foreground truncate text-xs font-medium">{n}</span>
                  </div>
                ))}
              </div>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {['App', 'MCP', 'OpenAPI', 'GraphQL', 'HTTP'].map((t) => (
                  <Badge key={t} variant={t === 'App' ? 'highlight' : 'outline'} size="sm">
                    {t}
                  </Badge>
                ))}
              </div>
