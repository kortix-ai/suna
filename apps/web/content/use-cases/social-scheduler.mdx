...
- **No social connectors, period.** The agent has no credential and no
  ...
  scope.
- **Draft only.** The only output that leaves the sandbox is a Slack message
  containing drafts. Nothing is scheduled, queued, or posted anywhere.
- **Isolation.** Every run happens in its own microVM sandbox, torn down when
  the run ends.
- **Scoped secrets.** The Notion connector is brokered server-side; no raw
  token is ever shown to the model or written to logs.
