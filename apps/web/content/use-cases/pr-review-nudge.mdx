...
are encrypted in the Secrets Manager and injected at runtime, never shown to the
...
- **Isolation.** Every run happens in its own microVM sandbox. The session can
  reach only GitHub (read-only) and Slack; nothing else leaves the sandbox.
- **Scoped secret.** The `GH_TOKEN` is encrypted in the Secrets Manager and
  injected into the sandbox at runtime, never exposed to the model or the logs.
