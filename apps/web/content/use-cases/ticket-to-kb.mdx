...
- **Resolved tickets from Plain** — read-only, via a raw API key injected at
  runtime, never shown to the model.
...
- **Isolation.** Every run resumes in its own microVM sandbox. The session can
  reach only Plain and the KB repo, and only the pull request leaves the
  sandbox.
