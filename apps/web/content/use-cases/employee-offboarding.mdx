...
- **Isolation.** Every departure case runs in its own microVM sandbox. The
  session can reach only the systems it's scoped to, and only the checklist
  result leaves the sandbox.
- **Scoped secrets.** The Okta, Google Workspace, Google Drive, and GitHub
  ...
  sandbox at runtime, never exposed to the model or the logs.
...
<Figure
  caption="An approval gate holding the ownership transfer before it runs"
  aspect="16/9"
/>
