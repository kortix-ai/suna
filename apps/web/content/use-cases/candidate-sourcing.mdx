...
Greenhouse themselves, if they choose to. Nothing leaves the sandbox except
the drafts.
...
- **Isolation.** Every run happens in its own microVM sandbox, scoped to one
  open role. Only the drafted emails leave the sandbox.
- **Scoped secrets.** The Greenhouse and LinkedIn credentials are encrypted in
  the Secrets Manager and injected into the sandbox at runtime, never exposed
