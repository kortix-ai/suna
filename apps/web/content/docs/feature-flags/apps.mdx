...
receives it. The App's own `Authorization` header is forwarded unchanged.

When the project's `agent_principal` feature flag is on, the gate judges a
governed agent session as the agent, not as the person who started it. List the
...
agents:
  finance-report:
    ...
    apps: [finance-dashboards]   # App slugs, `all`, or `none` (the default)
```
...
`401 app_auth_required`. With the flag off, the gate uses the member and group
rules above for the person behind the session.
