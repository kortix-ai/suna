...
| `meta_agent` | Kortix Agent | Stable | On | `behavioral` |
...
- **`meta_agent`** — the platform-owned Kortix Agent. It is the composer's
  default: you talk to it, it picks the right project agent, starts sessions,
  and reports verified results. Turn it off to pick project agents directly.
  API, CLI, trigger, and channel sessions that name no agent keep running the
  project default.
