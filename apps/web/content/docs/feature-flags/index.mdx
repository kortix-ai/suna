...
<Cards>
  ...
  <Card icon={<PackageIcon />} title="Crafts" href="/docs/feature-flags/crafts">Install a Kortix project into another project, activate its triggers, and read its runs.</Card>
</Cards>
...
| `crafts` | Crafts | Experimental | Off | `routes` |
...
- **`review_center`** — one inbox for change requests, approvals, and agent
  output.
- **`crafts`** — install a craft (a GitHub repo whose `kortix.yaml` declares
  agents, skills, connectors and triggers) into a project, and read the run
  history of the triggers it owns. See [Crafts](/docs/feature-flags/crafts).
