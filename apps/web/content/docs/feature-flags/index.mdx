...
| `config_releases` | `CONFIG_RELEASES_ENABLED` is on (default: on) |

`CONFIG_RELEASES_ENABLED=false` is a kill switch: it turns config releases off
for every project at once, whatever each project chose, and the row disappears
from Settings. Every session then reads its workspace config directory at its
next boot or start.
