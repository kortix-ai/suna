...
Four flags read it from configuration; every other flag is always available:
...
| `config_releases` | `CONFIG_RELEASES_ENABLED` is on (default: off) |

`CONFIG_RELEASES_ENABLED` defaults to `false`. It is an operator switch, off
until an environment's prerequisites are met
(`docs/runbooks/config-releases-rollout.md`). While it is off, the flag is
unavailable platform-wide: the Settings row disappears, whatever a project
chose. Once an operator sets it to `true`, the flag becomes *available* — each
project still defaults to `enabled: false` (`platformDefault: () => false`),
so turning the switch on changes no session's behavior by itself. Setting it
back to `false` is a kill switch: it turns config releases off for every
project at once, whatever each project chose, and the row disappears from
Settings. Every session then reads its workspace config directory at its next
boot or start.
...
- **`pi_harness`** — run this project's sessions on the pi agent harness
  ...
  agent in `.kortix/opencode/agents/` with `mode: subagent`, and the transcript
  shows each subagent's steps in place. pi sessions also load pi.dev packages:
  see [`harnesses:`](/docs/project/manifest#harnesses).
