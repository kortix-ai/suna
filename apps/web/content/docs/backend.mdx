...
Kortix owner, one workspace, and one cost record.
...
that service account the required workspace actions before use.
...
| `secrets`            | Narrows the selected agent's workspace-secret grant. An empty list delivers no workspace secrets. Only backend-origin callers can set it.                   |
...
A connector defines the tool surface. It contains a workspace-unique
...
The canonical API and SDK authorization strategy has two values:

- `workspace` accepts active workspace connections.
- `user` accepts only an active connection owned by the acting workspace
  member.

A service account has no member identity, so it cannot use a member's `user`
connection. Use `workspace` connectors for service-account sessions.
...
connectors:
  - slug: gmail-read
    ...
    provider: pipedream
    app: gmail
    # The manifest keeps `project` as a stable compatibility value.
...
The manifest schema continues to use `authorization_strategy: project | user`.
The canonical Workspace API and SDK translate `workspace` to the persisted
`project` value. Existing manifests do not require a migration.

The SDK exposes connections under `workspace.connectors.connections`:

```ts
const connection = await kortix.workspace(workspaceId).connectors.connections.reconcile({
...
await kortix.workspace(workspaceId).connectors.connections.activate(connection.connection_id);
```
...
```bash
curl -sS \
  "$KORTIX_API_URL/workspaces/$KORTIX_WORKSPACE_ID/sessions/$SESSION_ID/scope" \
  -H "Authorization: Bearer $KORTIX_API_KEY"
```

```ts
const scope = await kortix.session(workspaceId, sessionId).scope();
```

Replace scope with `PUT` or `rescope()`:

```ts
const nextScope = await kortix.session(workspaceId, sessionId).rescope({
  secrets: ['STRIPE_KEY'],
  connector_bindings: {
...
```bash
curl -sS \
  "$KORTIX_API_URL/usage/session-costs?workspace_id=$KORTIX_WORKSPACE_ID&limit=25&offset=0" \
  -H "Authorization: Bearer $KORTIX_API_KEY"

curl -sS \
  "$KORTIX_API_URL/usage/session-costs/$SESSION_ID?workspace_id=$KORTIX_WORKSPACE_ID" \
  -H "Authorization: Bearer $KORTIX_API_KEY"
```

The list returns session, workspace, owner, LLM, compute, total, request, token,
...
const sameDetail = await kortix.session(workspaceId, sessionId).cost();
```
...
```ts
const handle = kortix.session(workspaceId, session.session_id);
...
Use `useSession(workspaceId, sessionId)` for React hosts. It owns startup,
...
- Workspace guardrails apply above connector-connection policies.
...
| `404` create / `403` rescope | `CONNECTOR_CONNECTION_NOT_FOUND`                    | The connection does not exist in this workspace or violates the connector's authorization strategy. |
| `404`                        | `SECRET_IDENTIFIER_NOT_FOUND`                    | The secret allowlist contains an unknown workspace-secret identifier.                |
...
| `409`                        | `CONNECTOR_NOT_PIPEDREAM`                        | The alias is a connector on the workspace but not a Pipedream one, so no Quick Connect link exists for it. |
