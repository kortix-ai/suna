...
[secret](/docs/workspace/secrets). It does not appear on the Workspace Secrets
...
From the dashboard, open the Workspace Channels page and connect Slack there
...
| `projects` | Deprecated alias for `workspaces` |
...
`policy` accepts `project_open`, `owner_approval`, or `owner_only`.
`project_open` is the stable persisted value for the default policy. It lets
any workspace member who mentions the bot start a session.
