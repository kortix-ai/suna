...
The field records **provenance**: which installed subproject contributed this trigger. It is materialized into `project_trigger_runtime.subproject_slug` when the manifest reconciles, so a query can attribute a fire back to its subproject.

It is not a switch. There are exactly **two** switches over a trigger — its own `enabled` field, and the project-wide pause above. A subproject installs with every trigger **off**, and each one is enabled individually, through this page's surface. Nothing enables or disables a whole subproject at once.
