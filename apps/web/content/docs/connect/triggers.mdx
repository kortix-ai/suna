...
| `craft` | no | — | The craft that contributed this trigger. Written by a craft install, never by hand. See [Craft-owned triggers](#craft-owned-triggers). |
...
## Craft-owned triggers

Installing a [craft](/docs/feature-flags/crafts) merges its triggers into this project's `kortix.yaml`. Each one carries `craft: <slug>`, naming the craft it came from. A hand-authored trigger has no `craft:` field.

That field does two things:

- **It groups activation.** Enabling or disabling one craft flips every trigger it owns, in one commit. A craft installs with every trigger **off**, so nothing fires until someone enables it. This is a third switch, distinct from a trigger's own `enabled` field and from the project-wide pause above.
- **It joins the run history.** The craft-run report reads `project_trigger_runtime.craft_slug`, materialized from this field. A trigger without `craft:` never appears in a craft report; its fires are still in the project's own trigger history.

Craft-owned triggers are ordinary triggers otherwise. They obey the same scheduler, the same session strategy, the same limits, and the same project pause. Field reference: [Manifest → `crafts:`](/docs/project/manifest#crafts).
