...
| `subproject` | no | — | The subproject that contributed this trigger. Written by a subproject install, never by hand. See [Subproject-owned triggers](#subproject-owned-triggers). |
...
## Subproject-owned triggers

Installing a [subproject](/docs/feature-flags/subprojects) merges its triggers into this project's `kortix.yaml`. Each one carries `subproject: <slug>`, naming the subproject it came from. A hand-authored trigger has no `subproject:` field.

That field does two things:

- **It groups activation.** Enabling or disabling one subproject flips every trigger it owns, in one commit. A subproject installs with every trigger **off**, so nothing fires until someone enables it. This is a third switch, distinct from a trigger's own `enabled` field and from the project-wide pause above.
- **It joins the run history.** The subproject-run report reads `project_trigger_runtime.subproject_slug`, materialized from this field. A trigger without `subproject:` never appears in a subproject report; its fires are still in the project's own trigger history.

Subproject-owned triggers are ordinary triggers otherwise. They obey the same scheduler, the same session strategy, the same limits, and the same project pause. Field reference: [Manifest → `subprojects:`](/docs/project/manifest#subprojects).
