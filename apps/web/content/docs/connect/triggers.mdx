...
- **webhook** — runs when an external service sends a signed request to the Workspace webhook URL.
...
`kortix ship` commits `kortix.yaml` and pushes it. The schedule goes live after this lands on the Workspace default branch.
</Step>
...
Kortix builds the webhook URL from your Workspace id and the trigger slug:
...
Every trigger-fired session uses Workspace visibility. Any Workspace member can see it. It is not private to the member who configured or fired the trigger.
...
Fires on `POST /v1/webhooks/workspaces/{workspaceId}/{slug}`. Kortix checks the request in this order, with a constant-time comparison. The deprecated `/v1/webhooks/projects/{projectId}/{slug}` alias remains available.
...
| --- | --- | --- |
| `GET /v1/workspaces/{workspaceId}/triggers` | `project.trigger.read` | Lists triggers, runtime state, and manifest parse errors. A bad entry appears in `errors[]`; it does not break the other triggers. |
| `POST /v1/workspaces/{workspaceId}/triggers` | `project.trigger.create` | Creates a trigger. Commits to the manifest directly. |
| `PATCH /v1/workspaces/{workspaceId}/triggers/{slug}` | `project.trigger.update` | Partial update, merged onto the current entry. |
| `DELETE /v1/workspaces/{workspaceId}/triggers/{slug}` | `project.trigger.delete` | Also clears the trigger runtime state. |
| `PATCH /v1/workspaces/{workspaceId}/triggers/activation` | `project.trigger.update` | Body `{ paused: boolean }`. See [Pause and resume](#pause-and-resume). |
| `POST /v1/workspaces/{workspaceId}/triggers/{slug}/fire` | `project.trigger.fire` | Manual fire. Any Workspace member can fire a trigger. This permission does not require the editor or manager role. |
| `POST /v1/webhooks/workspaces/{workspaceId}/{slug}` | signature or token | Public URL, gated by the webhook secret. |

## Pause and resume

A Workspace-level switch stops every trigger in the Workspace at once, independent of each trigger `enabled` field. While paused, the scheduler skips the Workspace and inbound webhooks return `200` with `{ status: "skipped" }`. No session fires. A manual fire still works. Use this when the same repository runs on two control planes, such as dev and production, so cron does not fire twice. CLI: `kortix triggers pause` and `kortix triggers resume`. See [CLI](/docs/cli) for the full `kortix triggers` command group.
...
- Each Workspace allows 3 triggered sessions to provision at once by default. The account plan-tier active-session cap can also apply. A fire past either limit returns `queued` (`202`) instead of failing. It runs after a slot becomes available.
