...
Open the connector in **Customize → Connectors**. The **Accounts** tab lists
every account you can use or manage in one list. Each card shows who can use
the account: **Only you**, **Everyone in project**, or the groups and people it
is shared with.

Select **Add account**, name the account, and choose **Who can use it**:

- **Only you** — a private account. Any member can add one.
- **Everyone in the project** — a shared account.
- **Specific people or groups** — a shared account, narrowed to the people and
  groups you pick.

The two shared choices need `project.connector.connections.manage`. For
**Specific people or groups**, Kortix writes the grants before you authorize
the account, so the account never holds a credential while it is open to the
whole project. An account name must be new: a name that an account of the same
owner already uses is refused.

When an agent needs an account, it posts a **Connect** card in the chat. The
card opens the same dialog, with the name the agent suggests already filled in
and the audience it asked for preselected. Kortix creates a new account, never
re-authorizes an existing one, and tells the agent the new account's name so it
can run calls as that account.

### Choose who can use a shared account

A shared account is usable by everyone in the project until you narrow it.
Select **Share** on the account card. The dialog is the same **Grant access**
dialog used for agents. It lists who can use the account now, and you add:

- **Everyone in the project** — the default.
- **Groups** — every member of each group.
- **People** — one member each. Pick only yourself to keep the account to you.

Nothing changes until you select **Save**. Removing the last group or person
makes the account usable by everyone again; the dialog states the result before
you save.

A narrowed account follows the private-account rules, with its audience in
place of the owner:

- Only a person in the audience can use it, and only in a private session.
- A service account (an agent or a trigger acting unattended) never uses it.
- A session that binds it cannot be shared.

A person outside the audience does not see the account. A person who manages
the project's connections still sees it, marked **Not shared with you**, so
they can change who can use it.

### Share your own private account

Your own **Only you** account has a **Share** button too. Sharing it turns it
into a shared account for the people and groups you pick (you stay on it unless
you remove yourself), or for everyone in the project. From then on it is a
shared account: people who manage the project's connections can manage it too.

- Sharing needs `project.connector.connections.manage`, the same right as
  adding a shared account. Without it the button is disabled and says why.
- Kortix writes the grants first and turns the account shared in one update,
  so it is never open to the whole project in between.
- A shared account of the same name on the connector blocks it (`409`): rename
  one first.

```text
POST /projects/{projectId}/connections/{connectionId}/share
     {"principals": [{"principal_type": "group", "principal_id": "<group-id>"}]}
```

An empty `principals` list shares it with everyone. From the CLI:
`kortix connectors connections share <connection-id> --group <group-id>`
(also `--user <email|id>`, repeatable, or `--everyone`).

Each audience entry is one `connection` object assignment in the grant store:

```text
POST   /accounts/{accountId}/iam/assignments
       {"principal_type": "group", "principal_id": "<group-id>",
        "role_key": "agent-user", "scope_type": "project", "scope_id": "<project-id>",
        "object_type": "connection", "object_id": "<connection-id>"}
DELETE /accounts/{accountId}/iam/assignments/{assignmentId}
```

From the CLI, the same writes are:

```bash
kortix access grant --group <group-id> --connection <connection-id>
kortix access grant --everyone --connection <connection-id>
kortix access revoke <assignment-id>
kortix connectors connections ls   # the WHO CAN USE column
```

- `principal_type` is `user`, `group`, or `project` (everyone in the project;
  `principal_id` is the project id).
- Granting or revoking one needs `project.connector.connections.manage`.
- Only a shared account can be narrowed through these grants. A private account,
  or an id that is not a connection in this project, answers `404`; share your
  own private account with the `share` route above instead.
- `GET /projects/{projectId}/connections` returns each shared account's grants
  in `shared_with`, and `usable: false` on an account the caller sees only to
  manage it. The session picker offers only usable accounts.
...
- **composio** — managed OAuth for supported SaaS apps
- **pipedream** — legacy managed OAuth, kept for rollback only
...
### Apps that need your own OAuth app

Composio holds its own OAuth app for most toolkits. For some it holds none:
X (`twitter`) since February 2026, and OAuth-only toolkits such as `xero`,
`spotify`, `google_chat`, `googleforms`, and `zoom_chat`. On 2026-09-26 there
were 47. Composio cannot connect them until the Composio project holds an
auth config with your own OAuth app.

- Until that auth config exists, the Connectors catalogue does not list the
  toolkit. A connector already declared for it fails to sync with `422`
  `composio_auth_config_required`, and the message names the toolkit.
- To enable one, register an OAuth app with the provider. In the Composio
  dashboard, open **Auth Configs**, create an auth config for the toolkit, and
  enter the app's client ID and secret. Add the redirect URI the dashboard
  shows to the provider app.
- Kortix finds the enabled auth config itself. You do not restart or redeploy.
  The catalogue lists the toolkit within 60 seconds. The next sync and connect
  use the auth config immediately.
- When more than one enabled auth config exists for a toolkit, Kortix uses the
  newest. Disable the old one to rotate an OAuth app.
- Toolkits that Composio connects with its own app keep using it. Kortix never
  selects an auth config for them.

## Authentication and policy

A connection authenticates with:

- OAuth through Composio, a channel install, or a native OAuth2 grant
