...
A connector links a Workspace to an external tool or service. The agent
...
[manifest reference](/docs/workspace/manifest) for every field. Kortix declares
...
- `workspace` for connections available to eligible Workspace members
- `user` for a connection owned by the acting Workspace member
...
override them. Workspace guardrails apply above connector-connection policies.
...
connector's unmatched-action default, including reads. Explicit Workspace
...
verifies that the account can access and approve actions in the Workspace.
...
<Steps>
<Step>
### Open the Workspace Connectors page

Open the Workspace. Select **Connectors**, then select the app.

</Step>
<Step>
### Select the connection scope

Select **Workspace** for a shared Workspace connection. Select **User** for a
connection owned by the acting Workspace member.
...
The manifest keeps `project` as a compatibility value. The canonical API and
SDK expose this strategy as `workspace`.
...
Use `GET /workspaces/{workspaceId}/sessions/{sessionId}/scope` to read the effective
