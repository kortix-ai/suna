...
Goals are authored in the v2 `kortix.yaml`. These commands read that authored
state, request goal pushes, and write metric observations.

| Command | Effect |
| --- | --- |
| `kortix goals ls [--json]` | List goals and manifest parse errors. `list` is an alias. |
| `kortix goals show <slug> [--json]` | Show one goal. |
| `kortix goals push <slug> [--json]` | Request an immediate run for an active goal that has `push`. |
| `kortix goals observe <slug> --metric <name> --value <number> --source <text> [options]` | Record one finite metric value. |
| `kortix goals observations <slug> --metric <name> [options]` | List observations for one declared metric. `observation` and `obs` are aliases. |

`observe` accepts `--session <id>` and `--observed-at <ISO>`. The session must
belong to this project. The timestamp must include `Z` or a numeric offset. It
defaults to the current time. `--json` prints the SDK response.

`observations` accepts `--from <ISO>`, `--to <ISO>`, `--limit <1..10000>`, and
`--json`. Both timestamps must include a time-zone offset. `to` defaults to the
current time. `from` defaults to 30 days before `to`.

### Tasks

Tasks are durable PostgreSQL work contracts. They do not edit `kortix.yaml`.
`goal_slug` is optional. Use it only when work belongs to a Git-authored goal.

| Command | Effect |
| --- | --- |
| `kortix tasks ls [options]` | List tasks. `list` is an alias. |
| `kortix tasks show <id> [--json]` | Show one task. |
| `kortix tasks current [--json]` | Show the task bound to the current session principal. |
| `kortix tasks new --title <text> [--goal <slug>] [options]` | Create a task. `create` is an alias. |
| `kortix tasks run <id> [options]` | Create and claim a cloud coordinator session, then send its task prompt. |
| `kortix tasks watch <id> [--once] [--interval <seconds>] [--json]` | Poll durable task state until it becomes terminal. |
| `kortix tasks claim <id> --session <id> [--lease-seconds <30..86400>] [--json]` | Atomically claim ready work and set its status to `doing`. All dependencies must be `done`. The lease defaults to 900 seconds. |
| `kortix tasks contract <id> [options]` | Revise the task intent, constraints, verification requirements, or review policy. |
| `kortix tasks evidence <id> [--list] [options]` | List or append immutable evidence for one candidate. |
| `kortix tasks submit <id> --candidate <digest> [--session <id>] [--json]` | Request server-gated V1 completion. |
| `kortix tasks blockers <id> [--json]` | List open and historical blockers. |
| `kortix tasks blocker <id> [options]` | Create an idempotent typed blocker and reminder. |
| `kortix tasks resolve-blocker <id> --blocker <id> [--json]` | Resolve a blocker as a human caller. |
| `kortix tasks events <id> [--limit <n>] [--json]` | Read the append-only task timeline. |
| `kortix tasks sessions <id> [--json]` | Read coordinator, worker, and verifier lineage. |
| `kortix tasks cancel <id> --reason <text> [--json]` | Cancel task responsibility as a human caller. |
| `kortix tasks done <id> --session <id> --evidence <ref> [--summary <text>] [--json]` | Complete a legacy task. V1 tasks must use `evidence` and `submit`. |
| `kortix tasks block <id> --session <id> --reason <text> [--json]` | Block a claimed task and record the blocker. |
| `kortix tasks worker <id> [options]` | Bind one bounded worker and deliver its initial prompt. |
| `kortix tasks progress <id> [options]` | Record semantic progress for the current worker turn. |
| `kortix tasks no-progress <id> [options]` | Continue once, then block and escalate repeated no-progress. |

`tasks new --status` accepts `backlog` or `todo`. It defaults to `backlog`.
The AGI agent creates ready work with `--status todo`. Later statuses require
claim, review, completion, blocking, or cancellation transitions.

`ls` accepts `--goal <slug>`, one `--status <status>`, and `--limit <1..1000>`.
Status is `backlog`, `todo`, `doing`, `blocked`, `review`, `done`, or
`cancelled`.

`new` requires `--title`. It accepts optional `--goal <slug>`, `--body <text>`,
`--priority <integer>`, `--agent <name>`, `--blocked-by <id,id>`, `--origin <text>`,
`--fingerprint <text>`, and `--json`. `origin` defaults to `cli`. A
repeated non-null fingerprint for the same project reuses the existing task
instead of creating a duplicate.

The `claim`, `done`, `block`, `worker`, `progress`, and `no-progress` session
must belong to the project. A live claim by another session returns a conflict.
V1 completion requires evidence that satisfies the current contract revision.
Human review policies remain in `review` until a human submits or approves the
verified candidate.

Every goals and tasks command accepts `--project <id>` and `--host <name>`.
Without `--project`, the CLI resolves the linked or default project.
...
The in-sandbox agent's interface to every connector. Every call is checked, resolved, and audited server-side; the CLI never holds a third-party credential. Auth: `KORTIX_CLI_TOKEN`. Output is JSON.
