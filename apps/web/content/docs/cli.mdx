...
A space groups sessions under a named effort inside the project — its own
default agent, its own scheduled work, its own grants. Its file
(its `spaces.<slug>` block in `kortix.yaml`) is the source of truth; every write below commits to it.
See [Spaces](/docs/project/manifest#spaces).

| Command | Effect |
| --- | --- |
| `kortix spaces ls [--json]` | List the spaces you can see. |
| `kortix spaces show <slug> [--json]` | Show one space in full. |
| `kortix spaces create <name> [--slug <s>] [--description <text>] [--agent <name>] [--sessions private\|shared]` | Declare a new space. |
| `kortix spaces update <slug> [--name <text>] [--description <text>] [--agent <name>] [--sessions private\|shared]` | Change fields on an existing space. |
| `kortix spaces rm <slug> [--yes]` | Delete a space. Sessions keep their history but lose the grouping; triggers naming it are un-scoped, not deleted. Prompts for confirmation on a TTY without `--yes`; exits `2` without one on a non-interactive terminal. |
| `kortix spaces grant <slug> (--member <id\|email> \| --group <id>) [--expires YYYY-MM-DD]` | Let a member or group use this space. |
| `kortix spaces revoke <slug> (--member <id\|email> \| --group <id>)` | Remove that grant. |

On `update`, an explicit empty value (`--description=`, `--agent=`) clears that
field; `name` cannot be cleared. `--sessions shared` makes every session in the space visible to
...
`resource_type: 'space'` row `kortix grants` and the dashboard's access
...
Sessions and triggers can be scoped to a space:

| Command | Effect |
| --- | --- |
| `kortix sessions new --space <slug>` | Start a session inside a space. Inside a sandbox, defaults to `$KORTIX_SPACE` (the coordinator session's own space) when the flag is omitted. |
| `kortix sessions ls --space <slug>` | List only that space's sessions. `--space=` (empty) lists sessions with no space. |
| `kortix triggers add <slug> --apply --space <slug>` / `kortix triggers set <slug> --space <slug>` | Scope a live trigger's fired sessions to a space. `--space=` clears it. |
...
| Command | Effect |
| --- | --- |
| `kortix validate [--file <path>] [--json] [--scopes]` | Validate the manifest against the canonical schema. Resolves `kortix.yaml` first, then `kortix.toml`. Exit codes: `0` valid, `1` errors, `2` file missing. This includes every `spaces.<slug>` block and the rules across them (unique agent names, `from` references, default agents, trigger back-references). |
