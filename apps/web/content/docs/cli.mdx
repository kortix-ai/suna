...
An existing GitHub `origin` uses the account's Nango connection. Any other existing `origin` is registered as-is. No `origin` creates a managed repository through the selected platform Nango connection.
...
| `kortix self-host connect-github` | Deprecated. Prints the Nango setup values and dashboard path. |
