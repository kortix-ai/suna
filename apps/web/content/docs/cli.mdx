...
| Command | Effect |
| --- | --- |
| `kortix validate [--file <path>] [--json] [--scopes]` | Validate the manifest against the canonical schema. Resolves `kortix.yaml` first, then `kortix.toml`. Exit codes: `0` valid, `1` errors, `2` file missing. Also validates every sibling `kortix-<slug>.yaml` space file and the set as a whole (unique agent names, `from` references, default agents, trigger back-references). |
