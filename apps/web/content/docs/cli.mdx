...
A craft is a Kortix project you install into another project: a repository whose
`kortix.yaml` declares agents, skills, connectors and triggers. See
[Crafts](/docs/feature-flags/crafts).
...
| Command | Effect |
| --- | --- |
| `kortix crafts publish <owner/repo>` | Index a craft from a GitHub repo. Private by default. Aliases: `submit`. |
| `kortix crafts list` | Browse the catalog. Aliases: `ls`, `search`. |
| `kortix crafts show <id-or-slug>` | One craft: what it declares and what it needs. Alias: `view`. |
| `kortix crafts remove <id>` | Withdraw from the catalog. Does **not** uninstall it from any project. Aliases: `rm`, `delete`. |
| `kortix crafts installed` | What the target project has installed. |
| `kortix crafts install <id-or-slug>` | Start the agent session that installs it. |
| `kortix crafts uninstall <slug>` | Start the agent session that removes it. |
| `kortix crafts enable <slug>` / `disable <slug>` | Turn this craft's triggers on or off. |
| `kortix crafts runs [slug]` | Run history. Omit the slug for every craft. |
...
you review. A craft installs with every trigger **off** — `enable` is what starts
...
`kortix crafts` never uploads an archive. Publishing from a `.zip` exists on the
web, for a folder that is not a repo yet. A craft that tracks a repo gets
re-crawled when it moves; an uploaded snapshot never does.

Crafts is a per-project [feature flag](/docs/feature-flags), off by default.
