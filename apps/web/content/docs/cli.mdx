...
| Command | Effect |
| --- | --- |
| `kortix subprojects publish <owner/repo>` | Index a subproject from a GitHub repo. Private by default. Aliases: `submit`. |
...
| `kortix subprojects enable <slug>` / `disable <slug>` | Turn this subproject's triggers on or off. |
| `kortix subprojects runs [slug]` | Run history. Omit the slug for every subproject. |

`publish` options: `--ref <branch-or-tag>` (aliases `--branch`, `--tag`) pins the
ref; `--public` lists it in the public catalog.

Options on every subcommand: `--project <id>`, `--host <name>`, `--limit <n>`,
`--query <text>` (alias `-q`), `--json`.

`--limit` defaults to 50 and the CLI caps it at 200. The server clamps again:
`runs` returns at most 200 rows, `list` at most 100.
...
you review. A subproject installs with every trigger **off** — `enable` is what starts
it working, and it commits the change to `kortix.yaml`. That is not the
project-wide pause switch, which is `kortix triggers pause`.
