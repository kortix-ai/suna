...
| `kortix sessions new [--prompt "<text>"] [--agent <name>] [--model <id>] [--wait] [--connect] [--json]` | Start a session. `--connect` attaches the OpenCode TUI once it is ready (implies `--wait`); on an interactive terminal without it, the CLI asks whether to connect after creation. `--model <id>` overrides the project's default model; a model that runs on an API key or a ChatGPT subscription uses every key you may use for it, and they rotate. `--wait` blocks until it is running (up to ~5 minutes). Use `--secret <id>` or `--no-secrets` to narrow Secret access. These Secret flags require a backend token. Use `--connector <alias>=<authorization-id>` or `--no-connectors` to set Connector access. Use `--require-connector <alias>` to require an authorization before provisioning. Scope flags are repeatable. Use `--context <key>=<value>` for non-secret runtime context. |
...
| `kortix sessions model <id> <model-id> [--json]` | Change the model a session runs, mid-session. A live sandbox is re-pointed and its runtime restarts, which ends the turn running right now; a stopped session stores the value for its next start. A session with no keys selected for the new model's provider gets every key you may use there. |
...
| Command | Effect |
| --- | --- |
| `kortix models ls [--json]` | List every model you can run: state, origin, how it is paid for (`VIA`: Kortix, API key, or ChatGPT — your own, shared with you, or shared with the project), provider. |
...
Email and `bind` writes need `project.connector.write`.
...
| `kortix connectors connections ls [--all] [--json]` | List connections with label, `CONNECTED AS`, owner, `WHO CAN USE` (`owner only`, `everyone`, or the grant labels, with `(not you)` when you list it only to manage it), status, and default. |
| `kortix connectors connections rename <id> <label…>` | Rename a connection. Only the label changes; no re-authorization. |
| `kortix connectors connections share <id> [--group <id>]… [--user <email\|id>]… [--everyone]` | Share your own private account: it becomes a shared account only they may use (`--everyone`: the whole project). Needs the right to manage the project's connections. |
...
assignment), `--connection <id>` (narrow the use of one shared connector
account; the first grant limits the account to its grantees, and revoking the
last opens it to the whole project again), `--everyone` (grant to everyone with
access to the project; it holds an `--agent` or a `--connection`, never a
role), and `--expires <iso>` (auto-revoke timestamp). `--principal` filters
a list, and takes `user:<id>`, `group:<id>`, `service_account:<id>`,
`project:<id>`, or `pending:<email>`; a bare id is read as a user. A grant to
everyone lists as `everyone`. An agent's identity is a
