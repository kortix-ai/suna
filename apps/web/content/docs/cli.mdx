...
A template is a Kortix project you install into your own: a public repository
whose `kortix.yaml` declares agents, skills, connectors and triggers. See
[Marketplace](/docs/feature-flags/marketplace).

| Command                             | Effect                                                           |
| ----------------------------------- | ---------------------------------------------------------------- |
| `kortix marketplace list`           | Browse the catalog. `--query` filters. Aliases: `ls`, `search`.  |
| `kortix marketplace show <slug>`    | One template: what it declares and what it needs. Alias: `view`. |
| `kortix marketplace install <slug>` | Start the agent session that installs it.                        |

Options on every subcommand: `--project <id>` (install only, default the linked
project), `--host <name>`, `--query <text>` (alias `-q`), `--json`.

`install` does not change the project. It starts an agent session that reads
both manifests, resolves name collisions, and opens a change request you
review. Everything the template adds lands in that one change request, so
reverting it is the uninstall — there is no `installed` or `uninstall`
subcommand. A template installs with every trigger **off**. Enable them one at
a time with `kortix triggers enable <slug>`, which commits to `kortix.yaml`.

The catalog is curated and ships with the API; there is no `publish`.

Marketplace is a per-project [feature flag](/docs/feature-flags), off by
default. While it is off, `install` prints the feature-disabled sentence and
exits `1`.
