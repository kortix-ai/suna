...
cannot already read the session. A shared session runs only on provider keys
shared with the whole project: a model that ran on your own keys switches to
the project's keys for that provider, and with none that runs it, the share is
refused (`409 SHARED_SESSION_NEEDS_PROJECT_KEY`).
