...
`kortix projects upgrade` needs project write. The default agent rewrites the
manifest, runs `kortix validate`, and opens a change request. It never merges:
...
`kortix init` does not include a capability picker. Adding a skill is an agent import: start a session and ask the agent to bring one in.
...
A subproject is a Kortix project you install into another project: a repository whose
`kortix.yaml` declares agents, skills, connectors and triggers. See
[Subprojects](/docs/feature-flags/subprojects).

Two halves. `publish`, `list`, `show`, and `remove` are account-scoped — the
catalog. The rest act on one project, default the linked one.

| Command                                   | Effect                                                                                          |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `kortix subprojects publish <owner/repo>` | Index a subproject from a GitHub repo. Your whole account can see it. Aliases: `submit`.        |
| `kortix subprojects list`                 | Browse the catalog. Aliases: `ls`, `search`.                                                    |
| `kortix subprojects show <id-or-slug>`    | One subproject: what it declares and what it needs. Alias: `view`.                              |
| `kortix subprojects remove <id>`          | Withdraw from the catalog. Does **not** uninstall it from any project. Aliases: `rm`, `delete`. |
| `kortix subprojects installed`            | What the target project has installed.                                                          |
| `kortix subprojects install <id-or-slug>` | Start the agent session that installs it.                                                       |
| `kortix subprojects uninstall <slug>`     | Start the agent session that removes it.                                                        |

There is no `enable`, `disable` or `runs` subcommand. A subproject has no on/off
state and no runs of its own — see the note below.

`publish` options: `--ref <branch-or-tag>` (aliases `--branch`, `--tag`) pins the
ref; `--private` narrows the subproject to you alone. There is **no** flag that
publishes to the global catalog: those rows are curated, and arrive by migration,
seeder or direct insert.

Options on every subcommand: `--project <id>`, `--host <name>`, `--limit <n>`,
`--query <text>` (alias `-q`), `--json`.

`--limit` defaults to 50 and the CLI caps it at 200. The server clamps `list`
again, to 100.

`install` and `uninstall` do not change the project. Each starts an agent session
that reads both manifests, resolves name collisions, and opens a change request
you review. A subproject installs with every trigger **off**. Enable them one at
a time with `kortix triggers enable <slug>`, which commits to `kortix.yaml`.
That is not the project-wide pause switch, which is `kortix triggers pause`.

`kortix subprojects` never uploads an archive. Publishing from a `.zip` exists on the
web, for a folder that is not a repo yet. A subproject that tracks a repo gets
re-crawled when it moves; an uploaded snapshot never does.

Subprojects is a per-project [feature flag](/docs/feature-flags), off by default.
While it is off, every project-scoped subcommand prints the feature-disabled
...
This list is always the system skills, never a mix.
...
an assignment names them or one of their groups.
`kortix access grant --agent <name>` writes the same row.
