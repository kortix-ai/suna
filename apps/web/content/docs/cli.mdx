...
`kortix init` creates a workspace directory with the general-purpose starter. Its
...
`kortix ship` lints your `kortix.yaml`, commits local changes, pushes your branch, and prompts for any missing secret or connection. Run it each time you want your local changes on the cloud workspace. The first run also creates the cloud workspace and repo if you have not linked one yet.
</Step>
...
3. The deprecated `KORTIX_PROJECT_ID` environment variable.
4. `.kortix/link.json` in the exact working directory.
5. The global default set by `kortix workspaces use`.
...
Existing automation does not require a flag day. `kortix projects` remains a
deprecated adapter for `kortix workspaces`. The `--project` flag remains a
deprecated alias for `--workspace` on workspace-scoped commands.
`KORTIX_PROJECT_ID` remains a fallback after `KORTIX_WORKSPACE_ID`.
`--no-project` remains an alias for `--no-workspace` during login.

New automation must use Workspace commands and names. The compatibility adapter
continues to call `/v1/projects` so older response fields such as `project_id`
remain stable.

### Workspace scaffold

`kortix init [workspace-name] [options]` creates a new workspace directory. The
...
`.kortix/link.json`. `kortix ship` or `kortix workspaces link` create that file.
...
| `kortix sessions new [--prompt "<text>"] [--agent <name>] [--model <id>] [--wait] [--connect] [--json]` | Start a session. `--connect` attaches the OpenCode TUI once it is ready (implies `--wait`); on an interactive terminal without it, the CLI asks whether to connect after creation. `--model <id>` overrides the workspace's default model. `--wait` blocks until it is running (up to ~5 minutes). Use `--secret <id>` or `--no-secrets` to narrow Secret access. These Secret flags require a backend token. Use `--connector <alias>=<authorization-id>` or `--no-connectors` to set Connector access. Use `--require-connector <alias>` to require an authorization before provisioning. Scope flags are repeatable. Use `--context <key>=<value>` for non-secret runtime context. |
...
| `kortix connectors rm <slug>` | Remove a connector from the workspace. |
...
The audit commands read the centralized reconstruction log. Account and workspace
...
| `kortix audit workspace <workspace-id> [filters] [--all] [--json]` | List one workspace's events. |
| `kortix audit session <session-id> --workspace <workspace-id> [--all] [--json]` | Reconstruct one session in order. |
| `kortix audit export [filters] --format csv\|jsonl --out <file>` | Resume every export page into one file. |

Filters: `--actor`, `--actor-type`, `--workspace`, `--session`, `--source`,
...
entitlement. Workspace-wide lists require `project.members.manage` because they can
include private-session metadata. Session reconstruction requires
`project.session.read` and visibility of that session. These IAM wire actions
