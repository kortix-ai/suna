...
first, and `+ New session` starts a fresh sandbox. The selected runtime determines
the terminal interface. Pass an id to skip the picker:
`kortix connect <session-id>`.

YAML v3 Pi sessions use the Kortix terminal. It streams responses and tool status.
Questions show numbered options, including multiple selections and free text.
Permission requests show the tool, patterns, and arguments. Reply `once`,
`always`, or `reject`. `/reject` dismisses the current question or permission.
`/stop` or Ctrl-C stops the turn. `/exit` or Ctrl-D detaches and leaves the turn
running. Reconnecting restores history and pending requests. Typed messages use
the durable prompt inbox. This terminal is available on the Pi preview branch.

YAML v2 sessions use the OpenCode TUI. The CLI manages its binary: on first connect it downloads
...
`--port` and arguments after `--` apply only to OpenCode sessions.
...
This opens an interactive chat with your most recent session. Add an id to target a specific session: `kortix sessions chat <id>`. Pi uses the same terminal interface as `connect`. `chat --prompt` remains a one-shot command; `--json` prints one final JSON response.
...
| `kortix sessions new [--prompt "<text>"] [--agent <name>] [--model <id>] [--wait] [--connect] [--json]` | Start a session. `--connect` opens the selected runtime’s terminal once it is ready (implies `--wait`); on an interactive terminal without it, the CLI asks whether to connect after creation. `--model <id>` overrides the project's default model. `--wait` blocks until it is running (up to ~5 minutes). Use `--secret <id>` or `--no-secrets` to narrow Secret access. These Secret flags require a backend token. Use `--connector <alias>=<authorization-id>` or `--no-connectors` to set Connector access. Use `--require-connector <alias>` to require an authorization before provisioning. Scope flags are repeatable. Use `--context <key>=<value>` for non-secret runtime context. |
...
| `kortix sessions connect [<id>] [-- <opencode args>]` | Open the Kortix terminal for Pi or the OpenCode TUI for OpenCode. Also available top-level: `kortix connect` / `kortix attach`. With no id, opens a session picker (running, stopped-with-restart, or new). For OpenCode sessions, the CLI auto-downloads the version-matched `opencode` binary (cache: `~/.kortix/opencode/<version>/`; override: `KORTIX_OPENCODE_BIN`). |
