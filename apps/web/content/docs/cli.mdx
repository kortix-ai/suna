...
default agent, its own scheduled work, its own grants. Its file
(`kortix-<slug>.yaml`) is the source of truth; every write below commits to it.
See [Subprojects](/docs/project/manifest#subprojects).

| Command | Effect |
| --- | --- |
| `kortix subprojects ls [--json]` | List the subprojects you can see. |
| `kortix subprojects show <slug> [--json]` | Show one subproject in full. |
| `kortix subprojects create <name> [--slug <s>] [--description <text>] [--agent <name>] [--sessions private\|shared]` | Declare a new subproject. |
| `kortix subprojects update <slug> [--name <text>] [--description <text>] [--agent <name>] [--sessions private\|shared]` | Change fields on an existing subproject. |
...
On `update`, an explicit empty value (`--description=`, `--agent=`) clears that
field; `name` cannot be cleared. `--sessions shared` makes every session in the subproject visible to
