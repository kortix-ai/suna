...
Tasks are generated PostgreSQL state. They do not edit `kortix.yaml`.

| Command | Effect |
| --- | --- |
| `kortix tasks ls [options]` | List tasks. `list` is an alias. |
| `kortix tasks show <id> [--json]` | Show one task. |
| `kortix tasks new --goal <slug> --title <text> [options]` | Create a task for a declared goal. `create` is an alias. |

`tasks new --status` accepts `backlog`, `todo`, `doing`, `review`, or `cancelled`. It defaults to `backlog`. The meta agent creates ready work with `--status todo`.
...
| `kortix tasks done <id> --session <id> --evidence <ref> [--summary <text>] [--json]` | Complete a claimed task with explicit evidence. A liveness-bound task must cite its server-recorded progress ref. |
...
`new` requires `--goal` and `--title`. It also accepts `--body <text>`,
...
The `claim`, `done`, and `block` session must belong to the project. A live
claim by another session returns a conflict. `done` requires non-empty cited
evidence. Both `done` and `block` release the claim when the transition
succeeds.
