...
directory pointed at the workspace's `.kortix/opencode/` folder.
...
platform access. See [the manifest reference](/docs/workspace/manifest) for
...
Legacy `kortix.toml` (v1) workspaces list agents in an array, not a map, and
grant full access by default. Set specific grants to restrict access. See
[the manifest reference](/docs/workspace/manifest).
...
session. If you omit `agent_name`, Kortix uses the workspace's `default_agent`.
Set the default agent from the dashboard, the API, or the SDK. Each writes
`default_agent` to `kortix.yaml` and commits the change to the workspace's
default branch.
