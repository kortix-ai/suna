...
Kortix still supports v1 manifests. The platform resolves a workspace manifest in this order: `kortix.yaml`, then `kortix.yml`, then `kortix.toml`. The current starter creates one v2 `kortix.yaml`. A v1 workspace keeps working with no forced upgrade.
