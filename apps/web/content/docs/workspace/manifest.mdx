...
| `project.name` / `project.description`              | no       | legacy manifest metadata; the platform does not read `project.name` for the workspace display name |
...
Optional, human-facing metadata: `name` and `description`. The key remains
`project` for manifest compatibility. The platform does not read this table for
anything. A workspace display name comes from its database record. Treat this
block as documentation for people reading the repo.
...
| `authorization_strategy` | no       | `project` or `user`. Defaults to `project` when omitted. The `project` value remains for manifest compatibility and means active workspace connections. A user strategy uses only the acting member's connection. |
| `enabled`                | no       | Defaults to `true`.                                                                                                                                                    |
| `credential`             | no       | Only `shared` is supported. The retired `per_user` mode is a hard error.                                                                                               |
| `sensitive`              | no       | Defaults to `false`. `true` makes `require_approval` the unmatched-action default, including reads. Explicit workspace and connector rules still resolve first.        |
...
| Field                 | Default | Notes                                                                                                                                                                                                     |
| --------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _(map key)_           | —       | `[a-z0-9][a-z0-9_-]{0,127}`, unique per workspace. Matches the `.md` filename it governs.                                                                                                                 |
...
is workspace-owned or member-owned.

`kortix_cli` grants come from a fixed set of workspace-scoped actions. Their
wire names remain `project.*` for compatibility (`project.read`,
`project.cr.open`, `project.cr.merge`, `project.trigger.*`, `project.secret.*`,
`project.connector.*`, and more). Account-scoped actions — `member.*`,
`billing.*`, `project.create` — can never be granted to an agent. Run
`kortix validate --scopes` from inside a session to print the live, grantable list.
