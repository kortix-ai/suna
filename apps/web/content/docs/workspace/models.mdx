...
experimental feature flag, and it is on by default for cloud workspaces. Check or
...
## Per-workspace model enablement

The workspace controls which models its pickers offer. By default, the newest
model of each family is offered automatically. Kortix-managed models and any
model your workspace's defaults or routing policy reference are always offered —
a guard never prunes them.

You can override the default for individual models on the **Manage models**
page (Customize → Models). An exception is stored per workspace and takes effect
...
workspace's default model cannot be turned off — set a different default first.
...
`llm_credentials_project_wide` error. This code remains for wire compatibility. Update the shared key on the
