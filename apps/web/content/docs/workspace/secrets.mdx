---
title: Secrets
description: How Kortix stores workspace credentials and controls where each value is allowed to go.
---
...
Change the mode in the workspace's Secrets page, or from the CLI:
...
<Callout type="warn" title="Session-scoped delivery needs a named agent grant">
  Network boundary and the HTTPS broker attach a credential to one running session. For those two
  modes an agent's `secrets` list must name the identifier explicitly. `secrets: all` does not
  count, and a workspace with no `agents:` block in `kortix.yaml` never receives one.
</Callout>
...
Open the workspace's Secrets page, enter the key and value, and save. Kortix
...
workspace, on a best-effort basis. For model or gateway credentials, Kortix
...
delete it from the workspace's Secrets page.

<Callout type="warn" title="Removal is immediate, propagation is not">
  Kortix deletes a shared secret right away. Push to already-running sandboxes is best-effort, the
  same as rotation.
</Callout>
...
<Callout type="warn" title="Three things must be true, or the header is silently missing">
  Nothing inside the sandbox can detect a wrong setup. The request leaves without the header, and
  the upstream API answers `401`.
</Callout>

<Steps>

<Step>
### Pin the workspace to Platinum
...
network-boundary secret behaves exactly as if no grant existed. A workspace with
...
| _(blank)_           | `authorization: sk_live_...`        |
...
<Callout type="warn" title="An echo service is the worst possible test target">
  If a response would send the secret back into the sandbox, Platinum kills the connection. `curl`
  reports `curl: (52) Empty reply from server`. A correct setup therefore looks exactly like a
  broken one when you test against an endpoint that echoes request headers.
</Callout>
...
| `kortix secrets sync`                                                        | Re-push workspace secrets to this session's sandbox           |
...
`project.secret.write` permission. These IAM wire actions retain their legacy names. A custom workspace role can withhold
either permission, even from a manager.

## REST routes

Canonical routes sit under `/v1/workspaces/{workspaceId}`. Deprecated
`/v1/projects/{projectId}` routes remain supported for existing clients.
...
A workspace on Kortix's managed model access needs no key of its own. To bring
