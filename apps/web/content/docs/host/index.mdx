...
through the existing Serper proxy. Use a Serper-compatible base URL and API key,
not an incompatible native API endpoint.

Set `SERPER_API_URL` and `SERPER_API_KEY` in the API container environment,
then restart the API. For a source checkout, use the gitignored
`apps/api/.env.local` file instead. Never commit a real API key.

For example, Litescrape's web-search shim uses:

```dotenv
SERPER_API_URL=https://api.litescrape.com
SERPER_API_KEY=your-provider-api-key
```
