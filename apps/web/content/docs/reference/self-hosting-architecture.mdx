...
and for the cloud agent sandbox to call back to. A local machine can run the
identical artifact for evaluation, but only via a Cloudflare tunnel
(ephemeral URL) — with neither a domain nor a tunnel configured, agent
sessions won't work because the sandbox has no way to call back to this API;
neither path is a deployment target.
...
runs on a local machine, any VPS, or a cloud VM (EC2, Droplet, …) — a public
domain is just an env var (`KORTIX_DOMAIN`) the same stack reacts to, not a
...
flowchart TB
    ...
    subgraph box["One host — local machine, VPS, or cloud VM"]
