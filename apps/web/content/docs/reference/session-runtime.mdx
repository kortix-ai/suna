...
`queued` and `branching` exist in the enum but are not part of the live session flow; `completed` is defined but not currently written. The sandbox itself is tracked in a separate `session_sandboxes` row with its own states (`provisioning → active → stopped | error`).
...
| `KORTIX_TOKEN` | Project-scoped token. The `kortix` CLI reads it; the daemon also signs proxy requests with it. |
| `KORTIX_EXECUTOR_TOKEN` | Acts as the launching user, scoped to the project (used by the Executor). |
| `ENV_MODE` | `cloud` on Daytona-provisioned sandboxes. |

Not injected: `KORTIX_WORKSPACE` (`/workspace`) is baked into the image, not per session. No `KORTIX_GITHUB_TOKEN` — git credentials are fetched just-in-time by the daemon (below). No LLM routing variable — model provider keys travel as ordinary project secrets OpenCode reads at boot.
