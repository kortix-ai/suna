...
Points at the Dockerfile that becomes the base for session sandboxes; the Kortix runtime layer (opencode CLI, agent daemon, entrypoint) is layered on top automatically. Optional — omitting the table or any key falls back to defaults.
