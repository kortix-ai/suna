...
In v3, runtime routing lives under `runtimes:`. Each runtime profile selects an ACP harness and optional harness-native config directory.
...
| Field        | Type   | Default by harness | Notes                                                  |
| ------------ | ------ | ------------------ | ------------------------------------------------------ |
| `harness`    | string | —                  | `claude`, `codex`, `opencode`, or `pi`. |
| `config_dir` | string | Harness default    | Repo-relative dir mapped to the harness-native config env var. |

The selected harness owns its runtime semantics; Kortix compiles routing and governance into an ACP launch plan. See [Kortix vs runtime config](/docs/reference/config-boundary).

### Legacy `opencode:` / `[opencode]`

v2 projects may still declare where OpenCode's config dir lives. Optional, with a default. The agent daemon launches the OpenCode harness with `OPENCODE_CONFIG_DIR` pointed here.
