...
3. A pinned Python package floor via `pip install --break-system-packages`, verified importable at build time: web/parsing (`beautifulsoup4`, `lxml`, `markdownify`, `markitdown[pptx]`, `requests`), data (`numpy`, `pandas`, `scipy`, `scikit-learn`), PDF/office (`pdf2docx`, `pdf2image`, `pdfplumber`, `pymupdf`, `pypdf`, `pypdfium2`, `python-docx`, `python-pptx`, `openpyxl`, `reportlab`), imaging/browser (`pillow`, `pytesseract`, `playwright`), and plotting (`matplotlib`, `plotly`, `seaborn`). This backs the starter's document/data/PDF/presentation skills and is present even if your Dockerfile never touches Python.
