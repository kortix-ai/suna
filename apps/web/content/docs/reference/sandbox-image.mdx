...
`cpu`, `memory`, `disk`, and `gpu` on a `[[sandbox.templates]]` entry size the box your sessions run on. All four are optional; an omitted field takes the runtime provider's default. `cpu` is vCPU cores, `memory` and `disk` are GiB, `gpu` is GPU units (`0` / omitted = none).
