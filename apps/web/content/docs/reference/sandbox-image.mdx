...
```
┌─────────────────────────────────────────┐
│  Kortix runtime layer (auto-injected)   │  ← ACP harness adapters + kortix-agent + ENTRYPOINT
├─────────────────────────────────────────┤
...
4. Pinned, version-probed installs of every ACP harness adapter — `npm install -g` runs once per package, and the build fails if the binary doesn't land on `PATH` or the probe command errors:
   - `opencode-ai@<pinned-version>` — the native ACP process; probed with `opencode --version`.
   - `@agentclientprotocol/claude-agent-acp@<pinned-version>` — probed with `claude-agent-acp --version`.
   - `@agentclientprotocol/codex-acp@<pinned-version>` — probed with `codex-acp --version`.
   - `pi-acp@<pinned-version>`, alongside the `@earendil-works/pi-coding-agent@<pinned-version>` CLI it wraps — probed with `pi-acp --help` and `pi --version`.

   All four are baked at build time; a session request never installs or upgrades an adapter (no runtime `npx`/`npm install`). The layer also installs the `bun` runtime and `agent-browser` (for the headless-browser tool).
5. A baked OpenCode-specific tool-dependency cache under `/opt/kortix/` (so the boot-time `bun install` OpenCode runs in your config dir is a no-op, not a network round-trip). The other three ACP adapters don't use this cache — they have no equivalent config-dir dependency install step.
...
See [Harnesses](/docs/reference/harnesses) for each adapter's stability and authentication, and for the `KORTIX_ACP_<HARNESS>_PATH` / `KORTIX_ACP_<HARNESS>_ARGS` environment variables that let you override an adapter's launch command for controlled testing.
...
Snapshots are content-addressed: the platform hashes your Dockerfile's own bytes + the hardware spec + a platform runtime fingerprint. That fingerprint folds in the pinned version of every ACP adapter (OpenCode, Claude, Codex, Pi), the runtime layer and sandbox version constants, and the `kortix-agent` binary's source alongside the other runtime files it COPYs in (entrypoint script, `kortix` CLI's in-sandbox command surface, `slack-cli`, `executor-sdk`). Unchanged hash reuses the snapshot; otherwise a new one builds — bumping any pinned adapter version rebuilds every project's snapshot. Rebuilds happen on any push that changes the Dockerfile's text, or any change to the `sandbox` spec — pure code commits, and edits to files your Dockerfile `COPY`s in without touching the Dockerfile itself, reuse the snapshot for free (the hash isn't computed over those copied files today).
