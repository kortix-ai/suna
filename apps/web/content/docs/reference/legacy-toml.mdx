...
This page covers the v1 manifest (`kortix.toml`, `kortix_version: 1`). For the current manifest, see [Manifest reference](/docs/reference/manifest).
...
Agent behavior — system prompt, `model`, `mode`, `temperature`, and more — never moves. It already lives in each agent's `.md` frontmatter under `.kortix/opencode/agents/`, in both v1 and v2. See [Agents](/docs/concepts/agents).

Check the result with `kortix validate --file kortix.yaml`. Fetch the full v2 schema with `kortix schema --version 2`. See [CLI reference](/docs/reference/cli) for both commands.
