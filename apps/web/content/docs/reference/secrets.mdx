...
In `kortix.yaml`, each entry in the `agents:` map has a `secrets` grant — which of the project's declared names that agent receives as sandbox env vars. It's deny-by-default: an agent with no `secrets` key gets none, not every project secret. Give it `secrets: all` for the full set, or an explicit list to scope it down:
...
- Names match `^[A-Z_][A-Z0-9_]{0,63}$` — env-var-shaped, capped at 64 chars. The manifest parser is looser (`^[A-Z_][A-Z0-9_]*$`, no length cap), so a name longer than 64 chars is accepted in `env:` (or `[env]`) but can never get a value. Keep names ≤ 64 chars.
