...
| Harness | Manifest `harness` value | Default `config_dir` | ACP process |
| --- | --- | --- | --- |
| OpenCode | `opencode` | `.opencode` | `opencode acp` (native) |
| Claude Code | `claude` | `.claude` | `@agentclientprotocol/claude-agent-acp` |
| Codex | `codex` | `.codex` | `@agentclientprotocol/codex-acp` |
| Pi | `pi` | `.pi` | `pi-acp` |

OpenCode is the default harness every new project starts on. Claude Code,
Codex, and Pi are equally available — declare a runtime profile for one (see
below, or **Customize → Agents → Runtime profiles** in the dashboard) and
select it, no separate feature flag required. The Claude Code and Codex
processes are official adapters maintained in the Agent Client Protocol
organization; they are baked into the sandbox image at pinned versions, never
...
launch. The session composer's model picker is harness-aware: the gateway/BYOK
provider catalog grouped by provider for OpenCode, and each harness's own
default plus preset list for Claude Code, Codex, and Pi. There is one picker
component and no experimental flag gating it.
