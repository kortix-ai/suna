---
title: CLI reference
description: Install steps, the auth model, and every stable kortix command with its flags.
---

The `kortix` command line interface (CLI) controls Kortix from a terminal — your laptop or a session sandbox. This page lists every stable command, its flags, and the token model that authenticates it. For the everyday dev loop, see [Developing with the CLI](/docs/guides/developing).
...
Every token starts with `kortix_pat_`. Two scopes exist: a user token, from `kortix login`, that sees every account and project you belong to; and a project token, auto-minted for a session sandbox, that is scoped to one project. See [Token scope](#token-scope).

```sh
kortix login                                   # browser flow → pick account → pick default project
kortix init my-app && cd my-app                # scaffold a project
kortix ship                                    # create the cloud project + repo, push
kortix sessions new --prompt "Build the login page"
kortix cr ls                                   # review what the agent opened
kortix cr merge 1                              # land it on main
```
...
A command resolves "the project" in order: the `--project` flag, then the `KORTIX_PROJECT_ID` environment variable, then `.kortix/link.json` in the exact working directory, then the global default set by `kortix projects use`.
...
Flags: `--name <project>`, `--primary <agent>`, `--agents <csv>` (configure local coding-agent integrations to wire up), `--template <general-knowledge-worker|minimal>` (default: `general-knowledge-worker`), `--force` (configure the current directory in place instead of scaffolding a new one), `--overwrite`, `--no-git`, `-y|--yes`.
...
A change request (CR) merges one branch into another on any git host. It is the only way for an agent to land session work on the default branch. See [Change requests](/docs/reference/change-requests).
...
| `kortix cr open --title "<text>" [--description "<text>"] [--head <ref>] [--base <ref>]` | Open a CR. Aliases: `new`, `create`. Inside a sandbox, `--head` and the session default automatically. `--base` defaults to the project's default branch. |
...
`--platform slack|teams` selects the platform; default `slack`. See [Connectors](/docs/concepts/connectors).
...
A trigger starts a session from a schedule or a webhook. `add`, `rm`, `enable`, `disable` edit the local manifest — run `kortix ship` to apply. `pause`/`resume` flip a separate, server-side switch. See [Triggers](/docs/reference/triggers).
...
See [Manifest reference](/docs/reference/manifest).

## Self-host

`kortix self-host` runs one Docker-based stack, identical on a laptop, a VPS, or a cloud VM. See [Self-hosting](/docs/guides/self-hosting) and [Self-hosting architecture](/docs/reference/self-hosting-architecture).
...
Every token starts with `kortix_pat_`: a user token, scoped to every project on your accounts, or a project token, scoped to one project and auto-injected into that project's sandboxes. See the full token-family reference at [Session runtime](/docs/reference/session-runtime).
