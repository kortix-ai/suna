...
| `kortix self-host logs [service]` | Tail Docker Compose logs (Docker) or CloudWatch/SSM service logs (AWS VPC: `updater`, `supabase`, `app`, `watchdog`, `api`, `gateway`, `frontend`, `caddy`). |
