...
the optional integrations you may not need yet: Freestyle for app deployments
and Pipedream for app connectors. Local ports, Supabase, generated secrets, and
...
| `kortix self-host configure` | Re-run the Daytona, Freestyle, GitHub, and Pipedream credential wizard. |
...
| Command | Effect |
| --- | --- |
| `kortix channels status` | Show the current Slack connection. |
| `kortix channels connect [--bot-token <xoxb-…>] [--signing-secret <…>]` | Save bot token + signing secret to project secrets. Flags, `SLACK_BOT_TOKEN`/`SLACK_SIGNING_SECRET` env vars, or `-` to read from stdin. |
| `kortix channels disconnect` | Drop the project's Slack secrets. |
| `kortix channels manifest` | Print the Slack app manifest JSON — paste into `api.slack.com/apps` → "From a manifest". |
...
`kortix init` and `kortix <project-name>` both create a **new, standalone** directory — like `create-next-app`, neither ever scaffolds into an existing folder or the current directory, and `init` refuses to run against a non-empty one. They are not aliases of each other, though: `init` runs the full interactive wizard (coding-agent symlinks, starter-template choice, marketplace skill picker), while `kortix <project-name>` is the lighter direct form that skips the wizard and auto-commits the scaffold with git.
...
| `kortix <project-name> [options]` | Makes a fresh directory next to your shell cwd, runs `git init -b main`, drops the same OpenCode-native scaffold at the repo root, then stages everything and makes an initial commit (skip with `--no-commit`; skip git entirely with `--no-git`). No coding-agent wizard, no template/marketplace prompts. |
