...
| Command | Effect |
| --- | --- |
| `kortix init [project-name] [options]` | Prompts for a project name if omitted, then creates that directory and scaffolds it: `kortix.yaml` (the v3 starter manifest, declaring all four ACP runtime profiles — `.kortix/opencode`, `.claude`, `.codex`, `.pi`) plus each runtime's config dir (OpenCode gets the default `kortix` agent + the `kortix-system` skill + tools; Claude/Codex/Pi get their own native starter files). Runs the wizard: pick the local coding agent(s) to wire up for editing (symlinks `.opencode`/`.claude`/`.agents` + `AGENTS.md` into `.kortix/opencode` so skills/agents are shared) — this is local editor/CLI compatibility only, separate from the `runtimes` profiles above that govern cloud sessions. Does **not** write `.kortix/link.json` — that's created later by `projects link` or `ship`. |
