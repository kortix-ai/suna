...
`kortix self-host` is one generic Docker-native system — no separate "target"
to pick. `init` generates a `docker-compose.yml` + `.env` (plus a `Caddyfile`
and `updater.sh` when a domain is configured) into
`~/.config/kortix/self-host/<instance>/`, and `start` pulls the images and
runs `docker compose up`. It's identical on a local machine, any VPS, or a cloud VM —
there is no AWS profile, no Terraform, no TUF/signing, no SSM.

`start` creates config if needed, starts Supabase/API/gateway/frontend
locally, and collects the Daytona sandbox key, managed GitHub access, and
optional Pipedream connector integration via the `configure` wizard. Local
ports, generated secrets, and image tags are handled automatically. A public
domain + TLS is opt-in: set `KORTIX_DOMAIN` (and optionally
`KORTIX_API_DOMAIN`, default `api.<domain>`) via `env set` to turn on the
bundled Caddy reverse proxy, which terminates TLS via ACME HTTP-01 on ports
80/443 — leave it unset for the default loopback-port setup.

An always-present in-compose `kortix-updater` service (gated by
`KORTIX_AUTO_UPDATE`, on by default) pulls the configured channel's images on
an interval (`KORTIX_UPDATE_INTERVAL`, default daily) and rolls the stack
forward when anything actually changed — `update`/`reconcile` just runs that
same logic once, immediately. The full operator runbook lives at
`docs/runbooks/self-hosting.md` in the repository, and the architecture is
documented at [Self-hosting architecture](/docs/reference/self-hosting-architecture).

For a bare Linux box (VPS/EC2), `scripts/kortix-selfhost-up.sh` installs
Docker + the CLI and runs `init`/`start` for you — see the runbook.

| Command | Effect |
| --- | --- |
| `kortix self-host init` | Create or refresh the self-host config, without starting the stack. Non-mutating to a running stack. |
| `kortix self-host configure` | Interactive wizard for integrations (Daytona, GitHub, Pipedream) and update policy (channel, auto-update, interval). |
| `kortix self-host doctor` | Validate local Docker tooling and the rendered Compose config. Non-mutating. |
| `kortix self-host start` | Create config if needed, start the stack, and register the `selfhost` host. |
| `kortix self-host update [--tag <v> \| --channel stable\|latest]` | Pull the configured (or given) channel/tag's images and recreate the stack in place — migrates first, then rolls forward. |
| `kortix self-host reconcile` | Alias for `update` — converge to the configured channel/version right now. |
| `kortix self-host rollback --release <v>` | Roll back to an explicit older version (same mechanics as `update`, pinned). |
| `kortix self-host version` | Show the running version, configured channel, and whether a newer release is available. |
...
| `kortix self-host env ls [--show]` | Show every persistent value, grouped by service, masking populated secrets (`--show` reveals). |
| `kortix self-host env set KEY=VALUE …` | Set any value (domain, integrations, update policy, secrets, …) and restart only the services it affects. |
| `kortix self-host env rotate KEY \| --all-generated` | Regenerate a rotatable CLI-generated secret in place. |
| `kortix self-host logs [service]` | Tail Docker Compose logs. |
| `kortix self-host stop` | Stop the stack. |
| `kortix self-host uninstall` | Stop the stack and permanently delete this instance's containers, volumes, and config. Interactive confirmation, or `--yes`. |

Common flags: `--instance <name>` (default `default`), `--tag <version>` /
`--release <version>` (pin an explicit image tag), `--channel stable|latest`
(default `stable`), `--auto-update on|off` (default `on`),
`--update-interval <seconds>` (default `86400`), `--json`, and `--yes`.
...
| Command | Effect |
| --- | --- |
| `kortix secrets ls` | List secrets by identifier + manifest `env` spec (`env:` in `kortix.yaml`, `[env]` in legacy `kortix.toml`), showing `→ KEY` when the injected env key differs; marks required-but-missing. |
| `kortix secrets set NAME=VALUE …` | Upsert one or more. `NAME=-` reads VALUE from stdin (so values never appear in shell history). Add `--identifier <id>` (alias `--id`) with exactly one `KEY=VALUE` pair to store a second value under the same env key. |
...
Integrations agents call as tools (Pipedream apps, MCP servers, Postman collections/workspaces, and OpenAPI/GraphQL/HTTP endpoints) — mirrors the dashboard's Customize → Connectors. Connectors are project-wide visible; the access gate is which agents may call one (`agents.<name>.connectors` in `kortix.yaml`; the legacy `kortix.toml` array-of-tables form is `[[agents]].connectors`; see [Grants](#grants)). `add` / `rm` / `policy set` edit the **local** `kortix.yaml` (the source of truth) — run `kortix ship` to apply, then `sync` to reconcile — unless you pass `--apply`, which skips ship/CR and applies on the cloud project immediately.
...
`add` options: `--name <label>` (default: slug), `--provider <pipedream\|mcp\|openapi\|postman\|graphql\|http>`, `--app <slug>` (Pipedream), `--url <url>` (MCP), `--transport <http\|sse>` (MCP), `--endpoint <url>` (GraphQL), `--base-url <url>` (HTTP), `--spec <url\|path>` (OpenAPI/Postman/GraphQL/HTTP), `--auth-type <none\|bearer\|basic\|custom>`, `--credential shared`. A Postman `--spec` may be Collection v2 JSON, a `.postman/api` manifest, a public GitHub repository managed by Postman, or a public Postman workspace URL. Workspace URLs require the host operator to configure `POSTMAN_API_KEY`; repository and collection imports do not.
...
| `kortix channels connect` | **Connect Slack in one step.** On Kortix Cloud (or any host with the shared Slack app configured) it prints a one-click "Add to Slack" install link — open it, pick the workspace, click Allow. Add `--wait` to poll until the install lands (`--timeout <sec>`, default 300). Hosts without the shared app fall back to manual token mode automatically. |
...
| `kortix sessions connect [<id>] [-- <runtime attach args>]` | Attach to the session runtime when the selected harness supports a local attach flow. |
...
`add` options: `--type <cron\|webhook>` (default `cron`), `--prompt <text>` (required — initial prompt for the spawned session), `--agent <name>` (default: the project's `default_agent`), `--cron <expr>` (6-field cron, e.g. `"0 0 9 * * 1-5"`), `--timezone <tz>` (default UTC), `--secret-env <NAME>` (HMAC secret env var, webhook type), `--name <label>` (default: slug), `--disabled` (create it disabled).
...
Per-agent settings on the linked project. Today: which model each agent runs on — the dynamic gateway default, applied instantly with no `kortix.yaml` commit. A session that asks for the synthetic `auto` model resolves to this pick, falling back to the project → account → platform default. (`[[agents]].model` is v1-legacy TOML syntax and dead there too — parsed and round-tripped but never applied at runtime; removed outright in v2/v3, where a harness's model lives in its own native config instead. See [`[[agents]]` (v1, legacy)](/docs/reference/manifest#agents-v1-legacy).)
...
| Command | Effect |
| --- | --- |
| `kortix init [project-name] [options]` | Prompts for a project name if omitted, then creates that directory and scaffolds it: `kortix.yaml` (the v3 starter manifest, declaring all four ACP runtime profiles — `.opencode`, `.claude`, `.codex`, `.pi`) plus each runtime's config dir (OpenCode gets the default `kortix` agent + the `kortix-system` skill + tools; Claude/Codex/Pi get their own native starter files). Runs the wizard: pick the local coding agent(s) to wire up for editing (symlinks `.opencode`/`.claude`/`.agents` + `AGENTS.md` into `.opencode` so skills/agents are shared) — this is local editor/CLI compatibility only, separate from the `runtimes` profiles above that govern cloud sessions. Does **not** write `.kortix/link.json` — that's created later by `projects link` or `ship`. |

`kortix init` options: `--name <project>` (alias for the positional name), `--primary <agent>` (opencode\|claude\|codex\|cursor), `--agents <list>` (comma-separated extras to wire up alongside `--primary`), `--template <general-knowledge-worker\|minimal>` (default `general-knowledge-worker`, the full skill kit; `minimal` is base plumbing only), `--no-git` (skip `git init`), `-y`/`--yes` (skip prompts — requires a project name). Adding marketplace items is not part of `init` — start a session and ask the agent to import one.
...
- **Not a separate runtime.** Use `kortix self-host start` to run the generic Docker self-host stack (local machine, VPS, or cloud VM — all identical), then switch between Cloud and self-hosted APIs with `kortix hosts use`.
- **Not a `git` replacement.** `kortix cr` composes with `git` rather than wrapping it.
- **Not the runtime.** The selected ACP harness executes the agent in the sandbox; the CLI is the control plane. See [Kortix vs runtime config](/docs/reference/config-boundary).
