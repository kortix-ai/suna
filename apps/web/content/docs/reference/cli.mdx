...
A sandbox's project token is minted at session create and lives for that session sandbox. Restarting an already materialized sandbox stop/starts the same provider runtime and keeps its token. A replacement runtime is allocated only if the original sandbox never materialized; deleting the session ends that sandbox token.
