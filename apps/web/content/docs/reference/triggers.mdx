---
title: Triggers
description: Field reference for trigger config, session strategy, template variables, endpoints, and limits.
---

This page lists every trigger field, the session strategy fallback order, template variables, endpoints, and limits. For the plain-language version, see [triggers](/docs/concepts/triggers).

A trigger starts a [session](/docs/concepts/sessions) with no person present, on a schedule (`cron`) or an inbound webhook. There is no third type. Triggers live in the project manifest — `triggers:` in `kortix.yaml`. Legacy `kortix.toml` uses the same fields in a different container; see [legacy TOML](/docs/reference/legacy-toml).

Creating, updating, or deleting a trigger through the API, SDK, or dashboard writes directly to the default branch. It does not go through a [change request](/docs/reference/change-requests) (CR). Editing `kortix.yaml` inside a session and running `kortix ship` follows the normal branch and CR flow instead.

Runtime state (`last_fired_at`, `last_status`, `last_error`) lives outside the manifest, in the database. There is no fire count.
...
2. **`keyed`** — render `session_key` against the payload, then look up the most recent non-failed session previously stamped with that exact key. If the key renders empty, or no session matches, fall through to a fresh session directly (never to another key's session).
...
A project-level switch stops every trigger in the project at once, independent of each trigger's own `enabled` field. While paused, the scheduler skips the project and inbound webhooks return `200` with `{ status: "skipped" }` — no session fires. A manual fire still works. Use this when the same repository runs on two control planes (for example, dev and production) so cron does not fire twice. CLI: `kortix triggers pause` and `kortix triggers resume`. See [CLI reference](/docs/reference/cli) for the full `kortix triggers` command group.
