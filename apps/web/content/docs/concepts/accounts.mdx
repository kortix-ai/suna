...
An account holds your [projects](/docs/concepts/projects) and the people who work in them. When you sign up, Kortix creates a personal account for you. Invite a teammate and it becomes a team account. Personal and team accounts use the same roles, billing, and limits.
...
Kortix signs you in with a personal access token, prefixed `kortix_pat_`. It acts as you, with your full account access, unless you scope it to one project when you create it.

A service account is a separate, non-human identity, prefixed `kortix_sa_`. It has no project access by default — you must grant it access explicitly, or every call returns 403. Use a service account only when you need a credential that does not act as a specific person.

See [token families](/docs/reference/session-runtime) for the full list.
