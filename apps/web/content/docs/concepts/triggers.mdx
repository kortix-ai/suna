...
Use `session_mode: pinned` with a `session_id` when a trigger must first try to
loop a specific existing session. If that pinned session is gone or unavailable,
Kortix gracefully falls back to the trigger's reusable session, then to a fresh
session.
