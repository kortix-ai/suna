...
[The client](/docs/sdk/the-client). Every provider runs the same sandbox
image.

For the full status enum, injected environment variables, and daemon
endpoints, see [Session runtime](/docs/reference/session-runtime). For how a
session picks its agent, see [Agents](/docs/concepts/agents). For sessions a
schedule or webhook starts, see [Triggers](/docs/concepts/triggers). To land
session work on the default branch, see
[Change requests](/docs/concepts/change-requests).
