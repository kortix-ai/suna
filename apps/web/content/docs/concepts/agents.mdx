...
[session](/docs/concepts/sessions). This page covers agent files, skills, and
how a session picks an agent.

## OpenCode runs every session

OpenCode runs every session. The `kortix-agent` daemon starts it as
`opencode serve`, with its config directory pointed at the project's
`.kortix/opencode/` folder.
...
platform access. See [the manifest reference](/docs/reference/manifest) for
...
[change request](/docs/concepts/change-requests) merges them into the default
