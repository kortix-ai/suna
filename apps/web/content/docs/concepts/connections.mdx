...
- **Declared as `[[connectors]]` in `kortix.toml`** — provider is one of
  **Pipedream, MCP, OpenAPI, GraphQL, HTTP, or Channel** (chat platforms like
  Slack). The definition lives in git; the **credentials live in the platform,
  never in the repo**.
- **Channel connectors auto-materialize.** Connecting Slack
  ([Channels](/docs/concepts/channels)) creates a `slack` connector for you — no
  `kortix.toml` entry needed; its credential is the install token, resolved
  server-side.
