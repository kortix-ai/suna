...
- **Everyone in the project is a principal too.** In the **Grant access** dialog, pick **Everyone in {project}** to give an agent to every member at once (`principal_type: "project"`, `principal_id` = the project id). It holds object assignments only — never a role — so each person keeps their own project role.
