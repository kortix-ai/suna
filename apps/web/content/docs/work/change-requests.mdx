...
A change request (CR) merges one git branch into another. Kortix creates a CR from a session branch (`head_ref`) onto the Workspace default branch (`base_ref`, usually `main`). The CR row is metadata. The merge, diff, and conflict checks run as real git operations against the Workspace repository. A CR is the only way session work reaches the default branch.
...
An agent must open a CR to land any change on the Workspace default branch. The agent does not merge its own CR. Merging is the user's decision. Follow this contract:
...
3. Open the CR: `kortix cr open --title "..." --description "..."`. Inside a session sandbox, `--head` and `--session` default to `$KORTIX_BRANCH_NAME` and `$KORTIX_SESSION_ID`. `--base` defaults to the Workspace default branch.
...
A unique persisted index assigns the CR short number within its Workspace.
...
5. On success, Kortix sets `status` to `merged`, records `merged_at`, `merged_by`, and `merge_commit_sha`, and invalidates the Workspace git cache.
...
All canonical routes sit under `/v1/workspaces/:workspaceId/change-requests`.
...
Read routes need read access to the Workspace. Write routes need write access.
Each write action also needs a capability. The `project.*` action strings below
remain stable IAM wire values.
