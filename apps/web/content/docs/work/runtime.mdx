...
- Kortix cuts the branch from `base_ref`, which defaults to the Workspace default branch, at session-create time.
...
The agent finishes with `kortix sessions reload "$KORTIX_SESSION_ID" --workspace "$KORTIX_WORKSPACE_ID" --no-repo --force --yes`. `--no-repo` is required because the agent already reconciled the branch. The reload replaces the OpenCode runtime after the replacement becomes healthy. It ends the current turn, so send `continue` after the runtime returns.
...
Kortix injects these variables at boot, alongside the Workspace [secrets](/docs/workspace/secrets), which arrive as plain environment variables.

| Variable | What |
| --- | --- |
| `KORTIX_WORKSPACE_ID` | UUID of this Workspace. |
| `KORTIX_PROJECT_ID` | Deprecated alias for `KORTIX_WORKSPACE_ID`. |
...
| `KORTIX_DEFAULT_BRANCH` | The Workspace default branch. |
...
| `KORTIX_WORKSPACE_SECRET_NAMES` | Comma-separated names of the Workspace secrets. |
| `KORTIX_PROJECT_SECRET_NAMES` | Deprecated alias for `KORTIX_WORKSPACE_SECRET_NAMES`. |
...
| `KORTIX_SANDBOX_TOKEN`, `KORTIX_TOKEN`, `KORTIX_CLI_TOKEN` | The token family. See below. |
...
`KORTIX_SANDBOX_TOKEN` is not a Workspace API token. The `kortix` CLI does not
read it, and Workspace-scoped API routes reject it. Use `KORTIX_CLI_TOKEN` for
any call that should act as the launching user.

## Pushing from a session

The daemon fetches a short-lived clone credential from
`GET /v1/workspaces/:id/git/clone-credential`, authenticated with
`KORTIX_SANDBOX_TOKEN`. No static git token sits in the environment. With that
credential, `git push origin HEAD` sends commits to the session branch. Landing
on the default branch requires a merged [change request](/docs/work/change-requests).

## The agent runtime

The daemon launches OpenCode as `opencode serve --port 4096 --hostname 127.0.0.1`, with `OPENCODE_CONFIG_DIR` set to the Workspace config directory (default `.kortix/opencode`) inside the cloned repo. See [Agents](/docs/workspace/agents) for how a session picks an agent and its config.
...
If the Workspace has no Dockerfile, Kortix builds sessions from a bare `ubuntu:24.04` image plus the runtime layer below.
...
Set exactly one of `dockerfile` or `image` on each entry. Dockerfile paths must
stay inside the repository. Set `default` to the template slug your sessions
should use. Omit it to use the platform default image. Older Workspaces on
`kortix.toml` follow the same fields. See [legacy `kortix.toml`](/docs/workspace/legacy-toml).
Full field list: [manifest reference](/docs/workspace/manifest).
