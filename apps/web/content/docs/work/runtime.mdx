...
The daemon keeps the session's own work. It skips step 1 when the session has uncommitted edits or its own commits under the config directory, and reports `agent_files: kept-yours`. Two kinds of file do not count as session work, because the platform writes them: the `@opencode-ai/plugin` version in the config directory's `package.json`, and each skill directory that the managed-skill overlay ships. The daemon restores both after the sync. A file that `base_ref` deleted is deleted in the session too.
