...
The convergence is automatic and runs in the background. It never ends a turn. Directly after a wake, OpenCode needs a few seconds before it answers; Kortix retries after 5, 10, 15, and 30 seconds. If the session is mid-turn, Kortix retries after 6 minutes and again after 20 minutes. If the session stays busy, the next resume tries again. `kortix sessions reload` runs the same operation on demand.
