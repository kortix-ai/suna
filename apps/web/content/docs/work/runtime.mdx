...
3. If step 1 replaced a file, or the etags differ, the API pushes the compiled config. The push restarts OpenCode. If neither is true, nothing restarts.

The convergence is automatic and runs in the background. It never ends a turn. If the session is mid-turn, Kortix retries after 6 minutes and again after 20 minutes. If the session stays busy, the next resume tries again. `kortix sessions reload` runs the same operation on demand.
