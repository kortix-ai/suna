...
- **Running a tool, reasoning, or between steps.** The prompt steers. The server
  places it into the running turn and the model reads it at its next step
  boundary. The work in progress is kept and one answer accounts for both.
- **Streaming text.** A text stream has no step boundary, so nothing reads a
  steer until the text finishes. The server ends that response immediately and
  runs the prompt as the next turn. Only a prompt submitted after the response
  started ends it; a prompt that was already waiting steers or waits.

Reasoning and the answer text are one step. A prompt that steers while the
model is still reasoning is read after that step's text finishes.
...
even when the removed prompt was already waiting before that turn started.
