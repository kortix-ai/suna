...
| `useProjectSessionPages(projectId, { scope?, pageSize? })` | The project's sessions as a TanStack infinite query: `data` is the loaded sessions, de-duplicated; call `fetchNextPage()` while `hasNextPage`. |
| `useProjectSessionRow(projectId, sessionId)` | One session row: the copy a loaded session page holds, else `GET /sessions/:id`. Use it instead of searching a full session list. |
