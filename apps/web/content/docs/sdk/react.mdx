...
or queued. A row with no placement and a held row are never grouped, and the
two placements never share a group.

Pass `placement: 'composer'` to keep it in the editable list above the input.
It waits for the active response to finish, never steers, and never ends a
response. When the response finishes, EVERY waiting composer row is delivered
together as one grouped turn — one reply addresses all of them, in order — the
same `noReply` grouping the transcript placement uses. Each placement keeps
submission order.
