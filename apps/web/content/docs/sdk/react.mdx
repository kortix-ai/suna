...
create.mutate({ project_id: projectId, title: 'Fix the flaky test', verification_condition: 'CI green' });
```
...
const notifications = computeNotifications(events, identity.handle, readLastSeen(projectId, identity.handle));
```
