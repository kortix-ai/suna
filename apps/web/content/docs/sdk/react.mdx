...
The server runs pending prompts in order after the active turn completes.
...
Placement changes presentation only. Both forms run automatically.
...
inbox. This keeps Stop available. When `pendingDelivery` is `true`, show a queued
status instead of a thinking indicator. Active runtime turns and fresh runtime
activity clear that flag. A timed-out cancel does not confirm that execution stopped.
...
Both use one automatic FIFO. Waiting prompts show their delivery state and remain
after delivered turns. Queue List entries remain editable until delivery starts.
...
acceptance request is unresolved. Quick Queue shows Sending until the inbox confirms
the prompt, then Waiting until delivery. Queue List uses the same acceptance states.
The server inbox continues to own execution order.
