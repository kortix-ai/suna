...
message sync, the prompt mutations, and the OpenCode-id resolver. A normal host
should use `useSession` and never touch them. They remain exported only for the
in-progress `apps/web` migration and advanced cases; new hosts: do not import
them.
</Callout>
