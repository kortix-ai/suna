...
<Callout type="warn">
  **Internal plumbing.** These are the pieces `useSession` wraps — the SSE provider, message sync,
  the prompt mutations, and the runtime-session resolver. A chat-focused host uses `useSession` and
...
| `useSendRuntimeMessage()` · `useAbortRuntimeSession()`                                     | prompt · abort through the active runtime client                                                                                                                                     |
