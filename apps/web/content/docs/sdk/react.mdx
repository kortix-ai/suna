...
`useSession` is the batteries-included binding — most chat UIs never need
anything below it. The underlying pieces are exported too, for hosts that
want just one of them: `useAcpSession({ projectId, sessionId,
runtimeSessionId? })` for the raw `AcpSession` store binding without the
`/start` lifecycle, `useModelPicker({ projectId, agentName })` for the
harness-aware model picker (see [below](#harness-aware-model-pickers)), and
`usePermissionPolicy(projectId)` for the persistent, deny-by-default project
permission policy.
...
`s.acp` is the same snapshot shape [`AcpSession`](/docs/sdk/acp#acpsession)
exposes framework-free — see [ACP](/docs/sdk/acp) for what's inside
`acp.envelopes` and the projections (`projectAcpChatItems` and friends) that
derive from it.
...
<Callout type="warn">
  **Internal plumbing.** These are the pieces `useSession` wraps — direct ACP transport, native
  runtime readers, and the runtime-session resolver. A chat-focused host uses `useSession` and
...
| hook                                                                                      | composed role                                                                                                                                                                        |
| ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `useAcpSession({ projectId, sessionId, runtimeSessionId })`                               | own the ACP conversation transport directly — bootstrap, live stream, envelope batching, optimistic send/respond/cancel echoes                                                      |
