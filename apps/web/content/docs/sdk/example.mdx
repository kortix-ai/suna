...
async function main() {
  ...
  // 4. Ready the session. This provisions or resumes the real cloud runtime.
  //    ensureReady() polls retriable stages until ready. The callback receives
  //    the server's stage and reason values for accurate boot progress.
  const { runtimeSessionId } = await session.ensureReady({
    deadlineMs: 300_000,
    onProgress: ({ stage, reason }) => console.log('boot:', stage, reason),
  });
...
The first `ensureReady()` call on a fresh session provisions a real cloud
runtime. The default deadline is five minutes. Later calls resume the same
runtime and finish faster.
