...
The existing session composer enables uploads when its running worker advertises
image support. First-prompt creation screens remain gated. Documents retain the
environment upload path. Native tool-result images are not implemented yet.
