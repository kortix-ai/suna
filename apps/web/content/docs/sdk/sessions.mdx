...
`ensureReady()` polls retriable boot stages for up to five minutes by default.
It throws `RUNTIME_UNAVAILABLE` when the deadline expires or the server returns
a terminal stage. See [Wait for a cold boot](#wait-for-a-cold-boot).
...
## Wait for a cold boot

`ensureReady()` polls the session's `/start` endpoint through retriable
provisioning and runtime boot stages. On a warm session it resolves
immediately. On a cold boot it waits up to five minutes by default.

Concurrent calls for the same session share one start operation. Every
registered `onProgress` callback receives the observed server payloads.

```ts
const ready = await session.ensureReady({
  deadlineMs: 300_000,
  pollIntervalMs: 1_500,
  onProgress: ({ stage, reason }) => {
    renderBootProgress({ stage, reason });
  },
});
```

`RUNTIME_UNAVAILABLE` means the deadline expired or the server returned a
terminal stage. Inspect the last `stage` and `reason` from `onProgress`.
...
The runtime `sessionID` here is not the session ID you pass to
...
`RUNTIME_UNAVAILABLE` is the second error. It means `ensureReady()` reached its
deadline or received a terminal boot stage. See
[Wait for a cold boot](#wait-for-a-cold-boot). In React, `useSession` exposes
the boot state through `phase` and `reason`.
