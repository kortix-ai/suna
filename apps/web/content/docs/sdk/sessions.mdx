...
## Stage (Monitoring board)

`stage` is where a session is in its work, not whether its sandbox is alive.
The Monitoring page (feature flag `monitoring`) renders every visible session as
one card in one of six columns: `backlog`, `planning`, `ready`, `in_progress`,
`review`, `done` (`SESSION_STAGES`, in that order). A session with `stage: null`
renders in Backlog.

```ts
import { SESSION_STAGES, sessionTriggerSlug } from '@kortix/sdk';

await s.setStage({ stage: 'ready', needs_approval: true, note: 'Plan in PLAN.md' });

const session = await s.get();
session.stage?.value;          // 'ready'
session.stage?.needs_approval; // true — parked until a human approves
session.stage?.updated_by;     // 'agent' or a user id
sessionTriggerSlug(session);   // the trigger that created it, or null
```

Inside a sandbox the agent moves its own card with
`kortix sessions stage ready --needs-approval`. `setStage` returns `403` with
`code: 'feature_disabled'` until the project enables the flag. Only the
session's agent moves the card: a person's `setStage` is accepted solely to
approve (`in_progress`) or send back (`planning`) a card parked in `ready`
with `needs_approval`, and answers `403` with `code: 'stage_agent_only'`
otherwise. In React, `useSetSessionStage(projectId)` is the same call as a
mutation that refreshes every session list.

An agent that parks its card by asking a question is waiting on that
question, not on a new prompt. Read and answer it with the durable question
calls — the answer reaches the agent as its next turn and moves a parked card
to `in_progress`:

```ts
import { answerSessionQuestion, getSessionOpenQuestion } from '@kortix/sdk';

const open = await getSessionOpenQuestion(projectId, sessionId); // or null
if (open) {
  await answerSessionQuestion(projectId, sessionId, {
...
The durable question exists only when the sandbox daemon relayed the ask. When
it did not, the agent's `question` tool may still be blocked inside the
running sandbox; `answerSessionRuntimeQuestion(projectId, sessionId, text)`
answers that one directly through the session's own runtime and returns
`'answered'` or `'none'`. The Monitoring board tries the durable question, then
the runtime question, then falls back to a prompt.
