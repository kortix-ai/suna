...
The controller loads the newest 50 messages first. `loadOlder()` follows the
server cursor in pages of 100. `loadHttpSessionHistory()` follows every cursor
for explicit exports.

Each transcript controller binds one OpenCode session id to one runtime URL.
When the last React consumer leaves, automatic retries stop. A final turn-end
read can finish on its original runtime. Its failure cannot restart retries.
The hook creates no controller until it has a runtime URL. Server mirror
messages can render during that wait.

OpenCode's JSON `404` with `name: 'NotFoundError'` produces
`SessionNotFoundOnRuntimeError`, available from `@kortix/sdk`. The controller
keeps `freshness: 'error'` and stops reading that session/runtime pair.
Reprovisioning creates a new controller for the replacement runtime.
Proxy HTML and not-running `404` responses remain wakeable.
...
// Render uploads.attachments; disable Send when !uploads.canSend.

async function submit() {
  const parts = uploads.getReadyParts();
  const ids = uploads.getSnapshot().attachments.map((item) => item.id);
...
`getReadyParts()` checks readiness synchronously. Call it in the submit handler
to block a same-tick paste-and-Enter before the next React render.
The returned file part contains `attachment_id`, `filename`, and `mime`.
...
Acknowledged bytes determine progress. `processing` can remain at 100% until the
server completes the upload. Limits are 50 MiB per file, 100 MiB per message,
and 20 files. Files must be nonempty. Two files upload concurrently by default.

`retry(id)` resumes the same handle. `abort(id)` cancels unfinished work.
`remove(id)` deletes an unbound upload; a bound upload returns a typed 409.
`forget(ids)` clears accepted selections without deleting server objects.
Call it only after successful submission. Preserve selections when sending fails.
The hook aborts unfinished work on unmount or project change, and preserves ready
uploads so server command binding can finish.

Persist only ready `item.attachment` metadata in a user/project-bound draft.
`restore(metadata)` validates the fields and expiry. Never persist File objects,
blob URLs, or signed URLs. Expired restored files must be attached again.
Abandoned uploads expire after 24 hours.
...
Retain `onUpload` metadata for manual same-ID resumption. Completion retries typed
processing responses and retryable server failures for up to five minutes, plus
the current request's 30-second deadline. Caller aborts and client timeouts do not
auto-retry. Each chunk is at most 64 KiB and retries at most twice.
