...
`code: 'feature_disabled'` until the project enables the flag, and a sandbox
caller may only move its own session. In React, `useSetSessionStage(projectId)`
is the same call as a mutation that refreshes every session list.
