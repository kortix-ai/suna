...
- `s.transcript(options?)` — a compact server-side transcript (text and tool calls, no tool inputs or outputs). This works with a project-scoped session token, so use it from a scoped or embedded host.

## Readiness is a handshake

A session's sandbox is not always running. Before you send a prompt, call
`ensureReady()`. It provisions the sandbox if needed, waits for the runtime
to boot, and returns the resolved runtime.

```ts
const { opencodeSessionId, runtimeUrl, sandboxId } = await s.ensureReady();
```

`ensureReady()` is idempotent. If two callers call it for the same session
at the same time, the SDK sends one `start` request and both callers get the
same result.

On a cold boot, the sandbox can take longer to start than `ensureReady()`
waits for. When that happens, it throws an `ApiError` with
`code: 'RUNTIME_UNAVAILABLE'`. This means the sandbox is still provisioning,
not that it failed. Retry `ensureReady()` in a loop with a deadline. See
[Error handling](/docs/sdk/error-handling) for the retry pattern.
