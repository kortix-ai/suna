...
kortix.runtime(); // the runtime client for the active runtime (escape hatch)
```
...
| `auth.discover(input)`                                              | preview auth advertised by OpenAPI, Postman, or an endpoint     |
...
`p.connectors.discover` browses the direct integrations.sh catalogue and resolves a record's domain surfaces:

| method                           | what                                                   |
| -------------------------------- | ------------------------------------------------------ |
| `discover.list(query?, cursor?)` | searchable direct OpenAPI, MCP, GraphQL, and CLI page |
| `discover.detail(id)`            | normalized executable and documentation-only variants |

`p.connectors.pipedream` is the optional managed-OAuth path. `listApps` returns OAuth apps only; API-key apps should connect directly instead:

| method                        | what                         |
| ----------------------------- | ---------------------------- |
| `pipedream.listApps(params?)` | browsable OAuth app catalog  |
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `list()`                           | all triggers                                                                                                                   |
| `create(input)`                    | create — `{ name, type: 'cron' \| 'webhook', prompt_template, slug?, agent?, model?, enabled?, session_mode?, session_id?, cron?, run_at?, timezone?, secret_env? }` |
| `update(triggerId, input)`         | edit a trigger                                                                                                                 |
| `remove(triggerId)`                | delete a trigger                                                                                                               |
| `fire(triggerId)`                  | run it now                                                                                                                     |
| `setActivation(paused)`            | pause / resume all of the project's triggers server-side; per-trigger enablement uses `update(triggerId, { enabled })`         |
...
then commits it as `default_agent` in the project&apos;s `kortix.yaml`. New
...
`kortix.runtime()` returns the structural daemon/runtime client for the active runtime.
Prefer `kortix.session(pid, sid).runtime`, the same client scoped to a session.
Conversation traffic should use the ACP-first session facade (`send`, `abort`,
`stream`) unless you explicitly need a lower-level daemon helper.
