...
<Callout>
  Connectors are how tools reach a session (the **Executor** surface). The runtime-side MCP
  management hooks are being folded into this — see the [note in React
  hooks](/docs/sdk/react#mcp-servers). The deeper Executor-connector surface is still being brought
  into the SDK.
</Callout>
...
<Callout type="warn">
  `apps.updateConfig(input)` still exists for parity with the underlying client but is
  **deprecated** — use `p.updateExperimentalFeature('apps', enabled)` instead.
</Callout>
