...
<Callout>
Connectors are how tools reach a session (the **Executor** surface). A logical
connector remains manifest/project configuration; `profiles` select the
concrete identity for a session. Session creation accepts
`connector_bindings: { alias: { profile_id } }`. Credentials remain encrypted
in the control plane and are resolved for each Executor request, so profile
revocation is immediate and no credential enters the sandbox.
</Callout>
