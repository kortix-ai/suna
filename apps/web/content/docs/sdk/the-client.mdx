...
kortix.runtime()         // structural daemon/runtime client for the active runtime (escape hatch)
```
...
<Callout type="warn">
`apps.updateConfig(input)` still exists for parity with the underlying client
but is **deprecated** — use `p.updateExperimentalFeature('apps', enabled)` instead.
</Callout>
