...
a prompt, and turn classification. Each numbered step links to a concept page.
...
<Callout>
  The first `ensureReady()` call on a fresh session provisions a real cloud sandbox. Expect the
  ready step to take a while on the first run. Later runs resume the same sandbox and are fast.
</Callout>
...
| 8    | `classifyTurn` and the exhaustive part union         | [Turns](/docs/sdk/turns)           |
...
`packages/sdk/examples/`: a minimal client, streaming, a server wrapper,
transcript rendering, and files and secrets.
