---
title: Modules
description: The @kortix/sdk subpath imports — ACP, files, session runtime, runtime helpers, auth, REST, and the live stores.
---
...
| import | what |
|---|---|
| `@kortix/sdk/acp` · `@kortix/sdk/acp/transcript` | ACP client, protocol types, project-session endpoints, and transcript projections |
...
| `@kortix/sdk/runtime-client` | structural daemon/runtime helpers and wire types |
...
(see [below](#other-subpaths-internal-plumbing)) tracks low-level health/reconnect
...
Host code imports only from `@kortix/sdk/*` — never a native harness SDK, never
raw `backendApi` / `authenticatedFetch` from elsewhere.
</Callout>
...
`getRuntimeClient()` returns the structural daemon/runtime client for the
**active** session runtime, with auth injected. Conversation traffic is ACP-first;
this module is the lower-level escape hatch for daemon helpers that are not yet
covered by the session facade.
