...
if (result.ok && isRuntimeReady(result.health)) {
  // the runtime is up and the harness is ready
}
```
...
Harness conversation state is exposed only through the session-scoped ACP
surface (`kortix.session(projectId, sessionId)` and `useSession`). The removed
`runtime-client` subpath and its OpenCode-shaped global/session/provider client
do not have a compatibility replacement. Use typed platform clients for
control-plane data and ACP transcript projections for live turns.
...
UI, prefer the [hooks](/docs/sdk/react) (`useSession`) — they derive
