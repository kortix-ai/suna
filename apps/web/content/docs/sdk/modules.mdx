...
Session-specific identities use server-side connection profiles. An operator
can reconcile a managed external profile, while a normal member calls
`profiles.reconcileMember({ connector_alias, label })`; member ownership is
always derived from the bearer token. The session supplies only the profile id:
...
be available to the caller, and remain within the agent's manifest connector
grant. Member profiles are owner-only even for managers and force the session
to remain private. Revoking one denies the next Executor catalog/call request
without exposing the credential to runtime.
