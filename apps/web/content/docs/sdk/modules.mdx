---
title: Modules
description: 'The functional areas inside @kortix/sdk — ACP, files, session runtime, runtime helpers, auth, REST, turns — all importable from the canonical root entry.'
---
...
| ACP                  | client helpers, protocol types, and transcript projections | root and `/acp`                                                                        |
| runtime helpers      | structural daemon/runtime helper client and wire types | root and `/runtime-client`                                                             |
...
<Callout type="warn">
  ...
  code imports only from `@kortix/sdk/*` — never a native harness SDK, never raw `backendApi` /
...
## `@kortix/sdk/runtime-client`

`getRuntimeClient()` returns the structural daemon/runtime client for the **active**
runtime, with auth injected. Conversation traffic is ACP-first; this module is the
lower-level escape hatch for daemon helpers that are not yet covered by the session
facade.

```ts
import { getRuntimeClient, type RuntimeClient, type Session, type Part } from '@kortix/sdk/runtime-client';

const client = getRuntimeClient();
const { data } = await client.session.list({ limit: 100 });
```

Also `getRuntimeClientForUrl(url)` (a client pinned to a specific runtime),
`dropRuntimeClientForUrl(url)`, and `resetRuntimeClient()` (used on a runtime
switch). **When to use:** a daemon/runtime call the facade doesn't yet wrap.
...
wrapper metadata. Kortix persists it across replacement runtimes and injects
...
```ts
import { useServerStore, getActiveRuntimeUrl } from '@kortix/sdk/server-store';
import { useSyncStore } from '@kortix/sdk/sync-store';

const runtimeUrl = getActiveRuntimeUrl(); // outside React
...
`deriveSubdomainOpts`, `getActiveDbSandboxId`, `getActiveRuntimeUrl`,
...
- **`@kortix/sdk/runtime-pending-store`** — a zustand store of in-flight
  permission/question requests from the runtime (`useRuntimePendingStore`),
...
- **`@kortix/sdk/runtime-errors`** — normalizes runtime `ConfigInvalidError`
  payloads into readable config-error helpers.
