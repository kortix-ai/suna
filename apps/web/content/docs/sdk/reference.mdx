...
API requests with semantic session, executor, approval, and computer events.
...
connector arguments in the centralized event. Executor events can include a
...
#### `p.agents.profile(agentName)` — unified capability profile

The profile handle binds a canonical agent name. It manages one shared server
draft across instructions, integrations, private knowledge, skills,
automations, and advanced settings.

| method | what |
| --- | --- |
| `get()` | read the published profile, current draft, sources, revision, and active editors |
| `updateDraft({ expectedRevision, sections })` | replace draft sections with optimistic concurrency protection |
| `preview()` | return the deterministic impact summary, risk changes, and technical diff |
| `testDraft(input)` | create an expiring branch-backed test session |
| `publish(input)` · `discard(input)` | update one change request · discard the shared draft |
| `skills.importArchive(input)` · `skills.installMarketplace(input)` | stage a validated archive · stage a marketplace skill |
| `skills.importGitHub(input)` · `skills.generate(input)` | stage a GitHub folder · create a skill from a brief |
| `knowledge.list()` · `knowledge.createSource(input)` | list private sources · add a URL or connected resource |
| `knowledge.uploadFile(input)` | upload and complete one private source through signed storage |
| `knowledge.sync(sourceId)` · `knowledge.revoke(sourceId)` | queue synchronization · revoke access immediately |
| `pauseAutomation(slug)` | pause a schedule immediately without changing the published manifest |

`updateDraft`, `testDraft`, `publish`, and `discard` require the current
revision. A stale write returns HTTP `409` with conflicting sections and active
editors. High-risk publication also requires `acknowledgeHighRisk: true`.

```ts
const profile = p.agents.profile('researcher');
...
console.log(preview.impact.data_access);
```

Private knowledge is distinct from `.kortix/memory`. A source becomes available
to normal sessions only after its assignment merges into the default branch.
Session retrieval never accepts an agent name:

```ts
const result = await kortix.session(projectId, sessionId).knowledge.search({
  query: 'What is the refund policy?',
});

const citedPassage = await kortix
  .session(projectId, sessionId)
  .knowledge.read(result.results[0].citation.citation_id);
```

Search returns eight cited results by default. `mode: 'lexical'` and
`degraded_reason` make embedding fallback visible.
...
| `setAuthorizationStrategy(slug, strategy)` | select `project` or `user` authorization ownership |
...
| `authorizations.list()` · `authorizations.reconcile(input)` | list · create/update connected accounts |
| `authorizations.updateCredential(authorizationId, input)` | rotate an authorization credential |
| `authorizations.revoke(authorizationId)` · `authorizations.activate(authorizationId)` | deny · restore an authorization live |
...
A connector profile defines the tool, provider app, authorization strategy, and
policies. `authorizations` stores its connected accounts. A session can select
one with `connector_bindings: { alias: { authorization_id } }`. Credentials
stay encrypted and resolve per request.

The deprecated `profiles` facade aliases `authorizations`. New code uses
...
`create(input)` accepts `connector_bindings` keyed by connector-profile slug.
Each value names an `authorization_id`. It also accepts `secrets` for
