...
#### `p.goals` — authored goals and metric observations

Goals come from the project's default-branch v2 manifest. Observation writes
create PostgreSQL time-series state. They do not edit the manifest.

| method | what |
| --- | --- |
| `list()` | Return `{ goals, errors }`, including per-entry manifest parse diagnostics. |
| `get(slug)` | Return one authored goal. |
| `health(slug)` | Return deterministic aggregate and per-metric health with evaluation identity. |
| `push(slug)` | Request the goal's generated trigger. Returns `evaluation_id`, `evaluation_state`, and delivery state. |
| `observations.record(slug, input)` | Record `{ evaluation_id, metric, value, source, observed_at? }`. Session attribution comes from authentication. |
| `observations.list(slug, options)` | List one metric by `{ metric, from?, to?, limit? }`. |

`observed_at` defaults to now. Observation-list `to` defaults to now. Its
`from` defaults to 30 days before `to`. `limit` is 1–10,000.

An observation is an authenticated metric assertion. It never completes a goal.
The SDK has no goal-completion method. Goal status changes only through the
reviewed `kortix.yaml` declaration.

```ts
const project = kortix.project(projectId);
const { goals, errors } = await project.goals.list();
const pushed = await project.goals.push('reduce-api-latency', { idempotencyKey: crypto.randomUUID() });
const { observation } = await project.goals.observations.record('reduce-api-latency', {
...
Each declared metric is `unmeasurable` until an evaluation-bound observation
exists. Three consecutive fired evaluations with the same finite value produce
`stalled`. Other observed metrics produce `measuring`. The aggregate prioritizes
`stalled`, then `unmeasurable`, then `measuring`. `desired_status` is copied from
Git and never inferred from health.

A `ProjectGoal` contains `slug`, `path`, `title`, `done_when`, `status`,
`push_cron`, `timezone`, `agent`, and `metrics`. Status is `active`, `achieved`,
`paused`, or `abandoned`. Metric direction is `increase` or `decrease`.

#### `p.tasks` — generated task state

| method | what |
| --- | --- |
| `current()` | Return the task bound to the authenticated session principal. |
| `list(options?)` | List tasks by `goal_slug?`, `statuses?`, and `limit?`. |
| `get(taskId)` | Return one project task. |
| `create(input)` | Create a task, or reuse the matching `origin_fingerprint`. Returns `{ task, created }`. |
| `claim(taskId, input)` | Claim ready work with `{ session_id, lease_seconds? }` and set its status to `doing`. All dependencies must be `done`. The lease defaults to 900 seconds. |
| `releaseClaim(taskId, input)` | Idempotently release an unused coordinator claim after launch compensation. |
| `registerWorker(taskId, input)` | Bind one spawned worker with `{ session_id, worker_session_id, prompt, contract }`. The API persists the immutable bounds and initial prompt atomically. |
| `recordProgress(taskId, input)` | Persist `{ session_id, worker_session_id, settlement_id, ref }` for the matching live worker binding. |
| `settleNoProgress(taskId, input)` | Submit `{ session_id, worker_session_id, settlement_id, reason }`. The first distinct settlement queues one continuation. The second or an exhausted bound blocks and escalates. |
| `reviseContract(taskId, input)` | Revise intent, constraints, verification requirements, or review policy. |
| `evidence.list/add(taskId, ...)` | Read or append candidate-scoped immutable evidence. |
| `requestCompletion(taskId, input)` | Ask the server to verify the current contract and move the task to `done` or `review`. |
| `blockers.list/create/resolve(taskId, ...)` | Manage typed, idempotent blockers and reminder state. |
| `events(taskId)` · `sessions(taskId)` | Read the append-only timeline and session lineage. |
| `messages.list/send/acknowledge(taskId, ...)` | Exchange durable coordinator, worker, and verifier messages. |
| `cancel(taskId, input)` | Cancel task responsibility as a human caller. |
| `refinements.list/propose/rollback(...)` | Propose and roll back scoped harness improvements. |
| `complete(taskId, input)` | Complete a legacy task with cited evidence. V1 tasks use `evidence.add()` and `requestCompletion()`. |
| `block(taskId, input)` | Block with `{ session_id, blocker }`. |

`create(input)` requires `title` and `origin`. `goal_slug` is optional. Set it
only when the task belongs to a Git-authored goal. Optional fields include the
outcome contract, hierarchy, assignee, dependencies, priority, and
`origin_fingerprint`. A creation status can be `backlog` or `todo`. Claim and
transition operations own all later statuses. The two assignee fields are
mutually exclusive.

```ts
const { task, created } = await project.tasks.create({
  title: 'Profile the slowest endpoint',
  intent: 'Identify and verify the largest latency source.',
  verification_requirements: [{
    id: 'profile',
    kind: 'artifact',
    description: 'An independent profile identifies the slowest endpoint.',
    required: true,
  }],
  status: 'todo',
  priority: 10,
  origin: 'session',
  origin_fingerprint: 'profile:slowest-endpoint:v1',
});
...
const registration = await project.tasks.registerWorker(task.task_id, {
  ...
  prompt: 'Profile the endpoint and return a verifier reference.',
  contract: {
...
console.log(registration.worker.state); // "queued" or "drained"
```
...
await project.tasks.recordProgress(task.task_id, {
  ...
  ref: 'profile-report://runs/4f34a2d',
});
await project.tasks.evidence.add(task.task_id, {
  requirement_id: 'profile',
  kind: 'artifact',
  ref: 'profile-report://runs/4f34a2d',
  summary: 'The independent profile confirms the result.',
  candidate_digest: 'sha256:4f34a2d',
  state: 'passed',
});
await project.tasks.requestCompletion(task.task_id, {
  session_id: coordinatorSessionId,
  candidate_digest: 'sha256:4f34a2d',
});
```

If the worker settles without evidence or a blocker, call
`settleNoProgress()` with the current server-owned `task.liveness_turn_id`. Retrying the same ID returns
the same decision. The response contains `action`, `command_id`,
`measured_usage`, and the updated task.

A `ProjectTask` contains its project ID, optional goal ID, hierarchy (`parent_id`),
dependencies (`blocked_by`), assignee, status, priority, origin, claim lease,
versioned verification contract, worker contract, current `liveness_turn_id`,
progress, settlement, escalation, result, and timestamps.
`parent_id` is structure. Only `blocked_by` declares a dependency. The API
rejects a claim held by another live session with `409`.

For a liveness-bound task, the server enforces evidence provenance and
transition order. The worker is a distinct child of the coordinator.
`recordProgress()` must match that live binding. Progress and no-progress share
one settlement ledger; the same settlement id cannot record both outcomes.
Candidate evidence must cite the persisted progress result. The terminal result includes server-owned
coordinator, worker, progress-ref, and timestamp provenance. A general task
without `control_plane_version` preserves the legacy `complete()` contract.
V1 tasks cannot bypass the verification gate through that legacy route.

The generic API does not dereference or evaluate arbitrary evidence refs. Their
content remains an authenticated caller assertion. Goal observation `source`
and `value` fields have the same boundary. This SDK has no goal-completion route.
A goal becomes `achieved` only through its reviewed Git declaration, not from an
observation or a model response. Independent domain verifier adapters that fetch
external systems remain outside this generic SDK contract.
