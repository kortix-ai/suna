...
kortix.marketplace; // the public template catalog — list, get
...
### Marketplace — `kortix.marketplace`

The public template catalog. Reading needs no token; installing is per-project
(`project(id).marketplace`). See [Marketplace](/docs/sdk/marketplace).

| method                         | what                                                                                   |
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `list(options?)` · `get(slug)` | every template; `options.q` narrows by title, description, repo or slug · one template |

`get` throws a `MarketplaceError` with `status: 404` for a slug that is not in
the catalog.
...
#### `p.marketplace` — install a template

Behind the `marketplace` [feature flag](/docs/feature-flags/marketplace): the
call answers `403` `feature_disabled` while it is off. See
[Marketplace](/docs/sdk/marketplace).

| method          | what                                               |
| --------------- | -------------------------------------------------- |
| `install(slug)` | start the agent session that installs the template |

That is the whole surface. `install` returns a **session id**, not a result:
the agent opens one change request a human reviews, the project changes when it
merges, and reverting it is the uninstall. There is no installed list, no
uninstall method and no activation method. Enable the template's triggers one at
a time through `p.triggers` once the change request merges.
