...
the platform API. The `workspace(id)` and `session(workspaceId, sessionId)`
handles bind ids so you never repeat them.
...
| method | what |
| --- | --- |
| `audit.log(accountId, filters?)` | list events by workspace, session, actor, source, outcome, request, correlation, resource, action, or time |
...
Each event includes `workspace_id`, `session_id`, `actor_type`, `source`, `outcome`,
...
| `provision(input)` | new workspace on a new Kortix-managed repo, seeded with a starter template — `{ name, account_id?, seed_starter?, starter_template?, marketplace_items?, source_item_id?, idempotency_key? }` |
...
`kortix.projects` and `kortix.project(id)` remain available as deprecated
compatibility surfaces. They retain the legacy `project_id` wire shape and
`/v1/projects` routes. New integrations use the Workspace surfaces above.
...
without a key those all create real duplicate workspaces. Reuse one key for every
attempt at a single logical create and the repeats return the workspace the first
attempt made (201, same `workspace_id`, `push_token: null`). The key identifies the
attempt, not the payload — reusing one with a different `name` returns the first
workspace and ignores the new value, so mint a fresh key per distinct create.
Creating a second workspace with the same **name** and no key still works.

A repeat that arrives while the first call is still provisioning gets `409` with
`code: 'provision_in_flight'` rather than a `workspace_id` that call may still roll
...
`sessionCosts.list()` accepts `accountId`, `workspaceId`, `limit`, and `offset`.
...
#### `workspace.tokens` — Workspace-scoped API keys
...
#### `workspace.setupLinks` — agent-minted setup links
...
#### `workspace.access` — members, invites, requests
...
`workspace.access.resourceGrants` grants a member or group access to one resource
(an agent, a skill, or a secret) instead of the whole Workspace.
...
#### `workspace.connectors` — tool connectors
...
| `setAuthorizationStrategy(slug, strategy)` | select `workspace` or `user` connection ownership |
...
`workspace.connectors.discover` browses the direct-connector catalog. It is
**experimental** and off by default. Enable it per Workspace under Customize →
Feature flags → "Connectors API Discover". Easy Connect (Pipedream) remains the
default connector marketplace.

| method | what |
| --- | --- |
| `discover.list(query?, cursor?)` · `discover.detail(id)` | search OpenAPI/MCP/GraphQL/CLI entries · one entry's detail |

`workspace.connectors.pipedream` is the optional managed-OAuth path; `listApps`
...
| method | what |
| --- | --- |
| `list()` · `set(policies)` | the Workspace policies · replace the set |

#### `workspace.triggers` — cron and webhook automations
...
#### `workspace.marketplace` / `workspace.registry` — installed items

Installs a catalog item's files onto the Workspace default branch.
...
#### `workspace.files` — repo files (read)

Read-only access to the Workspace git tree. To read and write files inside a
...
#### `workspace.changeRequests` — lifecycle and merge
...
#### `workspace.sessions` — and the session handle

| method | what |
| --- | --- |
| `list()` | the Workspace sessions |
...
#### `workspace.approvals` — the connector approval inbox
...
#### `workspace.channels` — Slack / email / voice
...
#### `workspace.modelDefaults` — default model preferences
...
#### `workspace.setDefaultAgent` — Workspace default agent

`workspace.setDefaultAgent(agentName)` checks that the agent is declared and
enabled, then sets it as `default_agent` in the Workspace `kortix.yaml`. New sessions
prefer this agent unless a user picks another one.

#### `workspace.updateFeatureFlag` — feature flags

`workspace.updateFeatureFlag(feature, enabled)` turns one feature flag on or off
for the Workspace. Pass `enabled: null` to clear the override and fall back to
the platform default. It calls `PATCH /v1/workspaces/:id/features`.

`feature` is one of `FEATURE_FLAG_KEYS` (exported from `@kortix/sdk`, typed as
`FeatureFlagKey`). The caller needs the `project.customize.write` IAM action.
That wire action remains stable for compatibility. The route answers `403`
otherwise.
...
`workspace.updateExperimentalFeature(feature, enabled)` is the **deprecated** alias. It
keeps calling the deprecated route alias `PATCH /v1/workspaces/:id/experimental`
so consumers pinned to an older deployed API keep working. Use
`workspace.updateFeatureFlag` in new code.

#### `workspace.sandbox` — templates and snapshot builds
...
`kortix.session(workspaceId, sessionId).runtime` (call `ensureReady()` first)
instead, which
