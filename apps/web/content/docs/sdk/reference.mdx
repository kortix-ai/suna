...
#### `p.subprojects` — what this project has installed

Behind the `subprojects` [feature flag](/docs/feature-flags/subprojects): every
method here answers `403` `feature_disabled` while it is off. See
[Subprojects](/docs/sdk/subprojects).
...
That is the whole surface. There is no activation method and no runs method: a
subproject has no on/off state, and a run belongs to the **trigger** that fired,
not to the subproject that contributed it. Enable triggers one at a time through
`p.triggers`, and read runs from there.
...
`uninstall` keys on the manifest `slug`, not a subproject id — the slug is what
the manifest records, and an install outlives its catalog entry.
