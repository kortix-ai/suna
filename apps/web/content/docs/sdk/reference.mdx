...
kortix.crafts; // the craft catalog — publish, browse, withdraw
...
### Craft catalog — `kortix.crafts`

The catalog of crafts — account-scoped, and distinct from
`project(id).crafts`, which is what one project has installed. A craft comes
from a GitHub repo or an uploaded `.zip`. See [Crafts](/docs/sdk/crafts).

| method | what |
| --- | --- |
| `list(options?)` · `get(craftId)` | every public craft plus your own private ones; `options.q` filters · one craft |
| `submit(input)` | index a craft from `owner/repo`, optionally `@branch-or-tag` |
| `submitArchive(input)` | index a craft from an uploaded `.zip` (`{ file, visibility? }`) |
| `remove(craftId)` | withdraw from the catalog — does **not** uninstall it from any project |

Both submit calls resolve to `{ craft, warnings }`. `warnings` never blocks: the
craft **is** indexed. Rejections carry a machine `code` —
...
#### `p.crafts` — installed crafts and their runs

What this project has installed, plus activation and run history. Behind the
`crafts` [feature flag](/docs/feature-flags/crafts): every method here answers
`403` `feature_disabled` while it is off. See [Crafts](/docs/sdk/crafts).

| method | what |
| --- | --- |
| `list()` | the installed crafts, read from the project's manifest — not the catalog |
| `install(craftId)` · `uninstall(slug)` | start the agent session that installs · removes it |
| `author(description)` | describe a craft and have one built and published |
| `setActivation(slug, enabled)` | turn ONE craft's triggers on or off |
| `runs(options?)` · `runsFor(slug, options?)` | runs across every craft · one craft's runs plus aggregate `stats` |
...
`uninstall` and `setActivation` key on the manifest `slug`, not a craft id. A
craft installs with every trigger off, so `setActivation(slug, true)` is what
