...
| `spaces` | named containers inside the project — see `p.spaces` below |
...
#### `p.spaces` — named containers inside a project

A space groups sessions, owns scheduled work, may declare agents of its own, and is granted to members or groups like an agent. Each one is a `spaces.<slug>` block of the root manifest, and that block is the source of truth, so every write below commits to `kortix.yaml`. A `Space` carries `path` (the manifest it is declared in) and `agents` — the agents usable inside it beyond the globals: the ones its block owns or references. `agentsUsableIn(roster, space)` filters any roster whose entries carry `space` (the project detail's `config.agents` do) down to what a session there may run; the React hooks accept the same `space` option.

| method | what |
| --- | --- |
| `list()` | `{ spaces, errors }` — the ones the caller can access, sorted by slug |
| `get(slug)` | one space (`404` when undeclared or inaccessible) |
| `create(input)` | declare one — commits its `spaces.<slug>` block |
| `update(slug, input)` | partial merge (`null` clears a field; the slug is immutable) |
| `remove(slug)` | delete the file, after stripping `space:` from its triggers |

`create(input)` takes `{ name, slug?, description?, agent?, sessions? }`. `name` is required; `slug` defaults to `slugify(name)`. `agent` is a default, not a binding — granting a space does not grant its agent. `sessions` is `'private'` (default) or `'shared'`; `'shared'` makes every session in the space readable by everyone granted it, with lifecycle rights unchanged.

A session takes its slug at create time and can be moved afterwards:

```ts
await p.sessions.create({ space: "marketing" });
await p.sessions.list({ space: "marketing" });
await p.sessions.list({ space: "" });   // the sessions with NO space

await p.session(sessionId).update({ space: "marketing" }); // move it in
await p.session(sessionId).update({ space: null });        // move it out
```

A move is owner-governed like sharing is (a `shared` space makes its sessions readable by everyone granted it), so it needs `can_manage_sharing`; the target must be declared (`400 SPACE_NOT_DECLARED`) and granted to the caller (`403 space_not_accessible`), and the session's own agent must be usable there (`400 AGENT_NOT_IN_SPACE`).

A space is a closed object: a member with no grant row sees none, and `sessions.create({ space })` returns `403 space_not_accessible`. Grant one through the generic resource-grant surface:

```ts
await p.access.resourceGrants.create({ resourceType: "space", resourceId: "marketing", principalType: "member", principalId: userId });
```
...
connectors. `space` joins the session to a declared space at create
time (`400` when undeclared, `403 space_not_accessible` when the caller
holds no grant) — see `p.spaces` above.
