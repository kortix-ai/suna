...
| `create(input)` | declare one — commits `kortix.yaml` |
...
| `addContext(slug, { path, content })` | commit a text file and append it to `context` |
| `removeContext(slug, path)` | drop the `context` entry (the repo file stays) |

`create(input)` takes `{ name, slug?, description?, instructions?, context?, agent?, sessions? }`. `name` is required; `slug` defaults to `slugify(name)`. `instructions` is inline markdown. `context` entries are repo-relative paths (a file, or a `dir/`). `agent` is a default, not a binding — granting a subproject does not grant its agent. `sessions` is `'private'` (default) or `'shared'`; `'shared'` makes every session in the subproject readable by everyone granted it, with lifecycle rights unchanged.

Sessions carry the slug at create time and never move between subprojects:
