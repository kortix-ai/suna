...
| `subprojects` | named containers inside the project — see `p.subprojects` below |
...
#### `p.subprojects` — named containers inside a project

A subproject groups sessions, owns scheduled work, may declare agents of its own, and is granted to members or groups like an agent. Each one is a file in the repo beside the root manifest, `kortix-<slug>.yaml`, and that file is the source of truth, so every write below commits to it. A `Subproject` carries `path` (the file) and `agents` — the agents usable inside it beyond the globals: the ones its file owns or references. `agentsUsableIn(roster, subproject)` filters any roster whose entries carry `subproject` (the project detail's `config.agents` do) down to what a session there may run; the React hooks accept the same `subproject` option.

| method | what |
| --- | --- |
| `list()` | `{ subprojects, errors }` — the ones the caller can access, sorted by slug |
| `get(slug)` | one subproject (`404` when undeclared or inaccessible) |
| `create(input)` | declare one — commits `kortix-<slug>.yaml` |
| `update(slug, input)` | partial merge (`null` clears a field; the slug is immutable) |
| `remove(slug)` | delete the file, after stripping `subproject:` from its triggers |

`create(input)` takes `{ name, slug?, description?, agent?, sessions? }`. `name` is required; `slug` defaults to `slugify(name)`. `agent` is a default, not a binding — granting a subproject does not grant its agent. `sessions` is `'private'` (default) or `'shared'`; `'shared'` makes every session in the subproject readable by everyone granted it, with lifecycle rights unchanged.

A session takes its slug at create time and can be moved afterwards:

```ts
await p.sessions.create({ subproject: "marketing" });
await p.sessions.list({ subproject: "marketing" });
await p.sessions.list({ subproject: "" });   // the sessions with NO subproject

await p.session(sessionId).update({ subproject: "marketing" }); // move it in
await p.session(sessionId).update({ subproject: null });        // move it out
```

A move is owner-governed like sharing is (a `shared` subproject makes its sessions readable by everyone granted it), so it needs `can_manage_sharing`; the target must be declared (`400 SUBPROJECT_NOT_DECLARED`) and granted to the caller (`403 subproject_not_accessible`), and the session's own agent must be usable there (`400 AGENT_NOT_IN_SUBPROJECT`).

A subproject is a closed object: a member with no grant row sees none, and `sessions.create({ subproject })` returns `403 subproject_not_accessible`. Grant one through the generic resource-grant surface:

```ts
await p.access.resourceGrants.create({ resourceType: "subproject", resourceId: "marketing", principalType: "member", principalId: userId });
```
...
connectors. `subproject` joins the session to a declared subproject at create
time (`400` when undeclared, `403 subproject_not_accessible` when the caller
holds no grant) — see `p.subprojects` above.
