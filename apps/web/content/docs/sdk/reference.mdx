...
A subproject groups sessions, gives the agent standing instructions and reference files, owns scheduled work, and is granted to members or groups like an agent. The repo manifest (`kortix.yaml` → `subprojects.<slug>`) is the source of truth, so every write below commits to the repo.
...
| `remove(slug)` | delete the block and strip `subproject:` from its triggers |
