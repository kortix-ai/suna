...
| `observations.record(slug, input)` | Record `{ evaluation_id, metric, value, source, session_id?, observed_at? }`. |
...
| `recordProgress(taskId, input)` | Persist `{ session_id, worker_session_id, ref }` for the matching live worker binding. |
...
`settleNoProgress()` with a stable `settlement_id`. Retrying the same ID returns
...
worker contract, progress, settlement, escalation, result, and timestamps.
...
`recordProgress()` must match that live binding. `complete()` must cite the
