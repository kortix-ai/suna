...
kortix.subprojects; // the subproject catalog — publish, browse, withdraw
...
| `provision(input)` | new project on a new Kortix-managed repo, seeded with a starter template — `{ name, account_id?, seed_starter?, starter_template?, idempotency_key? }` |
...
### Subproject catalog — `kortix.subprojects`

The catalog of subprojects — account-scoped, and distinct from
`project(id).subprojects`, which is what one project has installed. A subproject comes
from a GitHub repo or an uploaded `.zip`. See [Subprojects](/docs/sdk/subprojects).

| method | what |
| --- | --- |
| `list(options?)` · `get(subprojectId)` | every public subproject plus your own private ones; `options.q` filters · one subproject |
| `submit(input)` | index a subproject from `owner/repo`, optionally `@branch-or-tag` |
| `submitArchive(input)` | index a subproject from an uploaded `.zip` (`{ file, visibility? }`) |
| `remove(subprojectId)` | withdraw from the catalog — does **not** uninstall it from any project |

Both submit calls resolve to `{ subproject, warnings }`. `warnings` never blocks: the
subproject **is** indexed. Rejections carry a machine `code` —
`manifest_not_found`, `manifest_invalid`, `ref_not_found`, `repo_not_found`,
`upstream_unavailable`, `invalid_archive`, `archive_refused`.
...
`p.connectors.discover` browses the direct-connector catalog. It is **experimental** and off by default — enable it per project under [Settings → Feature flags](/docs/feature-flags) → "Connectors API Discover". Easy Connect (Pipedream) remains the default connector catalog.
...
#### `p.subprojects` — installed subprojects and their runs

What this project has installed, plus activation and run history. Behind the
`subprojects` [feature flag](/docs/feature-flags/subprojects): every method here answers
`403` `feature_disabled` while it is off. See [Subprojects](/docs/sdk/subprojects).

| method | what |
| --- | --- |
| `list()` | the installed subprojects, read from the project's manifest — not the catalog |
| `install(subprojectId)` · `uninstall(slug)` | start the agent session that installs · removes it |
| `author(description)` | describe a subproject and have one built and published |
| `setActivation(slug, enabled)` | turn ONE subproject's triggers on or off |
| `runs(options?)` · `runsFor(slug, options?)` | runs across every subproject · one subproject's runs plus aggregate `stats` |

`install`, `uninstall`, and `author` return a **session id**, not a result. Each
opens a change request a human reviews; the project changes when that CR merges.

`uninstall` and `setActivation` key on the manifest `slug`, not a subproject id. A
subproject installs with every trigger off, so `setActivation(slug, true)` is what
starts it working — and it commits to `kortix.yaml`. It is not the project-wide
pause, which is `triggers.setActivation`.
