...
| `complete(taskId, input)` | Complete with `{ session_id, evidence: [{ ref, summary? }] }`. Evidence cannot be empty. A liveness-bound task must cite its server-recorded `last_progress_ref`. |
| `block(taskId, input)` | Block with `{ session_id, blocker }`. |

`create(input)` requires `goal_slug`, `title`, and `origin`. Optional fields are
`parent_id`, `body`, `status`, `priority`, `assignee_agent`,
`assignee_user_id`, `blocked_by`, and `origin_fingerprint`. A creation status
can be `backlog`, `todo`, `doing`, `review`, or `cancelled`. The two assignee
fields are mutually exclusive.

```ts
const { task, created } = await project.tasks.create({
  goal_slug: 'reduce-api-latency',
...
A `ProjectTask` contains its project and goal IDs, hierarchy (`parent_id`),
dependencies (`blocked_by`), assignee, status, priority, origin, claim lease,
worker contract, current `liveness_turn_id`, progress, settlement, escalation, result, and timestamps.
...
`recordProgress()` must match that live binding. Progress and no-progress share one settlement ledger; the same settlement id cannot record both outcomes. `complete()` must cite the
persisted `last_progress_ref`. The terminal result includes server-owned
coordinator, worker, progress-ref, and timestamp provenance. A general task
without a liveness worker preserves the original contract: `complete()` is the
explicit evidence transition, and its evidence array cannot be empty.
