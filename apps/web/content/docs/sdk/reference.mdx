...
kortix.github; // Nango-backed GitHub connection + repo linking
...
Account-scoped GitHub connection lifecycle and repository linking through Nango.

| method | what |
| --- | --- |
| `createConnectSession({ accountId })` · `createReconnectSession(...)` | start Nango Connect · repair an existing connection |
| `refreshConnection(...)` · `disconnectConnection(...)` | reconcile provider state · disconnect |
| `getInstallation(accountId)` · `listInstallations(accountId)` | this account's connection · all account connections |
| `listRepositories(accountId, installationId?)` · `listRepositoryBranches(...)` | repositories the connection can see · branches and the GitHub default |
| `linkRepository(input)` | import a repo as a project |

`saveInstallation`, `linkInstallation`, `deleteInstallation`, and
`listLinkableInstallations` remain as deprecated compatibility methods. They do
not transmit caller-supplied GitHub credentials.
...
| `gitToken()` | deprecated compatibility call; Nango-backed GitHub projects return `git_proxy_required` |

Use the project's `git_origin_url` for GitHub clone and push operations. The
Kortix Git proxy resolves the Nango credential on the server.
