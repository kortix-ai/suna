...
service account). A shared account can be narrowed to groups and people:
`connections.list()` returns its grants in `shared_with`, and
`connectionSharedWithEveryone(connection)` answers whether everyone in the
project may use it. A narrowed account follows the private rules for the
people it names, and `usable: false` marks one the caller sees only to manage
it. Grant or revoke with `createAssignment` / `revokeAssignment`
(`object: { type: 'connection', id }`) — see
[Choose who can use a shared account](/docs/connect/connectors#choose-who-can-use-a-shared-account).
`p.connectors.accounts(slug)` lists every account this caller may reach,
default first:
...
`files.write(path, blob)` replaces the file at `path` and fails if `path` is a
folder. `files.create(path)` creates an empty file and never overwrites: when a
file or folder already exists at `path`, it throws `FileExistsError`
(`status` `409`, `code` `'FILE_EXISTS'`).

```ts
import { FileExistsError, files } from '@kortix/sdk';

try {
  await files.create('/workspace/notes.md');
} catch (error) {
  if (error instanceof FileExistsError) console.log(`${error.path} already exists`);
  else throw error;
}
```
...
| `@kortix/sdk/wire-message-id` | The OpenCode wire message-id clock alone (one module, no imports). The root exports the same names |

#### Wire message ids

A prompt's `messageId` is its position in the transcript. OpenCode orders
messages by `msg_` + a 48-bit clock (`Date.now() * 0x1000`, low 48 bits) +
14 base62 characters, and the clock wraps every 2^36 ms (about 2.2 years).
Never encode it by hand.

```ts
import { mintWireMessageId } from '@kortix/sdk';

// `after`: the ids already in this session's transcript.
const messageId = mintWireMessageId({ after: transcriptIds });
```

The mint is dated two minutes back, then lifted one tick above the newest id
in `after` that is at most one hour ahead. A caller that holds the newest
position as a clock instead of ids uses `mintWireMessageIdAbove({ nowMs,
newestKnownTime })`, which returns `{ id, time }`. `newestWireIdClock(ids)`
finds that clock. Every comparison is on the ring, so ids on both sides of a
wrap keep their true order.

The 48-bit clock wraps every ~2.2 years, so never compare two clocks with `>`
or `<=`: an id minted just after a wrap is numerically smaller than one minted
just before it. Use `wireIdClockDelta(clock, reference)`, the signed distance
on the ring (positive when `clock` is ahead), and `maxWireIdClock(clocks)` for
the newest of several:

```ts
import { maxWireIdClock, wireIdClockDelta } from '@kortix/sdk/wire-message-id';

const floor = maxWireIdClock([transcriptNewest, deliveredNewest]);
const stale = floor !== null && wireIdClockDelta(clientClock, floor) <= 0n;
```
