...
#### `p.agents` — agent config and reviewed creation

Agent config uses two repo homes. `kortix.yaml` stores Kortix governance.
`.kortix/opencode/agents/<name>.md` stores OpenCode behavior and the prompt.

| method | what |
| --- | --- |
| `getConfig(agentName)` | read the merged governance + behavior block |
| `preview(input)` | validate a draft and return behavior markdown plus `preview_revision` |
| `create(input)` | write `kortix.yaml` and behavior markdown to a branch, then open a change request |
| `repairBehaviorFile(agentName, input)` | create a missing behavior markdown file through a change request |

```ts
const preview = await p.agents.preview({
...
`preview` does not return the full manifest content. Use the change request
diff after `create` or `repairBehaviorFile` to review the manifest change.
