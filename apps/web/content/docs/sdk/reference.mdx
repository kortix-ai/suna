...
| `complete(taskId, input)` | Complete with `{ session_id, evidence: [{ ref, summary? }] }`. Evidence cannot be empty. |
...
await project.tasks.complete(task.task_id, {
  session_id: sessionId,
  evidence: [{ ref: 'commit:4f34a2d', summary: 'Adds the profile and result.' }],
});
```
...
result, and timestamps. `parent_id` is structure. Only `blocked_by` declares a
dependency. The API rejects a claim held by another live session with `409`.
