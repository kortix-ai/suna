...
| `push(slug)` | Request the goal's generated trigger. Returns `queued`, `fired`, or `deduped`. |
| `observations.record(slug, input)` | Record `{ metric, value, source, session_id?, observed_at? }`. |
