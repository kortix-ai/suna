...
`kortix apps access <app>` reads a policy. Add `--mode`, `--password`, `--members`, or `--groups` to update it. `kortix apps deploy` accepts the equivalent `--access`, `--password`, `--members`, and `--groups` flags.
