...
An agent-minted session token already carries its workspace scope. Use
`kortix.connectors` when the agent does not have a separate `workspaceId` value.
...
`createKortix` gives you an imperative client for workspaces, sessions, secrets, and triggers. The legacy `projects` and `project(id)` surfaces remain available for compatibility. `@kortix/sdk/react` gives you hooks for live UI data — `useSession` runs a whole session in one hook.
