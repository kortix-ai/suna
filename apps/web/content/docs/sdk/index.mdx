...
`@kortix/sdk` is the typed client for the Kortix platform. It wraps the Kortix REST API and ACP session runtimes in one interface. Your app never calls the raw API or a harness-specific client directly. The core client is fetch-based and runs in Node, Bun, and browsers.
...
2. Wait for the sandbox to accept work.
   ...
   `ensureReady()` starts or resumes the session runtime. It polls retriable boot stages for up to five minutes by default. Use its `onProgress` callback to render the server's boot status. See [Sessions](/docs/sdk/sessions).
