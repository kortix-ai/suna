...
- **Kortix config** — `kortix.yaml` at the repo root, plus any Dockerfile a sandbox template names. The platform reads this: the trigger sweep, the sandbox builder, session token minting, and the dashboard.
- **Agent config** — the agent files, skills, and harness files. The agent harness reads this, in the sandbox and locally.

| Path | What it holds |
| --- | --- |
| `kortix.yaml` | The manifest: agents and what each may access, triggers, connectors, env, sandbox templates. |
| `agents/<name>.md` | One agent: frontmatter and system prompt. `agents.<name>.file` names it. |
| `skills/<name>/SKILL.md` | One skill. Every harness loads it. |
| `memory/` | The project memory. |
| `harnesses/opencode/` | Files only OpenCode reads: `opencode.jsonc`, `plugins/`, `tools/`, `commands/`. `opencode.config_dir` points here. |

Projects created before 2026-09 keep agents and skills under `.kortix/opencode/` and memory under `.kortix/memory/`. Kortix reads both layouts.

The join between the two halves is the agent's `.md` file. A manifest key `agents.<name>` governs the file that `agents.<name>.file` names, or `agents/<name>.md` when `file` is omitted. Every behavioral field lives only in that `.md` file's frontmatter and body, never in `kortix.yaml`:
...
| `runtime`                                           | no       | `"opencode"` (the default) or `"pi"` — the agent harness a session boots inside its sandbox; `pi` runs pi-agent-core in-process in the sandbox daemon and reads the same agent `.md` files and skills. The project's `pi_harness` feature flag selects pi without this line. |
...
| `opencode.config_dir`                               | no       | default `harnesses/opencode`, then the legacy `.kortix/opencode`                           |
...
default_agent: kortix
imports:
  - triggers/                # a directory: every .yaml and .yml file below it
...
```text
imports: triggers "weekly-report" is declared in both triggers/a.yaml and triggers/b.yaml — one name, one file
```
...
| Field        | Type   | Default                                         | Notes                    |
| ------------ | ------ | ----------------------------------------------- | ------------------------ |
| `config_dir` | string | `harnesses/opencode`, then `.kortix/opencode`   | Repo-relative directory. |

When `config_dir` is not set, Kortix uses the first of `harnesses/opencode` and `.kortix/opencode` that holds an `opencode.json` or `opencode.jsonc`. That directory holds the files only OpenCode reads: `opencode.jsonc`, `plugins/`, `tools/`, and `commands/`. Agents and skills live at the repo root, in `agents/` and `skills/`. `opencode.jsonc` stays the OpenCode-native registry for plugins, MCP servers, providers, and permissions — do not duplicate those settings in the manifest. See [Agents](/docs/project/agents).
...
| Field                 | Default | Notes                                                                                                                                                                                                     |
| --------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| _(map key)_           | —       | `[a-z0-9][a-z0-9_-]{0,127}`, unique per project. Without `file`, it is also the `.md` filename.                                                                                                           |
| `file`                | `agents/<name>.md` | Repo-relative path of the agent's `.md`. See [The agent file](#the-agent-file).                                                                                                                |
...
`agents.<name>.file` names the agent's `.md`, relative to the repo root.
...
- The path must not start with `/` or `-`. It must not contain a `.` or `..` segment, or an empty segment.
- Each segment uses only letters, digits, `_`, `-`, `.`, and spaces.
- When `file` is omitted, Kortix reads `agents/<name>.md`, then `.kortix/opencode/agents/<name>.md`. With `opencode.config_dir` set, the second path is `<config_dir>/agents/<name>.md`.
- When the dashboard saves an agent that has a `.md`, it writes `file` with that path.
- The API refuses a request that changes `file` (`400`, code `invalid_config`). To move an agent, move its `.md` and update `file` in `kortix.yaml` in the same commit.

`kortix validate` and the change request merge check reject an invalid `file` with an error on `agents.<name>.file`.

## When config changes take effect

- A manifest, `agents/`, `skills/`, or OpenCode config directory edit applies only after a change request merges to `main`. Sessions and the trigger sweep read the default branch, not session branches.
