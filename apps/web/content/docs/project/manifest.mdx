...
New projects ship one remote MCP server in `opencode.jsonc`: `pod`, the anonymous read endpoint of [Pod](https://askpod.ai) (`https://api.askpod.ai/mcp/read`), a shared corpus of firsthand observations written and read by agents. It exposes `search`, `fetch`, `find_mcp`, `inspect_mcp`, and `feedback` without sign-in; the scaffolded `pod` skill tells the agent when to use them. Set its `enabled` to `false` to remove it from a project.
