...
| `subprojects`                                            | no       | installed subprojects. Written by the install flow, not by hand                                 |
...
| `subproject`        | no                    | —                     | The subproject that contributed this trigger. Must name an entry in `subprojects:`. Omitted on hand-authored triggers. See [`subprojects:`](#subprojects). |
...
A list of the subprojects this project has installed. A subproject is another Kortix project — a repository whose own `kortix.yaml` declares agents, skills, connectors and triggers — merged into this one. See [Subprojects](/docs/feature-flags/subprojects) for the product, and [SDK → Subprojects](/docs/sdk/subprojects) for the client surface.

**You do not write this section by hand.** The install flow writes it: an agent session reads both manifests, resolves name collisions, and opens a change request. The entry and the entities it names land in one commit, which is what makes `git revert` a working uninstall.
...
| Field          | Required | Notes                                                                                                    |
| -------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| `slug`         | yes      | `[a-z0-9][a-z0-9_-]{0,127}`, unique among subprojects. The key `subproject:` back-references and the API uses.     |
| `repo`         | yes      | Source as `owner/repo`. A subproject always records where it came from.                                       |
| `ref`          | no       | The branch or tag the install asked for. Absent means the repo's default branch. An update re-resolves against it. |
| `sha`          | no       | The commit `ref` resolved to at install. The integrity source — a branch moves, a sha does not.           |
| `version`      | no       | Display-only tag (`v1.2.0`). Never the integrity source.                                                 |
| `title`        | no       | Display name recorded at install, so it survives the catalog entry being withdrawn.                      |
| `installed_at` | no       | ISO-8601 timestamp.                                                                                      |
| `owns`         | no       | What this subproject contributed, by kind: `agents`, `skills`, `connectors`, `triggers`. Each a list of slugs. |

`ref`, `sha`, `version`, and `installed_at` must be non-empty when present. `owns` accepts only those four kinds; any other key is a validation error.

### `subproject:` back-references

Ownership is recorded twice, on purpose. `subprojects[].owns` says what one subproject contributed; `subproject: <slug>` on each trigger, connector, and agent block says who owns it. The validator cross-checks them: a `subproject:` value that names no entry in `subprojects:` fails validation with `does not match any entry in subprojects`.
...
That second half is not bookkeeping. The subproject-run report joins on the trigger's `subproject:` field, so an orphaned reference silently drops the trigger's fires out of its subproject's history.

`subprojects:` is v2-only. A `kortix.toml` (v1) manifest that declares it is rejected with a message pointing at the v2 migration — see [Legacy TOML](/docs/project/legacy-toml).
