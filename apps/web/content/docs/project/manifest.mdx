...
## Spaces — `kortix-<slug>.yaml`

A space is a named container inside the project: it groups sessions under one effort, owns the triggers that name it, may declare agents of its own, and is an IAM object granted like an agent. Each space is **one file beside the root manifest**, named `kortix-<slug>.yaml`; the slug is the filename. There is no `spaces:` key in `kortix.yaml` — the root validator rejects one.
...
A space file carries no `kortix_version`; the root's version applies and must be `2`. A `kortix.toml` (v1) project has no spaces. `instructions:` and `context:` were removed on 2026-09-07: a file that still carries either keeps parsing, `kortix validate` warns, and the next write through the API or the app drops the key.

**Global and space-owned agents.** An agent declared in the root `kortix.yaml` is global and usable everywhere. An agent declared in `kortix-<slug>.yaml` is owned by that space and usable only in sessions inside it — and in any other space that references it with `agents.<name>: { from: <slug> }`. A reference imports use, not governance: it names an agent the target space owns (never a global one, never another reference) and carries no other key. Agent names are plain and unique across the root and every space file; `kortix validate` reports a duplicate naming both files. `default_agent` and a trigger without a space must name a global agent; a trigger with `space: <slug>` may name what is usable there. A session that names an agent where it is not usable is refused with `400 AGENT_NOT_IN_SPACE` and the list of agents that are (`usable_agents`). The composer offers the same set: the project home lists global agents, and picking a space adds its own.

Create/edit/delete need `project.customize.write` (the manifest-editing leaf, manager tier). Every write through the API or the app is one commit on the space's file; deleting one first detaches the triggers naming it in the root manifest, then removes the file, so the manifest set is valid after every commit.
