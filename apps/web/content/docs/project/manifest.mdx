...
| `imports`                                           | no       | other YAML files or directories whose `triggers`, `connectors`, `agents`, `apps`, and `volumes` merge into this manifest. See [`imports:`](#imports) |
...
| `volumes`                                           | no       | name-to-block map of storage you own, mounted into agent sessions at `/volumes/<name>`. See [`volumes:`](#volumes) |
...
3. An imported file declares `triggers`, `connectors`, `agents`, `apps`, `volumes`, and `imports`. Every other key (`kortix_version`, `default_agent`, `runtime`, `project`, `env`, `opencode`, `sandbox`, `policy`, `policies`) stays in the root file. An imported file that sets one fails validation.
...
5. One name, one file. A trigger slug, connector slug, agent name, app name, or volume name declared in two files is an error that names both files. Nothing overrides silently.
...
| `volumes`             | none    | Which top-level [`volumes`](#volumes) this agent's sessions mount. A list mounts each one `read-only`; a map sets `read-only` or `read-write` per volume. Every credential secret of an attached volume must also be in `secrets`. |
...
A name-to-block map of storage you already own, mounted into an agent's
sessions as an ordinary directory at `/volumes/<name>`. Kortix hosts none of
it. Attach a volume to an agent with `agents.<name>.volumes`. The how-to,
security model, and limits are in [Volumes](/docs/project/volumes).
...
agents:
  analyst:
    secrets: [DATASETS_KEY_ID, DATASETS_SECRET]
    volumes:
      datasets: read-only
```
...
| Field               | Required | Notes |
| ------------------- | -------- | ----- |
| _(map key)_         | —        | `[a-z0-9][a-z0-9_-]{0,127}`; the mount path is `/volumes/<key>`. |
| `type`              | yes      | `s3` — Amazon S3 or any S3-compatible service. |
| `bucket`            | yes      | Bucket name. Letters, digits, `.`, `-`, `_`; no `/`. |
| `prefix`            | no       | Path inside the bucket; the mount shows only this subtree. No leading `/`, no `.` or `..` segments. |
| `region`            | no       | Region name. Amazon S3 buckets resolve without it; some S3-compatible services require it. |
| `endpoint`          | no       | `http(s)` URL of an S3-compatible service, without credentials. Omit for Amazon S3. |
| `access_key_id`     | no       | Identifier of the project secret that holds the access key ID. Never the value. |
| `secret_access_key` | no       | Identifier of the project secret that holds the secret access key. Set both keys, or neither for a public bucket. |

The validator also rejects an agent that attaches an undeclared volume, or a
volume whose credential secret is not in that agent's `secrets` grant. It
cannot check that a secret exists or has Environment exposure. A session
reports that volume as not mounted instead, and boots without it.
...
- A `volumes` change, or a new key for a volume, applies to sessions created after the merge. A running session keeps its mounts.
