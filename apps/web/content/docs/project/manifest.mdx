...
1. Merging the change request that edits this list builds the package bundle once, for the sandbox platform, with install scripts off. Projects with the same list share one bundle.
2. A new or restarted session downloads the bundle before pi starts. A session never runs `npm install`.
3. A session that starts before the bundle exists boots without the project packages and starts the build; the next session has them.
