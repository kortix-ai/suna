...
A slug-to-block map. Each entry is a named container inside the project: it groups sessions under one effort, gives the agent standing instructions and reference files, owns the triggers that name it, and is a grantable object exactly like an agent.
...
| Field          | Type       | Default | Notes                                                                                             |
| -------------- | ---------- | ------- | --------------------------------------------------------------------------------------------------- |
| _(map key)_    | string     | —       | `[a-z0-9][a-z0-9_-]{0,127}`, unique per project. The IAM object id and the immutable slug.           |
...
| `agent`        | string     | —       | Must name a key in `agents:`. A default for sessions started here, not a binding — granting the subproject does not grant this agent. |
| `sessions`     | string     | `private` | `private` or `shared` (see below).                                                               |

Create/edit/delete need `project.customize.write` (the manifest-editing leaf, manager tier).

**Access model.** A subproject is a closed object, exactly like an agent: the manager tier (owner, admin, project manager, service account, super-admin) sees every subproject; a member sees only the ones with a grant row naming them or one of their groups — zero grant rows means zero subprojects. Grant one through the generic resource-grant surface (`POST /projects/:id/resource-grants {resource_type:'subproject', resource_id:<slug>, principal_type, principal_id}`), the same endpoint that grants agents. Deleting a subproject leaves its grant rows orphaned, flagged rather than silently reinterpreted.
...
**Starting a session inside one.** In the web app, the composer on the project home carries a subproject picker beside the agent picker, and a subproject's own page preselects it. From code or the CLI: `sessions.create({ subproject })`, or `kortix sessions new --subproject <slug>`.

**What the sandbox receives.** A session started inside a subproject (`sessions.create({ subproject })`, or `kortix sessions new --subproject`) gets `KORTIX_SUBPROJECT=<slug>` and `KORTIX_SUBPROJECT_CONTEXT=<json>` (`{version, slug, name, description, instructions, context[], sessions}`; `instructions` truncated at 32 KB). The sandbox daemon renders that into `/tmp/kortix/subproject.md` — heading, description, instructions, and a listed `context` — and appends it to the OpenCode `instructions` file, alongside each `context` entry (a `dir/` entry expands to every markdown file under it). A session spawned from inside a subproject session inherits the same subproject unless it passes its own `--subproject`.
