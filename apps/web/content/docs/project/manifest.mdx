...
| `subproject`   | no                    | none                   | Must name a key in `subprojects:`. The fired session joins that subproject; unset means none.          |
...
## Subprojects — `kortix-<slug>.yaml`

A subproject is a named container inside the project: it groups sessions under one effort, owns the triggers that name it, may declare agents of its own, and is an IAM object granted like an agent. Each subproject is **one file beside the root manifest**, named `kortix-<slug>.yaml`; the slug is the filename. There is no `subprojects:` key in `kortix.yaml` — the root validator rejects one.

```yaml
# kortix-marketing.yaml
name: Marketing
description: Campaign work.
agent: writer                  # default for sessions started here; must be usable here
sessions: private
agents:
  writer:                      # owned by marketing — usable here and where referenced
    connectors: [slack]
    secrets: [BRAND_API_KEY]
  researcher:
    from: research             # borrowed from kortix-research.yaml, which owns it
```

| Field          | Type       | Default   | Notes                                                                                                    |
| -------------- | ---------- | --------- | -------------------------------------------------------------------------------------------------------- |
| _(filename)_   | —          | —         | `kortix-<slug>.yaml`, `<slug>` = `[a-z0-9][a-z0-9_-]{0,127}`, in the root manifest's directory. `.yaml` only. |
| `name`         | string     | slug      | Display label.                                                                                           |
| `description`  | string     | —         | One-liner shown in the picker and the subproject page.                                                   |
| `agent`        | string     | —         | Default for sessions started here, not a binding. Must be usable here (global, owned, or referenced).    |
| `sessions`     | string     | `private` | `private` or `shared` (see below).                                                                       |
| `agents`       | map        | —         | Agents this subproject **owns**, in the same block shape as the root's `agents:`, or **references** as `{ from: <slug> }`. |

A subproject file carries no `kortix_version`; the root's version applies and must be `2`. A `kortix.toml` (v1) project has no subprojects. `instructions:` and `context:` were removed on 2026-09-07: a file that still carries either keeps parsing, `kortix validate` warns, and the next write through the API or the app drops the key.

**Global and subproject-owned agents.** An agent declared in the root `kortix.yaml` is global and usable everywhere. An agent declared in `kortix-<slug>.yaml` is owned by that subproject and usable only in sessions inside it — and in any other subproject that references it with `agents.<name>: { from: <slug> }`. A reference imports use, not governance: it names an agent the target subproject owns (never a global one, never another reference) and carries no other key. Agent names are plain and unique across the root and every subproject file; `kortix validate` reports a duplicate naming both files. `default_agent` and a trigger without a subproject must name a global agent; a trigger with `subproject: <slug>` may name what is usable there. A session that names an agent where it is not usable is refused with `400 AGENT_NOT_IN_SUBPROJECT` and the list of agents that are (`usable_agents`). The composer offers the same set: the project home lists global agents, and picking a subproject adds its own.

Create/edit/delete need `project.customize.write` (the manifest-editing leaf, manager tier). Every write through the API or the app is one commit on the subproject's file; deleting one first detaches the triggers naming it in the root manifest, then removes the file, so the manifest set is valid after every commit.

**Access model.** A subproject is a closed object, exactly like an agent: the manager tier (owner, admin, project manager, service account, super-admin) sees every subproject; a member sees only the ones with a grant row naming them or one of their groups — zero grant rows means zero subprojects. Grant one through the generic resource-grant surface (`POST /projects/:id/resource-grants {resource_type:'subproject', resource_id:<slug>, principal_type, principal_id}`), the same endpoint that grants agents. A subproject grant does not grant its agents; the person needs those in their own right. Deleting a subproject leaves its grant rows orphaned, flagged rather than silently reinterpreted.

**`sessions:` visibility.** `private` (default) keeps the ordinary per-session model — a session is visible to its creator unless separately shared. `shared` makes every session inside the subproject readable by everyone granted the subproject, regardless of that session's own visibility; lifecycle rights (stop/restart/delete) are unchanged. `GET /projects/:id/sessions?subproject=<slug>` filters the list to one subproject; `?subproject=` (empty) filters to sessions with none.

**Starting a session inside one.** In the web app, the chat box on the project home carries a subproject picker in the strip under it, and a subproject's own page preselects it. From code or the CLI: `sessions.create({ subproject })`, or `kortix sessions new --subproject <slug>`.

**What the sandbox receives.** A session started inside a subproject gets `KORTIX_SUBPROJECT=<slug>`, and nothing derived from it. The compiled agent config for the session holds the global agents plus the ones usable in that subproject. A session spawned from inside a subproject session inherits the same subproject unless it passes its own `--subproject`.

**Moving a session.** `PATCH /projects/:id/sessions/:sessionId {"subproject": "<slug>"}` files an existing session under a subproject; `{"subproject": null}` moves it back to the project level. In the app it is `Move to` in the session's `⋯` menu, in the sidebar and on the session page. The move is owner-governed like sharing — a `shared` subproject makes its sessions readable by everyone granted it — so it needs `can_manage_sharing`, the target subproject must be declared and granted to the caller, and the session's own agent must be usable there (`400 AGENT_NOT_IN_SUBPROJECT` otherwise). A sandbox that is already running keeps the `KORTIX_SUBPROJECT` it booted with until its next start.
...
2. Inside a trigger entry, order fields `slug`, `name`, `type`, `agent`, `subproject` (when set), `enabled`, then type-specific fields, then `prompt` last.
