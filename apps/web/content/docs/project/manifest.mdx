...
| `refine` | no | mid-session continual-harness refinement cadence |
...
Opt-in mid-session refinement — the continual-harness loop. When enabled, the platform delivers a refinement prompt into each eligible running session every `every_turns` assistant turns. The agent then runs the four-pass protocol from the `kortix-harness-refinement` skill over its own recent turns: it scans for failure signatures, edits the harness components responsible (agent prompts, sub-agents, skills/tools, memory under `.kortix/`), commits `harness: …` to the session branch — edits take effect on the next turn, no restart — and keeps one change request updated toward `main`. Promotion to `main` always goes through CR review.
...
| Field | Required | Default | Notes |
|---|---|---|---|
| `enabled` | yes | `false` | Must be literally `true` to activate. |
| `every_turns` | no | `25` | Cadence in completed assistant turns, 1–10000. |
| `warmup_turns` | no | `10` | The first refinement waits `warmup_turns + every_turns` turns. |
| `max_per_session_per_day` | no | `6` | Hard cap per session per UTC day, 1–96. |
| `agents` | no | `[default_agent]` | Only sessions running these agents are refined. Reflector agents (`harness-reflector`, `memory-reflector`) are rejected. |

The sweep runs platform-side about every two minutes and only reaches sessions whose sandbox is live. Cross-session refinement — the `harness-reflector` cron trigger the starter ships disabled — is configured under `triggers:`, not here.
