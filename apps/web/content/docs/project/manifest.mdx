...
| `crafts`                                            | no       | installed crafts. Written by the install flow, not by hand                                 |
...
| `craft`        | no                    | —                     | The craft that contributed this trigger. Must name an entry in `crafts:`. Omitted on hand-authored triggers. See [`crafts:`](#crafts). |
...
A list of the crafts this project has installed. A craft is another Kortix project — a repository whose own `kortix.yaml` declares agents, skills, connectors and triggers — merged into this one. See [Crafts](/docs/feature-flags/crafts) for the product, and [SDK → Crafts](/docs/sdk/crafts) for the client surface.
...
| Field          | Required | Notes                                                                                                    |
| -------------- | -------- | -------------------------------------------------------------------------------------------------------- |
| `slug`         | yes      | `[a-z0-9][a-z0-9_-]{0,127}`, unique among crafts. The key `craft:` back-references and the API uses.     |
| `repo`         | yes      | Source as `owner/repo`. A craft always records where it came from.                                       |
...
| `owns`         | no       | What this craft contributed, by kind: `agents`, `skills`, `connectors`, `triggers`. Each a list of slugs. |
...
### `craft:` back-references

Ownership is recorded twice, on purpose. `crafts[].owns` says what one craft contributed; `craft: <slug>` on each trigger, connector, and agent block says who owns it. The validator cross-checks them: a `craft:` value that names no entry in `crafts:` fails validation with `does not match any entry in crafts`.
...
That second half is not bookkeeping. The craft-run report joins on the trigger's `craft:` field, so an orphaned reference silently drops the trigger's fires out of its craft's history.

`crafts:` is v2-only. A `kortix.toml` (v1) manifest that declares it is rejected with a message pointing at the v2 migration — see [Legacy TOML](/docs/project/legacy-toml).
