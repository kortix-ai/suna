...
| `subprojects`                                       | no       | slug-to-block map of named containers inside the project                                   |
...
| `instructions` | string     | —         | Inline markdown. Delivered to the sandbox by env and rendered into the agent's standing instructions.     |
| `context`      | `string[]` | `[]`      | Repo-relative paths — a file, or a `dir/`. No leading `/`, no `..` segment.                              |
...
A subproject file carries no `kortix_version`; the root's version applies and must be `2`. A `kortix.toml` (v1) project has no subprojects.
...
**What the sandbox receives.** A session started inside a subproject gets `KORTIX_SUBPROJECT=<slug>` and `KORTIX_SUBPROJECT_CONTEXT=<json>` (`{version, slug, name, description, instructions, context[], sessions}`; `instructions` truncated at 32 KB), read from the subproject's file at the session's ref. The sandbox daemon renders that into `/tmp/kortix/subproject.md` — heading, description, instructions, and a listed `context` — and appends it to the OpenCode `instructions` file, alongside each `context` entry (a `dir/` entry expands to every markdown file under it). The compiled agent config for the session holds the global agents plus the ones usable in that subproject. A session spawned from inside a subproject session inherits the same subproject unless it passes its own `--subproject`.
