...
1. Merging the change request that edits this list builds the packages once, for the sandbox platform, with install scripts off. Each extension is pre-built into one file. Projects with the same list share the build.
...
3. A package that cannot be pre-built (for example, one whose `pi.extensions` uses globs) still works: the session loads it from the installed packages instead, which takes longer.
