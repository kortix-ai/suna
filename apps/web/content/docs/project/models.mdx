...
name in its `secrets`. Changing a running session to such a model does the
same when the session has no selection for that provider. A selection made on
purpose, an empty one included, is left alone.
...
the default. Another member's shared connection requires an explicit
selection. The legacy project login remains available when the member has no
usable connection of their own. A shared gateway key cannot borrow its
creator's private ChatGPT connection. An **Only you** account works only in its
owner's own private sessions; shared and scheduled sessions cannot use it.
