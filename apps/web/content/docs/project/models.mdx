...
Open **Customize → Models → Providers**. Turn off the switch in the **Kortix** row to use only your own providers. Kortix appears in the same list as other providers, with its model count and credit-billing description. Every provider has the same access switch.
