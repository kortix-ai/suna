...
the default in their own private session. A session shared with the project
...
their own. A shared gateway key cannot borrow its creator's private ChatGPT
connection.
...
- ChatGPT models need a ChatGPT account that the whole project can use. A
  gateway key cannot use its creator's private connection.
