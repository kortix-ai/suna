...
Every session routes its model calls through the Kortix LLM gateway. There is
no per-project switch: the sandbox runtime sees one provider, `kortix`, and
the gateway resolves the upstream — a managed model or your own provider key.
Your key is a project secret the gateway spends server-side; it never enters
the sandbox.
