...
OpenAI and Anthropic models are not managed. A ChatGPT plan adds
`codex/gpt-6-astra`, `codex/gpt-6-sol`, and `codex/gpt-6-luna` (plus the
GPT-5.x models). A BYOK key offers its provider's whole catalog, for example
`anthropic/claude-opus-5-5` with an Anthropic key. The picker shows the newest
model of each family by default.
