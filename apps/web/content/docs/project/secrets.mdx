...
sessions when you save it, and every prompt re-syncs a session's secrets
regardless. No restart is needed.
...
| `kortix secrets sync` | Re-push project secrets to active sandboxes. People only; an agent session gets `403` |
