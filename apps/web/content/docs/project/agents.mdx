...
Open **Customize → Agents → New → Set up manually**. Enter a unique name,
description, instructions, and mode. Select **Create agent** to save it and
open its editor. Configure its model, access, tools, and workspace there.

Manual setup requires a v2 project. The form offers an upgrade for legacy
projects. Saving creates the agent's manifest entry and markdown file in one
commit on the project's default branch. The project default agent stays the
same. New agents have no connector, secret, skill, or CLI grants until you
