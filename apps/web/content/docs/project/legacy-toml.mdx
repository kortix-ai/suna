...
Agent behavior — system prompt, `model`, `mode`, `temperature`, and more — does not change. It lives in each agent's `.md` frontmatter. A v1 project keeps those files under `.kortix/opencode/agents/`, and v2 still reads them there. v2 projects created from 2026-09 keep them in `agents/` at the repo root, and `agents.<name>.file` names each one. See [Agents](/docs/project/agents).
...
| `[[agents]].model` | removed | Dead in v1 (parsed, never applied). Agent behavior always comes from the agent's `.md` frontmatter. |
| `[[agents]].file` | `agents.<name>.file` | Dead in v1 (parsed, never applied). In v2 it names the agent's `.md`. |
