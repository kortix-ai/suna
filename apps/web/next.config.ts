...
const nextConfig = (): NextConfig => ({
  ...
  async redirects() {
    return [
      ...
      // The changelog moved into the docs site: it is now generated by Blume
      // from this repo's GitHub Releases (blume.config.ts content.sources), so
      // the hand-rolled /changelog route is gone. This URL is indexed and is
      // linked from the footer and the menu registry, so it redirects rather
      // than 404s. /changelog stays in middleware's public list — middleware
      // runs before redirects, and dropping it would bounce a signed-out
      // visitor to /auth instead of letting this fire.
      {
        source: '/changelog',
