...
import { activeHost, sandboxEnvValue } from '../api/config.ts';
...
describe('in-sandbox auth resolution', () => {
  ...
  it('uses the sandbox agent env file when OpenCode tool env did not inherit tokens', () => {
    const dir = mkdtempSync(join(tmpdir(), 'kortix-agent-env-'));
    const envFile = join(dir, 'agent-env.sh');
    writeFileSync(
      envFile,
      [
        '# Generated by kortix-sandbox-agent-server.',
        "export KORTIX_TOKEN='kortix_sb_must_not_be_used'",
        'export KORTIX_CLI_TOKEN=$(echo must-not-run)',
        "export KORTIX_EXECUTOR_TOKEN='kortix_pat_from_file'",
        "export KORTIX_API_URL='https://tunnel.example/v1'",
        "export KORTIX_PROJECT_ID='proj-from-file'",
        "export KORTIX_DEFAULT_BRANCH='main'",
        'export KORTIX_FRONTEND_URL=$(echo must-not-run)',
        "export NOT_ALLOWLISTED='ignored'",
        '',
      ].join('\n'),
    );
    ...
    expect(executorProjectContext().projectId).toBe('proj-from-file');
    expect(sandboxEnvValue('KORTIX_DEFAULT_BRANCH')).toBe('main');
    expect(sandboxEnvValue('KORTIX_FRONTEND_URL')).toBeUndefined();
    expect(sandboxEnvValue('NOT_ALLOWLISTED')).toBeUndefined();
