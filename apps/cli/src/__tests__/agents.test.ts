...
import { CANONICAL_SKILL, reconcileLegacyOpencodeSymlink, wireCodingAgents } from '../agents';
...
describe('wireCodingAgents', () => {
  test('all agents → symlinks for claude/codex(.agents) onto the real .opencode + a single AGENTS.md', () => {
    ...
    expect(result.written.sort()).toEqual(
      ['.agents → .opencode', '.claude → .opencode', 'AGENTS.md'].sort(),
    );

    // Each link is a real symlink pointing straight at the real OpenCode dir.
    // codex wires `.agents` (its documented, cross-tool skills dir), not `.codex`.
    for (const link of ['.claude', '.agents']) {
      expect(lstatSync(join(dir, link)).isSymbolicLink()).toBe(true);
      expect(readlinkSync(join(dir, link))).toBe('.opencode');
  ...
  test('only wires the agents that were selected', () => {
    ...
    expect(result.written).toEqual(['.claude → .opencode']);
  ...
  test('preserves existing links/file without --overwrite, replaces them with it', () => {
    ...
    expect(second.skipped.sort()).toEqual(['.agents', 'AGENTS.md'].sort());
  ...
  test('wireCodingAgents removes a DANGLING legacy .opencode symlink (target already gone)', () => {
    ...
    const result = wireCodingAgents({ repoRoot: root, agents: ['claude'], overwrite: false });
  ...
  test('never touches a custom .opencode symlink pointing anywhere else, target present or missing', () => {
    ...
    wireCodingAgents({ repoRoot: present, agents: ['opencode', 'claude'], overwrite: false });
    ...
    wireCodingAgents({ repoRoot: missing, agents: ['opencode', 'claude'], overwrite: false });
  ...
  test('a real .opencode directory is always left alone (never a symlink target)', () => {
    ...
    const result = wireCodingAgents({ repoRoot: dir, agents: ['claude'], overwrite: false });
    expect(lstatSync(join(dir, '.opencode')).isSymbolicLink()).toBe(false);
    expect(result.skipped).not.toContainEqual(expect.stringContaining('.opencode'));
  });
  ...
  test('wireCodingAgents creates the .opencode compat symlink on a fresh legacy clone (.kortix/opencode real, no .opencode entry at all)', () => {
    ...
    mkdirSync(join(root, '.kortix/opencode/skills/kortix-system'), { recursive: true });
    writeFileSync(join(root, '.kortix/opencode/skills/kortix-system/SKILL.md'), 'canonical skill', 'utf8');
    expect(existsSync(join(root, '.opencode'))).toBe(false);

    const result = wireCodingAgents({ repoRoot: root, agents: ['claude', 'codex'], overwrite: false });

    expect(lstatSync(join(root, '.opencode')).isSymbolicLink()).toBe(true);
    expect(readlinkSync(join(root, '.opencode'))).toBe('.kortix/opencode');
    // The compat chain now resolves end to end: .claude/.agents -> .opencode
    // -> .kortix/opencode -> the real skill file.
    expect(lstatSync(join(root, '.claude')).isSymbolicLink()).toBe(true);
    expect(lstatSync(join(root, '.agents')).isSymbolicLink()).toBe(true);
    expect(readFileSync(join(root, '.claude/skills/kortix-system/SKILL.md'), 'utf8')).toBe('canonical skill');
    expect(readFileSync(join(root, '.agents/skills/kortix-system/SKILL.md'), 'utf8')).toBe('canonical skill');
    expect(result.skipped).not.toContainEqual(expect.stringContaining('.opencode'));
    rmSync(root, { recursive: true, force: true });
  });
  ...
  test('reconcileLegacyOpencodeSymlink never creates the compat link in replace mode (applyScaffold seam), even with a real legacy target', () => {
    ...
    const result = reconcileLegacyOpencodeSymlink(root, { keepIfTargetExists: false });
  ...
  test('reconcileLegacyOpencodeSymlink creates nothing when neither .opencode nor a real .kortix/opencode exist', () => {
    ...
    const result = reconcileLegacyOpencodeSymlink(root, { keepIfTargetExists: true });
