...
export type WorkspaceLinkInput = Omit<WorkspaceLink, 'workspace_id' | 'project_id'> & {
  workspace_id?: string;
  project_id?: string;
};
...
/** Is the cwd plausibly a Kortix workspace repository? We require either an existing
...
export function loadWorkspaceLink(cwd = process.cwd()): WorkspaceLink | null {
...
export function saveWorkspaceLink(link: WorkspaceLinkInput, cwd = process.cwd()): void {
...
export function clearWorkspaceLink(cwd = process.cwd()): void {
  const path = linkFilePath(cwd);
