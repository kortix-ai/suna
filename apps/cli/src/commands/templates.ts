/**
 * `kortix templates <subcommand>` — browse the template catalog and install a
...
const HELP = help`Usage: kortix templates <subcommand> [options]
...
function parseFlags(argv: string[]): TemplatesFlags {
  return {
...
async function fetchTemplate(client: ApiClient, slug: string): Promise<Template | null> {
  try {
    const res = await client.get<{ template: Template }>(
      `/public/templates/${encodeURIComponent(slug)}`,
    );
    return res.template;
  } catch {
    return null;
...
function printSource(template: Template): string {
  return (
...
async function templatesList(argv: string[], flags: TemplatesFlags): Promise<number> {
  ...
  try {
    const res = await ctx.client.get<{ templates: Template[] }>(
      `/public/templates${suffix}`,
    );
    if (flags.json) {
      emitJson(res);
      return 0;
    ...
    process.stdout.write(
      `\n  ${C.bold}Templates${C.reset} ${C.faded}- ${templates.length} template${templates.length === 1 ? '' : 's'}${C.reset}\n\n`,
    );
    ...
    process.stdout.write(
      `\n  ${C.dim}Details:${C.reset} ${C.cyan}kortix templates show <slug>${C.reset}\n`,
    );
    return 0;
  } catch (error) {
...
async function templatesShow(argv: string[], flags: TemplatesFlags): Promise<number> {
  const slug = positional(argv);
  if (!slug) {
    process.stderr.write(`${status.err('pass a template slug: kortix templates show <slug>')}\n`);
    return 2;
  }
  const ctx = resolveCatalogClient(flags.host);
  if (!ctx) return 1;
  ...
  process.stdout.write(
    `\n  ${C.dim}Install:${C.reset} ${C.cyan}kortix templates install ${template.slug}${C.reset}\n`,
  );
  return 0;
}
...
async function templatesInstall(argv: string[], flags: TemplatesFlags): Promise<number> {
  const slug = positional(argv);
  if (!slug) {
    process.stderr.write(
      `${status.err('pass a template slug: kortix templates install <slug>')}\n`,
    );
    return 2;
  }
  ...
  try {
    const result = await ctx.client.post<{ session_id: string }>(
      `/projects/${ctx.projectId}/templates/install-session`,
      { slug },
    );
    if (flags.json) {
...
export async function runTemplates(argv: string[]): Promise<number> {
  ...
  switch (sub) {
    ...
    case 'ls':
    case 'search':
      return templatesList(rest, flags);
    case 'show':
    case 'view':
      return templatesShow(rest, flags);
    case 'install':
      return templatesInstall(rest, flags);
    default:
