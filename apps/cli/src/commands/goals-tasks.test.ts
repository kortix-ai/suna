...
const GOAL = {
  ...
  timezone: 'UTC',
  agent: 'operator',
  metrics: [{ name: 'p95_ms', direction: 'decrease' as const, target: 200, unit: 'ms' }],
};
        ...
        return {
          ...
          tasks: {
            ...
            recordProgress: (...args: unknown[]) =>
              record('tasks.recordProgress', args, {
                task: { ...TASK, status: 'doing' },
                action: 'recorded',
              }),
...
describe('goals command', () => {
  test('executes every SDK operation and preserves raw JSON responses', async () => {
    ...
    code = await runGoals(commandArgs(['push', GOAL.slug, '--idempotency-key', 'push-test-1']), {
      kortixFromAuth: sdk,
    });
    expect(code).toBe(0);
    ...
    expect(calls).toEqual([
      ...
      {
        method: 'goals.push',
        args: [GOAL.slug, { idempotencyKey: 'push-test-1' }],
        projectId: 'project_1',
      },
      {
  ...
  test('rejects invalid values, timestamps, and observation limits before the SDK call', async () => {
    ...
    for (const [args, message] of [
      [
        ['observe', GOAL.slug, '--metric', 'p95_ms', '--value', 'Infinity', '--source', 'x'],
        'finite number',
      ],
      [
        [
  ...
  test('surfaces ApiError through the shared CLI error formatter', async () => {
    ...
    expect(calls).toEqual([{ method: 'goals.get', args: ['missing'], projectId: 'project_1' }]);
  });
});
...
describe('tasks command', () => {
  ...
  test('executes every SDK operation with validated and translated arguments', async () => {
    ...
    let code = await runTasks(
      commandArgs(['ls', '--goal', GOAL.slug, '--status', 'todo', '--limit', '25', '--json']),
      { kortixFromAuth: sdk },
    );
    expect(code).toBe(0);
    ...
    code = await runTasks(
      commandArgs(['claim', TASK.task_id, '--session', 'session_1', '--lease-seconds', '900']),
      { kortixFromAuth: sdk },
    );
    expect(code).toBe(0);
    ...
    code = await runTasks(
      commandArgs(['block', TASK.task_id, '--session', 'session_1', '--reason', 'Waiting for API']),
      { kortixFromAuth: sdk },
    );
    expect(code).toBe(0);
    ...
    expect(calls).toEqual([
      ...
      },
      {
        method: 'tasks.block',
        projectId: 'project_1',
        args: [TASK.task_id, { session_id: 'session_1', blocker: 'Waiting for API' }],
      },
      {
        method: 'tasks.registerWorker',
        projectId: 'project_1',
        args: [
          TASK.task_id,
          {
  ...
  test('rejects statuses, numeric bounds, and missing evidence before the SDK call', async () => {
    ...
    for (const [args, message] of [
      ...
      [
        ['claim', TASK.task_id, '--session', 'session_1', '--lease-seconds', '29'],
      ...
      [['done', TASK.task_id, '--session', 'session_1'], '--evidence is required'],
      [['done', TASK.task_id, '--session', 'session_1', '--evidence='], '--evidence is required'],
    ] as const) {
      resetOutput();
