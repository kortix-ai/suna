...
export function attachmentDigest(url: string): string | null {
  return url.startsWith(PREFIX) && /^[a-f0-9]{64}$/.test(url.slice(PREFIX.length))
    ? url.slice(PREFIX.length)
    : null;
}

export function parsePromptAttachment(part: Record<string, unknown>): PromptAttachment {
  for (const field of Object.keys(part)) {
    if (!['type', 'mime', 'url', 'filename'].includes(field))
      throw new AttachmentInputError(`file part field "${field}" is not supported`);
  }
  if (typeof part.mime !== 'string' || !IMAGE_TYPES.has(part.mime))
    throw new AttachmentInputError('Pi image attachments require PNG, JPEG, GIF, or WebP');
...
export class SessionAttachmentStore {
  private readonly cache = new Map<string, { bytes: Buffer; contentType: string }>();
  ...
  async read(file: PromptAttachment, signal?: AbortSignal) {
    ...
    if (!sha256 || !this.baseUrl || !this.sessionId)
      throw new AttachmentInputError('immutable session attachment storage is unavailable');
    signal?.throwIfAborted();
    const cached = this.cache.get(sha256);
    if (cached) {
      if (cached.contentType !== file.mime)
        throw new AttachmentInputError('attachment MIME type does not match stored bytes');
    ...
    if (response.status === 404) throw new AttachmentInputError('session attachment not found');
    if (!response.ok) throw new Error(`session attachment storage returned ${response.status}`);
    ...
    while (this.cacheBytes + size > MAX_CACHE_BYTES && this.cache.size) {
  ...
  async validate(files: PromptAttachment[], signal?: AbortSignal) {
    ...
    for (const file of files) {
      ...
      if (total > MAX_CACHE_BYTES)
        throw new AttachmentInputError('prompt image attachments exceed 16 MiB');
    }
  }

  async hydrate<T extends object>(messages: T[], signal?: AbortSignal): Promise<T[]> {
    const result: T[] = [];
    for (const message of messages) {
      ...
      for (const block of source.content) {
        if (block?.type === 'image' && block.kortixAttachment) {
          ...
          content.push({ type: 'image', mimeType: file.mime, data: bytes.toString('base64') });
