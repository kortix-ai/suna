...
import {
  resolveAccountTier,
  sessionLlmPolicyForTier,
} from "../../shared/account-limits";
...
async function resolveLlmContext(c: any) {
  ...
  if (!verified.ok) {
    throw new HTTPException(401, {
      message: `Invalid LLM token: ${verified.reason}`,
    });
  }
...
sessionLlm.openapi(
  createRoute({
    ...
    tags: ["router"],
    summary:
      "Session-scoped OpenAI-compatible chat completions (session-LLM token, SSE streaming)",
    ...auth,
  ...
  async (c) => {
    ...
    const limited = await enforceSessionLlmRateLimit(
      c,
      ctx,
      "/v1/router/llm/chat/completions",
    );
...
sessionLlm.openapi(
  createRoute({
    ...
    tags: ["router"],
    summary:
      "List available models for a session-LLM token (OpenAI-compatible)",
    ...auth,
    responses: {
