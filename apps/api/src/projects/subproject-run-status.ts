/**
 * What one subproject run's status IS — a pure derivation, in its own leaf module so
 ...
 *     into `starting` would show a subproject that is failing every attempt as
...
export interface SubprojectRunStatusInput {
 ...
 */
export function subprojectRunStatus(input: SubprojectRunStatusInput): SubprojectRunStatus {
...
export function subprojectRunIsSettled(status: SubprojectRunStatus): boolean {
  return status === 'done' || status === 'failed' || status === 'stopped' || status === 'skipped';
}

export interface SubprojectRunStatsInput {
...
export interface SubprojectRunStats {
   ...
   * on the subproject, and counting them as failures would defame a subproject whose
   * sandbox was reaped.
   */
...
export function subprojectRunStats(runs: readonly SubprojectRunStatsInput[]): SubprojectRunStats {
  let done = 0;
  let failed = 0;
  let durationSum = 0;
  let durationCount = 0;
