/**
 * `reconcileProjectCraftsWithStore` — the pure craft reconciler.
 *
 ...
 *      be the thing that uninstalls a craft.
 *   2. Skipping a write that IS a change. The row is what the run report joins
 *      against, so a missed upsert reads as "this craft did nothing".
 */
import { describe, expect, test } from 'bun:test';
import {
  type ProjectCraftCatalogStore,
  type ProjectCraftRow,
...
function spec(overrides: Partial<CraftEntrySpec> = {}): CraftEntrySpec {
  return {
    slug: 'seo-watch',
...
function fakeStore(initial: ProjectCraftRow[] = []) {
  const rows = new Map(initial.map((r) => [r.slug, r]));
  const upserts: string[] = [];
  const removes: string[] = [];
  const store: ProjectCraftCatalogStore = {
    async list() {
      return [...rows.values()];
    },
...
function matchingRow(overrides: Partial<ProjectCraftRow> = {}): ProjectCraftRow {
  const s = spec();
  return {
    slug: s.slug,
    repoOwner: s.repoOwner,
...
describe('reconcileProjectCraftsWithStore', () => {
  test('a newly declared craft is written', async () => {
    const { store, upserts, removes } = fakeStore();
    const result = await reconcileProjectCraftsWithStore(PROJECT, [spec()], store);
    expect(upserts).toEqual(['seo-watch']);
    expect(removes).toEqual([]);
    expect(result).toEqual({ upserted: 1, removed: 0 });
  });

  test('an unchanged craft is NOT rewritten — updated_at stays meaningful', async () => {
  ...
  const changes: Array<[string, Partial<CraftEntrySpec>]> = [
    ['a moved sha (the craft was updated)', { resolvedSha: 'def456' }],
    ['a different branch/tag', { gitRef: 'v2' }],
    ['a renamed repo', { repoName: 'seo-craft-2' }],
    ['a new owner', { repoOwner: 'other' }],
    ['a retitled craft', { title: 'SEO watchdog' }],
    ['a craft that gained a trigger', { owns: { triggers: ['seo-weekly', 'seo-daily'] } }],
    ['a craft that lost everything it owned', { owns: {} }],
  ];
  for (const [label, overrides] of changes) {
    test(`${label} is written`, async () => {
      const { store, upserts } = fakeStore([matchingRow()]);
      await reconcileProjectCraftsWithStore(PROJECT, [spec(overrides)], store);
      expect(upserts).toEqual(['seo-watch']);
    });
  }
  ...
  test('owns comparison ignores key order and list order', async () => {
    ...
    await reconcileProjectCraftsWithStore(
      PROJECT,
  ...
  test('an empty owns list equals an absent one', async () => {
    ...
    await reconcileProjectCraftsWithStore(PROJECT, [spec({ owns: {} })], store);
    expect(upserts).toEqual([]);
  });

  test('a craft the manifest no longer declares is removed', async () => {
  ...
  test('pruneStale:false never removes — a stale READ cannot uninstall a craft', async () => {
    const { store, removes, rows } = fakeStore([
      matchingRow(),
  ...
  test('an empty manifest section with pruning removes every row', async () => {
    ...
    const result = await reconcileProjectCraftsWithStore(PROJECT, [], store);
  ...
  test('a legacy row with null columns converges instead of being seen as equal', async () => {
    ...
    await reconcileProjectCraftsWithStore(PROJECT, [spec()], store);
    expect(upserts).toEqual(['seo-watch']);
  });
});
