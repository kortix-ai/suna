/**
 * Craft runs — the read side of "what has this craft actually done".
 *
 ...
 * before this. Nothing new is written for craft runs; the join is what was
 * missing.
 *
 * The join is `project_trigger_runtime.craft_slug`, materialized from each
 * trigger's manifest `craft:` field. That is why it is indexed
 * (`idx_project_trigger_runtime_craft`, partial on `craft_slug IS NOT NULL`):
 * one lookup finds the craft's trigger slugs, and the executions come back on
 * the composite `(project_id, slug)`.
 *
 * Outcome derivation lives in `./craft-run-status.ts` — a leaf, because the
...
export interface CraftRunRecord {
  execution_id: string;
  /** The craft this run belongs to. Present on the all-crafts listing. */
  craft_slug: string;
...
export interface ListCraftRunsInput {
  projectId: string;
  /** One craft, or every craft in the project when omitted. */
...
/**
 * List a project's craft runs, newest first.
 *
 * Scoped to triggers whose `craft_slug` is set, so a hand-authored trigger's
 * executions never appear in a craft report — and, with `craftSlug` given, to
 * that one craft.
 */
export async function listCraftRuns(
  input: ListCraftRunsInput,
): Promise<{ runs: CraftRunRecord[]; total: number }> {
  const scope = input.craftSlug
    ? and(
        eq(projectTriggerExecutions.projectId, input.projectId),
        eq(projectTriggerRuntime.craftSlug, input.craftSlug),
      )
    : and(
        eq(projectTriggerExecutions.projectId, input.projectId),
        isNotNull(projectTriggerRuntime.craftSlug),
      );
  ...
  const [rows, counted] = await Promise.all([
    db
      ...
      // attributed to a craft, so it is not a craft run.
      .innerJoin(
        projectTriggerRuntime,
        and(
  ...
  const runs: CraftRunRecord[] = rows.map((row) => {
...
/** Aggregate stats for one craft, computed over the runs handed in. */
export function summarizeCraftRuns(runs: readonly CraftRunRecord[]): CraftRunStats {
  return craftRunStats(runs.map((r) => ({ status: r.status, durationMs: r.duration_ms })));
}
