...
// users should see real target projects through /projects, not old migration
// artifacts.
...
function serializeMigration(row: MigrationRow | null) {
  if (!row) return null;
  const phaseIndex = row.phase
    ? PHASE_ORDER.indexOf(row.phase as (typeof PHASE_ORDER)[number])
    : -1;
  return {
...
export function registerLegacyMigrationRoutes(app: OpenAPIHono<AppEnv>): void {
  ...
  app.openapi(
    ...
    effectHandler(async (c: any) => {
      ...
      const legacy = await db
        ...
        .from(sandboxes)
        .where(
          and(eq(sandboxes.accountId, accountId), inArray(sandboxes.status, [...LISTED_STATUSES])),
        )
      ...
      const items = (
        await Promise.all(
          legacy.map(async (s) => {
            ...
            // Archived machines are old migrated artifacts, not actionable
            // projects. Real access to the target project is handled by /projects.
            if (!shouldListLegacyMigrationItem({ status: s.status })) return null;
            return {
  ...
  app.openapi(
    createRoute({
      ...
      responses: {
        200: json(
          z.object({ created: z.boolean(), migration: z.any() }),
          'Existing in-flight migration',
        ),
    ...
    effectHandler(async (c: any) => {
      ...
      if (owned.provider !== MIGRATABLE_PROVIDER) {
        return c.json(
          { error: `Only JustAVPS machines can be migrated (got provider "${owned.provider}")` },
          400,
        );
      }
