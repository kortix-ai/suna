/**
 * Subproject index crawl — turn a GitHub repo into one `kortix.subprojects` row.
 *
 * A subproject repo needs ONLY a `kortix.yaml`. There is no `registry.json`
 * requirement: the manifest already declares the agents, triggers and
 * connectors the subproject contributes, so the card is derived from it with the
 ...
 * subproject the runtime would refuse to read.
 *
...
export type SubprojectCrawlErrorCode =
  | 'invalid_address'
  | 'repo_not_found'
  | 'ref_not_found'
  | 'manifest_not_found'
...
export class SubprojectCrawlError extends Error {
  constructor(
    readonly code: SubprojectCrawlErrorCode,
    message: string,
    /** Manifest validation findings, when `code === 'manifest_invalid'`. */
    readonly issues: ManifestIssue[] = [],
  ) {
    super(message);
    this.name = 'SubprojectCrawlError';
  }
}

/** One agent a subproject contributes, as the store card shows it. */
export interface SubprojectCardAgent {
  name: string;
  description: string | null;
}

/** One trigger a subproject contributes — the cadence the card advertises. */
export interface SubprojectCardTrigger {
  slug: string;
  name: string;
  type: string;
  cron: string | null;
...
/** One connector a subproject needs. This is a REQUIREMENT list, not a state list. */
export interface SubprojectCardConnector {
  slug: string;
  provider: string;
  app: string | null;
}
...
export interface SubprojectCrawlResult {
  slug: string;
  sourceKind: SubprojectSourceKind;
  ...
  agents: SubprojectCardAgent[];
  triggers: SubprojectCardTrigger[];
  connectors: SubprojectCardConnector[];
  skills: string[];
  envRequired: string[];
  /** The manifest path the subproject's own repo uses (`kortix.yaml` etc.). */
  manifestPath: string;
  /**
   * For an upload: the subproject's text files, which the install prompt embeds
   * because there is no repo to fetch from. Empty for a github subproject.
   */
 ...
 */
export interface SubprojectCrawlOptions {
  fetchImpl?: typeof fetch;
}

/**
 * Fetch through `shared/github-fetch.ts`'s token-attaching, exact-host-matched fetch.
 *
...
async function githubJson(
  url: string,
  notFound: SubprojectCrawlErrorCode,
  fetchImpl: typeof fetch,
): Promise<unknown> {
  ...
  if (res.status === 404 || res.status === 422) {
    throw new SubprojectCrawlError(notFound, `not found: ${url}`);
  }
  if (!res.ok) {
    throw new SubprojectCrawlError(
      'upstream_unavailable',
...
async function resolveSha(
  ...
  if (!sha) {
    throw new SubprojectCrawlError('ref_not_found', `could not resolve ref "${ref}" to a commit`);
  }
  return sha;
}
...
async function readManifestAtSha(
  ...
  throw new SubprojectCrawlError(
    'manifest_not_found',
    `no kortix.yaml in ${owner}/${repo} at ${sha.slice(0, 7)} — a subproject repo must declare one (${candidates
      .map((c) => c.path)
      .join(', ')})`,
  );
}
...
/**
 * Which skills the subproject's agents may invoke, as the union of their `skills:`
 * grants.
 *
 * Derived from the manifest rather than from a directory listing, because a
 * subproject repo is only required to ship a `kortix.yaml`. The `all` / `none`
 ...
 * subproject that wants its skills advertised should grant them by name.
 *
...
/** Env vars the subproject declares as required. */
...
/** A subproject's default slug: its repo name, lowercased into slug shape. */
export function subprojectSlugFromRepo(repoName: string): string {
  return (
    repoName
      .toLowerCase()
      .replace(/[^a-z0-9_-]+/g, '-')
 ...
 *
 * Extracted so a zip and a repo can never disagree about what a subproject IS. The
...
function deriveCardFromManifest(
  ...
  // A subproject's OWN manifest should not declare installed subprojects. If it does, say
  // so rather than indexing a nested install nobody will honor.
  const nested = extractSubprojects(manifest);
  if (nested.specs.length > 0) {
    warnings.push(
      `subprojects: ${nested.specs.length} installed-subproject entr${nested.specs.length === 1 ? 'y' : 'ies'} in the source manifest are ignored — a subproject does not install other subprojects`,
    );
  }
...
/**
 * Read a subproject out of an uploaded ZIP archive.
 *
 * The zip is somebody's repo with no git behind it, so there is no sha to pin
 * and no raw URL to fetch from later. The subproject's text files therefore travel
 ...
 * Bounded by `SUBPROJECT_ZIP_LIMITS`: this is an installable subproject, not a file host.
 */
export function crawlSubprojectZip(
  archive: ArrayBuffer | Uint8Array,
  uploadName: string,
): SubprojectCrawlResult {
  ...
  if (!candidate) {
    throw new SubprojectCrawlError(
      'manifest_not_found',
      `${uploadName} has no kortix.yaml at its root — a subproject archive must declare one (${manifestCandidatePaths(
        null,
      )
        .map((c) => c.path)
        .join(', ')})`,
  ...
  // Not a failure, and the common case for a whole-repo zip: a subproject is its
  ...
  if (read.ignored.length > 0) {
    card.warnings.push(
      `${read.ignored.length} file(s) are not part of a subproject and were left behind — a subproject is its ` +
  ...
  return {
    slug: subprojectSlugFromRepo(base),
    sourceKind: 'upload',
    repoOwner: null,
    repoName: null,
    gitRef: null,
    resolvedRef: null,
    resolvedSha: null,
    title: card.title ?? subprojectSlugFromRepo(base),
...
/**
 * Crawl one repo into a subproject card. Throws {@link SubprojectCrawlError} for every
 ...
 */
export async function crawlSubprojectRepo(
  address: string,
  options: SubprojectCrawlOptions = {},
): Promise<SubprojectCrawlResult> {
  const fetchImpl = options.fetchImpl ?? githubLoaderOptions.fetchImpl;
  const parsed = parseSubprojectRepo(address);
  if (!parsed) {
    throw new SubprojectCrawlError(
      'invalid_address',
  ...
  // list a subproject whose manifest the platform would refuse. Shared with the zip
  // path so a repo and an archive can never disagree about what a subproject IS.
  const card = deriveCardFromManifest(
    file.raw,
    file.format,
    file.path,
  ...
  return {
    slug: subprojectSlugFromRepo(repo),
    sourceKind: 'github',
    repoOwner: owner,
    repoName: repo,
    gitRef: ref,
    ...
    // subproject, while the repo blurb describes the repository.
    ...
    // A github subproject's files are fetched at `resolvedSha` when it installs, so
