    ...
    allocateSessionRuntime({
      ...
      buildEnvVars: () =>
        buildSessionSandboxEnvVars({
          ...
          // A restarted AGI coordinator must keep its AGI runtime: without
          // this the rebuilt env loses KORTIX_PROJECT_AUTO_CLONE=0 and the
          // AGI agent config, so the daemon clones the project over the AGI
          // workspace and wipes /workspace/AGENTS.md.
          platformAgiAgent: isAgiAgentName(session.agentName ?? ''),
