...
export async function releaseInboxHold(sessionId: string): Promise<number> {
  const [marked] = await db
    ...
    .limit(1);
  if (!marked) {
    // No row is held, but the Stop itself may still be recorded: a Stop whose
    // turn had nothing queued marks the session and holds no row. Releasing
    // must end it here too, or every later send typed before that Stop keeps
    // joining a hold that is over, and repairs stay parked (2026-09-23 merge:
    // this fast path skipped `holdInboxPrompts`, the only place it cleared).
    await clearUserStop(sessionId).catch(() => undefined);
    return 0;
  }
