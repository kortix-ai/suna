...
/** The real shape of a stuck craft run (ca470c15 in the incident). */
...
describe('lostBootPromptOverrides', () => {
  test("carries the craft's own agent so the rerun is not the runtime default", () => {
...
describe('reconcileLostBootPrompts', () => {
  ...
  test('a deduped enqueue is not counted as a rerun and is not loud', async () => {
    ...
    // craft from being run twice by a repeating maintenance tick.
    queryRows = [candidateRow()];
    dedupeNext = true;
