...
function isCurrentFence(where: SQL): boolean {
  ...
  return (
    query.params.includes(currentOwner) && query.params.includes(currentAttempt)
  );
}
...
describe("lifecycle command claim fencing", () => {
  test("lease covers the 5 minute readiness wait plus 45 second prompt delivery window", () => {
    expect(LIFECYCLE_COMMAND_LEASE_MS).toBeGreaterThanOrEqual(
      5 * 60_000 + 45_000,
    );
  });
  ...
  test("a stale worker cannot overwrite success after another worker reclaims the command", async () => {
    ...
    for (const call of updateCalls) {
      ...
      expect(params).toContain(
        call === updateCalls[0] ? "worker-a" : "worker-b",
      );
    }
  });
  ...
  test("only the current claim can heartbeat and extend its lease", async () => {
    ...
    expect(
      await renewLifecycleCommandLease(
        "cmd-1",
        { lockedBy: "worker-a", attempt: 1 },
        now,
      ),
    ).toBe(false);
    expect(
  ...
  test("claimed rows expose lockedBy plus attempt as the immutable claim token", () => {
    expect(
      lifecycleCommandClaim({ lockedBy: "worker-b", attempts: 2 }),
    ).toEqual({
      lockedBy: "worker-b",
      attempt: 2,
    });
    expect(() =>
      lifecycleCommandClaim({ lockedBy: null, attempts: 2 }),
    ).toThrow("missing lock owner");
  });
});
