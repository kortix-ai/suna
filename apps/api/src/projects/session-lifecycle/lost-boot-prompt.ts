 ...
 *
 * A session created with `initial_prompt` (every trigger fire, every craft run)
 ...
 * an empty transcript, and the session reaped 15 minutes later. Every craft run
 ...
 * cron fire is the right recovery — re-running a day-old craft would publish
 ...
 *
 * The agent is carried so the run happens under the craft's own agent rather
