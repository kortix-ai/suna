...
import { openUsersAbove } from './forwarded-placement';
import { readInboxRowsByWireIds } from './inbox-rows';
import { inboxSendOrderMs, underPlacementKeepsSendOrder } from './inbox-order';
import { inboxRowWaited } from './quick-queue-group';
import {
...
export async function executeQueuedContinue(
  ...
  try {
    ...
    // tail rows are already stamped when their turn comes (`admitted`). The
    // tail is claimed by this same drain, so its lease is this row's
    // `lockedBy` under its own command id.
    if (admission.admit && !options.admitted) {
      await Promise.all(
        [row.commandId, ...(options.groupCommandIds ?? [])].map((commandId) =>
          lifecycleStore.markInboxDeliveryStarted({ commandId, lockedBy: row.lockedBy }),
        ),
      );
    }
    tl.mark('admission');
