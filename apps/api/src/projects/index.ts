...
async function buildSessionSandboxEnvVars(input: {
  ...
  // used by Platinum sandbox sessions). kortix-agent's auto-clone would
  // invoke `git clone local://…`, which has no git remote helper → blocks
  // the boot sequence indefinitely. Skip auto-clone for these projects.
