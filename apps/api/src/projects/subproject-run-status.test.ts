/**
 * `subprojectRunStatus` — the derivation that decides what a subproject run SHOWS.
 *
...
import {
  ...
  subprojectRunIsSettled,
  subprojectRunStats,
  subprojectRunStatus,
} from './subproject-run-status';

function status(overrides: Partial<Parameters<typeof subprojectRunStatus>[0]> = {}): SubprojectRunStatus {
  return subprojectRunStatus({
...
describe('subprojectRunStatus — dispatch states', () => {
  ...
  test('a queued slot that already burned an attempt is RETRYING, not starting', () => {
    // Collapsing this into `starting` would show a subproject failing every attempt
...
describe('subprojectRunStatus — outcome comes from the session, not the execution', () => {
  ...
  test('a deleted session leaves the run stopped, never failed', () => {
    ...
    // it ended. Reporting `failed` would defame a subproject whose session was
    // merely cleaned up.
...
describe('subprojectRunIsSettled', () => {
  test('verdicts and skips are settled; dispatch states are not', () => {
    for (const s of ['done', 'failed', 'stopped', 'skipped'] as SubprojectRunStatus[]) {
      expect(subprojectRunIsSettled(s)).toBe(true);
    }
    for (const s of ['starting', 'retrying', 'running'] as SubprojectRunStatus[]) {
      expect(subprojectRunIsSettled(s)).toBe(false);
    }
  });
});

describe('subprojectRunStats', () => {
  const run = (s: SubprojectRunStatus, durationMs: number | null = null) => ({ status: s, durationMs });

  test('no runs yields no rate', () => {
    expect(subprojectRunStats([])).toEqual({
      total: 0,
      done: 0,
      failed: 0,
      successRate: null,
  ...
  test('the rate is done over settled VERDICTS only', () => {
    const stats = subprojectRunStats([run('done'), run('done'), run('done'), run('failed')]);
    expect(stats.successRate).toBe(75);
    expect(stats.total).toBe(4);
  });

  test('stopped and skipped are excluded from BOTH sides of the rate', () => {
    // A subproject whose sandbox was reaped, or whose filter declined a delivery,
    // must not be reported as failing.
    const stats = subprojectRunStats([run('done'), run('stopped'), run('skipped'), run('starting')]);
  ...
  test('with no settled verdict the rate is null, not 0', () => {
    ...
    expect(subprojectRunStats([run('skipped'), run('running')]).successRate).toBeNull();
  });

  test('the average ignores runs with no duration', () => {
    const stats = subprojectRunStats([run('done', 2_000), run('done', 4_000), run('running', null)]);
    expect(stats.avgDurationSeconds).toBe(3);
  });

  test('a negative duration is ignored rather than skewing the mean', () => {
    const stats = subprojectRunStats([run('done', 6_000), run('done', -5_000)]);
    expect(stats.avgDurationSeconds).toBe(6);
  });

  test('the average rounds to whole seconds', () => {
    expect(subprojectRunStats([run('done', 1_400)]).avgDurationSeconds).toBe(1);
    expect(subprojectRunStats([run('done', 1_600)]).avgDurationSeconds).toBe(2);
  });
});
