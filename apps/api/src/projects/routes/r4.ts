...
function whatsappChannelEnabled(metadata: unknown): boolean {
  return resolveExperimentalFeature(metadata, 'whatsapp');
}

/** Where the gateway should POST events for this Kortix deployment. */
function whatsappWebhookUrl(requestUrl: string): string {
  const base = (config.KORTIX_URL || new URL(requestUrl).origin).replace(/\/+$/, '');
  return `${base}/v1/webhooks/whatsapp/gateway`;
}

/** Shared guard: membership + connector.write + the experimental flag. */
async function loadWhatsAppProject(c: any, projectId: string) {
  const loaded = await loadProjectForUser(c, projectId, 'read');
  if (!loaded) return { error: c.json({ error: 'Not found' }, 404) } as const;
  await assertProjectCapability(
    c,
    loaded.userId,
    loaded.row.accountId,
  ...
  if (!whatsappChannelEnabled(loaded.row.metadata)) {
    return {
      error: c.json(
        { error: 'WhatsApp is experimental and must be enabled for this project' },
        403,
      ),
    } as const;
  }
...
projectsApp.openapi(
  createRoute({
    method: 'get',
    path: '/{projectId}/channels/whatsapp/installation',
    tags: ['channels'],
    summary: 'GET /:projectId/channels/whatsapp/installation',
    ...auth,
    request: { params: z.object({ projectId: z.string() }) },
    responses: { 200: json(z.any(), 'OK'), ...errors(404) },
  }),
  async (c: any) => {
    ...
    const loaded = await loadProjectForUser(c, projectId, 'read');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    if (!whatsappChannelEnabled(loaded.row.metadata)) return c.json(null);
    const install = await loadWhatsAppInstall(projectId);
    return c.json(
      install
        ? {
            gatewayUrl: install.gatewayUrl,
            accountId: install.accountId,
            phoneNumber: install.phoneNumber,
            displayName: install.displayName,
            webhookUrl: whatsappWebhookUrl(c.req.url),
          }
        : null,
    );
  },
...
// Step 1 of connecting: list the connections a gateway URL + key can reach, so
// the operator picks a phone number instead of pasting an opaque id.
projectsApp.openapi(
  createRoute({
    method: 'post',
    path: '/{projectId}/channels/whatsapp/connections',
    tags: ['channels'],
    summary: 'POST /:projectId/channels/whatsapp/connections',
    ...auth,
    request: {
      params: z.object({ projectId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
    },
    responses: { 200: json(z.any(), 'OK'), ...errors(400, 401, 403, 404, 502) },
  }),
  async (c: any) => {
    const projectId = c.req.param('projectId');
    const guard = await loadWhatsAppProject(c, projectId);
    ...
    if (!body.gateway_url || !body.api_key) {
      return c.json({ error: 'gateway_url and api_key are required' }, 400);
    }
    try {
      const gatewayUrl = normalizeGatewayUrl(body.gateway_url);
      const connections = await listGatewayConnections(gatewayUrl, body.api_key);
      return c.json({ gatewayUrl, connections });
    } catch (err) {
      const status = err instanceof WhatsAppGatewayError ? err.status : 502;
      return c.json({ error: (err as Error).message }, status as 400 | 401 | 502);
    }
  },
);

// Step 2: bind one connection to this project and register the webhook.
projectsApp.openapi(
  createRoute({
    method: 'post',
    path: '/{projectId}/channels/whatsapp/connect',
    tags: ['channels'],
    summary: 'POST /:projectId/channels/whatsapp/connect',
    ...auth,
    request: {
      params: z.object({ projectId: z.string() }),
      body: { content: { 'application/json': { schema: AnyObject } } },
  ...
  async (c: any) => {
    ...
    if (!body.gateway_url || !body.api_key || !body.account_id) {
      return c.json({ error: 'gateway_url, api_key and account_id are required' }, 400);
    }

    try {
      const gatewayUrl = normalizeGatewayUrl(body.gateway_url);
      const connections = await listGatewayConnections(gatewayUrl, body.api_key);
      const connection = connections.find((entry) => entry.id === body.account_id);
      if (!connection) {
        return c.json({ error: 'That connection is not visible to this API key' }, 400);
      }

      // Replace any previous endpoint so reconnecting does not leave orphans.
      const previous = await loadWhatsAppInstall(projectId);
      if (previous?.webhookEndpointId) {
        await deleteGatewayWebhook(gatewayUrl, body.api_key, previous.webhookEndpointId);
      }

      const webhook = await createGatewayWebhook(gatewayUrl, body.api_key, {
        url: whatsappWebhookUrl(c.req.url),
        accountId: connection.id,
        description: `Kortix project ${projectId}`,
      });

      const install = await saveWhatsAppInstall({
        projectId,
        gatewayUrl,
        apiKey: body.api_key,
        accountId: connection.id,
      ...
      await reconcileChannelConnectors(projectId);
      return c.json({ ...install, webhookUrl: whatsappWebhookUrl(c.req.url) });
    } catch (err) {
      const status = err instanceof WhatsAppGatewayError ? err.status : 502;
      return c.json({ error: (err as Error).message }, status as 400 | 401 | 502);
    }
...
projectsApp.openapi(
  createRoute({
    ...
    path: '/{projectId}/channels/whatsapp/installation',
    tags: ['channels'],
    summary: 'DELETE /:projectId/channels/whatsapp/installation',
    ...auth,
    request: { params: z.object({ projectId: z.string() }) },
  ...
  async (c: any) => {
    ...
    const install = await loadWhatsAppInstall(projectId);
    const credentials = install ? await loadWhatsAppCredentials(projectId) : null;
    if (install?.webhookEndpointId && credentials) {
      await deleteGatewayWebhook(
        credentials.gatewayUrl,
        credentials.apiKey,
        install.webhookEndpointId,
      );
    }
    await deleteWhatsAppInstall(projectId);
    await reconcileChannelConnectors(projectId);
    return c.json({ status: 'disconnected' });
  },
);
