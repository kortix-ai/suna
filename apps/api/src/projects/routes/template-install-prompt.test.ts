...
import {
  ...
  templateFetchRef,
  templateRawBase,
} from './template-install-prompt';

const SHA = '9f3c1a7ecb4d21f0a8b3c5d7e9f1a2b3c4d5e6f7';

function template(overrides: Partial<TemplateCatalogEntry> = {}): TemplateCatalogEntry {
  return {
    slug: 'seo-watch',
    title: 'SEO watch',
...
describe('buildTemplateInstallPrompt — provenance', () => {
  test('reads the template at the pinned SHA, not at the branch', () => {
    const p = buildTemplateInstallPrompt(template(), TARGET);
  ...
  test('falls back to the branch only when no sha was recorded', () => {
    ...
    expect(templateFetchRef(t)).toBe('main');
    expect(templateRawBase(t)).toBe('https://raw.githubusercontent.com/acme/seo-template/main');
  });

  test('falls back to HEAD when there is neither a sha nor a ref', () => {
    expect(templateFetchRef(template({ resolved_sha: '', git_ref: null }))).toBe('HEAD');
  });
  ...
  test('a project with no manifest yet is stated, not hidden', () => {
    expect(buildTemplateInstallPrompt(template(), null)).toContain('no manifest yet');
  });

  test('tells the agent everything it needs is in the prompt', () => {
    expect(buildTemplateInstallPrompt(template(), TARGET)).toContain('do not search the web');
  });
});

describe('buildTemplateInstallPrompt — the merge must not break the project', () => {
  test('never changes default_agent', () => {
    expect(buildTemplateInstallPrompt(template(), TARGET)).toContain(
  ...
  test('keeps an existing connector rather than redefining it', () => {
    ...
    expect(buildTemplateInstallPrompt(template(), TARGET)).toContain('KEEP the existing one');
  });

  test('validates before committing', () => {
    expect(buildTemplateInstallPrompt(template(), TARGET)).toContain('kortix validate');
  });
  ...
  test('lands at the project root — a subproject only if the user asks', () => {
    // A template is an example project setup, not a subproject. The agent may
    // offer once; it must not create one on its own initiative.
    const p = buildTemplateInstallPrompt(template(), TARGET);
    expect(p).toContain('Install at the project root');
    expect(p).toContain('do not add it as one unless I say so');
    expect(p).toContain('You may ask me once');
  });
});

describe('buildTemplateInstallPrompt — nothing starts firing on install', () => {
  ...
  test('a template with no triggers gets no enable step at all', () => {
    const p = buildTemplateInstallPrompt(template({ triggers: [] }), TARGET);
    expect(p).not.toContain('enabled: false');
    expect(p).toContain('what the template can now do');
  });
});

describe('buildTemplateInstallPrompt — credentials', () => {
  ...
  test('tells the agent not to re-ask for something already connected', () => {
    expect(buildTemplateInstallPrompt(template(), TARGET)).toContain('already connected');
  });

  test('a template needing nothing gets no credential walkthrough', () => {
    const p = buildTemplateInstallPrompt(template({ env_required: [], connectors: [] }), TARGET);
    expect(p).not.toContain('request_secret');
  });
});

describe('buildTemplateInstallPrompt — only the steps that apply', () => {
  test('a template with no skills gets no skill-copy step', () => {
    expect(buildTemplateInstallPrompt(template({ skills: [] }), TARGET)).not.toContain(
      'opencode/skills/<name>/',
    );
  });

  test('a template with no agents gets no agent-copy step', () => {
    expect(buildTemplateInstallPrompt(template({ agents: [] }), TARGET)).not.toContain(
      'agents/<name>.md',
    );
  });

  test('a template that declares nothing still produces a coherent prompt', () => {
    const p = buildTemplateInstallPrompt(
  ...
  test('a manifest that will not serialize degrades to JSON instead of throwing', () => {
    ...
    expect(() =>
      buildTemplateInstallPrompt(template({ manifest: cyclic }), TARGET),
    ).not.toThrow();
  });
});
