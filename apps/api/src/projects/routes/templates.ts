/**
 * `POST /:projectId/templates/install-session` — start the agent-driven
 * install of one templates template into this project.
 *
 * That is the whole project-scoped surface. Browsing is the anonymous
 * `/v1/public/templates` read, and nothing records what a project
...
import { findTemplateCatalogEntry } from '../../templates/catalog';
...
import { buildTemplateInstallPrompt } from './template-install-prompt';
...
projectsApp.openapi(
  createRoute({
    method: 'post',
    path: '/{projectId}/templates/install-session',
    tags: ['templates'],
    summary: 'POST /:projectId/templates/install-session',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    // learning whether this project has templates enabled.
    const loaded = await loadProjectForUser(c, projectId, 'write');
    if (!loaded) return c.json({ error: 'Not found' }, 404);
    const gate = requireFeatureFlag(c, loaded.row.metadata, 'templates');
    if (gate) return gate;
    ...
    const template = findTemplateCatalogEntry(slug);
    ...
    const result = await createSession({
      ...
      body: {
        initial_prompt: buildTemplateInstallPrompt(template, await manifestRawOrNull(project)),
