 ...
 */
export function templateFetchRef(
  template: Pick<TemplateCatalogEntry, 'resolved_sha' | 'git_ref'>,
): string {
...
export function templateRawBase(
  template: Pick<TemplateCatalogEntry, 'repo' | 'resolved_sha' | 'git_ref'>,
): string {
  return `${RAW_BASE}/${template.repo}/${templateFetchRef(template)}`;
}
...
export function buildTemplateInstallPrompt(
  ...
  const ref = templateFetchRef(template);
  const rawBase = templateRawBase(template);
  ...
  const steps: string[] = [
    ...
    'Install at the project root. A template is an example project setup, not a subproject: do not add it as one unless I say so. You may ask me once whether I want it as a subproject instead.',
  ];
