...
import {
  filterConfigResourcesForUser,
  denierFromConfig,
  resourceDenierForRequest,
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        const gitProject = yield* attemptRoute(() =>
          withProjectGitAuth(loaded.row),
        );
        const { specs } = yield* attemptRoute(() =>
          loadProjectApps(gitProject),
        );
        const spec = specs.find((s) => s.slug === slug);
        if (!spec) return yield* failNotFound();

        const latest = yield* attemptRoute(() =>
          getLatestDeployment(projectId, slug),
        );
        ...
        const fresh = yield* attemptRoute(() =>
          getLatestDeployment(projectId, slug),
        );
        return routeJson(
          {
    ...
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        if (latest.freestyleId) {
          yield* attemptRoute(() => provider.stop(latest.freestyleId!)).pipe(
        ...
        return routeJson({
          ok: true,
          deployment: updated ? serializeDeploymentRow(updated) : null,
        });
      }),
    );
  },
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        const provider = getDeploymentProvider(latest.provider ?? undefined);
        const data = yield* attemptRoute(() =>
          provider.logs(latest.freestyleId ?? ""),
        ).pipe(
          Effect.catchAll((err) =>
            failJson(
              {
                ok: false,
                error: err instanceof Error ? err.message : "logs unavailable",
              },
              502,
            ),
          ),
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const visibleFiles = denier
      ? files.filter((f) => !denier.isDenied(f.path))
      : files;
    return c.json({
  ...
  effectHandler(async (c: any) => {
    ...
    try {
      files = await listRepoFiles(
        gitProject,
        c.req.query("ref") || loaded.row.defaultBranch,
        c.req.query("path"),
      );
    } catch (error) {
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    try {
      const stream = await archiveRepoSubtree(
        await withProjectGitAuth(loaded.row),
        ref,
        path,
      );
      const fileName =
        (path?.split("/").filter(Boolean).pop() || "workspace") + ".zip";
  ...
  effectHandler(async (c: any) => {
    ...
    try {
      ...
      if (contentSearch) {
        ...
        const results = denier
          ? matches.filter((m) => !denier.isDenied(m.path))
          : matches;
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const skip = Number(c.req.query("skip") || "0");
    try {
      const result = await listCommits(await withProjectGitAuth(loaded.row), {
        ref,
        path,
        limit,
        skip,
      ...
      return c.json(
        {
          error:
            error instanceof Error
              ? error.message
              : "Failed to compute diff preview",
        },
        400,
      );
    }
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const manifestPath = normalizeString(
      body.manifest_path ?? body.manifestPath,
    );
