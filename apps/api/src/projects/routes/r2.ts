...
import {
  ...
  loadGitProject,
  registerGitHubLinkedProject,
  registerPatLinkedProject,
  resolveGitHubImport,
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const repoFullName = normalizeString(
      body.repo_full_name ?? body.repoFullName,
    );
    ...
    const repoUrl = repoFullName
      ...
      : repoUrlInput;
    if (!repoUrl)
      return c.json({ error: "repo_url or repo_full_name is required" }, 400);

    const manifestPath =
      normalizeString(body.manifest_path ?? body.manifestPath) ?? "kortix.toml";
    ...
    if (githubToken) {
      ...
      try {
        patImport = await resolveGitHubImportWithPat({
          repoUrl,
          token: githubToken,
          defaultBranch: normalizeString(
            body.default_branch ?? body.defaultBranch,
          ),
        });
      } catch (error) {
        return c.json(
      ...
      const patQuota = await enforceProjectQuota(c, scope.accountId, {
        repoUrl: patImport.repo.clone_url,
      });
      ...
      return c.json(
        {
          ...
          git_connection: serializeProjectGitConnection(
            await getProjectGitConnection(row.projectId),
          ),
        },
        201,
      );
    ...
    try {
      imported = await resolveGitHubImport({
        accountId: scope.accountId,
        repoUrl,
        installationId: normalizeString(
      ...
      if (error instanceof GitHubInstallationRequiredError) {
        return c.json(
          {
            error: error.message,
            install_url: await createGitHubInstallationInstallUrl(
              error.accountId,
              scope.userId,
            ),
          },
      ...
      return c.json(
        {
          error:
            (error as Error).message || "Failed to validate GitHub repository",
        },
        400,
      );
    }

    const linkQuota = await enforceProjectQuota(c, scope.accountId, {
      repoUrl: imported.repo.clone_url,
    });
  ...
  effectHandler(async (c: any) => {
    ...
    if (!/^[a-zA-Z0-9._-]+$/.test(name)) {
      return c.json(
        {
          error:
            "name must contain only letters, numbers, hyphens, underscores or dots",
        },
        400,
      );
    }
    ...
    try {
      githubAuth = await resolveGitHubRepoAuth(
        scope.accountId,
      ...
      const message =
        (error as Error).message || "GitHub is not configured on the server";
    ...
    for (let attempt = 0; attempt < 12 && !repo; attempt += 1) {
      ...
      try {
        ...
        return c.json(
          {
            error:
              (error as Error).message || "Failed to create GitHub repository",
          },
          502,
        );
      }
    ...
    const projectName =
      ...
      deriveProjectName(repo.full_name);
    ...
    for (const file of starter) {
      try {
        ...
        const message =
          (err as Error).message || "Failed to scaffold starter file";
        console.warn(
          `[projects/create-repo] Failed to scaffold ${file.path} into ${repo.full_name}:`,
          message,
        );
        return c.json(
          { error: `Failed to scaffold starter file ${file.path}: ${message}` },
          502,
        );
      }
    }
    ...
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        const body = yield* attemptRoute(
          () => c.req.json() as Promise<{ raw?: unknown }>,
        ).pipe(Effect.catchAll(() => Effect.succeed({} as { raw?: unknown })));
        const raw = typeof body.raw === "string" ? body.raw : null;
        if (!raw) {
          return yield* failJson(
            { error: "Missing `raw` (TOML string) in body." },
            400,
          );
        }

        const { validateManifest } = yield* attemptRoute(
          () => import("@kortix/manifest-schema"),
        );
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        const templates = yield* attemptRoute(() =>
          listSandboxTemplates(project),
        ).pipe(
          Effect.catchAll((err) => {
            const message = err instanceof Error ? err.message : String(err);
            return failJson(
              { error: `Failed to list sandbox templates: ${message}` },
              500,
            );
          }),
        );
        return routeJson({
          ...
          default_slug:
            templates.find((t) => t.isDefault)?.slug ??
            templates[0]?.slug ??
            null,
        });
      }),
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        const templateResult = yield* Effect.either(
          attemptRoute(() => listSandboxTemplates(project)),
        );
        ...
        yield* attemptRoute(() => reconcileStaleBuilds({ projectId })).pipe(
          Effect.catchAll(() => Effect.void),
        );
        const builds = yield* attemptRoute(() =>
          listSnapshotBuilds(projectId, { limit: 25 }),
...
async function buildSandboxHealth(
  ...
  const isBuilding =
    ...
    (primary
      ? ["pulling", "building"].includes(primary.daytonaState.toLowerCase())
      : false);
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        ).pipe(Effect.catchAll(() => Effect.succeed(SANDBOX_HEALTH_DEGRADED)));
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const info = describeSnapshotError(
      category as ReturnType<typeof classifySnapshotError>,
    );
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    if (!slug || !/^[a-z0-9][a-z0-9_-]{0,63}$/.test(slug)) {
      return c.json(
        { error: "slug must be lowercase letters/digits/_- (1-64 chars)" },
        400,
      );
    }
    if (slug === DEFAULT_SANDBOX_SLUG) {
      return c.json(
        { error: 'slug "default" is reserved for the platform template' },
        409,
      );
    }
    ...
    const dockerfilePath =
      typeof body.dockerfile_path === "string" && body.dockerfile_path.trim()
        ? body.dockerfile_path.trim()
        : undefined;
    if ((image && dockerfilePath) || (!image && !dockerfilePath)) {
      return c.json(
        { error: "Provide exactly one of `image` or `dockerfile_path`." },
        400,
      );
    }
    const entrypoint =
    ...
    const memoryGb =
      typeof body.memory_gb === "number" ? body.memory_gb : undefined;
    ...
    try {
      ...
      if (
        ...
        message.includes("idx_sandbox_templates_project_slug")
      ) {
        return c.json(
    ...
    const patch = {
      ...
      image:
        "image" in body
          ...
          : undefined,
      dockerfilePath:
        "dockerfile_path" in body
          ? typeof body.dockerfile_path === "string"
            ? body.dockerfile_path.trim() || null
            : null
          : undefined,
      entrypoint:
        "entrypoint" in body
          ? typeof body.entrypoint === "string"
            ? body.entrypoint.trim() || null
            : null
          : undefined,
      cpu:
        "cpu" in body
      ...
      memoryGb:
        "memory_gb" in body
          ? typeof body.memory_gb === "number"
            ? body.memory_gb
            : null
          : undefined,
      diskGb:
        "disk_gb" in body
          ? typeof body.disk_gb === "number"
            ? body.disk_gb
            : null
          : undefined,
    };
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    if (row.isShared)
      return c.json(
        { error: "Shared platform templates cannot be deleted." },
        409,
      );
