...
function parseBoundedPositiveInt(
  ...
  if (!Number.isInteger(value) || value < min || value > max) {
    return {
      ok: false,
      error: `${label} must be an integer between ${min} and ${max}`,
    };
  }
      ...
      Effect.gen(function* () {
        ...
        const rows = yield* attemptRoute(() =>
          db
            ...
            .orderBy(
              asc(projectGroupGrants.createdAt),
              asc(projectGroupGrants.groupId),
            ),
        );
        ...
        if (groupIds.length > 0) {
          ...
          for (const m of memberRows) {
            const stats = statsByGroup.get(m.groupId) ?? {
              total: 0,
              overrideCount: 0,
            };
            stats.total += 1;
        ...
        return routeJson({
          grants: rows.map((r) => {
            const stats = statsByGroup.get(r.groupId) ?? {
              total: 0,
              overrideCount: 0,
            };
            return {
  ...
  async (c: any) => {
    return runProjectRouteEffect(
      ...
      Effect.gen(function* () {
        ...
        if (!groupId)
          return yield* failJson({ error: "group_id is required" }, 400);
        if (!role) {
          return yield* failJson(
            { error: "role must be manager, editor, or user" },
            400,
          );
        }
        ...
        const [group] = yield* attemptRoute(() =>
          db
            ...
            .where(
              and(
                eq(accountGroups.groupId, groupId),
        ...
        if (!group)
          return yield* failJson(
            { error: "group not found in this account" },
            404,
          );
        ...
        yield* attemptRoute(() =>
          db
            ...
            .onConflictDoUpdate({
              target: [
                projectGroupGrants.projectId,
                projectGroupGrants.groupId,
              ],
              set: {
                role,
                grantedBy: loaded.userId,
        ...
        return routeJson(
          { project_id: projectId, group_id: groupId, role },
          201,
        );
      }),
    );
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const listableRows = rows.filter((r) => {
      ...
      return (
        r.status !== "stopped" || resumableStoppedSessionIds.has(r.sessionId)
      );
    });
    ...
    const ownerIds = [
      ...new Set(
        visible
          .map((r) => r.createdBy)
          .filter((id): id is string => !!id && id !== loaded.userId),
      ),
    ];
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const ownerEmail =
      visible.row.createdBy && !visible.isOwner
        ? ((await lookupEmailsByUserIds([visible.row.createdBy])).get(
            visible.row.createdBy,
          ) ?? null)
        : null;
    return c.json(
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const maxChars = parseBoundedPositiveInt(
      c.req.query("chars"),
      700,
      80,
      5000,
  ...
  effectHandler(async (c: any) => {
    ...
    if (!visible.canManageSharing) {
      return c.json(
        {
          error:
            "Only the session owner or a project manager can change sharing",
        },
        403,
      );
    }
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const attemptedServerField = serverManagedFields.find((field) =>
      hasOwn(body, field),
    );
    if (attemptedServerField) {
      return c.json(
        { error: `field is server-managed: ${attemptedServerField}` },
        400,
      );
    }
    ...
    const opencodeManagedField = [
      "opencode_session_id",
      "opencodeSessionId",
    ].find((f) => hasOwn(body, f));
    if (opencodeManagedField) {
      return c.json(
        { error: `field is server-managed: ${opencodeManagedField}` },
        400,
      );
    }
    ...
    if (unknownField) {
      return c.json(
        { error: `field is not user-editable: ${unknownField}` },
        400,
      );
    }
  ...
  effectHandler(async (c: any) => {
    ...
    if (!visible.canManageSharing) {
      return c.json(
        {
          error:
            "Only the session owner or a project manager can stop this session",
        },
        403,
      );
    }
...
projectsApp.openapi(
  createRoute({
    ...
    responses: {
      200: json(z.any(), "Resource grants + grantable resources"),
      ...errors(404),
    },
  }),
  effectHandler(async (c: any) => {
    ...
    const memberIds = [
      ...new Set(
        grants
          .filter((g) => g.principalType === "member")
          .map((g) => g.principalId),
      ),
    ];
    const groupIds = [
...
projectsApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    if (!isUuid(principalId))
      return c.json({ error: "principal_id must be a valid id" }, 400);
    const expires = parseExpiresAtBody(body.expires_at);
    if (!expires.ok) return c.json({ error: expires.error }, 400);
    ...
    if (resourceType === "secret") {
      const [s] = await db
        ...
        .where(
          and(
            eq(projectSecrets.projectId, projectId),
            eq(projectSecrets.name, resourceId),
          ),
        )
        .limit(1);
      if (!s)
        return c.json(
          { error: `no secret '${resourceId}' in this project` },
          400,
        );
    } else {
      let config;
      ...
      if (!projectHasResource(config, resourceType, resourceId)) {
        return c.json(
          { error: `no ${resourceType} '${resourceId}' in this project` },
          400,
        );
      }
    }
...
projectsApp.openapi(
  createRoute({
    ...
    ...auth,
    request: {
      params: z.object({ projectId: z.string(), grantId: z.string() }),
    },
