...
const ensureGatewayPermission = (
  ...
  allowed: (c: any, projectId: string, accountId: string) => Promise<boolean>,
  message: string,
) =>
...
const parseWindowDays = (raw: string | undefined): number =>
  Math.min(Math.max(Number(raw) || 30, 1), 365);
...
// Deliberate exception: playground fan-out keeps the per-model promise catches
// local so one upstream failure does not fail the whole comparison request.
