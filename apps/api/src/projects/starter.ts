/**
 * Generic OpenCode-native Kortix project starter.
 *
 * When a brand-new GitHub repo is created via `POST /v1/projects/create-repo`,
 * we commit this scaffold so the user starts from a clean, working
 * OpenCode shape: a `kortix.toml` manifest, an `.opencode/` directory with
 * one primary agent, one subagent, an example slash-command, an example
 * skill, plus a README + CONTEXT.md.
 *
 * Layout matches the OpenCode docs (https://opencode.ai/docs/):
 *   - agents:    .opencode/agents/<name>.md             — primary/subagent personas
 *   - commands:  .opencode/commands/<name>.md           — slash-commands
 *   - skills:    .opencode/skills/<name>/SKILL.md       — on-demand instructions
 *   - config:    .opencode/opencode.jsonc               — runtime config
 *
 * Files are written via the GitHub Contents API after the repo exists,
 * one commit per file under the default branch.
 */
...
Every session you start on Kortix is its own isolated sandbox — a fresh
Daytona VM with this repo cloned onto its own git branch (named after the
session UUID). OpenCode runs inside; the dashboard connects to it via the
sandbox's proxied URL. The branch is yours to merge back into \`main\` when
you're done.

## Editing the project

- **Agents** → \`.opencode/agents/*.md\` — markdown with YAML frontmatter
  (\`mode\`, \`description\`, \`permission\`, optional \`model\`).
- **Commands** → \`.opencode/commands/*.md\` — slash-commands. Body is a
  prompt template; supports \`$ARGUMENTS\`, \`$1\`, \`!\`cmd\`\` shell
  injection, \`@path\` file references.
- **Skills** → \`.opencode/skills/<name>/SKILL.md\` — on-demand instructions
  loaded by agents via the \`skill\` tool. Must have \`name\` +
  \`description\` in frontmatter.
- **Config** → \`.opencode/opencode.jsonc\` — providers, default agent,
  permissions, MCP, plugins.

Commit your changes — every new session picks them up on clone.

## Local dev

\`\`\`sh
git clone https://github.com/${i.repoFullName} ${i.projectName}
cd ${i.projectName}
# Open in Kortix or run \`opencode\` directly:
...
# Source of truth for project-wide config that lives in git alongside the
# code. Sessions clone this repo at boot and read \`.opencode/\` + this
# file to bootstrap their runtime.
...
- You operate inside an isolated session sandbox. The project repo is
  cloned to \`/workspace/.kortix\` on a branch named after the session UUID.
...
The session's UUID branch becomes the PR source; \`main\` is the target.
`;
...
const GITIGNORE = () => `# Local Kortix runtime state — should never be committed.
.kortix/state/
.opencode/sessions/
.opencode/log/
.opencode/cache/
