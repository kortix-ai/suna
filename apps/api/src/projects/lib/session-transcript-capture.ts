  ...
  try {
    ...
    const capture = async (): Promise<CaptureResult | null> => {
      ...
      */
      ...
      if (rows.length === 0 && !completeRead) {
        // An empty bounded read is a failed capture, not an empty session:
        // say so, so a stale mirror is traceable to the read that missed.
        if (!fullHistory) {
