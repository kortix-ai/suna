...
describe('production credential rotation wiring', () => {
  test('mints the worker runtime token through the shared purpose-bound helper', async () => {
    const source = await Bun.file(
      new URL('./legacy-runtime-bootstrap-wiring.ts', import.meta.url),
    ).text();
    expect(source).toContain('mintSessionRuntimeToken({');
    expect(source).toContain("runtimeKind: 'worker'");
    expect(source).toContain('runtimeId: row.sandboxId');
    expect(source).not.toContain('async function mintSessionToken');
  });
});
...
describe('renderLegacyBootstrapScript', () => {
  ...
  test('embeds the entrypoint when given, and the script prefers the manifest copy', () => {
    const s = renderLegacyBootstrapScript({
      relaunch: 'pt-app',
      entrypointSource: '#!/bin/bash\necho supervisor\n',
    });
    expect(s).toContain(
      `EMBEDDED_EP_B64='${Buffer.from('#!/bin/bash\necho supervisor\n').toString('base64')}'`,
    );
  ...
  test('rejects an unsafe opencode home', () => {
    expect(() =>
      renderLegacyBootstrapScript({ relaunch: 'pt-app', opencodeHome: '/x; rm -rf /' }),
    ).toThrow();
...
describe('parseScriptReport', () => {
  const exec = (stdout: string, exitCode = 0): SandboxExecResult => ({
    exitCode,
    stdout,
    stderr: '',
  });
  test('reads the last JSON line', () => {
    const r = parseScriptReport(
...
function makeDeps(
  over: Partial<LegacyBootstrapDeps> & {
    health?: unknown[];
...
describe('bootstrapLegacyRuntime', () => {
  test('legacy + idle platinum box: execs the script, waits for a current serving daemon, records converged', async () => {
    ...
    const states = calls.patches
      .map((p) => (p[LEGACY_BOOTSTRAP_METADATA_KEY] as { state?: string } | undefined)?.state)
      .filter(Boolean);
    expect(states).toEqual(['running', 'converged']);
    const finalRecord = calls.patches.at(-1)?.[LEGACY_BOOTSTRAP_METADATA_KEY] as Record<
      string,
      unknown
    >;
    expect(finalRecord.attempts).toBe(1);
    expect(finalRecord.manifestBuild).toBe(1788044234);
    expect(finalRecord.to).toEqual({
      agentSha256: 'a',
      entrypointSha256: 'e',
      runtimeBuild: 1788044234,
    });
  ...
  test('an install during the boot pass (opencode=updated) triggers exactly one more relaunch', async () => {
    ...
    const updated = {
      daemon: 'ok',
      opencode: 'ok',
      runtime: { build: 1788044234, components: { agent: 'current', opencode: 'updated' } },
    };
    const r = await bootstrapLegacyRuntime(
      input(),
      makeDeps({ health: [LEGACY_HEALTH, updated, updated, updated] }, calls),
    );
    expect(r.outcome).toBe('converged');
  ...
  test('current box: stamps the check and does nothing else', async () => {
    ...
    expect((calls.patches[0][LEGACY_CHECK_METADATA_KEY] as { klass: string }).klass).toBe(
      'current',
    );
  });
  ...
  test('recently checked current box is not probed again inside the TTL', async () => {
    ...
    const meta = {
      [LEGACY_CHECK_METADATA_KEY]: {
        at: new Date(clock.t - LEGACY_CHECK_TTL_MS / 2).toISOString(),
        klass: 'current',
      },
    };
    let probed = false;
  ...
  test('busy OpenCode is never touched', async () => {
    ...
    const r = await bootstrapLegacyRuntime(
      input(),
      makeDeps({ status: { ses_1: { type: 'busy' } } }, calls),
    );
  ...
  test('failed attempt: cooldown, then budget exhausted on the same build, fresh budget on a new build', async () => {
    ...
    expect(
      (
        await bootstrapLegacyRuntime(
          input(failedRecord(1, 1788044234, 60_000)),
          makeDeps({}, calls, clock),
        )
      ).outcome,
    ...
    expect(
      (
        await bootstrapLegacyRuntime(
          { ...input(failedRecord(1, 1788044234, 60_000)), force: true },
          makeDeps({}, calls, clock),
        )
      ).outcome,
    ).toBe('converged');
        ...
        await bootstrapLegacyRuntime(
          input(
            failedRecord(
              LEGACY_BOOTSTRAP_MAX_ATTEMPTS,
              1788044234,
              LEGACY_BOOTSTRAP_COOLDOWN_MS * 2,
            ),
          ),
          makeDeps({}, calls, clock),
        )
      ).outcome,
    ).toBe('skipped-exhausted');
    expect(
      (
        await bootstrapLegacyRuntime(
          {
    ...
    const retry = await bootstrapLegacyRuntime(
      input(failedRecord(1, 1788044234, LEGACY_BOOTSTRAP_COOLDOWN_MS * 2)),
      makeDeps({}, calls, clock),
    );
    expect(retry.outcome).toBe('converged');
    expect(
      (calls.patches.at(-1)?.[LEGACY_BOOTSTRAP_METADATA_KEY] as { attempts: number }).attempts,
    ).toBe(2);
    const newBuild = await bootstrapLegacyRuntime(
      input(failedRecord(LEGACY_BOOTSTRAP_MAX_ATTEMPTS, 1, 60_000)),
      makeDeps({}, calls, clock),
    );
  ...
  test('script failure is recorded with stage and error, audited as failure', async () => {
    ...
    const rec = calls.patches.at(-1)?.[LEGACY_BOOTSTRAP_METADATA_KEY] as {
      state: string;
      error: string;
    };
  ...
  test('relaunched but never converged inside the budget = failed with the last observation', async () => {
    ...
    const r = await bootstrapLegacyRuntime(
      input(),
      makeDeps({ health: [LEGACY_HEALTH, LEGACY_HEALTH] }, calls),
    );
    expect(r.outcome).toBe('failed');
    expect(
      (calls.patches.at(-1)?.[LEGACY_BOOTSTRAP_METADATA_KEY] as { error: string }).error,
    ).toContain('not converged');
  });
  ...
  test('daemon relaunched but its OpenCode install failed = failed, not converged', async () => {
    ...
    const failedOc = {
      daemon: 'ok',
      opencode: 'ok',
      runtime: { build: 1788044234, components: { agent: 'current', opencode: 'failed' } },
    };
    const r = await bootstrapLegacyRuntime(
      input(),
      makeDeps({ health: [LEGACY_HEALTH, failedOc] }, calls),
    );
    expect(r.outcome).toBe('failed');
    expect(r.detail).toBe('opencode convergence failed');
    expect(
      (calls.patches.at(-1)?.[LEGACY_BOOTSTRAP_METADATA_KEY] as { error: string }).error,
    ).toContain('OpenCode install failed');
  });
  ...
  test('daytona: stages and records staged; a staged record is not redone on the same build', async () => {
    ...
    const deps = makeDeps(
      {
        health: [LEGACY_HEALTH, LEGACY_HEALTH],
        exec: async (cmd) => {
          calls.execs.push(cmd);
          return {
            exitCode: 0,
            stdout: '{"ok":true,"stage":"staged","agent_sha256":"a","entrypoint_sha256":"e"}\n',
            stderr: '',
          };
        },
      },
    ...
    const lastPatch = calls.patches.at(-1);
    if (!lastPatch) throw new Error('expected the staged bootstrap metadata patch');
    const again = await bootstrapLegacyRuntime(input(lastPatch, 'daytona'), deps);
  ...
  test('a rotated session PAT is handed to the script; none when nothing to rotate', async () => {
    ...
    expect(() =>
      renderLegacyBootstrapScript({ relaunch: 'pt-app', kortixToken: "x'; rm -rf /" }),
    ).toThrow();
    // next-start providers never rotate: the provider owns the daemon's env.
    let minted = 0;
    const daytona = makeDeps(
      {
        health: [LEGACY_HEALTH, LEGACY_HEALTH],
        ...
        exec: async (cmd) => {
          calls.execs.push(cmd);
          return { exitCode: 0, stdout: '{"ok":true,"stage":"staged"}\n', stderr: '' };
        },
      },
      calls,
    );
  ...
  test('an in-progress attempt younger than the stale window is not duplicated', async () => {
    ...
    const meta = {
      [LEGACY_BOOTSTRAP_METADATA_KEY]: {
        ...
        lastAttemptAt: new Date(clock.t - 60_000).toISOString(),
      },
    };
    expect((await bootstrapLegacyRuntime(input(meta), makeDeps({}, calls, clock))).outcome).toBe(
      'skipped-in-progress',
    );
    expect(calls.execs).toHaveLength(0);
  });
  ...
  test('unsupported provider is skipped before any probe', async () => {
    const calls: Calls = { patches: [], audits: [], execs: [] };
    expect((await bootstrapLegacyRuntime(input(null, 'local'), makeDeps({}, calls))).outcome).toBe(
      'skipped-unsupported',
    );
  });
});
