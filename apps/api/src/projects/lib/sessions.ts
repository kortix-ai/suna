...
export async function createProjectSession(input: {
  ...
  const resolvedDefaultAgent = projectDefaultAgent ?? 'default';
  const agentName =
    requestedAgent && requestedAgent !== 'default'
      ? requestedAgent
      : callerIsMeta || requestedAgent === 'default'
        ? resolvedDefaultAgent
        : META_AGENT_NAME;
  const platformMetaAgent = isMetaAgentName(agentName);
