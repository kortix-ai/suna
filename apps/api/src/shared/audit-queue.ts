...
// 100, lowered from 500 (Essentia convoy fix, 2026-08-21): each row's BEFORE
// INSERT trigger takes a per-session `FOR UPDATE` lock held to the batch's
// COMMIT, so a large batch holds every touched session's lock for the whole
// commit and cross-blocks the other replica on many rows at once. Smaller
// batches commit sooner and shorten the lock-hold window. Tunable via
// KORTIX_AUDIT_FLUSH_MAX.
