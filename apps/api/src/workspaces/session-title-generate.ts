...
import { workspaceSessionMetadataMerge } from './lib/session-metadata-merge';
...
async function loadRow(sessionId: string, workspaceId: string): Promise<WorkspaceSessionRow | null> {
  const [row] = await db
    ...
    .where(and(eq(projectSessions.sessionId, sessionId), eq(projectSessions.workspaceId, workspaceId)))
    .limit(1);
...
function needsTitle(row: WorkspaceSessionRow): boolean {
...
function storedTitleSource(row: WorkspaceSessionRow): string | null {
  const metadata = (row.metadata ?? {}) as Record<string, unknown>;
...
function sessionModel(row: WorkspaceSessionRow): string | null {
  const metadata = (row.metadata ?? {}) as Record<string, unknown>;
 ...
 */
export async function persistTitle(row: WorkspaceSessionRow, title: string): Promise<void> {
  await db
    .update(projectSessions)
    .set({ metadata: workspaceSessionMetadataMerge({ name: title }), updatedAt: new Date() })
    .where(
      and(
        eq(projectSessions.sessionId, row.sessionId),
        eq(projectSessions.workspaceId, row.workspaceId),
...
export interface GenerateSessionTitleOptions {
  ...
  persist?: (row: WorkspaceSessionRow, title: string) => Promise<void>;
  fallbackModel?: (
    input: GenerateSessionTitleInput,
    excludedModel?: string,
  ) => Promise<string | null>;
...
export async function generateSessionTitleFromFirstPrompt(
  ...
  if (!input.sessionId || !input.workspaceId || !input.accountId || !input.userId || !suppliedText)
    return;
  ...
  const mint =
    options.mintKey ??
    (async (accountId, workspaceId, userId) => {
  ...
  // workspace's real keys in its list.
  const revoke =
    options.revokeKey ?? ((workspaceId, keyId) => deleteGatewayKey(workspaceId, keyId).then(() => {}));
  ...
  try {
    ...
    if (!model) {
      ...
      const minted = await mint(input.accountId, input.workspaceId, input.userId);
      if (minted) {
        try {
          ...
          await revoke(input.workspaceId, minted.keyId).catch(() => {});
        }
      }
    }
    if (!title) {
    ...
    const fresh = await load(input.sessionId, input.workspaceId);
