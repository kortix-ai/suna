...
export async function claimDueScheduleSlots(input: {
  ...
  const candidates = await db
    .select({
      workspaceId: projectTriggerRuntime.workspaceId,
    ...
    .innerJoin(projects, eq(projects.workspaceId, projectTriggerRuntime.workspaceId))
    .where(
      and(
    ...
    .orderBy(asc(projectTriggerRuntime.nextFireAt), asc(projectTriggerRuntime.workspaceId))
    .limit(input.limit);
  ...
  const results = await mapConcurrently(candidates, 8, async (candidate) => {
    ...
    return db.transaction(async (tx) => {
      ...
      const advanced = await tx
        ...
        .where(
          and(
            eq(projectTriggerRuntime.workspaceId, candidate.workspaceId),
        ...
        .returning({ workspaceId: projectTriggerRuntime.workspaceId });
      ...
      const inserted = await tx
        ...
        .onConflictDoNothing({
          target: [
            projectTriggerExecutions.workspaceId,
      ...
      const [existing] = await tx
        ...
        .where(
          and(
            eq(projectTriggerExecutions.workspaceId, candidate.workspaceId),
...
export async function countUncatalogedTriggerWorkspaces(): Promise<number> {
  const rows = await db
    .select({ count: sql<number>`count(distinct ${projectTriggerRuntime.workspaceId})` })
    .from(projectTriggerRuntime)
    .innerJoin(projects, eq(projects.workspaceId, projectTriggerRuntime.workspaceId))
