...
export function legacyRehydrateSpec(
  ...
  const fromWorkspace = asObject(asObject(workspaceMetadata).legacy_migration);
  const sourceSandboxId = firstString(fromSession.source_sandbox_id, fromWorkspace.source_sandbox_id);
...
export function rekeyOpencodeDb(dbPath: string, newWorkspaceId: string): { sessions: number } {
  const local = new Database(dbPath);
  try {
    local.exec('PRAGMA foreign_keys=OFF');
    local.prepare('UPDATE project SET id = ?').run(newWorkspaceId);
    local.prepare('UPDATE session SET project_id = ?').run(newWorkspaceId);
...
export async function rehydrateSessionChat(input: RehydrateInput): Promise<void> {
  ...
  const newWorkspaceId = await waitForOpencodeWorkspaceId(sandbox, 180_000);
  if (!newWorkspaceId) {
  ...
  //    migration's source sandbox id — for suna migrations, the new workspaceId).
  ...
  try {
    ...
    const { sessions } = rekeyOpencodeDb(join(workDir, dbName), newWorkspaceId);
    appLogger.info('[rehydrate] re-keyed archive', { sessionId, sessions, newWorkspaceId });
...
async function waitForOpencodeWorkspaceId(
