...
export async function repoStep(ctx: SunaMigrationContext): Promise<void> {
  ...
  await ctx.checkpoint({
    project_id: repo.workspaceId, repo_url: repo.upstreamUrl, repo_owner: repo.repoOwner,
...
export async function dbStep(ctx: SunaMigrationContext): Promise<void> {
  ...
  const workspaceId = ctx.progress.project_id as string;
  ...
  await (ctx.database as any).transaction(async (tx: any) => {
    await tx.insert(projects).values({
      workspaceId, accountId: ctx.accountId, name: 'Legacy (Suna) projects',
      ...
      metadata: {
        ...
        // keyed by workspaceId and shared by every session, so it is project-level.
        legacy_migration: { run_id: ctx.runId, source_sandbox_id: workspaceId, migrated_at: now.toISOString() },
      },
    }).onConflictDoNothing({ target: projects.workspaceId });
    ...
    await tx.insert(projectGitConnections).values({
      ...
      visibility: 'private',
      status: 'connected',
    } as any).onConflictDoNothing({ target: projectGitConnections.workspaceId });
  ...
  ctx.log('db: committed', { project_id: workspaceId, sessions: specs.length });
}
