 ...
 * `workspaceId` is the opencode projectID; the rehydrate ship re-keys project.id
 * and session.project_id to the live workspace's id, so any stable value works.
 */
export function writeConversations(dbPath: string, workspaceId: string, sessions: SessionToWrite[]): WriteResult {
  ...
  try {
    ...
    db.query(`INSERT OR IGNORE INTO "project" (id, worktree, vcs, name, icon_url, icon_color, time_created, time_updated, time_initialized, sandboxes, commands)
              VALUES (?, ?, 'git', '', '', '', ?, ?, ?, '[]', NULL)`).run(workspaceId, WORKSPACE_DIR, now, now, now);
    ...
    const tx = db.transaction(() => {
      ...
      for (const s of sessions) {
        ...
        insSession.run(sessionID, workspaceId, slugFromTitle(title, `session-${sIdx++}`), WORKSPACE_DIR, title, OPENCODE_VERSION, now, now);
        res.sessions++;
