...
function streamRouteSource(): string {
  const start = routes.indexOf("path: '/{workspaceId}/sessions/{sessionId}/reload-stream'");
  expect(start).toBeGreaterThan(-1);
  const end = routes.indexOf("path: '/{workspaceId}/sessions/{sessionId}/scope'", start);
  expect(end).toBeGreaterThan(start);
  return routes.slice(start, end);
}

describe('POST /workspaces/:workspaceId/sessions/:sessionId/reload-stream', () => {
  test('keeps the existing JSON route and adds a separate streamed route', () => {
    expect(routes).toContain("path: '/{workspaceId}/sessions/{sessionId}/reload'");
    expect(routes).toContain("path: '/{workspaceId}/sessions/{sessionId}/reload-stream'");
  });

  test('authorizes before opening the stream', () => {
    const source = streamRouteSource();
    expect(source.indexOf('assertWorkspaceCapability(')).toBeLessThan(
