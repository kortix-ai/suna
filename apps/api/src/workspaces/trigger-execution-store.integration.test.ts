...
const WORKSPACE_ID = '00000000-0000-4000-a000-000000009802';
...
describeWithDb('durable exact trigger scheduler — real PostgreSQL', () => {
  ...
  test('concurrent scheduler pods claim one slot, coalesce backlog, and advance atomically', async () => {
    ...
    const executions = await db
      ...
      .where(
        and(
          eq(projectTriggerExecutions.workspaceId, WORKSPACE_ID),
    ...
    const [runtime] = await db
      ...
      .where(
        and(eq(projectTriggerRuntime.workspaceId, WORKSPACE_ID), eq(projectTriggerRuntime.slug, SLUG)),
      );
  ...
  test('paused projects remain due but are not claimed or advanced', async () => {
    ...
    const executions = await db
      .select()
      .from(projectTriggerExecutions)
      .where(eq(projectTriggerExecutions.workspaceId, WORKSPACE_ID));
