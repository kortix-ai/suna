...
export function workspaceMetaAgentEnabled(metadata: unknown): boolean {
  return resolveFeatureFlag(metadata, 'meta_agent');
}

export function addPlatformMetaAgent(config: WorkspaceConfigSummary): WorkspaceConfigSummary {
  return {
    ...config,
 ...
 *
 * The workspace-bound PAT and the launching user's IAM role remain the outer
 * authorization boundaries. Workspace secrets and connectors stay unavailable
