...
describe("serializeWorkspace icon", () => {
  ...
  test("is null when metadata.icon is malformed", () => {
    expect(
      serializeWorkspace(workspaceRow({ icon: "not-an-emoji" })).icon,
    ).toBeNull();
  });

  test("is null when metadata.icon is oversized", () => {
    expect(
      serializeWorkspace(workspaceRow({ icon: "x".repeat(5000) })).icon,
    ).toBeNull();
  });
  ...
  test("metadata defaults to {} on the serialized workspace when the row is null", () => {
...
describe("serializeWorkspace — icon_glyph", () => {
  test("a stored glyph is exposed as a top-level field", () => {
    ...
    expect(serializeWorkspace(row).icon_glyph).toEqual({
      name: "Rocket",
      color: "blue",
    });
  });
...
// that, so a workspace whose manifest grants nothing still reported 'available'
...
function declarative(
  env: Array<string[] | "all" | undefined>,
): SecretAgentGrantConfig {
  return {
...
describe("secretDeliveryBlockedReason", () => {
  ...
  test("any one agent granting it is enough", () => {
    ...
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "egress", config),
    ).toBeNull();
  });
  ...
  test("a declarative config with zero agents is ambiguous, so it reports null", () => {
    ...
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "egress", declarative([])),
    ).toBeNull();
  });
  ...
  test("runtime and denied secrets never carry the reason", () => {
    ...
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "runtime", declarative([])),
    ).toBeNull();
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "denied", declarative([])),
    ).toBeNull();
  });

  // The commonest broken setup, and the one that sent a real user hunting for
  // hours: an egress secret on a workspace whose manifest declares no `agents:`.
  ...
  test("opencode discovery means no manifest grant exists, which is certain", () => {
    ...
    expect(secretDeliveryBlockedReason("BOUNDARY_TEST", "egress", config)).toBe(
      "no_agent_grant",
    );
  });
  ...
  test("runtime secrets are unaffected by opencode discovery", () => {
    ...
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "runtime", config),
    ).toBeNull();
  });
  ...
  test.each([
    ...
    [
      "a non-array agents field",
      { agent_discovery: "declarative", agents: null },
    ],
    [
      "an unknown discovery mode",
      { agent_discovery: "something-else", agents: [] },
    ],
    ...
    expect(
      secretDeliveryBlockedReason(
        "BOUNDARY_TEST",
        "egress",
        config as SecretAgentGrantConfig,
      ),
    ).toBeNull();
  });
  ...
  test("no config at all reports null", () => {
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "egress", null),
    ).toBeNull();
    expect(
      secretDeliveryBlockedReason("BOUNDARY_TEST", "egress", undefined),
    ).toBeNull();
  });
