...
import { workspaceSecretIsConfiguredForConsumer } from '../secrets';
...
export async function connectorConnectionIsConnected(input: {
  ...
  if (connector.providerType === 'channel') {
    ...
    const connectionSlug =
      typeof connection.metadata.connector_slug === 'string'
        ...
        : connector.slug;
    if (platform === 'slack') {
      return (await loadSlackInstall(connector.workspaceId).catch(() => null)) !== null;
    }
    if (platform === 'teams') {
      return (await loadTeamsInstall(connector.workspaceId).catch(() => null)) !== null;
    }
    if (platform === 'email') {
      const install = await loadAgentMailInstall(connector.workspaceId, connectionSlug).catch(
        () => null,
      );
  ...
  return stored?.authSecret
    ? workspaceSecretIsConfiguredForConsumer({
...
export async function ensureEmailSessionBinding(input: {
  ...
  const connectionId = await loadEmailInstallConnectionId(input.workspaceId, input.inboxId);
...
> {
  ...
  for (const [requestedAlias, binding] of Object.entries(input.bindings)) {
    ...
    if (!row) {
      return {
        ok: false,
        error: `Connection is not available for connector alias "${alias}" in this workspace`,
...
export async function resolveRequiredConnectorConnections(input: {
  ...
  // An unconfigured alias outranks a missing authorization. Only the workspace
...
export async function missingRequiredConnectorConnectionsForSession(input: {
  ...
  // flow for a connector the workspace does not have would strand them there.
  if (unavailable.length > 0) {
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(projectSessionConnectorBindings.workspaceId, input.workspaceId),
...
export async function resolveSessionConnectorConnection(input: {
    ...
    const [bound] = await db
      ...
      .innerJoin(
        ...
        and(
          ...
          eq(connectors.workspaceId, projectSessionConnectorBindings.workspaceId),
        ),
      )
      .where(
        and(
  ...
  // Hand the workspace-default fallback the SAME principal identity the original
  ...
  const fallbackFromDefault = await resolveWorkspaceDefaultConnectorConnection({
    accountId: input.accountId,
    workspaceId: input.workspaceId,
    alias,
    actingUserId,
...
/**
 * Workspace-default connection resolution — the fallback an UNBOUND alias resolves
 ...
 * workspace default can resolve it directly. Connector discovery and execution do
 * not call this helper. They use `resolveSessionConnectorConnection`, which also
 * enforces the stored session scope.
 */
export async function resolveWorkspaceDefaultConnectorConnection(input: {
  accountId: string;
  workspaceId: string;
  alias: string;
  actingUserId?: string;
