...
const { propagateWorkspaceSecretsToActiveSandboxes, pushSessionAgentConfigToSandbox } =
...
describe('propagateWorkspaceSecretsToActiveSandboxes', () => {
  test('reports a sandbox only after the daemon confirms revision, file write, and export count', async () => {
    const result = await propagateWorkspaceSecretsToActiveSandboxes('proj-1');
