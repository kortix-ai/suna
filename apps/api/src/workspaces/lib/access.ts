...
// project. Every isolated project counts, even when another workspace uses the
...
export async function enforceWorkspaceQuota(
  ...
  if (count >= limit) {
    // FREE_TIER_WORKSPACE_LIMIT is 1, so this string is pluralized rather than
    // hardcoded — "limited to 1 projects" reads as a bug to the user.
    const workspacesWord = limit === 1 ? 'project' : 'projects';
    return c.json(
      {
        error:
          limit === FREE_TIER_WORKSPACE_LIMIT
            ? `Free accounts are limited to ${limit} ${workspacesWord}. Upgrade to a paid plan to create more.`
            : `This account has reached its limit of ${limit} ${workspacesWord}.`,
...
async function loadWorkspaceSessionRow(
  ...
  const [row] = await db
    ...
    .where(and(
      ...
      eq(projectSessions.workspaceId, loaded.row.workspaceId),
 ...
 * workspace manager. A project member with no manage rights (e.g. an editor
...
const loadWorkspaceMemberRole = ttlMemo({
  ...
  loader: async (workspaceId: string, userId: string): Promise<WorkspaceRole | null> => {
    const [row] = await db
      .select({ workspaceRole: projectMembers.projectRole })
      .from(projectMembers)
...
export async function assertWorkspaceCapability(
  ...
  if (isRepositoryWorkspaceAction(action)) {
    await assertAgentSessionWorkspaceAllowsRepository(c, accountId, workspaceId);
  }
  const actingTokenId =
  ...
  await assertAuthorized(
    ...
    accountId,
    action,
    { type: 'project', id: workspaceId, ...(resource ? { resource } : {}) },
    actingTokenId,
...
export async function workspaceCapabilityAllowed(
  ...
  if (
    isRepositoryWorkspaceAction(action) &&
    !(await agentSessionWorkspaceAllowsRepository(c, accountId, workspaceId))
  ) {
    return false;
  }
  const actingTokenId =
...
export async function agentSessionWorkspaceAllowsRepository(
  ...
  return sessionWorkspaceAllowsRepositoryAccess({ sessionId, accountId, workspaceId });
}
...
export async function assertAgentSessionWorkspaceAllowsRepository(
  ...
  if (await agentSessionWorkspaceAllowsRepository(c, accountId, workspaceId)) return;
