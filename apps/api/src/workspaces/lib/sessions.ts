...
export async function buildSessionSandboxEnvVars(input: {
  ...
  /** Resolved per-workspace `llm_gateway` feature flag. Gateway ON →
  ...
  // value to the sandbox. Every OTHER secret is workspace-wide (secret
  ...
  return {
    ...
    // Rolling-deploy compatibility for sandbox images built before the
    // Workspace namespace became canonical. Remove only in a major image/API
    // cutover after every supported sandbox image reads KORTIX_WORKSPACE_*.
...
export async function createWorkspaceSession(input: {
  ...
  // connectors keeps the workspace-default fallback for the rest unless the caller
  ...
  const workspaceDefaultAgent = normalizeString(loadedAgents.defaultAgent) ?? mirroredDefaultAgent;
  // The meta coordinator is a per-workspace experimental opt-in
  ...
  // agent name.
  const metaAgentEnabled = workspaceMetaAgentEnabled(workspace.metadata);
  ...
  // to the workspace default (the observed failure was meta "spawning a worker"
  ...
  if (platformMetaAgent && callerIsMeta) {
    return {
      error: {
        ...
        body: {
          error:
            'The meta coordinator cannot spawn another meta coordinator — pick a workspace agent',
  ...
  if (
    !platformMetaAgent &&
    workspaceRequiresDeclaredAgents(workspace.metadata, config.KORTIX_REQUIRE_DECLARED_AGENTS)
  ) {
