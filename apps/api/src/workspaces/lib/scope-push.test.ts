...
// spawnChild re-runs mergeWorkspaceEnv + the gateway strip.
//
...
mock.module('../../llm-gateway/enablement', () => ({
  workspaceLlmGatewayEnabled: async () => gatewayEnabled,
}));
...
describe('pushSessionScopeToSandbox', () => {
  test('pushes the re-derived snapshot and asks for the opencode restart', async () => {
    ...
    // re-run mergeWorkspaceEnv. This is the load-bearing part of the fix: the
  ...
  test('re-derives from the freshly-committed allowlist, not a cached snapshot', async () => {
    ...
    // (The mocked listWorkspaceSecretsSnapshotForUser returns the same set for
