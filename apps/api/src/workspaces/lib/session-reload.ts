...
import { invalidateWorkspaceMirror, type GitBackedWorkspace } from '../git';
import {
  agentConfigEtag,
...
export type ReloadAgentFiles =
  ...
  | 'kept-yours'
  /** The workspace keeps no agent files in the repo. */
  | 'not-applicable'
...
function reloadOutcomeSentence(result: SessionReloadResult): string {
  switch (result.agent_files) {
    ...
    case 'not-applicable':
      return 'Reloaded. This workspace keeps no agent files in the repo, so only the compiled config changed.';
    case 'kept-yours':
...
export async function latestAgentConfigEtag(input: {
  ...
  const [[workspace], [session]] = await Promise.all([
    db
      ...
      .from(projects)
      .where(and(eq(projects.workspaceId, input.workspaceId), eq(projects.accountId, input.accountId)))
      .limit(1),
    input.sessionId
      ? db
          .select({
  ...
  if (!workspace?.defaultBranch) return null;
  const gitWorkspace: GitBackedWorkspace = {
  ...
  invalidateWorkspaceMirror(input.workspaceId);
  const compiled = await (
    !workspaceModeAllowsFullRepository(workspaceModeFromSessionMetadata(session?.metadata)) &&
    session?.agentName
      ? resolveSelectedAgentConfigForSession(gitWorkspace, session.agentName, input.baseRef)
      : resolveCompiledAgentConfigForSession(gitWorkspace, input.baseRef)
  ).catch(() => null);
 ...
 *
 * `null` when it cannot be told — the box is unreachable, or the workspace has no
...
export async function reloadSessionConfig(input: {
  ...
  // the push, the etag compare. See `invalidateWorkspaceMirror` above: a reload
