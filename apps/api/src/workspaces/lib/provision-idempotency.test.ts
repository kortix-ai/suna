...
function workspaceRow(over: Partial<WorkspaceRow> = {}): WorkspaceRow {
  const now = new Date('2026-08-05T12:00:00.000Z');
  return {
    workspaceId: '00000000-0000-4000-a000-0000000050ff',
    accountId: ACCOUNT,
 ...
 */
function fakeStore(rows: ReturnType<typeof workspaceRow>[] = []) {
...
describe('readProvisionIdempotencyKey', () => {
  ...
  test('an absent key is legal — provision without one keeps working', () => {
    expect(readProvisionIdempotencyKey({ name: 'My First Workspace' })).toEqual({
      ok: true,
      key: null,
    });
  });
...
describe('classifyProvisionReplay', () => {
  ...
  test('a seed still pending inside the window is in-flight, not a replay', () => {
    ...
    const workspace = workspaceRow({ metadata: pendingSeed, createdAt: new Date(T0 - 5_000) });

    expect(classifyProvisionReplay(workspace, T0)).toEqual({ kind: 'in_flight', workspace });
  });
  ...
  test('the same row past the window is a replay — the bound must expire', () => {
    ...
    expect(classifyProvisionReplay(workspace, T0)).toEqual({ kind: 'replay', workspace });
  });
  ...
  test('the boundary itself is a replay, not in-flight', () => {
    ...
    expect(classifyProvisionReplay(workspace, T0).kind).toBe('replay');
  });

  test('a verified seed is a replay however fresh it is', () => {
    const workspace = workspaceRow({ metadata: verifiedSeed, createdAt: new Date(T0 - 1) });
  ...
  test('a deliberate seed opt-out is a replay however fresh it is', () => {
    ...
    const workspace = workspaceRow({ metadata: optedOut, createdAt: new Date(T0 - 1) });
  ...
  test('a legacy row with no seed metadata is a replay, not a permanent 409', () => {
    for (const metadata of [{}, null, { git: 'not-an-object' }, { git: { seed: 7 } }]) {
      const workspace = workspaceRow({ metadata, createdAt: new Date(T0 - 1) });
      expect(classifyProvisionReplay(workspace, T0).kind).toBe('replay');
    }
  });
  ...
  test('the insert-race loser re-reading a just-inserted winner sees in_flight', () => {
    ...
    const winner = workspaceRow({ metadata: pendingSeed, createdAt: new Date(T0 - 3) });
  ...
  test('the window is caller-overridable so the bound itself is testable', () => {
    ...
    expect(classifyProvisionReplay(workspace, T0, 10_000).kind).toBe('in_flight');
    expect(classifyProvisionReplay(workspace, T0, 1_000).kind).toBe('replay');
  });
...
describe('findIdempotentProvision', () => {
  ...
  test('no key means no lookup at all — the database is never touched', async () => {
    const store = fakeStore([workspaceRow()]);
  ...
  test('two calls with one key yield one project and one shared project_id', async () => {
    ...
    const provision = async (key: string | null) => {
      ...
      const row = workspaceRow({
        workspaceId: `00000000-0000-4000-a000-00000000${String(reposCreated).padStart(4, '0')}`,
        idempotencyKey: key,
      });
      store.rows.push(row);
      return { workspaceId: row.workspaceId, created: true };
    };
  ...
  test('two calls with no key still create two projects — same name, deliberate duplicate', async () => {
    ...
    const provision = async (key: string | null) => {
      ...
      reposCreated += 1;
      store.rows.push(workspaceRow({ workspaceId: `p-${reposCreated}`, idempotencyKey: key }));
      return { created: true };
    };
...
describe('provisionReplayResponse', () => {
  const REPLAY_ACCESS = { workspaceRole: 'manager' as const, effectiveRole: 'manager' as const };
  const seededRow = workspaceRow({
    metadata: { git: { repo_id: '987654321', seed: { seeded: true, expected: true } } },
  });

  test('returns the same workspace_id the original create returned', () => {
    expect(provisionReplayResponse(seededRow, REPLAY_ACCESS).workspace_id).toBe(seededRow.workspaceId);
  });

  test('carries the create response fields the caller lost', () => {
    expect(provisionReplayResponse(seededRow, { workspaceRole: 'manager', effectiveRole: 'manager' }))
      .toMatchObject({
        account_id: ACCOUNT,
        name: 'My First Workspace',
        repo_id: '987654321',
  ...
  test('reports the grant the caller actually holds, not the creator\'s', () => {
    // `grantWorkspaceRole` runs only on the create path. A second account admin
    ...
    expect(replay.workspace_role).toBeNull();
    expect(replay.effective_workspace_role).toBe('manager');
  });
  ...
  test('reports a real grant when the caller holds one', () => {
    ...
    expect(replay.workspace_role).toBe('manager');
  });
  ...
  test('never mints a push credential as a side effect of a retry', () => {
    ...
    // POST /v1/projects/:workspaceId/git-token for it.
  ...
  test('does not leak the idempotency key back to the caller', () => {
    ...
    // contract (@kortix/api-contract WorkspaceSchema) has no field for it.
...
describe('describeProvisionedRepo', () => {
  ...
  test('metadata written before this shape existed degrades to nulls, never throws', () => {
    expect(describeProvisionedRepo(workspaceRow({ metadata: {} }))).toEqual({
      repoId: null,
      seeded: false,
    });
    expect(describeProvisionedRepo(workspaceRow({ metadata: null }))).toEqual({
      repoId: null,
      seeded: false,
    });
    expect(describeProvisionedRepo(workspaceRow({ metadata: { git: 'not-an-object' } }))).toEqual({
      repoId: null,
      seeded: false,
    });
    expect(describeProvisionedRepo(workspaceRow({ metadata: { git: { seed: 7 } } }))).toEqual({
      repoId: null,
      seeded: false,
    });
  });
  ...
  test('a non-string repo id is normalised to a string, matching the create response', () => {
    const row = workspaceRow({ metadata: { git: { repo_id: 987654321 } } });
