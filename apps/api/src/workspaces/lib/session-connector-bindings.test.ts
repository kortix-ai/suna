...
import {
  projectSessionIdForWorkspacePrincipal,
...
describe('session connector binding security contracts', () => {
  ...
  test('Supabase authentication session identity is not a Kortix project session identity', () => {
    expect(projectSessionIdForWorkspacePrincipal(undefined, 'supabase-auth-session')).toBeNull();
    expect(
      projectSessionIdForWorkspacePrincipal('11111111-1111-4111-a111-111111111111', 'kortix-session'),
    ).toBe('kortix-session');
  });
