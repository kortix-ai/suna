/**
 * Workspace resource registry + per-resource list filtering.
 *
 ...
 * one place that maps a WorkspaceConfigSummary → those ids, so the grant-validation
...
import { filterAccessibleWorkspaceResources, hasAnyResourceGrants } from '../../iam';
import { loadWorkspaceConfig, listRepoFiles } from '../git';
import { withWorkspaceGitAuth } from './git';

/**
 * Load a workspace's config WITH its repo file list. File-based agents/skills
 * (`.opencode/agent/*.md`, `skills/<slug>/`) are discovered by scanning the
 * files passed to loadWorkspaceConfig — calling it with `[]` finds NONE. Every
 * resource path (the grant picker, the visibility denier) must go through this.
 */
export async function loadConfigWithFiles(
  row: Parameters<typeof withWorkspaceGitAuth>[0] & { defaultBranch: string },
  ...
  return loadWorkspaceConfig(gitWorkspace, files);
}
...
/** Map a loaded workspace config → the grantable agent/skill resource ids. */
export function workspaceResourcesFromConfig(config: WorkspaceConfigSummary): WorkspaceResources {
  return {
...
/** Validate a (resourceType, resourceId) against what the workspace actually has. */
export function workspaceHasResource(
  config: WorkspaceConfigSummary,
  resourceType: 'agent' | 'skill',
  resourceId: string,
): boolean {
  const res = workspaceResourcesFromConfig(config);
...
export async function filterConfigResourcesForUser(
  config: WorkspaceConfigSummary,
  ctx: { userId: string; accountId: string; workspaceId: string; actingTokenId?: string },
): Promise<WorkspaceConfigSummary> {
  ...
  const [okAgents, okSkills] = await Promise.all([
    filterAccessibleWorkspaceResources(ctx.userId, ctx.accountId, ctx.workspaceId, 'agent', agentIds, ctx.actingTokenId),
    filterAccessibleWorkspaceResources(ctx.userId, ctx.accountId, ctx.workspaceId, 'skill', skillIds, ctx.actingTokenId),
  ]);
 ...
 * when the workspace actually scopes something (the common case is zero work — two
...
export async function resourceDenierForRequest(ctx: {
  ...
  if (!(await hasAnyResourceGrants(ctx.workspaceId))) return null;
  let config: WorkspaceConfigSummary;
  try {
