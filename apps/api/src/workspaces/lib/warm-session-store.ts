...
export async function withWarmWorkspaceSessionLock<T>(
  ...
  const lockIdentity = `warm_session:${scope.workspaceId}:${scope.userId}`;
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        eq(projectSessions.workspaceId, scope.workspaceId),
