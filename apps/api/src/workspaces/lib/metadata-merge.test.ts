...
async function readMeta(workspaceId: string): Promise<Record<string, unknown>> {
  const [row] = await db.select({ metadata: projects.metadata }).from(projects).where(eq(projects.workspaceId, workspaceId)).limit(1);
...
d('atomic merge preserves the routing pin (throwaway Postgres)', () => {
  ...
  test('the delete-key path removes a key while preserving the pin', async () => {
    ...
    await db.update(projects).set({ metadata: metadataMerge({ triggers_paused: true }) }).where(eq(projects.workspaceId, workspaceId));

    await db.update(projects).set({ metadata: metadataMerge({}, ['triggers_paused']) }).where(eq(projects.workspaceId, workspaceId));
  ...
  test('nested experimental toggles never clobber each other or the pin, and cleanup drops the empty object', async () => {
    ...
    await db.update(projects).set({ metadata: metadataMergeSubtree('experimental', { agent_tunnel: true }) }).where(eq(projects.workspaceId, workspaceId));
    await db.update(projects).set({ metadata: metadataMergeSubtree('experimental', { llm_gateway: false }) }).where(eq(projects.workspaceId, workspaceId));
    ...
    await db.update(projects).set({ metadata: metadataClearSubtreeKey('experimental', 'agent_tunnel') }).where(eq(projects.workspaceId, workspaceId));
    ...
    await db.update(projects).set({ metadata: metadataClearSubtreeKey('experimental', 'llm_gateway') }).where(eq(projects.workspaceId, workspaceId));
    meta = await readMeta(workspaceId);
