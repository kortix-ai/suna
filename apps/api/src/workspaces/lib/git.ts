...
export async function getWorkspaceGitConnection(workspaceId: string): Promise<WorkspaceGitConnectionRow | null> {
  const [row] = await db
    ...
    .where(eq(projectGitConnections.workspaceId, workspaceId))
    .limit(1);
  return row ?? null;
}
...
export async function getWorkspaceGitCredential(
  ...
  const [row] = await db
    ...
    .where(and(
      eq(projectGitCredentials.workspaceId, workspaceId),
...
export async function resolveWorkspaceGitAuth(workspace: WorkspaceRow): Promise<{
  ...
  if (remote.authMethod === 'project_credential') {
    ...
    if (credential) {
      const token = decryptWorkspaceSecret(workspace.workspaceId, credential.valueEnc);
  ...
  const legacyToken = await getWorkspaceSecretValueForConsumer({
    workspaceId: workspace.workspaceId,
    accountId: workspace.accountId,
    name: WORKSPACE_GIT_AUTH_SECRET_NAME,
    consumer: 'git_proxy',
  });
  if (legacyToken) {
    return {
 ...
 *    holds `project.gitops.push` / `.read` on it; a workspace-scoped PAT must
 ...
 * project row, then runs the normal `resolveWorkspaceGitAuth` resolution
