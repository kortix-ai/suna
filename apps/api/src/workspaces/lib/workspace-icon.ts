/**
 * Validator for `projects.metadata.icon` — the per-workspace emoji shown on the
 * project card.
 *
 * Every write path (provision / create-repo / link-repository) and the read
 * path (serializeWorkspace) run values through this one function, so a value that
 * reached the column can always be rendered.
 *
 * Returning `null` means "no icon". It never throws: a malformed icon must not
 * fail project creation, because the icon is decoration and the workspace is not.
 */
...
export function normalizeWorkspaceIcon(input: unknown): string | null {
  ...
  // would land in the jsonb column and render straight into WorkspaceCard.
