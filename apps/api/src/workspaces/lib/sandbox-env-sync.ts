...
export interface WorkspaceSecretPropagationTarget {
  session_id: string;
...
export interface WorkspaceSecretPropagationResult {
  ok: boolean;
  ...
  failed: number;
  exported: number;
  results: WorkspaceSecretPropagationTarget[];
}
...
export async function resolveSandboxEnvSnapshot(
  ...
  const resolved = await resolveOwnerRawEnv(workspaceId, sessionId, requestedAgent);
...
export async function propagateWorkspaceSecretsToActiveSandboxes(
...
): Promise<WorkspaceSecretPropagationResult> {
  const report: WorkspaceSecretPropagationResult = {
    ok: true,
    active_sandboxes: 0,
    targeted: 0,
    synced: 0,
  ...
  try {
    ...
    if (targets.length === 0) {
      console.info('[env-sync] propagate: no active sandboxes found', { workspaceId, totalRows: rows.length });
      report.failed = report.results.length;
      report.ok = report.failed === 0;
      return report;
    }
    console.info('[env-sync] propagate: pushing to sandboxes', { workspaceId, targetCount: targets.length });
    ...
    await runBounded(targets, FANOUT_CONCURRENCY, async (row) => {
      ...
      try {
        snapshot = await resolveSandboxEnvSnapshot(workspaceId, row.sessionId);
        if (!snapshot) throw new Error('session env snapshot is unavailable');
        const providerName = row.provider as ProviderName;
        const networkBoundary = await resolveSessionNetworkBoundary(workspaceId, row.sessionId);
  ...
  try {
    ...
    const compiled =
      ...
      session?.agentName
        ...
        : await resolveCompiledAgentConfigForSession(gitWorkspace, input.baseRef);
 ...
 *     CONFIG file only — it does not re-run `mergeWorkspaceEnv`, so opencode's
 ...
 * `spawnChild` re-runs `mergeWorkspaceEnv` + `withoutDeniedProviderEnv`. The
...
export async function pushSessionScopeToSandbox(input: {
  ...
  try {
    ...
    const llmGatewayEnabled = await resolveWorkspaceLlmGatewayEnabled(input.workspaceId);
    const { url, headers } = await resolveSandboxIngress(row.externalId, {
      port: SANDBOX_SERVICE_PORT,
      transport: 'http',
    });
    await postEnvToDaemon({
      ...
      serviceKey,
      snapshot,
      // Restarts opencode so spawnChild re-runs mergeWorkspaceEnv + the gateway
