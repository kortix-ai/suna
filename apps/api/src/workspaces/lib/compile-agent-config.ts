...
export async function resolveCompiledAgentConfigForSession(
  ...
  const ref = baseRef?.trim() || workspace.defaultBranch;
  try {
    const candidates = manifestCandidatePaths(workspace.manifestPath).map((c) => c.path);
    const found = await readManifestFromRepo(workspace, candidates, ref);
    ...
    await Promise.all(
      Object.keys(agents).map(async (name) => {
        ...
        try {
          agentMdFiles[path] = await readRepoFile(workspace, path, ref);
        } catch (err) {
          ...
          console.warn(
            `[compile-agent-config] workspace ${workspace.workspaceId}: agent "${name}" has no behavior file at "${path}"`,
          );
        }
      }),
    );
...
export async function resolveSelectedAgentConfigForSession(
  ...
  const ref = baseRef?.trim() || workspace.defaultBranch;
  const candidates = manifestCandidatePaths(workspace.manifestPath).map(
    (candidate) => candidate.path,
  );
  const found = await readManifestFromRepo(workspace, candidates, ref);
  if (!found) {
    throw new CompileAgentConfigError(
      `Workspace ${workspace.workspaceId} has no manifest for selected-agent compilation.`,
      agentName,
    );
  }
  ...
  if (manifestSchemaVersion(raw) !== 2) {
    throw new CompileAgentConfigError(
      `Workspace ${workspace.workspaceId} must use kortix_version 2 for selected-agent compilation.`,
      agentName,
    );
  }
