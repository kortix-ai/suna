...
import { loadWorkspaceAgents, requiredConnectorsForAgent } from '../agents';
...
export type PromptConnectorVerdict =
  | { ok: true }
  /** The alias is a connector on the workspace, but nothing usable is connected to it. */
  | {
      ok: false;
      kind: 'authorization_required';
      connections: RequiredConnectorConnection[];
    }
  /** The alias is not a connector on this workspace at all — only a manifest change fixes it. */
...
export async function missingPromptConnectorConnections(input: {
  ...
  let aliases: string[];
  try {
    const [[session], boundRows, [workspace]] = await Promise.all([
      db
    ...
    if (workspace?.defaultBranch) {
      ...
      //
      // `rethrowReadErrors` is load-bearing: by default loadWorkspaceAgents
