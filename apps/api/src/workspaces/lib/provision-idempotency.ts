...
import { normalizeString, serializeWorkspace } from './serializers';
 ...
 * ended up with several "My First Workspace" rows.
 *
 ...
 * the workspace the first attempt made. Two properties matter and neither is
 * optional:
 *
 ...
 *   2. The key lives on the workspace row, not in this process. The API runs
 ...
 * route: `@kortix/sdk`'s `provisionWorkspace` sends `timeout: 120_000`
 * (`provisionWorkspaceWithToken` uses 90s, and Bun's `idleTimeout: 45` closes the
...
export type ProvisionReplayDecision =
  | { kind: 'create' }
  | { kind: 'replay'; workspace: WorkspaceRow }
  | { kind: 'in_flight'; workspace: WorkspaceRow };
...
export function classifyProvisionReplay(
  ...
  if (!workspace) return { kind: 'create' };

  const seed = readManagedRepoSeedState(workspace.metadata);
  const seedPending = seed?.expected === true && seed.seeded === false;
  const age = now - workspace.createdAt.getTime();
  if (seedPending && age < windowMs) return { kind: 'in_flight', workspace };

  return { kind: 'replay', workspace };
}
...
/**
 * The two provision-response fields that are not on the workspace row itself.
 ...
 * hard-code `manager` because it just called `grantWorkspaceRole`; a replay
 * cannot. `grantWorkspaceRole` runs ONLY on the create path, so a second account
...
export function provisionReplayResponse(
  row: WorkspaceRow,
  access: { workspaceRole: WorkspaceRole | null; effectiveRole: WorkspaceRole },
) {
  const { repoId, seeded } = describeProvisionedRepo(row);
  return {
    ...serializeWorkspace(row, access),
    push_token: null,
    git_username: null,
    repo_id: repoId,
    seeded,
