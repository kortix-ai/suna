...
import { isResourceExplicitlyGranted, loadWorkspaceResourceGrants } from '../../iam/resource-grants';
import { loadConfigWithFiles } from './workspace-resources';
...
export async function resolveAssignedAgentNames(
  ...
  const grants = await loadWorkspaceResourceGrants(workspaceId, 'agent');
 ...
 * assigned to, from the workspace's git config. Fast-paths to empty when the user
 * has no agent assignments (the common case) — only then does it pay the config
 * read. Used by the read surfaces that already hold a WorkspaceRow (the Secrets
 * list, connector visibility). The runtime session path builds the same union
 * from the lighter `loadWorkspaceAgents` specs instead.
 */
export async function resolveInheritedResourceNames(
  row: Parameters<typeof loadConfigWithFiles>[0] & { workspaceId: string },
  userId: string,
  groupIds: readonly string[],
): Promise<DeclaredResourceUnion> {
  const assigned = await resolveAssignedAgentNames(row.workspaceId, userId, groupIds);
