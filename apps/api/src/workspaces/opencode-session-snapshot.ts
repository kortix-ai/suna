...
function normalizeSnapshot(session: OpenCodeSessionLike): OpenCodeSessionSnapshot | null {
  ...
  return {
    ...
    project_id: stringOrNull(session.projectID ?? session.project_id ?? session.workspaceId),
...
export async function syncOpencodeSessionSnapshot(input: {
  ...
  const [updated] = await db
    ...
    .set({
      metadata: workspaceSessionMetadataMerge({ opencode_sessions: scopedSessions }),
      updatedAt: new Date(),
    })
    .where(
      and(
        eq(projectSessions.sessionId, input.row.sessionId),
        eq(projectSessions.workspaceId, input.row.workspaceId),
...
export function scheduleOpencodeSnapshotSync(
  input: { sessionId: string; workspaceId: string; externalId: string; userId?: string },
