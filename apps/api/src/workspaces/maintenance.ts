...
export async function sweepExpiredSessionBranches(now = new Date()): Promise<{
  ...
  const rows = await db
    ...
    .innerJoin(projects, eq(projectSessions.workspaceId, projects.workspaceId))
    .where(
      and(
  ...
  try {
    const [
      ...
      cleanupExpiredConnectorAttachments().catch((err) => {
        console.warn(
          '[workspace-maintenance] connector-attachment cleanup failed:',
          err instanceof Error ? err.message : err,
        );
      ...
      reconcileRuntimeWakeFences().catch((err) => {
        console.warn(
          '[workspace-maintenance] runtime-wake reconcile failed:',
          err instanceof Error ? err.message : err,
        );
    ...
    // not being enforced.
    console.log(
      '[workspace-maintenance] heartbeat',
    ...
    try {
      ...
      if (staleLiveness > 0) {
        console.warn(
          `[workspace-maintenance] BILLING LIVENESS STALE: ${staleLiveness} active compute session(s) not observed alive within the grace — revenue is draining silently, check the reaper`,
        );
      }
    } catch (err) {
      console.warn(
