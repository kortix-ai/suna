...
d('provider transition — durable flow (throwaway Postgres)', () => {
  ...
  // workspaceDeclaresCustomTemplates (a project-scoped, non-default-slug row).
  const customTemplateProbe = (database: Database) => async (project: { workspaceId: string }) => {
    const rows = await database
      .select({ slug: sandboxTemplates.slug })
      .from(sandboxTemplates)
      .where(and(eq(sandboxTemplates.workspaceId, project.workspaceId), ne(sandboxTemplates.slug, 'default')))
      .limit(1);
