 ...
 *  - the generation is reserved ON THE WORKSPACE ROW (projects.sandbox_provider_generation)
...
export async function reserveSwitchTransition(
  ...
  return db.transaction(async (tx) => {
    const [workspace] = await tx
      ...
      .from(projects)
      .where(eq(projects.workspaceId, input.identity.workspaceId))
      .for('update')
      .limit(1);
...
export async function activateWithCas(
  ...
  return db.transaction(async (tx) => {
    const [workspace] = await tx
      ...
      .for('update')
      .limit(1);
    if (!workspace) return { activated: false, reason: 'project_missing' as const };
