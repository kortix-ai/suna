...
describe('generation CAS', () => {
  test('activation only wins with a strictly-greater generation', () => {
    expect(canActivateGeneration({ transitionGeneration: 2, workspaceRecordedGeneration: 1 })).toBe(true);
    expect(canActivateGeneration({ transitionGeneration: 1, workspaceRecordedGeneration: 1 })).toBe(false);
    expect(canActivateGeneration({ transitionGeneration: 1, workspaceRecordedGeneration: 2 })).toBe(false);
  });
