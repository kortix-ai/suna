...
describe('provider transition metrics', () => {
  test('events increment per-event and per-event:target tallies', () => {
    emitProviderTransitionEvent('requested', { target: 'platinum', source: 'daytona', workspaceId: 'p1' });
    emitProviderTransitionEvent('build_started', { target: 'platinum', workspaceId: 'p1', queueSeconds: 3 });
    emitProviderTransitionEvent('activation_completed', { target: 'platinum', workspaceId: 'p1', timeToReadySeconds: 42 });
  ...
  test('cold_fallback is tracked distinctly so a post-activation cold boot is observable (should be 0)', () => {
    ...
    emitProviderTransitionEvent('cold_fallback', { target: 'platinum', workspaceId: 'p1' });
