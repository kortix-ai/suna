 ...
 */
export function existingWorkspaceMirrorPath(workspace: GitBackedWorkspace): string | null {
...
export async function refreshMirror(workspace: GitBackedWorkspace, force = false) {
  ...
  if (current) {
    ...
    return refreshMirror(workspace, true);
  }
  const next = doRefreshMirror(workspace, force)
    ...
    .finally(() => {
      if (refreshLocks.get(workspace.workspaceId)?.promise === next) {
        refreshLocks.delete(workspace.workspaceId);
      }
    });
  refreshLocks.set(workspace.workspaceId, { promise: next, forced: force });
  return next;
}
