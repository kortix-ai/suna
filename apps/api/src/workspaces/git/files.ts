...
export async function readRepoFile(workspace: GitBackedWorkspace, filePath: string, ref?: string): Promise<string> {
...
export async function readManifestFromRepo(
  ...
  const treeRef = validateRef(ref || workspace.defaultBranch);
  const repoPath = await refreshMirror(workspace, opts?.forceRefresh);
