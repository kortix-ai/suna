...
beforeAll(async () => {
  ...
  await run('git', ['commit', '-m', 'chore: workspace setup'], { cwd: work, env });
...
describe('remoteBranchExists', () => {
  test('is true for a repo that carries the default branch', async () => {
    const exists = await remoteBranchExists(
      { workspaceId: 'p1', repoUrl: seededRepo, defaultBranch: 'main', manifestPath: 'kortix.yaml' },
      'main',
    );
  ...
  test('is false for a freshly created repo with no refs at all', async () => {
    const exists = await remoteBranchExists(
      { workspaceId: 'p2', repoUrl: emptyRepo, defaultBranch: 'main', manifestPath: 'kortix.yaml' },
      'main',
    );
  ...
  test('is false for a branch name that is absent but does not match a prefix', async () => {
    const exists = await remoteBranchExists(
      { workspaceId: 'p3', repoUrl: seededRepo, defaultBranch: 'main', manifestPath: 'kortix.yaml' },
      'mai',
    );
    expect(exists).toBe(false);
  });
