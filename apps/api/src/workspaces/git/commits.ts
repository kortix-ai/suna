 ...
 */
export async function resolveCommitSha(workspace: GitBackedWorkspace, ref?: string): Promise<string> {
...
export async function listCommits(
  ...
  const repoPath = await refreshMirror(workspace);
  const ref = validateRef(options.ref || workspace.defaultBranch);
