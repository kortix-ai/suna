...
export async function listBranches(workspace: GitBackedWorkspace): Promise<GitBranchInfo[]> {
  const result = await runGit(
    ['ls-remote', '--heads', workspace.repoUrl],
    undefined,
    true,
    workspace.gitAuthToken,
    undefined,
  ...
  const branches = parseRemoteBranches(result.stdout, workspace.defaultBranch);
  const repoPath = existingWorkspaceMirrorPath(workspace);
  if (!repoPath || branches.length === 0) return branches;

  const cached = await readCachedBranchMetadata(repoPath, workspace.defaultBranch).catch(() => null);
 ...
 * scaffold seed produced the default branch before it reports the workspace
...
export async function remoteBranchExists(
  ...
  const result = await runGit(
    ['ls-remote', '--heads', workspace.repoUrl, `refs/heads/${ref}`],
    undefined,
    true,
    workspace.gitAuthToken,
    undefined,
...
export async function createRemoteSessionBranch(
  ...
  try {
    ...
    try {
      ...
      const outcome = await ensureManagedRepoSeeded(workspace.workspaceId, 'session-branch');
...
export async function commitMultipleFilesToBranch(
  ...
  try {
    ...
    try {
      ...
      invalidateWorkspaceMirror(workspace.workspaceId);
      if (expectedFileRevision) {
        const remoteTip = await readRemoteBranchTip(workspace, repoPath, branch, authHost);
