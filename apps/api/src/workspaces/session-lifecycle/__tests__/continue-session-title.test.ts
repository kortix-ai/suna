      ...
      from: (table: unknown) => ({
        where: () => ({
          limit: async () => {
            ...
            if (table === projects) return [{ workspaceId: WORKSPACE_ID, accountId: ACCOUNT_ID }];
            return [];
          },
        }),
      }),
...
mock.module('../actor', () => ({
  resolveWorkspaceAutomationActor: async () => actor,
}));
...
async function deliver(metadata: Record<string, unknown>, text: string) {
  sessionRow = { accountId: ACCOUNT_ID, workspaceId: WORKSPACE_ID, status: 'running', metadata };
