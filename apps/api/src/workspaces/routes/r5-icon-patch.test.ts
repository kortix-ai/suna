/**
 * `PATCH /v1/projects/:workspaceId` — the emoji icon's tri-state semantics, AND
 ...
 * `normalizeWorkspaceIcon` collapses BOTH invalid input and an explicit null to
...
const WORKSPACE_ID = '00000000-0000-4000-a000-0000000099a0';
...
describe('PATCH /:workspaceId — icon absent', () => {
...
describe('PATCH /:workspaceId — icon set', () => {
  test('a valid emoji merges `{ icon }` into metadata SQL-side', async () => {
    returningRow = workspaceRow({ metadata: { icon: '🎯' } });
...
describe('PATCH /:workspaceId — icon cleared', () => {
...
describe('PATCH /:workspaceId — invalid icon is NOT a clear', () => {
  // The whole reason this handler cannot use `normalizeWorkspaceIcon(body.icon)`
