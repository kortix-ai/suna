...
import {
  ...
  WorkspaceConnectionSchema as ConnectionSchema,
  WorkspaceReconcileConnectionInputSchema as ReconcileConnectionInputSchema,
...
import { AnyObject, TriggerSchema, workspaceRoutesApp } from '../lib/app';
...
import { listWorkspaceSecretNamesForConsumer } from '../secrets';
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    const rows = await db
      ...
      .where(eq(connectorConnections.workspaceId, workspaceId));
    return c.json({
      connections: rows
        .filter((connection) =>
          mayReadConnection(
...
workspaceRoutesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/connections/all',
    tags: ['connectors'],
...
workspaceRoutesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/connections/me',
    tags: ['connectors'],
  ...
  async (c: any) => {
    ...
    if (connector.authorizationStrategy !== 'user') {
      return c.json(
        {
          error: 'This connector uses workspace-owned connections',
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    const requestedOwnerType = typeof body.owner_type === 'string' ? body.owner_type : 'external';
...
for (const operation of ['credential', 'revoke', 'activate', 'default'] as const) {
  workspaceRoutesApp.openapi(
    createRoute({
      method: 'put',
      path: `/{workspaceId}/connections/{connectionId}/${operation}`,
      tags: ['connectors'],
      summary: `${operation} connection`,
      ...auth,
      request: {
        params: z.object({ workspaceId: z.string(), connectionId: z.string().uuid() }),
        body: {
          content: {
            'application/json': {
              schema:
    ...
    async (c: any) => {
      ...
      if (!loaded) return c.json({ error: 'Not found' }, 404);
      const actingPrincipalIsServiceAccount = c.get('authType') === 'service_account';
      const mayManageSystemConnections = await workspaceCapabilityAllowed(
        c,
        loaded.userId,
        loaded.row.accountId,
        workspaceId,
      ...
      const [connection] = await db
        ...
        .innerJoin(
          ...
          and(
            ...
            eq(connectors.workspaceId, connectorConnections.workspaceId),
          ),
        )
        .where(
          and(
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    await reconcileWorkspaceTriggerRuntime(workspaceId, extractTriggers(committedManifest).specs);
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    return c.json({
      ...teamsMode(baseUrl, { enabled, workspaceId, byoAppId }),
      orgConsentUrl: byoAppId ? null : teamsOrgConsentUrl({ workspaceId, baseUrl, enabled }),
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    return c.json({
      ...install,
      connection_id: await loadEmailInstallConnectionId(workspaceId, install.inboxId),
    });
  },
);
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    if (agentName) {
      const loadedAgents = await loadWorkspaceAgents(loaded.row, {
        forceRefresh: true,
    ...
    const clientIds = agentMailProvisioningClientIds(workspaceId, connectorSlug);
    ...
    if (existingInboxId && existingEmail) {
      ...
      const owners = await listWorkspacesForWorkspace('email', existingInboxId);
      const foreignOwner = owners.find((id) => id !== workspaceId);
      if (foreignOwner) {
        return c.json({ error: 'AgentMail inbox is already connected to another workspace' }, 409);
      }
      inbox = {
    ...
    return c.json({
      ...summary,
      connection_id: await loadEmailInstallConnectionId(workspaceId, summary.inboxId),
    });
  },
);
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    // back to this workspace) or a project/session-scoped user PAT.
    const authType = (c as any).get('authType') as string | undefined;
    ...
    const platformWorkspaceId =
      body.workspace_id?.trim() || (await resolveWorkspaceIdForChannel(workspaceId, channel));
    ...
    await bindChatThread({ workspaceId, platformWorkspaceId, threadId: threadTs, sessionId });
  ...
  async (c: any) => {
    ...
    if (authType === 'apiKey' && apiKeyType === 'sandbox' && accountId && sandboxId) {
      ...
      const [workspace] = await db
        ...
        .from(projects)
        .where(and(eq(projects.workspaceId, workspaceId), eq(projects.accountId, accountId)))
        .limit(1);
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    // What `auto` resolves to for this workspace. Served below so the client can
    // LOCK its switch instead of offering a toggle that always 409s.
    const effectiveDefault = toWireModel(
      defaults.workspaces[workspaceId] ?? defaults.account ?? platformDefaultModelId() ?? '',
    );
    ...
    // Server-owned per-workspace enablement, resolved HERE and stamped onto each
    ...
    return c.json({
      ...
      // True while the workspace has made no exceptions at all — the only thing
...
// off the workspace's own default model (the picker would hide what `auto`
// resolves to).
...
workspaceRoutesApp.openapi(
  createRoute({
    method: 'put',
    path: '/{workspaceId}/model-enablement',
    tags: ['workspaces'],
    summary: 'PUT /:workspaceId/model-enablement',
    ...auth,
    request: {
      params: z.object({ workspaceId: z.string() }),
  ...
  async (c: any) => {
    ...
    const defaults = await getAccountModelDefaults(accountId, workspaceId);
    const effectiveDefault =
      defaults.workspaces[workspaceId] ?? defaults.account ?? platformDefaultModelId();
    if (effectiveDefault && modelOverrides[toWireModel(effectiveDefault)] === false) {
      return c.json(
        {
          error: 'Cannot disable the workspace default model — change the default first.',
    ...
    await setWorkspaceModelOverrides({ workspaceId, updatedBy: userId, modelOverrides });
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    // session_id is caller-supplied. Scoping it to :workspaceId closes the
...
workspaceRoutesApp.openapi(
  createRoute({
    method: 'get',
    path: '/{workspaceId}/sessions/{sessionId}/question',
    tags: ['sessions'],
    summary: 'GET /:workspaceId/sessions/:sessionId/question',
    ...auth,
  ...
  async (c: any) => {
    ...
    // other session-content reads use (r7.ts). `loadWorkspaceForUser(…, 'read')`
...
workspaceRoutesApp.openapi(
  createRoute({
    ...
    path: '/{workspaceId}/sessions/{sessionId}/question',
    tags: ['sessions'],
    summary: 'POST /:workspaceId/sessions/:sessionId/question',
    ...auth,
    request: {
  ...
  async (c: any) => {
    ...
    // WORKSPACE_SESSION_START)` is the usual bar for starting a turn, but that
