...
export function buildRegistryWorkspaceInstallPrompt(
  ...
  if ((item.files ?? []).length > 0 && ownFiles.length === 0) {
    throw new Error(
      `Workspace template "${item.name}" has no resolvable file content (likely an external registry item) — install-session merge only supports base project items today.`,
    );
  }
  const deps = item.registryDependencies ?? [];

  const lines: string[] = [
    `Integrate the "${item.title ?? item.name}" project template into THIS workspace — without breaking anything already here.`,
    '',
    item.description ?? '',
    '',
