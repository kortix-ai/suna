...
import { AnyObject, SandboxTemplateSchema, SnapshotSchema, workspaceRoutesApp } from '../lib/app';
 ...
 * a workspace's sessions can start. See snapshots/sandbox-status.ts for why this
 * is derived from live provider coverage rather than from the build log.
 */
function workspaceSandboxStatus(args: {
...
function serializeSandboxStatus(resolved: ReturnType<typeof workspaceSandboxStatus>) {
...
workspaceRoutesApp.openapi(
  ...
  if (patToken) {
    ...
    const row = await registerPatLinkedWorkspace({
      ...
      manifestPath,
      ...(iconGlyph
        ? { workspaceMetadata: { icon_glyph: iconGlyph } }
        : icon
          ? { workspaceMetadata: { icon } }
          : {}),
    });
...
workspaceRoutesApp.openapi(
  ...
  const workspaceName = normalizeString(body.workspace_name ?? body.project_name ?? body.projectName) ?? deriveWorkspaceName(repo.full_name);
...
workspaceRoutesApp.openapi(
  ...
  const resolved = workspaceSandboxStatus({
    templates,
    builds,
...
workspaceRoutesApp.openapi(
  ...
  try {
    ...
    templates = await listSandboxTemplates(await loadGitWorkspace(loaded), observation.listOptions);
  } catch {
    templatesUnavailable = true;
  }
  const { templateBuilds, status } = workspaceSandboxStatus({
    templates,
    builds,
    metadata: loaded.row.metadata,
    selectedProvider: observation.selectedProvider,
  ...
  if (!failed) {
    if (templatesUnavailable) {
      return c.json(
        {
          error:
            'Could not read this workspace’s sandbox templates, so the current build state is unknown. Try again in a moment.',
