...
import { AnyObject, CommitSchema, WorkspaceSchema, workspaceRoutesApp } from '../lib/app';
...
import { addPlatformMetaAgent, workspaceMetaAgentEnabled } from '../lib/platform-meta-agent';
...
workspaceRoutesApp.openapi(
  ...
  const config = workspaceMetaAgentEnabled(loaded.row.metadata)
...
workspaceRoutesApp.openapi(
  ...
  // image, not the workspace repo. Answer like any other missing file.
...
workspaceRoutesApp.openapi(
  ...
  const iconGlyph = iconGlyphPresent ? normalizeWorkspaceGlyph(body.icon_glyph) : null;
  const iconPresent = 'icon' in body;
  const icon = iconPresent ? normalizeWorkspaceIcon(body.icon) : null;
