...
import {
  propagateWorkspaceSecretsToActiveSandboxes,
  type WorkspaceSecretPropagationResult,
...
// A secret write may also have to reach every LIVE sandbox in the workspace. The
...
function summarizeDeliverySync(
  result: WorkspaceSecretPropagationResult,
): SecretDeliverySync {
  return {
    ok: result.ok,
...
/**
 * The other network-boundary secret in this workspace that already claims one of
 ...
 * in the workspace with an error the author cannot connect to their edit. Both
...
async function boundaryDestinationConflict(
  ...
  return findBoundaryDestinationConflict(
    ...
    rows.map((other) => ({
      ...
      policy: other.egressPolicy,
    })),
  );
}

function boundaryConflictBody(
  identifier: string,
  conflict: BoundaryDestinationConflict,
) {
  return {
    error:
...
async function connectorSecretBindings(
  workspaceId: string,
  identifier: string,
): Promise<string[]> {
  const rows = await db
...
// legacy `project_id`, so the token is useless outside this one workspace. They're
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    if (getAgentGrant(c)) {
      return c.json(
        { error: "Agent-session tokens cannot mint workspace tokens" },
        403,
      );
    }
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    if (getAgentGrant(c)) {
      return c.json(
        { error: "Agent-session tokens cannot manage workspace tokens" },
        403,
      );
    }
    const ok = await revokeAccountToken(
      tokenId,
      loaded.row.accountId,
      workspaceId,
    );
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    const tokenWorkspaceId = (c as any).get("tokenWorkspaceId") as
      string | undefined;
    ...
    if (authType === "pat") {
      if (tokenWorkspaceId !== workspaceId) {
        return c.json(
          {
            error: "clone credentials require a workspace-scoped runtime token",
          },
          403,
        );
      }
      ...
      if (!sandbox) {
        return c.json(
          { error: "sandbox token is not scoped to this workspace" },
          403,
        );
      }
      if (
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    if (await hasServerManagedGitAuth(loaded.row)) {
      return c.json(
        { error: "Git auth is already managed by Kortix for this workspace" },
        409,
      );
    }
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    try {
      ...
      agentGrants = workspaceConfig ?? null;
    } catch (err) {
  ...
  async (c: any) => {
    ...
    if (!isValidIdentifier(identifier)) {
      return c.json(
        {
          error:
            "identifier must be alphanumeric (A-Z, 0-9, _, ., -; max 128 chars)",
        },
        400,
      );
    }
    ...
    const requestedConsumer =
      body.consumer === undefined
        ? undefined
        : SecretConsumerSchema.nullable().safeParse(body.consumer);
    ...
    if (
      ...
      !["runtime", "broker", "egress", "denied"].includes(
      ...
      return c.json(
        {
          error:
            "secret creation supports runtime, broker, egress, or denied delivery",
        },
        400,
      );
    }
    ...
    const explicitStrategy = (requestedStrategy ??
      (defaultToGateway ? "broker" : undefined)) as
      "runtime" | "broker" | "egress" | "denied" | undefined;
    ...
    if (explicitConsumer === "http_broker" || explicitConsumer === "network") {
      ...
      if (
        ...
        policy.policy.backend !== "kortix_fetch"
      ) {
        return c.json(
          { error: "HTTP broker requires the kortix_fetch backend" },
          400,
        );
      }
      if (explicitConsumer === "network") {
        ...
        if (!networkBoundaryDeliveryAvailable()) {
          return c.json(
            {
              error:
                "Network-boundary delivery requires the Platinum sandbox provider",
              code: "secret_delivery_unavailable",
            },
            409,
          );
        }
        const conflict = await boundaryDestinationConflict(
          workspaceId,
          identifier,
          policy.policy,
        );
        if (conflict)
          return c.json(boundaryConflictBody(identifier, conflict), 409);
      }
      explicitPolicy = policy.policy;
    } else if (body.egress_policy !== undefined) {
      return c.json(
        { error: "This consumer does not accept an outbound policy" },
        400,
      );
    }
    const explicitHandlePrefix =
      explicitConsumer === "http_broker" &&
      typeof body.handle_prefix === "string"
        ? body.handle_prefix.trim()
        : null;
    if (explicitHandlePrefix && explicitHandlePrefix.length > 48) {
      return c.json(
        { error: "handle_prefix must contain at most 48 characters" },
        400,
      );
    }
    ...
    const boundaryDelivery =
      explicitStrategy === "egress" || existing?.strategy === "egress";
...
workspaceRoutesApp.openapi(
  createRoute({
    ...
    request: {
      ...
      body: {
        content: {
          "application/json": { schema: UpdateSecretStrategyInputSchema },
        },
      },
    },
    responses: {
  ...
  async (c: any) => {
    ...
    if (!parsed.success) {
      return c.json(
        { error: "strategy must be runtime, egress, broker, or denied" },
        400,
      );
    }
    ...
    if (getAgentGrant(c)) {
      return c.json(
        { error: "Agent sessions cannot change secret delivery policy" },
        403,
      );
    }
    ...
    if (parsed.data.strategy === "runtime" && nextConsumer !== "sandbox") {
      return c.json(
        { error: "runtime delivery requires the sandbox consumer" },
        400,
      );
    }
    ...
    if (parsed.data.strategy === "egress" && nextConsumer !== "network") {
      return c.json(
        { error: "egress delivery requires the network consumer" },
        400,
      );
    }
    if (
      ...
      return c.json(
        { error: "broker delivery requires a server consumer" },
        400,
      );
    }
    ...
    if (existing.strategyLocked && existing.strategy !== parsed.data.strategy) {
      return c.json(
        {
          error: "This secret delivery strategy is locked",
...
async function writeCodexAuthSecret(input: {
  ...
  if (sharing?.mode === "private") {
    const [written] = await db
      ...
      .returning({ secretId: projectSecrets.secretId });
    if (!written)
      throw new Error("Failed to store the private Codex credential");
    ...
    const [written] = await db
      ...
      .returning({ secretId: projectSecrets.secretId });
    if (!written)
      throw new Error("Failed to store the shared Codex credential");
    secretId = written.secretId;
  }
  ...
  return (
    views.find((v) => v.identifier === CODEX_AUTH_JSON_SECRET_NAME) ?? {
      identifier: CODEX_AUTH_JSON_SECRET_NAME,
      name: CODEX_AUTH_JSON_SECRET_NAME,
    }
  );
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    if (isSystemWorkspaceSecretName(identifier)) {
      return c.json(
        { error: `${identifier} is managed by Kortix and cannot be removed` },
        403,
      );
    }
...
// Force a re-push of all workspace secrets to all active sandboxes. Use after
...
workspaceRoutesApp.openapi(
  createRoute({
    ...
    tags: ["secrets"],
    summary:
      "POST /:workspaceId/secrets/sync — force re-push secrets to active sandboxes",
    ...auth,
    request: { params: z.object({ workspaceId: z.string() }) },
    responses: {
      200: json(
