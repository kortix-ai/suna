/**
 * Drives the real POST /:workspaceId/sessions/:sessionId/audit/events handler.
...
const WORKSPACE_ID = 'd7200000-0000-4000-a000-000000000001';
...
workspaceRoutesApp.use('*', async (c, next) => {
...
describe('POST /:workspaceId/sessions/:sessionId/audit/events', () => {
  test('cannot promote forged sandbox provenance into canonical audit columns', async () => {
    const response = await workspaceRoutesApp.request(
      `/${WORKSPACE_ID}/sessions/${SESSION_ID}/audit/events`,
      {
        method: 'POST',
