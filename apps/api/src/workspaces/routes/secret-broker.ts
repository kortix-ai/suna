...
import { decryptWorkspaceSecret, intersectSecretGrants } from '../secrets';
...
workspaceRoutesApp.openapi(
  createRoute({
    method: 'post',
    path: '/{workspaceId}/secrets/{identifier}/broker',
    tags: ['secrets'],
  ...
  async (c) => {
    ...
    if (
      c.get('authType') !== 'pat' ||
      c.get('tokenWorkspaceId') !== workspaceId ||
      !sessionId ||
      !agentGrant
    ) {
      return c.json(
    ...
    const [session] = await db
      ...
      .limit(1);
    if (!session) {
      return c.json({ error: 'Active workspace session not found', code: 'session_not_active' }, 403);
    }
    ...
    try {
      ...
      const [handle] = await db
        ...
        .where(
          and(
            eq(projectSessionSecretHandles.workspaceId, workspaceId),
      ...
      const secret = decryptWorkspaceSecret(workspaceId, encryptedValue);
