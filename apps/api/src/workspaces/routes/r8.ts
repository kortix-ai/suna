...
import { AnyObject, ChangeRequestSchema, SessionStartResultSchema, workspaceRoutesApp } from '../lib/app';
...
workspaceRoutesApp.openapi(
  ...
  async (c: any) => {
    ...
    // The capability check above is WORKSPACE-wide, and in Kortix-as-a-Backend the
    // sandbox's own token holds it — every KaaB session shares the wrapper's
    // credential, so "may push in this workspace" is true for every end-user's
  ...
  async (c: any) => {
    ...
    try {
      ...
      const diff = useSnapshot
        ? await getDiffBetweenShas(workspaceForGit, cr.baseCommitSha!, cr.headCommitSha!)
