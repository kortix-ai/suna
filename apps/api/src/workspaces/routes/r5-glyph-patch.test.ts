/**
 * `PATCH /v1/projects/:workspaceId` — the `icon` / `icon_glyph` mutual
 ...
 *
 * Both normalizers (`normalizeWorkspaceIcon`, `normalizeWorkspaceGlyph`) collapse
 ...
 *
 * This file drives the REAL `r5.ts` Hono handler (`workspaceRoutesApp.request(...)`)
...
const WORKSPACE_ID = '00000000-0000-4000-a000-0000000099b0';
...
let returningRow: Record<string, unknown> = workspaceRow();
...
mock.module('../lib/access', () => ({
  ...
  workspaceCapabilityAllowed: async () => true,
}));

// Registers r5.ts's routes onto the shared `workspaceRoutesApp` singleton. r1.ts
...
function patch(body: Record<string, unknown>) {
  return workspaceRoutesApp.request(`/${WORKSPACE_ID}`, {
    method: 'PATCH',
...
beforeEach(() => {
  setCalls = [];
  returningRow = workspaceRow();
});

describe('PATCH /:workspaceId — glyph set', () => {
...
describe('PATCH /:workspaceId — icon set', () => {
...
describe('PATCH /:workspaceId — explicit null clears exactly one key', () => {
  test('an explicit null deletes only the glyph key', async () => {
    returningRow = workspaceRow({ metadata: {} });
...
describe('PATCH /:workspaceId — malformed value is NOT a clear', () => {
...
describe('PATCH /:workspaceId — neither key present', () => {
...
describe('PATCH /:workspaceId — both keys present, the glyph wins', () => {
...
describe('PATCH /:workspaceId — both keys explicitly null clears both', () => {
