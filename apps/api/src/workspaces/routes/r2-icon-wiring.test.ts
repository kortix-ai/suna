 ...
 * (`../lib/workspace-registration.icon.integration.test.ts`) proves Task 2's
 * `workspaceMetadata` contract on `registerLinkedWorkspace`, but it hand-copies
 * `r2.ts`'s `const icon = normalizeWorkspaceIcon(body.icon)` / `...(icon ? {
 * workspaceMetadata: { icon } } : {})` instead of calling the real route — so it
 * would not catch a wrong body key (e.g. `body.Icon`) or the spread being
 * deleted from a call site in a later edit.
 *
 * THIS file drives the real `r2.ts` Hono handlers (`workspaceRoutesApp.request(...)`)
 * for all three call sites — `/link-repository` PAT path,
 * `/link-repository` GitHub-App path, `/create-repo` — and asserts on what
 * they actually pass to `registerGitHubLinkedWorkspace` /
 * `registerPatLinkedWorkspace`, which are mocked here so the test needs no
...
function fakeWorkspaceRow(over: Record<string, unknown> = {}) {
  const now = new Date();
  return {
    workspaceId: `proj-${Math.random().toString(36).slice(2)}`,
    accountId: FAKE_ACCOUNT_ID,
...
// ── Auth/quota/access — no DB. `resolveWorkspaceAccount` bypasses the real
// membership lookup entirely; `enforceWorkspaceQuota` always allows.
const realAccess = await import('../lib/access');
mock.module('../lib/access', () => ({
  ...realAccess,
  resolveWorkspaceAccount: async () => ({ userId: FAKE_USER_ID, accountId: FAKE_ACCOUNT_ID }),
  enforceWorkspaceQuota: async () => null,
}));
...
mock.module('../lib/git', () => ({
  ...
  getWorkspaceGitConnection: async () => null,
}));
...
const mockRegisterGitHub = mock(async (input: Record<string, unknown>) =>
  fakeWorkspaceRow({ metadata: (input.workspaceMetadata as Record<string, unknown>) ?? {} }),
);
const mockRegisterPat = mock(async (input: Record<string, unknown>) =>
  fakeWorkspaceRow({ metadata: (input.workspaceMetadata as Record<string, unknown>) ?? {} }),
);
mock.module('../lib/workspace-registration', () => ({
  registerGitHubLinkedWorkspace: mockRegisterGitHub,
  registerPatLinkedWorkspace: mockRegisterPat,
}));

// Registers r2.ts's routes onto the shared `workspaceRoutesApp` singleton. r1.ts
...
const { workspaceRoutesApp } = await import('../lib/app');
await import('./r2');

function post(path: string, body: Record<string, unknown>) {
  return workspaceRoutesApp.request(path, {
    method: 'POST',
...
describe('r2.ts icon wiring — POST /link-repository (PAT path)', () => {
  test('a valid icon is threaded into registerPatLinkedWorkspace as workspaceMetadata.icon', async () => {
    ...
    expect(mockRegisterPat).toHaveBeenCalledWith(
      expect.objectContaining({ workspaceMetadata: { icon: '🚀' } }),
    );
    expect(mockRegisterGitHub).not.toHaveBeenCalled();
  });

  test('a malformed icon reaches registerPatLinkedWorkspace with NO workspaceMetadata key at all', async () => {
    const res = await post('/link-repository', {
    ...
    expect('workspaceMetadata' in call).toBe(false);
  });
});

describe('r2.ts icon wiring — POST /link-repository (GitHub App path)', () => {
  test('a valid icon is threaded into registerGitHubLinkedWorkspace as workspaceMetadata.icon', async () => {
    const res = await post('/link-repository', {
  ...
  test('no icon in the body reaches registerGitHubLinkedWorkspace with NO workspaceMetadata key at all', async () => {
    const res = await post('/link-repository', {
...
describe('r2.ts icon wiring — POST /create-repo', () => {
  ...
  test('a two-grapheme icon reaches registerGitHubLinkedWorkspace with NO workspaceMetadata key at all', async () => {
    const res = await post('/create-repo', {
