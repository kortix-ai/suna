...
export class ManagedRepoSeedError extends Error {
  ...
  constructor(input: {
    ...
    super(
      `managed repo seed ${input.stage} failed for workspace ${input.workspaceId} ` +
...
export async function pushVerifiedSeed(input: {
  ...
  console.error(
    `[managed-repo-seed] workspace=${input.workspaceId} branch=${input.branch} ` +
...
/**
 * May Kortix seed this workspace's repo on demand?
 *
 * Yes for a MANAGED repo whose seed was expected but never verified, and for a
 * managed repo with no recorded state at all — every workspace created before
 ...
 * one `ls-remote` and nothing else), takes a per-workspace lock, pushes at most
...
export function ensureManagedRepoSeeded(
  ...
  const inFlight = repairLocks.get(workspaceId);
  if (inFlight) return inFlight;
  const run = repairManagedRepo(workspaceId, trigger).finally(() => {
    repairLocks.delete(workspaceId);
  });
  repairLocks.set(workspaceId, run);
  return run;
}

async function repairManagedRepo(workspaceId: string, trigger: string): Promise<SelfHealOutcome> {
  try {
    ...
    const { buildConnectionRef, getWorkspaceGitConnection, getWorkspaceGitRemote, withWorkspaceGitAuth } =
      await import('./lib/git');
    const remote = getWorkspaceGitRemote(row, await getWorkspaceGitConnection(workspaceId));
    ...
    const access = await withWorkspaceGitAuth(row);
    const { invalidateWorkspaceMirror, remoteBranchExists } = await import('./git');
    const gitWorkspace = {
      workspaceId,
    ...
    if (await remoteBranchExists(gitWorkspace, row.defaultBranch)) {
    ...
    const { buildWorkspaceSeedFiles, buildWorkspaceSeedFilesFromItem, defaultAgentFromSeedFiles } =
    ...
    console.warn(
      `[managed-repo-seed] repairing empty managed repo workspace=${workspaceId} ` +
    ...
    await pushVerifiedSeed({
      ...
      remoteHasBranch: () => remoteBranchExists(gitWorkspace, row.defaultBranch),
    });
    ...
    console.warn(`[managed-repo-seed] repaired empty managed repo workspace=${workspaceId}`);
    return { repaired: true };
  } catch (error) {
    console.error(
      `[managed-repo-seed] repair failed workspace=${workspaceId} trigger=${trigger}:`,
