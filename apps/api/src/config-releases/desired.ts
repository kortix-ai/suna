 ...
 * and SDK read). Spec: "Release builder", "Config mode", "Quarantine across
 * the project".
 */
...
import { explainConfigMode, type ConfigModeDecision, type WorkspaceReport } from './mode';
...
function descriptorFor(release: ConfigRelease, decision: ConfigModeDecision, repositoryAccess: boolean) {
  ...
  return { ...descriptor, reason: descriptor.reason ?? decision.note };
}

/**
 * Resolve the base tip, build its release, choose the mode, and apply the
 * project quarantine. A quarantined release is replaced by the newest release
...
export async function resolveDesiredRelease(
  ...
  const decision = await explainConfigMode({ project: input.project, baseSha, release: base, report: input.report });
  let descriptor = descriptorFor(base, decision, input.repositoryAccess);
    ...
    try {
      ...
      if (quarantined.has(descriptor.release_id)) {
        ...
        if (fallback && fallback.releaseId !== descriptor.release_id) {
          ...
          const candidate = descriptorFor(rebuilt, decision, input.repositoryAccess);
  ...
  return { baseSha, descriptor, decision, quarantinedReleaseId };
}
