/**
 * The template catalog — a static list of templates.
 *
 ...
 * (`../projects/routes/templates.ts`). Nothing about a template lives in the
...
export interface TemplateCatalogEntry extends Template {
...
function toCard(entry: TemplateCatalogEntry): Template {
 ...
 */
export function listTemplateCatalog(q?: string | null): Template[] {
...
export function getTemplateBySlug(slug: string): Template | null {
  const entry = findTemplateCatalogEntry(slug);
  return entry ? toCard(entry) : null;
}

/** One full entry by slug — the install route's read. */
export function findTemplateCatalogEntry(slug: string): TemplateCatalogEntry | null {
  return TEMPLATE_CATALOG.find((entry) => entry.slug === slug) ?? null;
}
