      ...
      if (!Number.isNaN(ts.getTime()) && lastId) {
        conditions.push(
          or(
            ...
            and(
              eq(auditEvents.occurredAt, ts),
              lt(auditEvents.eventId, lastId),
            ),
          )!,
        );
      }
...
function serializeWebhook(
  w: typeof auditWebhooks.$inferSelect,
  includeSecret = false,
) {
  return {
...
auditRouter.openapi(
  createRoute({
    ...
    request: {
      params: AccountIdParam,
      body: {
        content: { "application/json": { schema: AuditWebhookInputSchema } },
      },
    },
    responses: {
  ...
  effectHandler(async (c: any) => {
    ...
    if (name.length > 128)
      return c.json({ error: "name too long (max 128 chars)" }, 400);
...
auditRouter.openapi(
  createRoute({
    ...
    request: {
      ...
      body: {
        content: { "application/json": { schema: AuditWebhookPatchSchema } },
      },
    },
    responses: {
  ...
  effectHandler(async (c: any) => {
    ...
    const [before] = await db
      ...
      .where(
        and(
          eq(auditWebhooks.webhookId, webhookId),
          eq(auditWebhooks.accountId, accountId),
        ),
      )
      .limit(1);
    ...
    await recordAuditEvent({
      ...
      before: {
        ...
        action_prefix: before.actionPrefix,
      },
      after: {
        name: updated.name,
        enabled: updated.enabled,
        action_prefix: updated.actionPrefix,
      },
      ip:
        c.req.header("x-forwarded-for")?.split(",")[0]?.trim() ||
        c.req.header("x-real-ip") ||
