    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          const emails = yield* accountTry(() => lookupEmailsByUserIds(rows.map((r) => r.userId)));
          const projectGrantRows = yield* accountTry(() =>
            db
              .select({
          ...
          const groupsByUser = yield* accountTry(async () => {
            const byUser = new Map<string, Array<{ group_id: string; name: string }>>();
            try {
          ...
          const patCountByUser = yield* accountTry(async () => {
      ...
      `);
          ...
          const mfaByUser = yield* accountTry(async () => {
    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          if (!membership) return yield* accountFail({ error: 'Forbidden' }, 403);
          yield* accountTry(() =>
            assertAuthorized(userId, accountId, ACCOUNT_ACTIONS.MEMBER_INVITE),
          );

          const body = yield* accountTry(() => readBody(c));
          const email = normalizeEmail(body.email);
          if (!email) return yield* accountFail({ error: 'A valid email is required' }, 400);
          ...
          if (!accountRow) return yield* accountFail({ error: 'Account not found' }, 404);

          const targetUserId = yield* accountTry(() => lookupUserIdByEmail(email));

          if (targetUserId) {
            const existing = yield* accountTry(() => getMembership(targetUserId, accountId));
            if (existing) {
              return yield* accountFail({ error: 'Already a member' }, 409);
            }
          ...
          const delivery = yield* accountTry(() =>
    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          const expiresAt = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000);
          const [updated] = yield* accountTry(() =>
            db
          ...
          if (!updated) return yield* accountFail({ error: 'Invite not found' }, 404);
    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          const callerMembership = yield* accountTry(() => getMembership(callerUserId, accountId));
          if (!callerMembership) return yield* accountFail({ error: 'Forbidden' }, 403);
          yield* accountTry(() =>
            assertAuthorized(callerUserId, accountId, ACCOUNT_ACTIONS.MEMBER_REMOVE),
          );

          const targetMembership = yield* accountTry(() => getMembership(targetUserId, accountId));
          if (!targetMembership) return yield* accountFail({ error: 'Member not found' }, 404);

          // Admin cannot remove an owner — invariant preserved on top of IAM.
          if (
            callerMembership.accountRole === 'admin' &&
            targetMembership.accountRole === 'owner'
          ) {
            return yield* accountFail({ error: 'Admins cannot remove owners' }, 403);
          }

          if (targetMembership.accountRole === 'owner') {
            const owners = yield* accountTry(() => countOwners(accountId));
            if (owners <= 1) {
              return yield* accountFail({ error: 'Cannot remove the last owner' }, 409);
            }
          }
          ...
          yield* Effect.sync(() => invalidateIamCacheForUser(targetUserId));
    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          if (!callerMembership) return yield* accountFail({ error: 'Forbidden' }, 403);
          yield* accountTry(() =>
            assertAuthorized(callerUserId, accountId, ACCOUNT_ACTIONS.MEMBER_UPDATE),
          );

          const body = yield* accountTry(() => readBody(c));
          const newRole = parseRole(body.role, ['owner', 'admin', 'member']);
          if (!newRole)
            return yield* accountFail({ error: 'role must be one of owner|admin|member' }, 400);
          ...
          const ownerChangeAllowed =
            newRole === 'owner' || targetMembership.accountRole === 'owner'
              ? yield* accountTry(() =>
                  authorize(callerUserId, accountId, ACCOUNT_ACTIONS.MEMBER_SUPER_ADMIN_GRANT),
                )
              : { allowed: true };
          if (!ownerChangeAllowed.allowed) {
          ...
          if (newRole === 'owner' && callerMembership.accountRole !== 'owner') {
            return yield* accountFail({ error: 'Only owners can grant the owner role' }, 403);
          }

          if (targetMembership.accountRole === 'owner' && newRole !== 'owner') {
            const owners = yield* accountTry(() => countOwners(accountId));
            if (owners <= 1) {
              return yield* accountFail({ error: 'Cannot demote the last owner' }, 409);
            }
          }
    ...
    async (c: any) => {
      return runAccountWorkflow(
        ...
        Effect.gen(function* () {
          ...
          const membership = yield* accountTry(() => getMembership(userId, accountId));
          if (!membership) return yield* accountFail({ error: 'Not a member' }, 404);
          ...
          yield* Effect.sync(() => invalidateIamCacheForUser(userId));
