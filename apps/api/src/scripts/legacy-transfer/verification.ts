...
export function assertWorkspaceVerified(projectId: string, proof: {
  ...
  workspace_restore?: { restored_entries?: number; regular_files?: number; metadata_verified?: boolean; target?: string };
}): void {
  ...
  if (!/^[a-f0-9]{64}$/.test(capture.archive_sha256 ?? '') ||
      ...
      restore.metadata_verified !== true || restore.target !== `/workspace/${projectId}`) {
