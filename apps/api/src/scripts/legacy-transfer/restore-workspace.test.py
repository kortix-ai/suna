...
class RestoreTests(unittest.TestCase):
 def setUp(self):
  ...
  entries=[]
  for p in [self.source, self.source/'empty', self.source/'file']:
   st=p.stat(); row=dict(path=str(p.relative_to(self.source)), type='directory' if p.is_dir() else 'file',mode=st.st_mode & 0o7777,mtime_ns=st.st_mtime_ns)
