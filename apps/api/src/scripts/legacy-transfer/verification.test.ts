...
const valid = {workspace_status:'captured',source_sandbox_id:'source-box',workspace_capture:{archive_sha256:'a'.repeat(64),entries:4,files:1},workspace_restore:{restored_entries:4,regular_files:1,metadata_verified:true,root_directory_verified:true,exact_inventory_verified:true,file_hashes_verified:true,target:'/workspace/project'}};
...
test('a file count cannot consume the required root directory',()=>{
 expect(()=>assertWorkspaceVerified('project',{...valid,workspace_capture:{...valid.workspace_capture,entries:1},workspace_restore:{...valid.workspace_restore,restored_entries:1}})).toThrow();
});
