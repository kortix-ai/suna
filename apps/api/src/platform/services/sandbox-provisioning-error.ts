...
export const INVALID_SECRET_BOUNDARY_POLICY_MESSAGE =
  "A network-boundary secret in this workspace has an invalid outbound policy, so no session can start. " +
...
/**
 * The workspace's own network-boundary config is unusable, so `resolveNetworkBoundaryBindings`
 ...
 */
export function classifySandboxProvisioningFailure(
  error: unknown,
): SandboxProvisioningFailure {
  ...
  if (isGitAuth) {
    return {
      category: "git-auth",
      userMessage:
        "Couldn't access the workspace's Git repository. Check the workspace's Git credentials and try again.",
      isCapacity: false,
      isGitAuth: true,
    };
  }
