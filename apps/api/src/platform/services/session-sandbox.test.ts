...
describe('provisionSessionSandbox — mid-provision delete race', () => {
  ...
  test('always injects KORTIX_LLM_AI_SDK_NATIVE=true into the sandbox env', async () => {
    const opened = waitFor((resolve) => {
      onComputeOpened = resolve;
    });
    ...
    // buildKortixProvider reads it to select @ai-sdk/gateway. Native is the
    // only transport — there is no operator flag.
