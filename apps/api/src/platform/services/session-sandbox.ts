  ...
  const providerCreateInput: CreateSandboxOpts = {
    ...
    envVars: {
      ...
      ...(gatewayLlmKey
        ...
        : {}),
      // AI-SDK-native transport — always on. Tells the daemon's
      // `buildKortixProvider` to select `@ai-sdk/gateway` (native
      // `/language-model`) instead of `@ai-sdk/openai-compatible`. There is no
      // operator flag: it used to gate on `GATEWAY_AI_SDK_NATIVE`, which let the
      // standalone gateway drift OFF while sessions were baked native, 404-ing
      // every turn. Allowlisted in the daemon's env route
      ...
      // a session on native BYOK ignores it.
      KORTIX_LLM_AI_SDK_NATIVE: 'true',
    },
