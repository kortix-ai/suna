...
export class PlatinumProvider implements SandboxProvider {
  ...
  private async provisionFromTemplate(template: string, opts: CreateSandboxOpts): Promise<ProvisionResult> {
    ...
    if (dedup) {
      ...
      // Tags the box for a future orphan-box reaper — lets it identify +
      // age-gate Kortix-owned boxes without depending on the name format.
      createBody.metadata = { kortixSandboxId: opts.sandboxId, kortixEnv: config.INTERNAL_KORTIX_ENV };
    }
