...
function findRepoRoot(): string | null {
  ...
  for (const dir of candidates) {
    ...
    const hasComposeScripts = existsSync(
      resolve(dir, "scripts/compose/docker-compose.yml"),
    );
...
setupApp.openapi(
  createRoute({
    ...
    responses: {
      200: json(
        z.object({ installed: z.boolean().nullable() }),
        "Install status",
      ),
      503: json(
        z.object({ installed: z.null(), error: z.string() }),
  ...
  effectHandler(async (c: any) => {
    try {
      if (!hasDatabase) {
        ...
        return c.json(
          { installed: null, error: "Database not configured" },
          503,
        );
      }
      ...
      const queryResult = result as
        ...
        | Array<{ has_users?: boolean | "t" | "f" }>;
      const row = Array.isArray(queryResult)
        ? queryResult[0]
        : queryResult.rows?.[0];
...
setupApp.openapi(
  ...
  effectHandler(async (c: any) => {
    ...
    const capabilities: Record<
      string,
      { async: boolean; events: boolean; polling: boolean }
    > = {
    ...
    return c.json({
      providers: available,
      default: available.includes(config.getDefaultProvider())
        ? config.getDefaultProvider()
        : available[0] || "daytona",
      capabilities: Object.fromEntries(
        available.map((p) => [
          p,
          capabilities[p] || { async: false, events: false, polling: false },
        ]),
      ),
    });
  }),
    ...
    try {
      ...
      if (listed.error) {
        return c.json(
          {
            success: false,
            error: listed.error.message || "Could not inspect existing users",
          },
          500,
        );
      }
      ...
      if (error) {
        return c.json(
          { success: false, error: error.message || "Could not create owner" },
          500,
        );
      }
...
setupApp.openapi(
  createRoute({
    ...
    responses: {
      200: json(
        z.object({ complete: z.boolean(), completedAt: z.string().nullable() }),
  ...
  effectHandler(async (c: any) => {
    ...
    try {
      ...
      return c.json({
        complete,
        completedAt: account?.setupCompleteAt?.toISOString() ?? null,
      });
    } catch (e) {
...
setupApp.openapi(
  createRoute({
    ...
    responses: {
      ...
      500: json(
        z.object({ ok: z.boolean(), error: z.string() }),
        "Server error",
      ),
      ...errors(401),
    },
...
setupApp.openapi(
  createRoute({
    ...
    responses: {
      ...
      500: json(
        z.object({ step: z.number(), error: z.string() }),
        "Server error",
      ),
      ...errors(401),
    },
  ...
  effectHandler(async (c: any) => {
    ...
    try {
      ...
      return c.json(
        { step: 0, error: e instanceof Error ? e.message : String(e) },
        500,
      );
    }
  }),
