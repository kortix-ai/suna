...
export interface ConnectorDraft {
  ...
  credential?: 'shared';
  auth?: { type?: 'none' | 'bearer' | 'basic' | 'custom' | 'oauth1'; in?: 'header' | 'query' | 'path'; name?: string; prefix?: string };
}
...
export interface ConnectorConfigView {
  ...
  baseUrl: string | null;
  spec: string | null;
  auth: { type: 'none' | 'bearer' | 'basic' | 'custom' | 'oauth1'; in: 'header' | 'query' | 'path'; name: string | null; prefix: string | null };
}
