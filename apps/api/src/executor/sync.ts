...
import { goalTriggersEnabled } from '../projects/lib/triggers';
...
export async function syncProjectConnectors(
  ...
  if (manifest) {
    // R-8: goal-derived triggers must be declared to the runtime catalog HERE,
    // not just on a UI read. Post-refactor the catalog IS the fire registry and
    // this sweep is the only reconcile that runs for a project nobody opens —
    // and reconcile DELETES undeclared slugs (trigger-runtime-catalog-core.ts).
    // Omitting the opt-in is therefore worse than inert: goal rows created by
    // loadTriggersForResponse would be reaped on the next connector sweep, so a
    // `goals:` block would flicker in on a UI visit and vanish again.
    const triggers = extractTriggers(manifest, { goals: goalTriggersEnabled(row.metadata) });
