...
export async function getSsoProvider(accountId: string): Promise<SsoProvider | null> {
  return runEffectOrThrow(getSsoProviderEffect(accountId));
}

const getSsoProviderEffect = (accountId: string) =>
  Effect.gen(function* () {
...
export async function upsertSsoProvider(args: {
  ...
  return runEffectOrThrow(Effect.gen(function* () {
    const { database } = yield* DatabaseService;
    const existing = yield* getSsoProviderEffect(args.accountId);
    if (existing) {
      const [row] = yield* Effect.tryPromise(() =>
        database
...
export async function createSsoGroupMapping(args: {
  ...
  return runEffectOrThrow(Effect.gen(function* () {
    ...
    const [grp] = yield* Effect.tryPromise(() =>
      database
