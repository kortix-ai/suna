...
export async function addGroupMembers(args: {
  ...
  return runEffectOrThrow(Effect.gen(function* () {
    ...
    const validRows = yield* Effect.tryPromise(() =>
      database
    ...
    yield* Effect.sync(() => invalidateIamCacheForUsers(filtered));
...
export async function removeGroupMember(
  ...
  return runEffectOrThrow(Effect.gen(function* () {
    ...
    if (rows.length > 0) yield* Effect.sync(() => invalidateIamCacheForUser(userId));
    return rows.length > 0;
  }));
}
