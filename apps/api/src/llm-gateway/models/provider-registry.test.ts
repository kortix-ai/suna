...
describe('runtime catalog provider resolution', () => {
  ...
  // flow can build a kind:'bedrock' descriptor from a workspace's own key.
  ...
  // WORKSPACE's own AWS_REGION secret, not deployment config — this function has
  // no workspace context, so resolve-candidates.ts resolves the region-aware
