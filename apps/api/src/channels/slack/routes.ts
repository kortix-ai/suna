...
const verifySlackRequest = (
  ...
  Effect.sync(() =>
    ...
    Effect.flatMap((ok) =>
      ok ? Effect.void : failJson({ error: "Invalid signature" }, 401),
    ),
  );
  ...
  return Effect.gen(function* () {
    ...
    return yield* dependency(() =>
      ...
      Effect.catchAll((error) =>
        Effect.sync(() => {
          console.error(
            "[slack-webhook] slash command failed",
            errorCause(error),
          );
...
const oauthEventWorkflow = (envelope: SlackEnvelope) =>
  Effect.gen(function* () {
    ...
    const handledDmCommand = yield* dependency(() =>
      maybeHandleDmCommand(teamId, event),
    );
    if (handledDmCommand) return;

    const resolution = yield* dependency(() =>
      resolveOauthProject(teamId, event.channel),
    );
    if (resolution.kind === "project") {
      yield* dependency(() =>
        dispatchSlackEvent(resolution.projectId, envelope),
      );
    } else if (resolution.kind === "ambiguous") {
      yield* dependency(() =>
        maybePostPicker(teamId, resolution.projectIds, envelope),
      );
...
const oauthEventsRouteWorkflow = (c: Context) =>
  Effect.gen(function* () {
    ...
    fireAndLog(
      "[slack-webhook] oauth handler failed",
      oauthEventWorkflow(envelope),
    );
    return jsonResponse({ ok: true });
  });
  ...
  Effect.gen(function* () {
    ...
    if (!mode.available || !mode.signingSecret) {
      return jsonResponse(
        {
          response_type: "ephemeral",
          text: "OAuth mode not configured on this server.",
        },
        503,
      );
    }
...
const byoEventsRouteWorkflow = (c: Context) =>
  Effect.gen(function* () {
    ...
    const signingSecret = yield* dependency(() =>
      loadSlackSigningSecretForProject(projectId),
    );
    if (!signingSecret)
      return yield* failJson({ error: "Not configured" }, 404);
    ...
    fireAndLog(
      "[slack-webhook] byo handler failed",
      byoEventWorkflow(projectId, envelope),
    );
    return jsonResponse({ ok: true });
  });
...
slackWebhookApp.openapi(
  createRoute({
    ...
    tags: ["channels"],
    summary:
      "Per-project (BYO app) Slack slash command webhook (signature verified)",
    request: {
      params: z.object({ projectId: z.string() }),
      body: {
        content: { "application/x-www-form-urlencoded": { schema: z.any() } },
...
slackWebhookApp.openapi(
  createRoute({
    ...
    tags: ["channels"],
    summary:
      "Per-project (BYO app) Slack interactivity webhook (signature verified)",
    request: {
      params: z.object({ projectId: z.string() }),
      body: {
        content: { "application/x-www-form-urlencoded": { schema: z.any() } },
...
slackWebhookApp.openapi(
  createRoute({
    ...
    tags: ["channels"],
    summary:
      "Per-project (BYO app) Slack app manifest (single source of truth)",
    request: {
      params: z.object({ projectId: z.string() }),
      query: z.object({
