...
describe('channel signed state', () => {
  test('round-trip the payload for their own purpose', () => {
    const state = signChannelState('teams-oauth', { projectId: 'p1', userId: 'u1' }, 60_000);
    const payload = verifyChannelState('teams-oauth', state);
  ...
  test('a token signed for one purpose does not verify as another', () => {
    ...
    expect(verifyChannelState('slack-oauth', state)).toBeNull();
  ...
  test('expired, malformed, and empty-key tokens do not verify', () => {
    ...
    expect(verifyChannelState('slack-oauth', 'not-signed')).toBeNull();
