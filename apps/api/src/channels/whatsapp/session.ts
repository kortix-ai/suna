...
const WHATSAPP_TURN_INSTRUCTIONS = [
  ...
  '- Send replies with the `whatsapp` Executor connector, or the `w` CLI in the sandbox.',
  '  Credentials are resolved server-side; do not look for an API key in the sandbox.',
  '- Reply into the same chat with `whatsapp.send_text` using the chat JID below.',
  '- Read `.opencode/skills/whatsapp/SKILL.md` before writing: WhatsApp uses *single asterisks*',
  '  for bold, not Markdown, and messages should be short and informal.',
