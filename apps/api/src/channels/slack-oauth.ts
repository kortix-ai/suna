...
import { linkSlackIdentity, lookupSlackIdentity } from './slack/identity';
import {
  frontendBase,
  installHandoffUrl,
  stateForCaller,
...
export async function seedInstallerIdentity(input: {
  ...
  const existing = await lookupSlackIdentity(input.teamId, input.slackUserId);
  if (existing && existing.userId !== input.userId) return 'kept';
  await linkSlackIdentity(input);
  return 'linked';
}
