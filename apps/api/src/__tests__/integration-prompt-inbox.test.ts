...
describe('a prompt given up on records WHY as a code, and a retry clears it', () => {
  ...
  test('the code is merged into result, beside the markers the row already carried', async () => {
    ...
    await markCommandFailed(await hold(row), 'prompt accepted by the runtime but never became a message', {
      retryable: false,
      attempts: 1,
      sessionId: SESSION_ID,
  ...
  test('a producer that names no cause is `unknown`; a failure only re-queued records none', async () => {
    ...
    await markCommandFailed(await hold(dead), 'admission check failed: boom', {
      retryable: true,
      attempts: 5,
      sessionId: SESSION_ID,
    });
    ...
    await markCommandFailed(await hold(requeued), 'the session was not ready in time', {
      retryable: true,
      attempts: 1,
      sessionId: SESSION_ID,
  ...
  test('retry clears the code together with the error', async () => {
    ...
    await markCommandFailed(await hold(row), 'Out of credits. Top up to continue.', {
      retryable: false,
      attempts: 1,
      sessionId: SESSION_ID,
