...
describe('resource type guard', () => {
  test('agent, skill, secret + space are valid resource types (READ/REVOKE back-compat)', () => {
...
describe('creatable resource type guard — the two closed-by-default object types', () => {
  test('agent + space are creatable; skill/secret are NOT (governed by editor role + agent inheritance)', () => {
    expect(CREATABLE_RESOURCE_GRANT_TYPES).toEqual(['agent', 'space']);
    expect(isCreatableResourceType('agent')).toBe(true);
    expect(isCreatableResourceType('space')).toBe(true);
