/**
 * Parser-level tests for `apps:` in kortix.yaml (v2 manifest).
...
function parseAndExtract(body: string) {
  return extractApps(parseManifestString(manifestWith(body), 'yaml', 'kortix.yaml'));
}
...
`);
    ...
    expect(specs[0]!.path).toBe('kortix.yaml#apps.site');
  });
...
`);
  ...
  test('multiple apps: entries in one manifest sort A-Z by slug', () => {
    const { specs, errors } = parseAndExtract(`
apps:
  - slug: zeta
    domains:
...
`),
    ...
    const reparsed = extractApps(parseManifestString(serialized, 'yaml', 'kortix.yaml'));
...
describe('apps: — rejection paths', () => {
  test('top-level `apps` as a mapping (not a list) is rejected', () => {
    const { specs, errors } = parseAndExtract(`
apps:
  slug: wrong
`);
