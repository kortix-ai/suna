...
describe('legacy → per-seat adoption (regression)', () => {
  ...
  //
  // A deal that is BOTH Enterprise (entitlements) AND per-seat (billing) — e.g.
  // the LibreMax shape: $40K/yr Enterprise flat + $40/enabled-seat/mo with
  // 2,500 pooled credits/seat — must hold both at once. The previous webhook
  ...
  describe('enterprise + per-seat coexistence — tier not clobbered', () => {
    test('enterprise_entitled=true + per-seat sub update → billing_model reconciled, tier NOT set to per_seat', async () => {
      // The exact LibreMax contract shape: enterprise entitlements (via flag)
