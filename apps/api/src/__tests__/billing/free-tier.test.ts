...
describe('free tier account setup', () => {
  test('initializes a free account with one idempotent $5 expiring grant', async () => {
...
describe('free tier monthly credit rotation', () => {
  ...
  test('resets unused 300 display credits to exactly 500 fresh credits', async () => {
    freeAccountsDueResult = [
      createMockCreditAccount({
    ...
    expect(resetExpiringCreditsCalls[0][1]).toBe(5);
    expect(resetExpiringCreditsCalls[0][2]).toBe('Free tier monthly credit reset: 5 credits');
    expect(resetExpiringCreditsCalls[0][3]).toBe('free_tier_rotation_acc_300_left_2026-07');
  });

  test('resets unused 20 display credits to exactly 500 fresh credits', async () => {
    freeAccountsDueResult = [
      createMockCreditAccount({
  ...
  test('grants exactly 500 display credits even when the free wallet is empty', async () => {
    freeAccountsDueResult = [
      createMockCreditAccount({
        accountId: 'acc_empty',
        tier: 'free',
