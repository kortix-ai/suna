...
const envTest = read('apps/api/.env.test');
...
describe('the kortix-api suite actually runs on pull requests', () => {
  ...
  test('the unit suite runs off the committed fake env, not dotenvx', () => {
    ...
    expect(runLine).toContain('--env-file=.env.test');
  ...
  test('.env.test carries no ciphertext and no live-looking credential', () => {
    expect(envTest).not.toContain('encrypted:');
