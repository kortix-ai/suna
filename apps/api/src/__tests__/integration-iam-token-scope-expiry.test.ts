 ...
 * as e2e-untested —
 *   (1) a WORKSPACE-SCOPED token (PAT/session bound to a project) can act ONLY on
...
describe('grant expiry (filtered at authorization time)', () => {
  test('an EXPIRED project grant is ignored; a FUTURE-dated one still applies', async () => {
    ...
    await db.insert(projectMembers).values({
      accountId: ACCOUNT, workspaceId: P1, userId: expired, projectRole: 'editor',
    ...
    await db.insert(projectMembers).values({
      accountId: ACCOUNT, workspaceId: P1, userId: future, projectRole: 'editor',
