...
describe('resolveSessionOrigin', () => {
  ...
  test('scheduled + triggered invocations are classified by source, not token', () => {
    ...
    // Source wins even if a service-account token happened to drive the fire.
  ...
  test('human callers (pat/supabase/apiKey) on interactive surfaces are user', () => {
    expect(resolveSessionOrigin({ authType: 'supabase', source: 'ui' })).toBe('user');
    expect(resolveSessionOrigin({ authType: 'pat', source: 'cli' })).toBe('user');
