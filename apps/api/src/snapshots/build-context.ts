 ...
 * (`stageWarmRepoCheckout` / `stageWarmRepoArchive`), NOT inside the built
 * image: the resulting build context carries only a sanitized, origin-reset,
 * credential-scrubbed checkout/archive that the Dockerfile `COPY`s. Omit for
...
export interface WarmRepoContext {
   ...
   *
   * Exposed under BOTH names so the two staging pipelines can read it with their
   * own vocabulary: `commitSha` feeds the full-rebuild archive (`stageWarmRepoArchive`
   * → `.tar.gz`, consumed by the layer renderer), `tip` feeds the FROM-base fast-path
   * checkout (`stageWarmRepoCheckout`). Both are this same exact sha.
   */
  commitSha: string;
  tip: string;
...
export async function stageWarmRepoCheckout(
  contextDir: string,
  warmRepo: Omit<WarmRepoContext, 'commitSha'>,
...
export async function stageBuildContext(
  ...
  // Self-heal a stale/missing prebuilt agent binary in place (rebuilds from
  // source when dist/ predates src/), which also guarantees the binary exists —
  // superseding a plain existence + mtime-skew fail-loud check.
