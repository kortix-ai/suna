...
async def _invalidate_kb_cache_for_entry(entry_id: str) -> None:
    ...
    try:
        ...
        # Get all agent IDs that have this entry assigned
        agent_ids = await convex.get_agents_for_knowledge_entry(entry_id)

        # Invalidate cache for each agent
        for agent_id in agent_ids:
            await invalidate_kb_context_cache(agent_id)

        logger.info(f"Invalidated KB cache for {len(agent_ids)} agents after entry {entry_id} update")
...
class FileProcessor:
    ...
    def __init__(self):
        # Using Convex client for storage and database operations
        pass
    ...
    async def process_file_fast(
        ...
        try:
            ...
            # Generate storage path (this would normally go to S3 or Convex storage)
            # For now, we'll store the content reference
            storage_path = f"knowledge-base/{folder_id}/{entry_id}/{sanitized_filename}"

            # Create the entry in Convex
            try:
                entry = await convex.create_knowledge_base_entry(
                    entry_id=entry_id,
                ...
                logger.info(f"[PROCESSOR] Created entry {entry_id} in {time.time() - start:.2f}s")

                # Store file content for later retrieval (in production, this goes to S3/Convex storage)
                # For now, we'll note that storage needs to be implemented
                ...
                logger.error(f"Convex error creating entry: {e}")
                return {
                    'success': False,
    ...
    async def _generate_and_update_summary(
        ...
        try:
            ...
            convex = get_convex_client()
            await convex.update_knowledge_base_entry(
                entry_id=entry_id,
                summary=summary,
                status="completed"
            )

            # Invalidate cache for agents that have this entry assigned
            await _invalidate_kb_cache_for_entry(entry_id)

            logger.info(f"Summary generated and updated for entry {entry_id}")
            ...
            try:
                ...
                logger.error(f"Error updating entry status: {update_error}")
    ...
    async def process_file(
        ...
        try:
            ...
            content = self._extract_content(file_content, filename, mime_type)
            summary = await self._generate_summary(content, filename) if content else None
