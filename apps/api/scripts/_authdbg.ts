...
console.log('tokenId:', r.tokenId, 'workspaceId:', r.workspaceId, 'secret prefix:', String(r.secretKey).slice(0, 12));
...
console.log('latest token rows:', JSON.stringify(rows.map((x: any) => ({ tokenId: x.tokenId, workspaceId: x.workspaceId, revoked: x.revokedAt, expires: x.expiresAt }))));
