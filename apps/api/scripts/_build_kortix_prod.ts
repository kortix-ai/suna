...
console.log('[build] platinum=', process.env.PLATINUM_API_URL, ' project=', p.workspaceId);
const t0 = Date.now();
const r = await ensureSandboxImage(
  { workspaceId: p.workspaceId, repoUrl: p.repoUrl, defaultBranch: (p as any).defaultBranch ?? 'main', manifestPath: (p as any).manifestPath ?? null, gitAuthToken: null } as any,
