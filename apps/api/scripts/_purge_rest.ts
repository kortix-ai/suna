...
for (const p of live as any[]) {
  console.log('active:', p.workspaceId.slice(0, 8), p.name, 'acc', p.accountId.slice(0, 8));
  const tok = (await createAccountToken({ accountId: p.accountId, userId: p.accountId, name: 'purge2' })).secretKey;
  const r = await fetch(`http://localhost:8008/v1/projects/${p.workspaceId}`, { method: 'DELETE', headers: { Authorization: `Bearer ${tok}` } });
