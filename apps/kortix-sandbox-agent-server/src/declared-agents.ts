 ...
 * nothing else.
 *
 * STILL NEEDED ALONGSIDE config-releases (boot-config.ts). The release store
 * hash-verifies every file it serves, which independently closes this hole for
 * a project that has `config_releases` on — but that flag defaults OFF
 * (feature-flags/registry.ts), and `revertToPreReleaseConfig` in
 * harness/open-code/config-release.ts falls straight back to
 * `resolveOpencodeConfigDir`, the writable in-repo tree, for every project that
 * doesn't opt in. This call is unconditional (every spawn, regardless of which
 * config dir is live) rather than gated on that flag: pruning an already
 * hash-verified release directory against the SAME compiled config the release
 * was built from is a no-op (the declared set can only match), so there is
 * nothing to lose by running it there too, and one call site never drifts out
 * of sync with which path is active this week.
 */
