...
export function createEnvRouter(cfg: Config, projectEnv: ProjectEnvStore): Hono {
  ...
  router.post('/', async (c) => {
    ...
    if (bearerToken(c.req.header('Authorization')) !== cfg.sandboxToken) return c.json({ error: 'unauthorized' }, 401)
    if (c.req.header(KORTIX_USER_CONTEXT_HEADER)) return c.json({ error: 'forbidden' }, 403)
