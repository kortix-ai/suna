...
describe('execution-only environment', () => {
  test('reports ready without contacting or starting an agent runtime', async () => {
    ...
    expect(await response.json()).toMatchObject({ runtimeReady: true, environmentRuntimeVersion: 5, workload: 'environment', opencode: 'disabled', opencode_pid: null, opencode_port: null })
  ...
  test('the real daemon ignores inherited OpenCode warm-seed and initial-session flags', async () => {
    ...
    try {
      ...
      expect(health).toMatchObject({ runtimeReady: true, environmentRuntimeVersion: 5, workload: 'environment', opencode: 'disabled', opencode_pid: null, opencode_session_required: false })
