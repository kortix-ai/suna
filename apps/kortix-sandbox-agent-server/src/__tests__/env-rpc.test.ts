...
describe('env-rpc route', () => {
  ...
  test('preserves symlinks, dangling entries, and line semantics from the Pi filesystem contract', async () => {
    ...
    expect(
      list.body.value.find((entry: { name: string }) => entry.name === 'dangling.txt'),
    ).toMatchObject({
      path: path.join(workspace, 'tree', 'dangling.txt'),
      kind: 'symlink',
    });
  ...
  test('cancel waits for the remote process group to exit before acknowledging', async () => {
    const { app, workspace } = await makeApp();
    const requestId = 'cancel-process-group';
    ...
    const run = app.request('/', {
      ...
      headers: {
        'content-type': 'application/json',
        [KORTIX_USER_CONTEXT_HEADER]: mintUserContext(RPC_SECRET),
      },
      body: JSON.stringify({
        op: 'exec',
        requestId,
        cwd: workspace,
        args: {
          command: `echo $$ > ${JSON.stringify(pidFile)}; (sleep 0.8; echo late > ${JSON.stringify(marker)}) & wait`,
        },
      }),
    });
    ...
    const cancelled = await app.request('/cancel', {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        [KORTIX_USER_CONTEXT_HEADER]: mintUserContext(RPC_SECRET),
    ...
    expect(Date.now() - cancelledAt).toBeLessThan(600);
    const result = (await (await run).json()) as any;
    expect(result).toMatchObject({ ok: false, error: { code: 'ABORT_ERR' } });
    expect(() => process.kill(pid, 0)).toThrow();
    await Bun.sleep(900);
    expect(await fs.stat(marker).catch(() => null)).toBeNull();
  }, 5_000);

  test('an HTTP client disconnect cancels its remote process group', async () => {
    const { app, workspace } = await makeApp();
    const server = Bun.serve({ port: 0, fetch: app.fetch });
    const marker = path.join(workspace, 'disconnect-marker.txt');
    const pidFile = path.join(workspace, 'disconnect.pid');
    const controller = new AbortController();
    const run = fetch(`http://127.0.0.1:${server.port}/`, {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        [KORTIX_USER_CONTEXT_HEADER]: mintUserContext(RPC_SECRET),
  ...
  test('cancel acknowledges a non-exec mutation only after the mutation settles', async () => {
    const { app, workspace } = await makeApp();
    const fifo = path.join(workspace, 'mutation.fifo');
    const mkfifo = Bun.spawn(['mkfifo', fifo], { stdout: 'ignore', stderr: 'pipe' });
    expect(await mkfifo.exited).toBe(0);
    const requestId = 'cancel-file-mutation';
    const mutation = app.request('/', {
      ...
      body: JSON.stringify({
        ...
        requestId,
        cwd: workspace,
        args: { path: fifo, content: 'mutated' },
      }),
    });
    await Bun.sleep(30);
    ...
    const first = await Promise.race([
      cancellation.then(() => 'acknowledged' as const),
      Bun.sleep(50).then(() => 'waiting' as const),
    ]);
    const reader = first === 'waiting' ? fs.readFile(fifo) : null;
    const cancelled = await cancellation;
    const mutated = await mutation;

    expect(cancelled.status).toBe(200);
    expect(await cancelled.json()).toEqual({ ok: true, value: { cancelled: true } });
    expect((await mutated.json()) as any).toMatchObject({
      ok: false,
      error: { code: 'ABORT_ERR' },
    });
    expect(first).toBe('waiting');
    expect(reader).not.toBeNull();
    if (reader) expect(await reader).toHaveLength(0);
  }, 5_000);

  test('a cancellation that arrives before its request prevents the delayed mutation', async () => {
    const { app, workspace } = await makeApp();
    const requestId = 'cancel-before-request';
    const marker = path.join(workspace, 'delayed-mutation.txt');
    const headers = {
      'content-type': 'application/json',
      [KORTIX_USER_CONTEXT_HEADER]: mintUserContext(RPC_SECRET),
    };
    ...
    expect(await cancelled.json()).toEqual({ ok: true, value: { cancelled: false } });
    ...
    const mutation = await app.request('/', {
      ...
      body: JSON.stringify({
        ...
        requestId,
        cwd: workspace,
        args: { path: marker, content: 'must not exist' },
      }),
    });
