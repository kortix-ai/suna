...
describe('withModelLimits', () => {
  ...
  test('preserves a gateway-provided output limit when only context is missing', () => {
    const out = withModelLimits({ 'deepseek/deepseek-v4-flash': { name: 'Flash', limit: { output: 12_345 } } })
    expect(out['deepseek/deepseek-v4-flash']?.limit).toEqual({ context: 1_048_576, output: 12_345 })
  })
