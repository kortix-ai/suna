 ...
 * deliberately; `pi-wire-id.test.ts` runs this minter against the shared
 * frozen vectors (tests/spec/wire-message-id.vectors.json) that apps/api and
 * packages/sdk assert too, so the three cannot drift silently.
 */
...
function wireIdTime(messageId: string | null | undefined): bigint | null {
