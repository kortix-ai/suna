...
async function startSessionRuntime(
  ...
  if (bootState.initialOpenCodeSessionRequired) {
    ...
    // the agent" until a human clicked Restart (a customer session, 10+ min).
 ...
 * a customer session). This relay fires on the root's `busy`/`retry`
...
export async function relayTurnEndToApi(
  ...
  // session's completions: the 2026-08-18 customer incident was a CHILD session
...
async function readRootTurnState(
  ...
  try {
    ...
    // there unnamed EVERY relay for such sessions (live 2026-08-20, a customer
    // session: `relay_named:false` on each end, double finalizes
