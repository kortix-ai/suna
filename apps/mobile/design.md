...
| The project hamburger (iOS 26+) | `PlatformButton glass` via `MenuButton` — native Liquid Glass circle; the Host is padded 18pt so the glass shadow is not clipped (the plain `PlatformButton` stays glass-free for that reason) |
...
| Chat input position | Pinned to the bottom with `px-4`, above the dock (`insets.bottom + 72`); follows the keyboard to 8pt above it. Text, files, and model stay until the send succeeds |
...
| Nav rows | `NavPill` rows in a `px-2 pb-2` column, no gap: `Pressable` `flex-row items-center gap-3 rounded-xl px-3 py-2.5 active:bg-accent` (44pt), Lucide icon `size={18} strokeWidth={2.2}` `text-foreground` on the 20pt line, label `Text` `font-medium`, one line. Order: Sessions (`MessagesSquare`, → `sessions`), Files (`Folder`, → `files`), All projects (`LayoutGrid`, → `router.replace('/projects')`, the only way to the Projects list). Sessions and Files go through ProjectScreen's `navigateProjectRoute` (push or replace, see the table above) |
...
| New session | `Button size="lg" className="rounded-full"` — the default (primary) variant, 44pt, `Plus` icon (`size={20}`) + "New session", content width. `haptics.tap()` → `onClose()` → `onNewSession()` (ProjectScreen `returnHome`: reset the store and pop a covering route, so project home's composer starts the session) |
