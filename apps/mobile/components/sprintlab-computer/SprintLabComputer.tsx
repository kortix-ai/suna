...
export function SprintLabComputer({
  toolMessages,
  currentIndex,
  onNavigate,
  messages,
  ...
  log.log('[SprintLabComputer] currentPair:', currentPair ? 'has pair' : 'undefined');
  log.log('[SprintLabComputer] currentPair.toolMessage:', currentPair?.toolMessage?.message_id || 'null');
  log.log('[SprintLabComputer] currentPair.assistantMessage:', currentPair?.assistantMessage?.message_id || 'null');
  
  const { toolCall, toolResult, isSuccess, assistantTimestamp, toolTimestamp } = useMemo(() => {
    if (!currentPair?.toolMessage) {
      log.log('[SprintLabComputer] No toolMessage in currentPair, returning null');
      return { toolCall: null, toolResult: null, isSuccess: false, assistantTimestamp: undefined, toolTimestamp: undefined };
    }
    log.log('[SprintLabComputer] Calling extractToolCallAndResult');
