...
// The Workspace detail drawer lists its sessions (GET /workspaces/:id/
 ...
 * exists. Web parity: the workspace home stashes the prompt and sends it after the
...
export default function WorkspaceSessionScreen() {
  ...
  // middle · right drawer). workspaceId is the Workspace we scope the
  // new API onto.
  const { id: workspaceId } = useLocalSearchParams<{ id: string }>();
  const insets = useSafeAreaInsets();

  // Tabs are remembered per Workspace. Switch the tab store before the first paint.
  useLayoutEffect(() => {
  ...
  // A Workspace session that is provisioning. The middle pane shows its state.
  ...
  const handleNewSession = useCallback(async () => {
    ...
    try {
      ...
      log.error('❌ [Workspace] Failed to create session:', err?.message || err);
  ...
  const handleConfigureSession = useCallback(
    async (prompt: string) => {
      ...
      try {
        ...
        if (showUpgradeForError(err)) return;
        log.error('❌ [Workspace] Failed to start config session:', err?.message || err);
  ...
  // restartWorkspaceSession). Tears down and re-provisions the sandbox.
  ...
  // Back to the Workspace list. Used by the drawer's Kortix logo.
  ...
  const handleDashboardSend = useCallback(
    async (text: string, options: PromptOptions, mentions?: TrackedMention[]) => {
      ...
      try {
        ...
        if (showUpgradeForError(err)) return;
        log.error('❌ [Workspace] Dashboard send failed:', err?.message || err);
        Alert.alert('Error', err?.message || 'Failed to start session');
      } finally {
  ...
  const renderDrawerContent = useCallback(() => {
    ...
    return (
      <View className="flex-1 bg-chrome-background" style={{ paddingTop: insets.top }}>
        {/* Kortix wordmark — tap to return to the Workspace list. */}
        <View className="flex-row items-center justify-between px-5 pb-4 pt-3">
          <TouchableOpacity
            onPress={goToWorkspaces}
            activeOpacity={0.6}
        ...
        {/* Top-level actions: New session, search, and all Workspaces. */}
            ...
            <>
              ...
              sandboxErrorMsg ? (
                <View className="flex-1 items-center justify-center px-8">
                  ...
                  <TouchableOpacity
                    onPress={
                      sandboxUpgradeGate ? () => openUpgradeSheet(sandboxUpgradeGate) : goToWorkspaces
                    }
              ...
              activePageId === 'page:changes' && PAGE_TABS[activePageId] && !showTabsOverview ? (
                <ChangesPage
                  ...
                  isRightDrawerOpen={rightDrawerOpen}
                />
              ) : /* Active page tab — Files (Workspace repo browser) */
              activePageId === 'page:files-nav' && PAGE_TABS[activePageId] && !showTabsOverview ? (
                <FilesNavPage
                  ...
                  isRightDrawerOpen={rightDrawerOpen}
                />
              ) : /* Active page tab — Sandbox (Workspace runtime image) */
              ...
              activePageId === 'page:dev' && PAGE_TABS[activePageId] && !showTabsOverview ? (
                <DevPage
                  ...
                  isRightDrawerOpen={rightDrawerOpen}
                />
              ) : /* Active page tab — Members (Workspace access) */
              activePageId === 'page:members' && PAGE_TABS[activePageId] && !showTabsOverview ? (
                <MembersNavPage
                  ...
                  isRightDrawerOpen={rightDrawerOpen}
                />
              ) : /* Active page tab — Settings (Workspace settings) */
              ...
              showTabsOverview ? (
                ...
                <KeyboardAvoidingView
                  ...
                  <Animated.View
                    ...
                    clipped by the card (web parity: WorkspaceHome brandmark). */}
                    <View
                      pointerEvents="none"
                      style={[
