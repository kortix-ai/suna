...
| 6 | Runtime authentication | Both boxes shared one token, so the API could not identify the caller. | `account_tokens.runtime_kind` and `runtime_id` bind one token to one physical runtime. Legacy null fields resolve to the worker during rollout. |
