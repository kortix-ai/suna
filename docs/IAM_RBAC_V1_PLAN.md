...
- **Phase 3 (backend) — role + policy management API.** Built the dark `/iam/roles`, `/iam/roles/:id/permissions`, `/iam/actions`, `/iam/policies` (+ `:bulk-delete`) routes to the frontend SDK's exact contract; built-in presets incl. the read+run **"User"** tier; `ROLE_*`/`POLICY_*` wired into `ADMIN_EXTRAS`; allow-only (`effect=allow`, conditions ignored); every mutation busts the affected principals' cache (`invalidateIamCacheForRole`/`…ForPolicyPrincipal`). Pure presets + validator unit-tested.
...
2. **Finish Phase 1b route gates** (fail-OPEN if missed → safe to complete incrementally; verify each with ke2e): secrets create/delete → `project.secret.write` (r3); connector config → `project.connector.write`; `[[apps]]` CRUD → `project.customize.write` (r4); CR create/commit-push → `project.gitops.push` (r8). **File-based agents/skills/commands/memory** are git files (no per-type route) → they stay under the generic write/CR gate in v1; per-type granularity is Phase 4.
3. **Smaller backend follow-ups:** `/iam/policies` `PATCH`, `:bulk-import`, `:simulate` (the SDK calls them; non-core, so the UI's import/simulate buttons no-op until built); the **SA/token bridge** (engine union currently resolves member + group principals only — extend `resolveActorV2` to also match `principal_type='token'` for the acting service account, with a safe default so existing automation never 403s); a DB-backed integration test for the route handlers (gated on `TEST_DATABASE_URL`).
4. **Phase 4 (flagged, highest-risk) — bypass seams + resource grants.** `authorizeGitProxy` honoring `GitScope` + token caps (default-allow legacy); `iam_resource_grants` fail-closed for "can't even SEE the rest"; group tab in SharingPicker.
...
**Note:** the new leaves + Phase-1b gates are inert for built-in roles (everyone holds the leaves) — the first user-visible deactivation comes from a **custom role** (now buildable via the API) that omits a leaf. Custom roles can already deactivate the **existing** `project.trigger.*` (Schedules/Webhooks), `members.manage`, gateway.* leaves plus the Phase-1b-gated deploy/customize/gitops-merge; the rest light up as Phase-1b route-gating completes.
