...
**⏳ Deliverable 2 — wire agent SESSIONS to an SA identity (NEXT, the hot-path half).** Add `account_tokens.service_account_id` (migration), stamp it at BOTH mint sites (cold `session-sandbox.ts` + warm `warm-pool.ts` — the warm path silently dropped agentGrant once; do not repeat), set it on the auth context (widen `authType` union to include `service_account` first — existing drift), and have the engine intersect the SA's standing role with the session `agentGrant` (effective = standingRole ∩ grant ∩ env). Needs a durable agent→SA mapping (encode in `service_accounts.name` or add a column; uniqueness on (account, project, agent)). Resolve the SA role **fail-CLOSED** (unlike the agentGrant cap). Then the policy-assignment UI gains an "agent / service account" principal option. **⚠️ This touches session-start latency + Slack/cron — guard with a default-safe path (token without service_account_id behaves exactly as today).**
