...
### ⏳ Next — the agent-identity north star (own identity, own env, secure)

0. **Agent-as-teammate (NEW top priority — Ino, 2026-06-25).** Make agents first-class IAM principals (see `project_agent_identity_northstar` memory). Three pillars shipped (own env, secure, scoped capabilities); the gap is **own identity** — today an agent is only `userRole ∩ agentGrant`, bounded by its launcher. Build: extend `resolveActorV2` to resolve **agent / service-account (`principal_type='token'`) principals** so admins can bind `agent → custom role @ scope` (the `service-accounts` surface + SDK already exist as the identity anchor); mint agent-session tokens carrying the AGENT identity's effective permissions; then the policy-assignment UI gains an "agent" principal option (today it correctly excludes `token`). **Pivotal call for Ino:** standing identity (agent has its own role, sessions only NARROW via the kortix.toml grant + env — recommended) vs. always-bounded-by-launcher (today's model). This is the same `token`-principal seam the edge-case audit closed as inert — reopening it *correctly* is the whole feature.
