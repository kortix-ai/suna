...
- **DEFERRED — the visibility-isolation tier (explicit scope choice):** file routes (`/files/content`, `/files/search`, `grepRepoFiles`) can still READ an agent/skill's raw markdown that `/detail` hides. This is exactly the **git-proxy + deny-prefix** tier ("can't even SEE the file"), the additive next step — not a regression in the edit/use-control scope shipped here.
...
- **FAST-FOLLOW (low):** the Slack `/kortix agents` PICKER still lists agent *names* cross-department (metadata only — the use-bypass above is now closed). Root-cause note from the review: per-resource read-filtering is enforced route-by-route; a shared deny-prefix/visibility helper would close the whole "forgot to filter on a new read path" class at once (the file routes + Slack picker would both hang off it).
...
- **140 / 150 files clean.** Every IAM-feature file passes in isolation: `unit-iam-*` (engine, role-perms, role-presets, session-gate, sso, cache-invalidation), `unit-agent-scope`, `unit-project-access(-fold)`, `unit-platform-roles`, `unit-projects-policies`, `unit-project-limit-policy`, `unit-trigger-activation-route`, `e2e-project-apps(-gate)`, `e2e-project-session-contract`, `e2e-projects-contract`, `e2e-projects-provision`, `e2e-project-limit`. Both DB-backed integration tests pass (`integration-agent-scope-http` 3/0, `integration-agent-grant-roundtrip` 2/0) — the strongest end-to-end IAM proof.
