...
`project_sessions.agent_name` defaults to the literal string `default`, and no agent is ever named `default` — it is a placeholder that the runtime resolves to OpenCode's configured `default_agent` (conventionally `kortix`). A session stored as `default` therefore carries the *least-privileged* executor grant: `null` (full, capped at the user) for projects that have not adopted `[[agents]]` governance, and default-deny for those that have. There is no agent-specific grant for a switch to inherit.
