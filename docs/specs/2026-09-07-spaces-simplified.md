# Spaces, simplified — spec addendum (2026-09-07)

Extends `2026-09-03-spaces.md` and `2026-09-06-space-files-and-scoped-agents.md`.
Three decisions from the user on 2026-09-07:

1. `instructions` and `context` are removed from a space — everywhere, not
   just from the page.
2. The space page loses its right-hand panel. It is one column: heading,
   composer, Recents.
3. A session can be moved between spaces, and in and out of the project
   level.

## 1. Problem

A space was two things at once: a container that groups sessions, and a
...
What is coming instead is configuration written into the space's file by a
person or an agent, and per-space dashboards grouped in tabs across the top
...
/spaces/:slug/context`, `addProjectSpaceContext` /
`removeProjectSpaceContext` in `@kortix/sdk`, `kortix spaces context
add|rm` and the `--instructions-file` / `--context` flags, the
`KORTIX_SPACE_CONTEXT` env envelope
(`apps/api/src/projects/lib/space-envelope.ts`), the daemon's
`/tmp/kortix/space.md` renderer
(`apps/kortix-sandbox-agent-server/src/space.ts`), and
`withSpaceInstructions` in the agent-config compiler.

**Kept:** `KORTIX_SPACE=<slug>` in the sandbox env. The in-sandbox CLI
reads it so `kortix sessions new` inherits the space; nothing else in the
box needs it. A session inside a space still compiles the agents that
space owns or references.
...
down with it. `validateSpaceFileV2` reports each as a **warning** and the
...
with the space's name, the composer, and Recents under it. The `aside` slot
...
`PATCH /v1/projects/:projectId/sessions/:sessionId {"space": "<slug>" | null}`.
...
| The target space is declared | `400 SPACE_NOT_DECLARED` |
| The caller is granted the target | `403 space_not_accessible` (with `accessible_spaces`) |
| The session's agent is usable in the target | `400 AGENT_NOT_IN_SPACE` (with `usable_agents`) |

The sharing gate, not the manager tier, is the right one: a `sessions: shared`
space makes every session in it readable by everyone granted it, so a move
is a visibility decision. A manager who cannot read another person's private
session cannot re-file it either.

Only `project_sessions.space` changes. Every server-side gate reads that
column, so the move is complete when it commits. A sandbox that is already
running keeps the `KORTIX_SPACE` it booted with until its next start.

## 5. Coverage

`SPACE-1` asserts the two fields are `400` on create/update. `SPACE-7` covers the
