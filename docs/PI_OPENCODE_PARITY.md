...
| Shared environment resources | V2 and v3 accept the same seed/helper declarations. OpenCode installs before custom code starts; matching compiled boot needs no additional resource download | `GH-18` exercises both artifact routes, source pinning, authorization, and per-agent files. Installer tests preserve seed edits and deletions; full preview proof is recorded in PR #6998 |
...
| Original document uploads | Private PostgreSQL assets, original filenames, immutable message references, and lazy editable environment copies | Preview `5ef50b70c0`: SDK and browser CSV/PDF/SVG uploads with no environment; duplicate filenames; four exact file hashes through Bash; Read from conversation context; 167 text deltas; exact stop/resume history; stopped browser downloads |
...
| MCP configuration | Native Pi tools use the existing remote MCP connector gateway. Resource and prompt operations pass preview verification at `48e1acb70f` | Local stdio configuration and supervised environment execution pass process and RPC tests; PR #6998 records live verification. Subscriptions and a dedicated discovery UI remain open. `30bff99179` verifies recovered permission authority and stale completion rejection. `278b28f42c` verifies embedded MCP image resources, exact private bytes, native rendering, and replay |
