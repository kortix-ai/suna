...
- Preview/proxy routing in `apps/api/src/sandbox-proxy/index.ts` resolves the sandbox by `sandboxes.externalId` (Daytona is the model now; `justavps` resolves the same way); `/v1/p/{sandboxId}/{port}` returns `{"error":"Sandbox not found"}` when no matching `sandboxes.externalId` row exists — e.g. a stale/cloud `externalId` against a fresh local stack.
