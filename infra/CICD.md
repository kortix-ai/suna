...
| `prod` | PROD | Receives promotions only; deploys one clean version `X.Y.Z`.        |

- `main` is the working branch. Open PRs against `main`; CI gates them.
- `prod` only ever advances via the **Promote** workflow.
...
- **Promoted** (on `prod`): artifacts are the clean `$(cat VERSION)` =
...
3. The workflow bumps `VERSION`, commits it to `main`, tags `vX.Y.Z`, and
   fast-forwards `prod` to that commit.
4. The push to `prod` triggers **Deploy Prod** (`deploy-prod.yml`): it
...
| ------------------- | -------------------------------- | ---------------------------------------------------------------------------------------- |
| `ci.yml`            | PR → `main`,`prod`          | Typecheck/build gates (API, frontend, sandbox agent, CLI, desktop).                      |
| `codeql.yml`        | push/PR → `main`,`prod` + weekly | CodeQL SAST (SOC 2 CC7.1/CC8.1).                                                     |
| `secret-scan.yml`   | PR → `main`,`prod`          | gitleaks secret scan.                                                                     |
...
| `promote.yml`       | dispatch                         | Bump VERSION, tag `vX.Y.Z`, fast-forward `prod`.                                    |
| `deploy-prod.yml`   | push → `prod` + dispatch   | Retag images → `vX.Y.Z`+`latest`, build prod CLI + desktop, cut GitHub Release `vX.Y.Z`, [gated] roll prod ECS. |
...
- On `prod`, desktop is built into the unified `vX.Y.Z` release but is
...
- **Frontend (hosted):** `dev.kortix.com` and `kortix.com` are deployed by
  Vercel from `main` and `prod` respectively — not by these workflows.
...
2. **Create the `prod` GitHub Environment** with a required reviewer
...
4. **Configure Vercel:** set the Production Branch to `prod`, and assign a
