...
- **`api_secrets` MUST include the Nango API and webhook signing keys.**
  Configure `NANGO_BASE_URL`, both GitHub integration IDs,
  `KORTIX_GIT_PROXY=true`, and `GITHUB_CREDENTIAL_RESOLUTION=nango_only`.
  Select one organization connection in the platform Git settings. Project
  creation returns a configuration error when this connection is absent.
