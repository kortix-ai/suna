# ── prod-eks (platform layer) — in-cluster controllers + app namespace ────────
#
# Applied AFTER the `cluster` layer. Reads the cluster's outputs from remote
...
#
# Split from `cluster` on purpose: the kubernetes/helm providers can't be
# configured until the cluster endpoint exists, so keeping them in their own
# state lets each layer plan/apply cleanly.
