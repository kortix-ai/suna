...
resource "aws_iam_policy" "bedrock_count_tokens" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [{ Sid = "Statement1", Effect = "Allow", Action = ["bedrock:CountTokens"], Resource = "*" }] })
  tags   = local.tags
}
...
locals {
  ...
  groups = {
    administrators = {
      ...
      members  = ["markokraemer", "kubet", "saumya@kortix.com"]
    }
    bedrock-limited = {
